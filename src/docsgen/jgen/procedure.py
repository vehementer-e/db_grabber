...
def get_procedure_metadata(conn, schema: str, proc: str, database: str | None = None) -> dict:
    """
    Генерация JSON-описания процедуры.
    ...
    def _safe_fetchall(stage: str, sql: str, params=()):
        try:
    ...
    def _safe_fetchone(stage: str, sql: str, params=()):
        try:
            cur.execute(sql, params)
            return cur.fetchone()
        except Exception as e:
    ...
    row = cur.fetchone()
    if not row or row[0] is None:
        raise ValueError(f"Procedure not found: {schema}.{proc} in {db_name}")
    obj_id = int(row[0])
    ...
    # Если запрос упал или вернул ошибку метаданных — result_dataset остаётся None
    if rs_rows:
        has_error = any((r.error_number is not None) for r in rs_rows)
        if has_error:
            # добавим сообщение DMV-ошибки в generation_errors, чтобы было видно причину
            first_err = next((r for r in rs_rows if r.error_number is not None), None)
            if first_err is not None:
                generation_errors.append(
                    {
                        "stage": "result_dataset",
                        "message": f"{first_err.error_number}: {first_err.error_message}",
                        "sql": "sys.dm_exec_describe_first_result_set_for_object",
                    }
                )
            result_dataset = None
        else:
            ...
            result_dataset = cols if cols else None
    ...
    # внешние БД — запросы "в лоб", как у тебя
    ...
    for r in rb_rows:
        ...
        required_by.append({"objType": obj_type, "db": db_name, "schema": r.schema, "name": r.name, "relation": relation})
    ...
    # ---------- RESULT DATASET (first result set) ----------
    cur.execute(
        """
        SELECT
            column_ordinal,
...
#         meta = get_procedure_metadata(conn, schema, proc)
