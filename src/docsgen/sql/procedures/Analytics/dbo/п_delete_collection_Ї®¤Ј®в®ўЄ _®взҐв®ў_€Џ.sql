...
CREATE   proc  [dbo].[_collection_подготовка_отчетов_ИП]
@mode nvarchar(max) = ''
, @zero_rows int = 1
as
...
select CustomerId, 'Одновременно подтвержденный и неподтвержденный банкрот' Тип from _collection_CustomerStatus
...
select CustomerId, 'Отсутствует дата решения для подтвержденного банкротства' Тип from _collection_CustomerStatus
where CustomerState=('Банкрот подтверждённый' ) and IsActive=1 and DateResultOfCourtsDecisionBankrupt is null

union all


select CustomerId, 'Отсутствует дата решения для неподтвержденного банкротства' Тип from _collection_CustomerStatus
where CustomerState=('Банкрот неподтверждённый' ) and IsActive=1 and BankruptcyFilingDate is null

union all


select CustomerId, 'Отсутствует дата смерти' Тип from _collection_CustomerStatus
where CustomerState='Смерть подтвержденная' and IsActive=1 and date is null

union all


select distinct [id клиента], 'Торги состоялись или принят на баланс но нет статуса БВ' Тип from _collection
...
select distinct [id клиента], 'Есть дата иска. сумма платежей больше суммы по судебному решению. Но нет статуса БВ' Тип from _collection
...
select distinct [id клиента], 'Есть результат торгов но нет даты торгов' Тип from _collection
...
select distinct [id клиента], 'нет даты СП' Тип from  [_collection]
...
select distinct [id клиента], 'нет даты иска' Тип from  [_collection]
...
select distinct [id клиента], 'нет даты ИЛ' Тип from  [_collection]
...
select distinct [id клиента], 'нет даты получения ИЛ' Тип from  [_collection]
...
select distinct [id клиента], 'нет даты принятия ИЛ в работу' Тип from  [_collection]
