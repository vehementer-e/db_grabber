CREATE   proc [dbo].[create_report_naumen_lines_IOT]
as
begin
...
insert into #lines   select ('6015'), ('Сервис 6015')
insert into #lines   select ('6021'), ('Сервис 6021')
...
from NaumenDbReport.dbo.dm_call_legs_incoming
...
,[Дата звонка]           
,[Дата звонка день]           
,c.Месяц [Дата звонка месяц]
,c.Неделя [Дата звонка неделя]
,c.[Неделя начиная с пятницы] [Дата звонка неделя начиная с пятницы]
,null  [Месяц текст]
,null  [Неделя текст]
...
into #final_for_ins
from #final f 
outer apply (select top 1 [Дата звонка]  [Дата повторного обращения на линию] from #final fx where f.[Телефон клиента]=fx.[Телефон клиента] and f.[Дата звонка]<fx.[Дата звонка] and f.line=fx.line and f.[Дата звонка день]=fx.[Дата звонка день]) x
...
left join v_Calendar c on c.Дата=f.[Дата звонка день]
--order by [Телефон клиента], [Дата звонка]

--drop table if exists Analytics.dbo.report_naumen_lines_IOT
--select * into Analytics.dbo.report_naumen_lines_IOT from #final_for_ins

begin tran
delete from  Analytics.dbo.report_naumen_lines_IOT
insert into  Analytics.dbo.report_naumen_lines_IOT
...
--exec [C2-VSR-BIRS].[RS_Jobs].dbo.StartReportJob  'A37E760C-0832-4122-B77E-D59CF4316E43'
--перенес в джоб

--select * from Analytics.dbo.report_naumen_lines_IOT

--begin tran
--
---- drop table if exists Analytics.dbo.report_naumen_lines
---- select * into Analytics.dbo.report_naumen_lines from #final_for_ins
--delete from Analytics.dbo.report_naumen_lines where [Дата звонка день] between @start_date and @end_date
--insert into Analytics.dbo.report_naumen_lines 
--select * from #final_for_ins
--commit tran
