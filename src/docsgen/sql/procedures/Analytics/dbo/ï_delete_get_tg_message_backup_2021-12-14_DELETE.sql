...
CREATE       procedure 
--[dbo].[get_tg_message_backup_2021-12-09]
[dbo].[get_tg_message_backup_2021-12-14]
(
@period nvarchar(max),
@type nvarchar(max)
)
...
тип = 'ПС: '+  case when [ПлатежнаяСистема]='Киви' then 'Contact' else [ПлатежнаяСистема] end
   ...
   where 1=1  and
	 ...
	 , v as(
	  ...
	  case when @period = 'week'    then week
        ...
        , for_text_pts as(
      select Канал, count(*) Займов, sum([Сумма]) Выдано, avg([Сумма]) Чек from v where isInstallment=0 group by Канал union all
   select 'Всего',  count(*) Займов, sum([Сумма]) Выдано, avg([Сумма]) Чек from v where isInstallment=0
   

      ) , for_text_inst as(
      select Канал, count(*) Займов, sum([Сумма]) Выдано, avg([Сумма]) Чек from v where isInstallment=1 group by Канал union all
   select 'Всего',  count(*) Займов, sum([Сумма]) Выдано, avg([Сумма]) Чек from v where isInstallment=1
...
______________
Total:
План: '+(select format(isnull(sum(Сумма), 0) + isnull(sum([Сумма инстоллмент]), 0),'#,0')  from plans )+'
Факт: '+(select format(isnull(sum([Сумма]), 0) ,'#,0') from v )+'
% выполнения: ' + format(
                  
				  case when (select isnull(sum(Сумма), 0) + isnull(sum([Сумма инстоллмент]), 0)  from plans )>0 then
                            (select isnull(sum([Сумма]), 0) from v )/(select isnull(sum(Сумма), 0) + isnull(sum([Сумма инстоллмент]), 0)  from plans ) else 0 end
				, '0%')
into #sales_data
from 
(
...
begin
	  set @message = '
Если вы получили это сообщение и не знаете что делать напишите /help или @petyaaaaaaaa
'
