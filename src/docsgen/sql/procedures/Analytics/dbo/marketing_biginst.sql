 ...
 CREATE proc [dbo].[marketing_biginst] @mode nvarchar(max) = 'update'
as
...
drop table if exists ##biginst_detail
...
WHERE (
(  isnull(b.source,  a.source  )  IN ('infoseti-deepapi-pts') 
or isnull(b.source,  a.source  ) like '%big-inst%'
or isnull(b.source,  a.source  ) like '%big-deepapi%'
or ( b.productsubtype in (  'Big Installment Рыночный'  , 'Big Installment Рыночный - Самозанятый')  )
 
))
AND isnull(isnull(b.issued, b.call1), a.created)  between '20251202' and cast( getdate() +1  as date)  
...
*/
drop table if exists dbo.marketing_biginst_detail 
select * into dbo.marketing_biginst_detail  from  ##biginst_detail 
...
select *, row_number() over(partition by id order by case when productType2  ='BIG INST' then 1 end desc ) rn_lead 
, case when productTypeExternal<> 'Автокредит Инфосети' or productType2  ='BIG INST' then 1 else 0 end as [Признак большой инстоллмент]
, case when phone in (select mobile_number from [adhoc_20260116_BIG пилот]) and created>='20251224' then 1 else 0 end as pilot
...
--select a.*,  b.number, b.producttype, b.productSubType, b.source, b.created, b.status_crm2 from [adhoc_20260116_BIG пилот] a
--left join request b on a.mobile_number=b.phone and b.created>='20251224'
--order by b.productType2 desc,  b.created desc
