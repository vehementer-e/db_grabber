...
[dbo].[get_tg_message_backup_2022_01_27]
(
@period nvarchar(max),
@type nvarchar(max)
)
...
;
      ...
      select Канал, count(*) Займов, sum([Сумма]) Выдано, avg([Сумма]) Чек from #tmp_dm_sales where isInstallment=0 group by Канал union all
   select 'Всего',  count(*) Займов, sum([Сумма]) Выдано, avg([Сумма]) Чек from #tmp_dm_sales where isInstallment=0
   

      ) , for_text_inst as(
      select Канал, count(*) Займов, sum([Сумма]) Выдано, avg([Сумма]) Чек from #tmp_dm_sales where isInstallment=1 group by Канал union all
   select 'Всего',  count(*) Займов, sum([Сумма]) Выдано, avg([Сумма]) Чек from #tmp_dm_sales where isInstallment=1
