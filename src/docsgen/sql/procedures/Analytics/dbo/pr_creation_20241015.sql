create proc pr_creation_20241015
as
...
--select T = '27) 2023 ГОД 1 - 4 кв' ,* from #t1 where YEAR([Заем выдан день]  )=2023 and month([Заем выдан день]) between 1 and 12 --UNION ALL
select T = '27) 2023 ГОД 1 - 3 кв' ,* from #t1 where YEAR([Заем выдан день]  )=2023 and month([Заем выдан день]) between 1 and 9 UNION ALL
select T = '28) 2024 ГОД 1 - 3 кв' ,* from #t1 where YEAR([Заем выдан день]  )=2024 and month([Заем выдан день]) between 1 and 9 --UNION ALL

)ч

select * into pr_20241015 from #t2




select rn  [Марка модель ТС]
, max(case when    '27) 2023 ГОД 1 - 3 кв'       =T then Сегмент end)  '27) 2023 ГОД 1 - 3 кв'
, max(case when    '28) 2024 ГОД 1 - 3 кв'       =T then Сегмент end)  '28) 2024 ГОД 1 - 3 кв'
