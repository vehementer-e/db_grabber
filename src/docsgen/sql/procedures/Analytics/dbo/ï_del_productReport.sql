...
CREATE     proc  [dbo].[_productReport]			 
@mode nvarchar(max) = 'select'		   , @recreate int =0
--exec [_birs].[product_report] 'update', 1 

 /*
  update 	 product_report_debug_mode set is_debug = 1
 update 	 product_report_debug_mode set is_debug = 0  exec [C2-VSR-BIRS].[RS_Jobs].dbo.StartReportJob  '615B9902-77A6-4B5E-A7E4-D791D47F4B4B'
...
if @mode  = 'request' and (select is_debug from product_report_debug_mode)=1 select top 10000   *  from  request_bi  where created>=getdate()-30
if @mode  = 'request' and (select is_debug from product_report_debug_mode)=0 select  * from  request_bi  where created>='20230101'



if @mode  = 'plan' and (select is_debug from product_report_debug_mode)=1 select top 0   *  from  v_plan_budget_risk_xlsx  
if @mode  = 'plan' and (select is_debug from product_report_debug_mode)=0 select  * from  v_plan_budget_risk_xlsx  
...
if @mode  = 'balance20' and (select is_debug from product_report_debug_mode)=1 select top 0   *  from productReportBalanceBi
if @mode  = 'balance20' and (select is_debug from product_report_debug_mode)=0 select  * from  productReportBalanceBi
if @mode  = 'balance_for_plan_fact' and (select is_debug from product_report_debug_mode)=1 select   top 0    date, returntype, productType, sum(percentsPaid) percentsPaid  from  productReportBalance where percentsPaid <> 0  group by  date, returntype, productType
if @mode  = 'balance_for_plan_fact' and (select is_debug from product_report_debug_mode)=0 select  date, returntype, productType, sum(percentsPaid) percentsPaid  from  productReportBalance where percentsPaid <> 0  group by  date, returntype, productType
...
if @mode  = 'prolongation' and (select is_debug from product_report_debug_mode)=1 select top 0 
 ...
 , a.loyaltyBezzalog FROM Analytics.dbo.[productReportProlongation] a 
...
if @mode  = 'closed' and (select is_debug from product_report_debug_mode)=1   SELECT top 0 
           a.[type] ,   a.[type2] ,   a.[date_type] ,   a.[product] ,   a.[closed] ,   a.[ispts] ,   a.[request_0_d] ,   a.[request_5_d] ,   a.[request_14_d] ,   a.[request_30_d] ,   a.[request_90_d] ,   a.[request_180_d] ,   a.[free_term_days] ,   a.[Вид займа любой продукт] ,   a.[closed_dpd_begin_day] ,   a.[product_type] ,   a.[loyalty_pts] ,   a.[loyalty_nopts] ,   a.[Тип первого займа] FROM Analytics.dbo.[productReportClosed] a
...
--if @mode  = 'prolongation' --update 	 product_report_debug_mode set is_debug = 0 
--begin select * from v_prolongation end
   --SELECT 
   --         a.[type] ,   a.[type2] ,   a.[date_type] ,   a.[product] ,   a.[closed] ,   a.[ispts] ,   a.[request_0_d] ,   a.[request_5_d] ,   a.[request_14_d] ,   a.[request_30_d] ,   a.[request_90_d] ,   a.[request_180_d] ,   a.[free_term_days] ,   a.[Вид займа любой продукт] ,   a.[closed_dpd_begin_day] ,   a.[product_type] ,   a.[loyalty_pts] ,   a.[loyalty_nopts] ,   a.[Тип первого займа] FROM Analytics.dbo.[productReportClosed] a
if @mode  = 'update' --update 	 product_report_debug_mode set is_debug = 0 
begin
...
--EXEC sp_rename 'Analytics._birs.product_report_lead_request.userBrowser', 'browser'
--EXEC sp_rename 'Analytics._birs.product_report_lead_request.userBrowserVersion', 'browserVersion'
end
