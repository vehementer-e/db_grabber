...
--declare @report_quarter date = cast(DATEADD(qq, DATEDIFF(qq, 0, @report_date), 0) as date)
--
--declare @period nvarchar(10) = 'week'

set @message = (select top 1 text from [оперативная витрина с комиссиями агрегаты] where command=@type+'_'+@period  )
...
Прибыль = isnull(ПрибыльБезНДС, 0),
Сумма = Сумма,
Количество = 1, 
день =    [ДеньПлатежа],
...
--      select Канал, count(*) Займов, sum([Сумма]) Выдано, avg([Сумма]) Чек , isnull(sum(СтавкаНаСумму)/nullif(sum([Сумма]), 0), 0) Ставка from #tmp_dm_sales where product='ПТС' group by Канал union all
--   select 'Всего',  count(*) Займов, sum([Сумма]) Выдано, avg([Сумма]) Чек , isnull(sum(СтавкаНаСумму)/nullif(sum([Сумма]), 0), 0) Ставка from #tmp_dm_sales where product='ПТС' 
--   
--
--      ) , for_text_inst as(
--      select Канал, count(*) Займов, sum([Сумма]) Выдано, avg([Сумма]) Чек , isnull(sum(СтавкаНаСумму)/nullif(sum([Сумма]), 0), 0) Ставка  from #tmp_dm_sales where product='Инстоллмент' group by Канал union all
--   select 'Всего',  count(*) Займов, sum([Сумма]) Выдано, avg([Сумма]) Чек , isnull(sum(СтавкаНаСумму)/nullif(sum([Сумма]), 0), 0) Ставка  from #tmp_dm_sales where product='Инстоллмент'
--   
--      ), for_text_BI as(
--   select 'Всего' Канал,  count(*) Займов, sum([Сумма]) Выдано, avg([Сумма]) Чек, isnull(sum(СтавкаНаСумму)/nullif(sum([Сумма]), 0), 0) Ставка from #tmp_dm_sales where product='Бизнес инвест'
--   
--      )
--
--select 
...
--' +'Чек: ' + format(Чек , '#,0')+' руб.'+ case when канал = 'Всего' then '
--' +'Ставка: ' + format(Ставка , '0.0')+'%' else '' end
--
--text
--from for_text_pts
--      ) x
...
--'+ 'Выдано: ' +format(Выдано , '#,0')+' руб.
--' +'Чек: ' + format(Чек , '#,0')+' руб.'+ case when канал = 'Всего' then '
--' +'Ставка: ' + format(Ставка , '0.0')+'%' else '' end text
--from for_text_inst
--      ) x
--)inst
--
...
--set @message = (select top 1 text from [оперативная витрина с выдачами и каналами агрегаты] where [periods]=@period and b='ПТС' )
set @message = (select top 1 text from [оперативная витрина с выдачами и каналами агрегаты] where command=@type+'_'+@period  )
...
begin
 ...
 isnull(text, '') from _tg.projects_stat	 where type = 'vtb-ref'
select @message	   +'
/vtb'
return
end
...
begin
 ...
 isnull(text, '') from _tg.projects_stat	 where type = 'psb-ref'	 +'
/psb'
select @message
return
end
...
begin
	  set @message = '
Посмотреть результаты продаж - /channels_today
...
/comissions_week        
/comissions_month   
/comissions_lastmonth   
/comissions_quarter


По вопросам работы бота 112@carmoney.ru
'
...
set @message = (select top 1 text from [оперативная витрина с выдачами и каналами агрегаты] where command=@type   )
