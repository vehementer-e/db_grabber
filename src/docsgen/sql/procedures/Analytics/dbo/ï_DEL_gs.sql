 CREATE   proc [dbo].[_gs]   @mode nvarchar(max) = '' ,  @wait int = 1 as
declare @datet___ datetime   =  getdate()  



if @mode like  'get_sql %' begin select * from _gs_view where mode like '%' + replace( @mode, 'get_sql ', '') + '%' return end 
...
DRIVE_to_DWH_FILES(src = r"G:\Общие диски\Commercial Team\Internet Marketing\Расходы",
					   ...
					   name2 = "Продажа трафика2"
                       )
'	   , @wait
if  @wait=1 

waitfor delay '00:03:30'
if not exists (select top 1 * from stg.files.[продажа трафика2_stg] where created>=@datet___	) and @wait=1 
begin
RAISERROR ('Справочник stg.files.[продажа трафика2_stg] не обновился или пустой', -- Message text.
               16, -- Severity.
               1 -- State.
               );
			   select 1/0 
...
exec python  'update_marketing_costs()'	   , @wait
if  @wait=1 
...
RAISERROR ('Справочник  _gsheets.[dic_costs_sms] пустой ', -- Message text.
               16, -- Severity.
               1 -- State.
               );
...
, case when c.ispts=1 then   c.[_payMethodPts] 	   end as [_payMethodPts] 	  
