CREATE   PROC [dbo].[Стоимость займа CPA расчет стоимости CRIB2]
as
begin
...
drop table if exists #crib_api 

select 	b.requestNumber, b.lead_leadgen_name , b.leadgeneratorid
into #crib_api
from stg._crib.dm_crm_requests b
join stg._LCRM.lcrm_leads_full_calculated lcrm on lcrm.ID=b.lcrmid	and lcrm.uf_type='api' and 	b.lead_leadgen_name <>''


--select * from #crib_api 

drop table if exists #postback_request 
select request_number, lead_leadgen_name , leadgeneratorid into #postback_request from (
select request_number, lead_leadgen_name, leadgeneratorid, created, row_number() over(partition by request_number order by  created )  rn  -- select top 100 *
from stg._crib.dm_postbacks
where request_number is not null
 ) x where rn=1
--select * from #postback_request
...
with ft  as (
select id
, case when p.lead_leadgen_name is not null then p.lead_leadgen_name when ca.lead_leadgen_name is not null  then  ca.lead_leadgen_name  when nullif(UF_ROW_ID, '') is null and a.[Канал от источника]<>'CPA нецелевой' then UF_SOURCE     end UF_SOURCE
...
, UF_REGIONS_COMPOSITE
, UF_STEP
, case when p.lead_leadgen_name is not null then 'POSTBACK' when ca.lead_leadgen_name is not null  then  'api' when nullif(UF_ROW_ID, '')  is null then UF_TYPE    end   UF_TYPE
, UF_LOGINOM_DECLINE
, dateadd(year, -2000, z.Дата) UF_ACTUALIZE_AT
, isnull(b.[Канал от источника] , a.[Канал от источника]) [Канал от источника]
, UF_FULL_FORM_LEAD 
, case when p.lead_leadgen_name is not null then p.leadgeneratorid when ca.lead_leadgen_name is not null  then  ca.leadgeneratorid  when nullif(UF_ROW_ID, '') is null and a.[Канал от источника]<>'CPA нецелевой' then UF_PARTNER_ID     end  UF_PARTNER_ID 
...
left join #crib_api ca on ca.requestNumber=a.UF_ROW_ID
...
,      ВидЗайма
	into #stg_table
from ft with(nolock)
where  UF_SOURCE is not null or [Канал от источника]<>'CPA нецелевой'

;
with v as (select *, ROW_NUMBER() over(partition by uf_row_id order by (select 1 )) rn from #stg_table) delete from v where rn>1 and uf_row_id is not null
;
with v as (select *, ROW_NUMBER() over(partition by id order by (select 1 )) rn from #stg_table) delete from v where rn>1 and uf_row_id is  null
;
drop table if exists analytics.dbo.[Стоимость займа лиды с потенциальной оплатой Криб2] 
select top 0 * into  analytics.dbo.[Стоимость займа лиды с потенциальной оплатой Криб2]   from #stg_table
delete a from analytics.dbo.[Стоимость займа лиды с потенциальной оплатой Криб2] 	a join 	#stg_table b on a.id=b.id 
insert into analytics.dbo.[Стоимость займа лиды с потенциальной оплатой Криб2]
select * from #stg_table
--select * from  analytics.dbo.[Стоимость займа лиды с потенциальной оплатой Криб2]
------------------------------------------------------------------------------------------------------------
------------------------------------------------------------------------------------------------------------
------------------------------------------------------------------------------------------------------------
...
delete from analytics.dbo.[Стоимость займа лиды с потенциальной оплатой Криб2]
...
from  analytics.dbo.[Стоимость займа лиды с потенциальной оплатой Криб2]
...
from analytics.dbo.[Стоимость займа лиды с потенциальной оплатой Криб2] a
...
--	 order by 5
--
drop table if exists  Analytics.dbo.[Стоимость займа лиды с расчетной стоимостью Криб2]  
select top 0 * into Analytics.dbo.[Стоимость займа лиды с расчетной стоимостью Криб2] from #stg_table_result
delete from Analytics.dbo.[Стоимость займа лиды с расчетной стоимостью Криб2]
insert into Analytics.dbo.[Стоимость займа лиды с расчетной стоимостью Криб2]
select * from #stg_table_result
where (UF_SOURCE is not null and uf_row_id is not null) or (	  uf_row_id is  null and СТоимость is not null )



exec [C2-VSR-BIRS].[RS_Jobs].dbo.StartReportJob  '231BE714-A1AA-4A85-864A-BADE499D9837', 1
exec analytics.dbo.log_email 'Стоимость CPA опер CRIB2 finished', 'p.ilin@techmoney.ru'
