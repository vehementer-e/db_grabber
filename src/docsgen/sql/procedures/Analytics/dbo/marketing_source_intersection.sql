...
CREATE   proc [dbo].[marketing_source_intersection]
as
...
UPDATE a
SET a.hasLeadInfoseti = 1
FROM _request a
 JOIN v_lead2 b2 
    ON a.phone = b2.phone 
   AND b2.created BETWEEN DATEADD(day, -30, a.created) AND a.created 
   AND b2.source in ('infoseti-deepapi-installment', 'infoseti-deepapi-pts')
...
--alter table _request add [hasLeadInfoseti] tinyint
--alter table _request_log add [hasLeadInfoseti] tinyint
