...
create   proc [dbo].[Подготовка отчета стоимость CPA опер_backup_2022_11_14]
as
begin
...
from analytics.dbo.dm_report_lcrm_cpa_cpc_for_costs
where UF_SOURCE in ('pod-pts')  and МесяцЗайма ='20221001' 
group by МесяцЗайма
union all
select
...
from analytics.dbo.dm_report_lcrm_cpa_cpc_for_costs
where UF_SOURCE in ('ptsoff-leads')  and МесяцЗайма ='20221001' 
group by МесяцЗайма
union all
select
...
from analytics.dbo.dm_report_lcrm_cpa_cpc_for_costs
where UF_SOURCE in ('workie-installment-ref', 'workie-ref')  and МесяцЗайма ='20221001' and ЗаявкаAPI_или_API2=0
group by МесяцЗайма
...
from analytics.dbo.dm_report_lcrm_cpa_cpc_for_costs
where UF_SOURCE in ('click2')  and МесяцЗайма ='20221001' and ЗаявкаAPI_или_API2=0
group by МесяцЗайма
...
from analytics.dbo.dm_report_lcrm_cpa_cpc_for_costs
where UF_SOURCE in ('mastertarget')  and МесяцЗайма ='20221001' and ЗаявкаAPI_или_API2=0
group by МесяцЗайма
...
from analytics.dbo.dm_report_lcrm_cpa_cpc_for_costs
where UF_SOURCE in ('likemoney', 'likemoney-ref')  and МесяцЗайма ='20221001' --and ЗаявкаAPI_или_API2=0
group by МесяцЗайма
...
from analytics.dbo.dm_report_lcrm_cpa_cpc_for_costs
where UF_SOURCE in ('creditpts-api', 'crditpts')  and МесяцЗайма ='20221001' --and ЗаявкаAPI_или_API2=0
group by МесяцЗайма
union all
select
...
from analytics.dbo.dm_report_lcrm_cpa_cpc_for_costs
where UF_SOURCE in ( 'liknot')  and МесяцЗайма ='20221001' and ЗаявкаAPI_или_API2=1
group by МесяцЗайма
...
from analytics.dbo.dm_report_lcrm_cpa_cpc_for_costs
where UF_SOURCE in ( 'unicom24r', 'unicom24')  and МесяцЗайма ='20221001' and ЗаявкаAPI_или_API2=1
group by МесяцЗайма
...
from analytics.dbo.dm_report_lcrm_cpa_cpc_for_costs
where UF_SOURCE in ( 'cityads', 'Refflection-api')  and МесяцЗайма ='20221001' and ЗаявкаAPI_или_API2=0
group by МесяцЗайма
...
from analytics.dbo.dm_report_lcrm_cpa_cpc_for_costs
where UF_SOURCE in ( 'finuslugi')  and МесяцЗайма ='20221001' and ЗаявкаAPI_или_API2=0
group by МесяцЗайма
...
when UF_SOURCE='pod-pts' then 
											...
											when ДеньЗайма between '20220301' and '20220930' then
															                        '{"cost":"'+format(СуммаЗайма*0.05 ,'0.00')+'", "for":"'+'Займ'+'", "month":"'+МесяцЗаймаТекст+'", "since":"'+'20200110'+'", "till":"'+'now'+'", "condition":"'+''+'", "extra":"'+'5%'+'"}'  	
...
when UF_SOURCE='ptsoff-leads'               then 
													case when ДеньЗайма >='20221101' then 
...
when UF_SOURCE in ('creditpts-api', 'creditpts')
                                                   then 
													...
													when ДеньЗайма between '20221001' and '20221031' then 
													   ...
													   when ДеньЗайма between '20220527' and '20220930' then 
																								'{"cost":"'+format( СуммаЗайма*0.08,'0.00')+'", "for":"'+'займ'+'", "month":"'+МесяцЗаймаТекст+'", "since":"'+'now'+'", "till":"'+'now'+'", "condition":"'+''+'", "extra":"'+''+'"}'   
...
--																								                               else '{"cost":"'+format( 700,'0.00')+'", "for":"'+'заявку'+'", "month":"'+МесяцЗаявкиТекст+'", "since":"'+'20210319'+'", "till":"'+'now'+'", "condition":"'+'Условия действуют с 19.03.2021 г.: по API - 300 руб. за Заявку по Реферальной ссылке - 700 руб. за Заявку'+'", "extra":"'+'ЗаявкаAPI_или_API2=0'+'"}'   
													...
													when ДеньЗайма between '20221001' and '20221031'
