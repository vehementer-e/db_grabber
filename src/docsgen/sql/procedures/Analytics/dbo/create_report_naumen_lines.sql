CREATE   proc [dbo].[create_report_naumen_lines]
as
begin
...
,      [Дата звонка неделя]         = dateadd(day, datediff(day, '1900-01-01', cast(min(created)      as date)) / 7 * 7, '1900-01-01') --cast(DATEADD(wk, DATEDIFF(wk,0,min(created)  ), 0) as date) 
...
--update a 
--set a.[Дата звонка неделя] = dateadd(day, datediff(day, '1900-01-01', [Дата звонка день]) / 7 * 7, '1900-01-01')
--
--from
--Analytics.dbo.report_naumen_lines a
