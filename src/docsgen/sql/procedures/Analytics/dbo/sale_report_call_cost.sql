CREATE     proc [dbo].[sale_report_call_cost]	@mode nvarchar(max) = 'update' as
 --exec sp_create_job 'Analytics._sale_report_call_cost at 8:00' , 'sale_report_call_cost' , '1', '80000'
...
--declare @update_dt date = '20250301'
...
--select * into report_call_cost from  _birs.cost_of_calls


delete from report_call_cost
where attempt_start_date>=cast(@update_dt as date)


insert into report_call_cost -- order by 1 desc
select*from #dos
...
--insert into _birs.cost_of_calls-- order by 1 desc
--select*from #dos
...
EXEC  sp_birs_update 'B5BE1608-35B3-4E7F-AF6B-0904914ED0EA'
