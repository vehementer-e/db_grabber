...
where ishistory=0
	 ...
	 , v as(
	  ...
	  case when @period = 'week'    then week
        ...
        , for_text as(
      select Канал, count(*) Займов, sum([Сумма]) Выдано, avg([Сумма]) Чек from v group by Канал union all
   select 'Всего',  count(*) Займов, sum([Сумма]) Выдано, avg([Сумма]) Чек from v
      )
