...
CREATE   PROC [dbo].[create_report_leads_full_NEW]
...
,  case when Обработан is not null   then 1 else 0 end [Признак обработан лид]
,  case when Дозвон is not null   then 1 else 0 end [Признак дозвон]
...
,  case when fl.[Признак профильный]=1 or fl.[Номер заявки] is not null  then 1 else 0 end [Признак профильный лид]
...
--
AND a.UF_REGISTERED_AT = b.UF_REGISTERED_AT
...
SELECT [UF_REGISTERED_AT день]               [UF_REGISTERED_AT день]
	  ...
	  ,[Заем выдан день CRM]
	  ,[Заем выдан месяц CRM]
	  ,cast(count(*)                                             as bigint) [Количество лидов]
      ,cast(sum([Признак обработан лид]      )                   as bigint) [Признак обработан лид]      
      ,cast(sum([Признак дозвон]				)                as bigint) [Признак дозвон]				
      ,cast(sum([Признак профильный лид]		)                as bigint) [Признак профильный лид]		
      ,cast(sum([Признак заявка со звонка]	)                    as bigint) [Признак заявка со звонка]	
      ,cast(sum([Признак займ со звонка]		)                as bigint) [Признак займ со звонка]		
      ,cast(sum([Сумма займа со звонка]		)                    as bigint) [Сумма займа со звонка]	
	  ,cast(sum([Признак заявка CRM]         )                   as bigint) [Признак заявка CRM]
      ,cast(sum([Признак Предварительное одобрение CRM] )        as bigint) [Признак Предварительное одобрение CRM]
      ,cast(sum([Признак Контроль данных CRM] 		   )         as bigint) [Признак Контроль данных CRM] 			
      ,cast(sum([Признак Отказ Carmoney CRM] 				   ) as bigint) [Признак Отказ Carmoney CRM] 				
      ,cast(sum([Признак Одобрено CRM] 				   )         as bigint) [Признак Одобрено CRM] 				
      ,cast(sum([Признак Заем выдан CRM] 			   )         as bigint) [Признак Заем выдан CRM] 				
...
FROM Stg.tmp.TMP_AND_analytics_dbo_report_leads_full
WHERE [UF_REGISTERED_AT день] IN (SELECT [UF_REGISTERED_AT день] FROM #t1 WHERE [UF_REGISTERED_AT день] IS NOT NULL GROUP BY [UF_REGISTERED_AT день])

--INSERT INTO analytics.dbo.report_leads_full
INSERT INTO Stg.tmp.TMP_AND_analytics_dbo_report_leads_full
select 
*
from #t1
commit tran

--НЕ ЗАБЫТЬ РАСКОММЕНТАРИТЬ !!!
/*
