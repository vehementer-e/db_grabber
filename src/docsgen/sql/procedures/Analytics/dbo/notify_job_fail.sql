CREATE     proc [dbo].[notify_job_fail]
as
begin
...
drop table if exists #job_fails;
 ...
 with notify_job_fail_processed2 as ( select 
convert(datetime,stuff(stuff(stuff( left(right(step_id_job_id_run_dateandtime, 51) , 14)  , 9, 0, ' '), 12, 0, ':'),
        15, 0, ':')) run_dateandtime , *
		from notify_jobFailProcessed
...
into #job_fails
  ...
  and Finish_DateAndTime>=dateadd(minute, -30, isnull((select max(run_dateandtime) from notify_job_fail_processed2), cast(getdate() as date)))  and Run_Status<>'retry'
  and Finish_DateAndTime >='20250423 12:13'
  order by Run_DateAndTime



  select job_id, step_id, isNULL(updatesql , '') updatesql  into #upd from jobs
...
join notify_job_fail_processed2  b on a.step_id_job_id_run_dateandtime=b.step_id_job_id_run_dateandtime
;
with v as (
...
+'update script:'+ a.updatesql+ CHAR(10)
...
, 
(select 'p.ilin@smarthorizon.ru' [send_to]  ) a2 where Текст is not null
