...
CREATE PROCEDURE [dbo].[_request_old_light_creation] -- _request_old_light_creation  1 --exec   [_birs].[factor_analysis_light_creation] 1
...
,  po.НаименованиеПолное as [ПричинаОтказа] 
			...
			, sis.call2 as call2--c19.Период as 'P2P'
...
end as nvarchar(50))  [Канал от источника атрибуция]
...
drop table if exists #v_dm_place_of_creation_2
	...
	, case  
	   ...
	   from #R_t_source	  fa
		...
		drop table if exists #parent_guid
		select   number into #parent_guid from v_request_lk
		where request_source_guid is not null
...
--ВсеЗаявки.ЗаявкиЛид,
	...
	--		, ВсеЗаявки.причинаотказа as [Причина отказа]
		...
		--	end [Категория повторного клиента]
			...
			, ТипПРодукта.Наименование ТипПродукта
...
----
	...
	--		, cast( 0 as int) Дубль_ClientGuidОдинДень -- перенести в конец
		...
		--	,  L_C_R_M.[Канал от источника лид]   as [Канал от источника лид]
			...
			, case
			when pg.number is not null and [Заем выдан] is not null then 'Пересадка (выдан)'
			when pg.number is not null and [Заем выдан] is   null then 'Пересадка (не выдан)'
			else '' end parentDesc
...
left join   Stg._1cCRM.Справочник_тмТипыКредитногоПродукта ТипПРодукта on КредПродукт.ТипПродукта = ТипПРодукта.ссылка
...
left join #parent_guid pg on pg.number=ВсеЗаявки.НомерЗаявки
...
--alter table _birs.[factor_analysis_light_staging] add источник varchar(128)
--alter table _birs.[factor_analysis_light] add источник varchar(128)
--alter table _birs.[factor_analysis_light_to_del] add источник varchar(128)
