...
CREATE      proc  [dbo].[sale_partner_lead_report]
...
from sale_partner_lead where UF_STAT_CAMPAIGN = '-118' or Юрлицо = 'АО КБ "Юнистрим"'
order by 2 desc
end
...
from
	...
	left join v_fa fa (nolock) on fa.Номер = l.UF_ROW_ID
...
from
	...
	join v_fa fa (nolock) on fa.Номер = l.UF_ROW_ID
...
from
	...
	join stg._LK.requests lk_r (nolock) on lk_r.num_1c=fa.Номер
	join stg._1cCRM.Справочник_Офисы o on o.Код=lk_r.promo_code
where 	fa.ДатаЗаявкиПолная>='20250301'
...
delete from dbo.sale_partner_lead 
insert into dbo.sale_partner_lead
...
exec python 'sql2gs("""  select  * from dbo.sale_partner_lead  where  [Дата создания лида]>= getdate()-60 and  [Дата создания лида]>=''20250301''  order by 5 desc""" , "1CYRAUTrJR_KXA9cqEUH6CC8DU47McHdm4n2ssMzCn2k", "partner_lead")'
