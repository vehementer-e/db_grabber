CREATE   proc --exec
[dbo].[sp_create_job]  --'Analytics._birs Отчет по неделям в ТГ в 8:00'	  , 'exec [_birs].[kpi_report_mail]' , '1', '80000'
@job_name_ nvarchar(max) = ''
,@sql nvarchar(max) = ''
,@enabled nvarchar(max) = '1'
,@start nvarchar(max) = '90000'
,@each_N_min int = 0
as
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'''+@job_name_ +''', 
		...
		--@notify_email_operator_name=N''analytics alert operator'', 
		@job_id = @jobId OUTPUT
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'''+@job_name_+''', 
		...
		@freq_interval=1, 
		@freq_subday_type=4, 
		@freq_subday_interval='+format(@each_N_min, '0')+ ', 
		@freq_relative_interval=0, 
		@freq_recurrence_factor=0, 
		@active_start_date=20220617, 
		@active_end_date=99991231, 
		@active_start_time='+@start+', 
		@active_end_time=235959 '  end  +
...
--return
exec (@task)
select result = 'Джоб создан' 
,      JobName = @job_name_ 
,      Command = @sql 
,      'EXEC msdb.dbo.sp_start_job @job_name =  '''+@job_name_+ '''' START_JOB_SCRIPT
,      'EXEC msdb.dbo.sp_stop_job @job_name =  '''+@job_name_+ '''' STOP_JOB_SCRIPT
,      'EXEC msdb.dbo.sp_delete_job @job_name =  '''+@job_name_+ '''' DELETE_JOB_SCRIPT
  into #tt
  select * from #tt
--for json auto
