...
CREATE         proc   [dbo].[marketing_lead_monitoring]
...
select 1 cnt,id, phone, source , appmetrica , channel_group ,   date [UF_REGISTERED_AT_date] ,   channel, entrypoint    into #tm1   from v_lead
   where
 cast(created_at_time as date)>=@last_month_start
							   
 and datepart(hour, created ) <=		 @report_h
 and entrypoint not in ('DWH', 'CSV')




 --select * from v_entrypoint
...
union all	   select 1 as order_num,  type = '' , gr = source+ '(UNIAPI)' , *  from #tm1  a            where       entrypoint='UNI_API'
...
union all select 7 as order_num,  type = '' , gr = 'all' , * from #tm1  a -- where channel_group='CPA'
...
where	  (
type_group LIKE 'КОЛ-ВО%'

 AND
 [Лидов среднее с начала прошлого месяца]>1
 --and abs([Отклонение рабочие/выходные])>0.5
