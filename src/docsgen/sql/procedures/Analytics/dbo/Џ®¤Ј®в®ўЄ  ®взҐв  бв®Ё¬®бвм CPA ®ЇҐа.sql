CREATE PROC [dbo].[Подготовка отчета стоимость CPA опер]
as
begin
...
--where Номер='23113021485981'
select * from #z

drop table if exists #fa
select  Номер, [Заем выдан], [Выданная сумма], case when ispts=0 then 1 else 0 end isInstallment, [Признак Рефинансирование] [ПризнакРефинансирование], [Канал от источника], [Вид займа] into #fa 
...
--select top 0 * into  analytics.dbo.dm_report_lcrm_cpa_cpc_for_costs   from #stg_table
 ...
 --select top 100 * from analytics.dbo.dm_report_lcrm_cpa_cpc_for_costs
 --order by id desc

delete a from analytics.dbo.dm_report_lcrm_cpa_cpc_for_costs 	a join	 #stg_table b on a.id=b.id
