...
declare @productType nvarchar(max) = (select  string_agg(productType, ',')  from analytics.dbo.productType where productType in  (@productTypeSsrs) )

declare @isLoan nvarchar(max) = (select  string_agg(isloan, ',')  from (select 1 isloan union select 0) x where isloan in  (@isLoanSsrs) )


exec analytics.[dbo].[sale_report_request] @date_from, @date_to, @all_columns, @productType, @loginSsrs, @isLoan
*/


CREATE   proc     [dbo].[sale_report_request] 
...
SELECT trim(value) #productType into #productType FROM STRING_SPLIT(@productType, ',') 



drop table if exists #isloan

SELECT cast(trim(value) as int)  #productType into #isloan FROM STRING_SPLIT(@isloan, ',') 
...
hasLeadInfoseti hasLeadInfoseti, productTypeExternal productTypeExternal ,
 a.returnTypeByProduct, a._pack2PTS, a._pack2ProfileSignedPTS, a._pack2SignedPTS, a.abPhotoUprid, a._upridYes ,
 a.productTypeInitial, a.productSubType, a.isSelfEmployed, a._proofOfIncomeLoadedPTS, a.isTakePts, a.call03, a.call03approved,
 a.sumNoProofOfIncome, a.sumProofOfIncome, a.sumSelfemployed, a.needBki, a.firstLimitChoice, a.secondLimitChoice, a.addProductSumRequest, a.isAddProductRequest 
 , inn
 ,cast( null as nvarchar(100)) innIp 
 
 , [addProductInsuranceCancellSum]
, [addProductCancellSum]


into #request_new from request  a with(nolock) where 1=0
 


--declare @datefrom date = getdate()	 declare @dateto date = getdate() declare @all_columns int=0

--reports.dbo.dm_sales
  ...
  select 	fa.Номер,		  
	...
	into #t1 from [dm_Factor_Analysis]	 fa
	
Where
	cast(fa.[Верификация КЦ] as date) between @datefrom and @dateto	  --	  and 1=0
	and productType in (select * from #productType)
	and case when [Заем выдан] Is not null then 1 else 0 end in (select * from #isloan)
  ...
  select 	fa1.[Дата],
	...
	fa1.[Номер],
	fa1.[Текущий статус],
	fa1.[Место создания] [Место cоздания],
    ...
    fa1.[Сумма Рат Юруслуги],
		...
		fa1. [Признак Защита жизни и здоровья], 
		fa1. [Сумма Защита жизни и здоровья], 
	fa1.[Признак Безопасность семьи], 
	fa1.[Сумма Безопасность семьи], 
...
,  fa1.[Страхование Жизни сумма расторжения]
,  fa1.[Каско сумма расторжения]
,  fa1.[Спокойная Жизнь сумма расторжения]
,  fa1.[РАТ сумма расторжения]
,  fa1.[Помощь Бизнесу сумма расторжения]
,  fa1.[Защита от потери работы сумма расторжения]
,  fa1.[Телемедицина сумма расторжения] 
,  fa1.[Фарма сумма расторжения]
,  fa1.[Безопасность семьи сумма расторжения]
,  fa1.[Защита жизни и здоровья сумма расторжения]
,  fa1.КоличествоКП
,  fa1.СписокКП
	
	into #f1 from v_fa	 fa1
	
Where
	cast(fa1.[Верификация КЦ] as date) between @datefrom and @dateto		-- and 1=0

	and productType in (select * from #productType)
	and isLoan in (select * from #isloan)



if @all_columns=1 begin
insert into #request_new
select a.number, a.carBrand, a.carModel, a.carYear,  null as vin, a.hasLeadInfoseti hasLeadInfoseti, a.productTypeExternal, 
a.returnTypeByProduct, a._pack2PTS, a._pack2ProfileSignedPTS, a._pack2SignedPTS, a.abPhotoUprid, a._upridYes ,
a.productTypeInitial, a.productSubType, a.isSelfEmployed, a._proofOfIncomeLoadedPTS, a.isTakePts, a.call03, a.call03approved, 
a.sumNoProofOfIncome, a.sumProofOfIncome, a.sumSelfemployed, a.needBki, a.firstLimitChoice, a.secondLimitChoice, a.addProductSumRequest, a.isAddProductRequest
 ,a. inn
 ...
 , CASE 
        -- 1. Проверяем наличие хотя бы одного активного (status IS NULL или не 'Прекратило...')
        WHEN EXISTS (
            SELECT 1 
            FROM OPENJSON(REPLACE(REPLACE(innipjson, 'None', 'null'), '''', '"'), '$.ip') 
            WITH (status NVARCHAR(MAX) '$.status')
            WHERE status IS NULL OR status <> N'Прекратило деятельность'
        ) THEN 'ИП активный'

        -- 2. Проверяем, пустой ли массив (после очистки структуры)
        WHEN JSON_QUERY(REPLACE(REPLACE(innipjson, 'None', 'null'), '''', '"'), '$.ip') = '[]' THEN 'Не ИП'

        -- 3. Если записи есть, но активных не нашли — значит только прекратившие
       when innipjson is not null then 'ИП прекратил деятельность'
    END innIp
...
from request a with(nolock)  join #f1 b on a.number=b.Номер
...
fa1.[Номер точки при создании],
fa1.issmartinstallment issmartinstallment,
returnTypeByProduct [Видзайма в рамках продукта инстиоллмент] ,
fa1.СрокЛьготногоПериода,
...
#request_new.productTypeExternal ,

 #request_new.returnTypeByProduct,
 #request_new._pack2PTS,
 #request_new._pack2ProfileSignedPTS,
 #request_new._pack2SignedPTS, 
 #request_new.abPhotoUprid,
 #request_new._upridYes ,
 #request_new.productTypeInitial,
 #request_new.productSubType,
 #request_new.isSelfEmployed,
 #request_new._proofOfIncomeLoadedPTS, 
 #request_new.isTakePts,
 #request_new.call03,
 #request_new.call03approved,
 #request_new.sumNoProofOfIncome, 
 #request_new.sumProofOfIncome,
 #request_new.sumSelfemployed,
 #request_new.needBki,
 #request_new.firstLimitChoice,
 #request_new.secondLimitChoice,
 #request_new.addProductSumRequest,
 #request_new.isAddProductRequest ,
  #request_new.inn  ,
  #request_new.innIp   ,
    
   #request_new.[addProductInsuranceCancellSum] ,
    #request_new.[addProductCancellSum] ,
		fa1. [Признак Защита жизни и здоровья], 
		fa1. [Сумма Защита жизни и здоровья], 
	fa1.[Признак Безопасность семьи], 
	fa1.[Сумма Безопасность семьи] 
...
,   case when @all_columns=1 then isnull( a.isAddProductRequest ,  a.[ПризнакКП]  ) end as [ПризнакКП] 
,   case when @all_columns=1 then isnull(  a.addProductSumRequest  ,  a.[СуммаДопУслуг]  ) end as [СуммаДопУслуг] 
...
,   case when @all_columns=1 then a.productTypeExternal end as productTypeExternal  

,   case when @all_columns=1 then a._pack2PTS end as _pack2PTS  
,	case when @all_columns=1 then a._pack2ProfileSignedPTS end as _pack2ProfileSignedPTS
,   case when @all_columns=1 then a._pack2SignedPTS end as _pack2SignedPTS  
,   case when @all_columns=1 then a.abPhotoUprid end as abPhotoUprid  
,   case when @all_columns=1 then a._upridYes end as _upridYes  
,   case when @all_columns=1 then a.productTypeInitial end as productTypeInitial
,   case when @all_columns = 1 then a.productSubType            end as productSubType
,   case when @all_columns = 1 then a.isSelfEmployed            end as [Признак самозанятый]
,   case when @all_columns = 1 then a._proofOfIncomeLoadedPTS   end as proofOfIncomeLoadedPTS1
,   case when @all_columns = 1 then a.isTakePts   end as isTakePts
,   a.[call03]
,   a.[call03approved] 
,   a.sumNoProofOfIncome [Предложение без подтверждения дохода (сумма)]
,   a.sumProofOfIncome [Предложение с подтверждением дохода (сумма)]
,   a.sumSelfemployed [предложение для СЗ (сумма)]
,   a.needBki [признак с бки/без бки]
,   case a.firstLimitChoice
        when 'withIncomeConfirm'    then N'С подтв. дохода'
        when 'withoutIncomeConfirm' then N'Без подтв. дохода'
        when 'selfEmployed'         then N'Самозанятый'
        else null
    end as firstLimitChoice

,   case a.secondLimitChoice
        when 'withIncomeConfirm'    then N'С подтв. дохода'
        when 'withoutIncomeConfirm' then N'Без подтв. дохода'
        when 'selfEmployed'         then N'Самозанятый'
        else null
    end as secondLimitChoice
,   a.addProductSumRequest [Сумма КП в заявке]
,   a.isAddProductRequest [Признак КП в заявке]

,   case when @all_columns=1 then a.[Признак Защита жизни и здоровья] end as [Признак Защита жизни и здоровья] 
,   case when @all_columns=1 then a.[Признак Безопасность семьи] end as [Признак Безопасность семьи] 

,   case when @all_columns=1 then a.[Сумма Защита жизни и здоровья] end as [Сумма Защита жизни и здоровья] 
,   case when @all_columns=1 then a.[Сумма Безопасность семьи] end as [Сумма Безопасность семьи] 
, a.inn  
, a.innIp 
...
, case when @all_columns=1 then a.[Страхование Жизни сумма расторжения] end as [Страхование Жизни сумма расторжения]
, case when @all_columns=1 then a.[Каско сумма расторжения] end as [Каско сумма расторжения]
, case when @all_columns=1 then a.[Спокойная Жизнь сумма расторжения] end as [Спокойная Жизнь сумма расторжения]
, case when @all_columns=1 then a.[РАТ сумма расторжения] end as [РАТ сумма расторжения]
, case when @all_columns=1 then a.[Помощь Бизнесу сумма расторжения] end as [Помощь Бизнесу сумма расторжения]
, case when @all_columns=1 then a.[Защита от потери работы сумма расторжения] end as [Защита от потери работы сумма расторжения]
, case when @all_columns=1 then a.[Телемедицина сумма расторжения] end as [Телемедицина сумма расторжения]
, case when @all_columns=1 then a.[Фарма сумма расторжения] end as [Фарма сумма расторжения]
, case when @all_columns=1 then a.[Безопасность семьи сумма расторжения] end as [Безопасность семьи сумма расторжения]
, case when @all_columns=1 then a.[Защита жизни и здоровья сумма расторжения] end as [Защита жизни и здоровья сумма расторжения]
, case when @all_columns=1 then a.КоличествоКП end as [КоличествоКП]
, case when @all_columns=1 then a.СписокКП end as [СписокКП]
