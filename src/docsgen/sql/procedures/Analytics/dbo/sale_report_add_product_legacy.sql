...
CREATE     PROC [dbo].[sale_report_add_product_legacy]
@mode nvarchar(max) = '',
...
SELECT  
  ...
  SELECT  
       cast(a.День as date) [Дата]
      ,cast(format(a.День, 'yyyy-MM-01') as date) Месяц
      , [Сумма инстоллмент]  [Займы руб]
      , 1 [Факт/План]
      ,bezzalogAddProductSum [План КП]
      ,bezzalogAddProductSum  [План КП аналитический по дням]
      ,getdate() [created] 
	  , 'inst' productType --(+pdl)
  FROM  sale_plan_view a  
  where case when (select count(*) from #productType where productType in ('inst', 'pdl')) =2 then 1 else 0 end =1
  ...
  SELECT  
       cast(a.День as date) [Дата]
      ,cast(format(a.День, 'yyyy-MM-01') as date) Месяц
      , [Сумма Большой инст]  [Займы руб]
      , 1 [Факт/План]
      ,longInstAddProductSum [План КП]
      ,longInstAddProductSum  [План КП аналитический по дням]
      ,getdate() [created] 
	  , 'big inst' productType
  FROM  sale_plan_view a 
  ...
  SELECT  
       cast(a.День as date) [Дата]
      ,cast(format(a.День, 'yyyy-MM-01') as date) Месяц
      , [Сумма автокредит]  [Займы руб]
      , 1 [Факт/План]
      ,autocreditAddProductSum [План КП]
      ,autocreditAddProductSum  [План КП аналитический по дням]
      ,getdate() [created] 
	  , 'autocredit' productType
  FROM  sale_plan_view a 
...
SELECT [Дата_договора]
  ...
  FROM [dbo].add_product_refuse_view
  where 1=0
...
SELECT [Дата_договора]
  ...
  FROM [dbo].add_product_refuse_view a
  left join v_fa b on a.number=b.number
  where isnull(b.productType, 'pts') in (Select * from #productType)
...
[Сумма Автоспор]              [Сумма Автоспор] ,
 ...
 [Сумма Безопасность семьи]              [Сумма Безопасность семьи] ,
 [Сумма Безопасность семьи Carmoney]    [Сумма Безопасность семьи Carmoney] ,
 [Сумма Безопасность семьи Carmoney net] [Сумма Безопасность семьи Carmoney net] ,
 [Признак Безопасность семьи]           [Признак Безопасность семьи],

   
[Сумма Защита жизни и здоровья]              [Сумма Защита жизни и здоровья] ,
 [Сумма Защита жизни и здоровья Carmoney]    [Сумма Защита жизни и здоровья Carmoney] ,
 [Сумма Защита жизни и здоровья Carmoney net] [Сумма Защита жизни и здоровья Carmoney net] ,
 [Признак Защита жизни и здоровья]           [Признак Защита жизни и здоровья],
...
--where cast( [Заем выдан] as date) >=@start_date_ssrs  and isPts=1
where cast( [Заем выдан] as date) >=@start_date_ssrs  and agr.productType in 
