...
CREATE   proc [_birs].[product_report_product_report_prolongation_conversions_creation]
as
begin
...
select * into #v_prolongation_conversions from v_prolongation
...
-- join mv_loans c on a.number=c.number and c.ispdl=1 
 outer apply (select max(last_prolo.rn) prolongation_number  from #v_prolongation  last_prolo where a.number=last_prolo.number and last_prolo.date<a.date ) x_last_prolo
where
a.ispdl=1 and 
