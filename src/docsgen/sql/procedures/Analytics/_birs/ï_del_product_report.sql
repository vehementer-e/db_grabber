CREATE     proc [_birs].[product_report]			 
@mode nvarchar(max) = 'select'		   , @recreate int =0
--exec [_birs].[product_report] 'update', 1 

 /*
  update 	 product_report_debug_mode set is_debug = 1
 update 	 product_report_debug_mode set is_debug = 0 exec [C2-VSR-BIRS].[RS_Jobs].dbo.StartReportJob  'BEBD9382-4332-4CA8-B572-FBE02423347C'
 */--
...
if @mode  = 'request_external' and (select is_debug from product_report_debug_mode)=0 select  *        from  v_request_external2 


if @mode  = 'request' and (select is_debug from product_report_debug_mode)=1 select top 0   *  from  _request where created>='20230511'
if @mode  = 'request' and (select is_debug from product_report_debug_mode)=0 select  * from  _request where created>='20230511'
...
if @mode  = 'balance_for_plan_fact' and (select is_debug from product_report_debug_mode)=1 select   top 0  newid() row_id, date, returntype, productType, sum(percentsPaid) percentsPaid  from  productReportBalance where percentsPaid <> 0  group by  date, returntype, productType
if @mode  = 'balance_for_plan_fact' and (select is_debug from product_report_debug_mode)=0 select newid() row_id, date, returntype, productType, sum(percentsPaid) percentsPaid  from  productReportBalance where percentsPaid <> 0  group by  date, returntype, productType

if @mode  = 'prolongation' and (select is_debug from product_report_debug_mode)=1 select top 0 *  from  _birs.product_report_prolongation_conversions
if @mode  = 'prolongation' and (select is_debug from product_report_debug_mode)=0 select * from  _birs.product_report_prolongation_conversions
...
if @mode  = 'loan_overdue' and (select is_debug from product_report_debug_mode)=0 select       number, statusContract,	[fpd10], [fpd0],	[fpd4], 	[fpd7],	[fpd30],	[fpd15]   from v_loan_overdue
if @mode  = 'closed' and (select is_debug from product_report_debug_mode)=1 select top 1000 * from productReportClosed
if @mode  = 'closed' and (select is_debug from product_report_debug_mode)=0 select  * from productReportClosed
