...
CREATE PROCEDURE [_birs].[factor_analysis_light_creation] --exec   [_birs].[factor_analysis_light_creation] 1
...
BEGIN
	...
	Declare @dtDateDegin date  = dateadd(year, 2000, getdate()-30)
	Declare @dtDateDegin2000 date  = getdate()-30
...
where  ДатаИзменения>=@dtDateDegin
...
where  UF_ACTUALIZE_AT>=@dtDateDegin2000
...
into #analytics_mfo_channels   --select top 1 *
...
where isnumeric([ПризнакОформленияНовойЗаявки])=1 and  Дата >=@dtDateDegin
...
--select external_id, return_type return_type, 2 as ПриоритетИсточника	   --select top 1 * 
--	from dwh2.dbo.dm_return_type_old_data

insert into #return_types
select cast(number as nvarchar(20)) number, return_type, 1 as ПриоритетИсточника --select top 1 * 
from dwh_new.dbo.risk_apr_segment
where isnumeric(number)=1	  and try_cast( '20'+left(number, 6) as date)>=dateadd(day, -30, @dtDateDegin2000)
insert into #return_types
select номер, 
...
into #requests_with_cp	--select top 1 * 
...
where [Дата изменения]>= @dtDateDegin2000
...
drop table if exists #v_dm_place_of_creation_2
	...
	, case  
			...
			when ras.rbp_gr like '%proba%' then 'Исп. срок'
...
--ВсеЗаявки.ЗаявкиЛид,
	...
	--		, ВсеЗаявки.причинаотказа as [Причина отказа]
		...
		--	, ВсеЗаявки.АдресПроживания as [Регион проживания]
			...
			, p.RBP
		--	, case 
		--	when ВсеЗаявки.[Вид займа] in ('Докредитование', 'Параллельный') and isInstallment=0 then  isnull(c_dip.category_docredy , '') 
		--	when ВсеЗаявки.[Вид займа] in ('Повторный' ) and isInstallment=0 then  isnull(c_dip.category_povt , '') 
...
--select  * into ##t1 from #tt_end
--select  * into ##t2 from  reports.dbo.dm_factor_analysis_001

--select * from 	#tt_end
--where ispts=1
--
--   select distinct [Канал от источника] from 	#tt_end
--   select * from 	#tt_end	 where телефон='9126043829'
--   select * from 	#tt_end
--   where   [exceptions info]='/Изменена точка(UF_STAT_CAMPAIGN по site3_installment_lk)/'
--   order by Номер desc
--select * from   #tt_end

--select * from #tt_end
--where [Признак Рефинансирование]=1
--where isSmartInstallment=1--isInstallment

--select * from #tt_end
--where [Заем выдан] is not null and [Процентная ставка] is null

--
--select Номер, [офис ссылка], [Канал от источника], [Выданная сумма], isInstallment,ДатаЗаявкиПолная, [Номер партнера], Дубль, [Exceptions info], [Заем выдан], z.d  from #tt_end 
--a
--full outer join 
--
--(
--select '22122100624430' d union all
--select '22122720635329' d union all
--select '22121500618065' d union all
--select '22122820636684' d union all
--select '22112520595073' d union all
--select '22120820611808' d union all
--select '22123020639547' d union all
--select '22121900622030' d union all
--select '23010400643376' d union all
--select '22112420592474' d union all
--select '22120400606755' d union all
--select '22120820611850' d union all
--select '22121220615350' d union all
--select '22121620619385' d --union all
--) z on a.Номер=z.d
--where [Канал от источника]='Банки'-- or  [Канал от источника] is null
--order by d, [Заем выдан] 
--
--select distinct  from #tt_end
--where [Юрлицо при создании]  like '%союз%'
--order by [Верификация КЦ]
--
--select * from #analytics_mfo_channels
--where признакоформленияновойзаявки='19020729770002'
--
--select * from #columns_updated
--where  Номер in (
--'19020728120001' ,
--'19020729770002')
...
--'19020728120001' ,
--'19020729770002')
----select * from #tt_end where Телефон='9068822598'
----select * from  cte_fa _001  where Телефон='9068822598'

--select x.Номер, x.[Канал от источника] , x.[Exceptions info], b1.[Канал от источника лид], b.Источник ,b.[Канал от источника], b.[Exceptions info], b.[Место cоздания] from (
--select Номер, [офис ссылка], [Канал от источника], ДатаЗаявкиПолная, [Номер партнера],[Место cоздания],  Дубль, [Exceptions info], телефон   from #tt_end --where [Заем выдан] is not null 
--except
--select Номер, [офис ссылка], [Канал от источника], ДатаЗаявкиПолная, [Номер партнера],[Место cоздания], Дубль, [Exceptions info], телефон   from  reports.dbo.dm_factor_analysis_001
--) x 
--left join  reports.dbo.dm_factor_analysis_001  b on x.Номер=b.Номер
--left join  #tt_end  b1 on x.Номер=b1.Номер
--order by 2,3,1
--order by 4
--select Номер, [офис ссылка], [Канал от источника], ДатаЗаявкиПолная, [Номер партнера]   from #tt_end except
--select Номер, [офис ссылка], [Канал от источника], ДатаЗаявкиПолная, [Номер партнера]   from  cte_fa _001 
--order by 4
--
--select Номер,   Дубль   from #tt_end except
--select Номер,   Дубль   from  cte_fa _001 
--order by 1
--
--
--select Номер, [офис ссылка], [Канал от источника], ДатаЗаявкиПолная, [Номер партнера], Дубль, [Место cоздания], [Exceptions info], [Дата отчета], телефон from #tt_end
--where Номер='23011820663773'
--
--select Номер, [офис ссылка], [Канал от источника], ДатаЗаявкиПолная, [Номер партнера], Дубль, [Место cоздания], [Exceptions info], [Дата отчета], телефон from  cte_fa _001
--where Номер='23011820663773'
--select * from #tt_end
----where [Канал от источника]='Банки'
--where Телефон='9192648387'
--order by ДатаЗаявкиПолная

--select * from #tt_end where [lcrm id] in 
--(
--413155803,
--413077519,
--413033015,
...
--)
--select * from #tt_end
--where номер='22052400393693'
--select Номер, РП from #tt_end except select Номер, РП from dm_factor_analysis_001  where номер='22031600303278'
--select Номер, [канал от источника] from #tt_end except select Номер, [канал от источника] from dm_factor_analysis_001 where номер='19111310000240'
--where номер='22030300284053'
...
--, a.[Верификация КЦ]
--
--from #tt_end a left join  cte_fa _001 b on a.Номер=b.Номер
--left join stg._LCRM.lcrm_leads_full_channel_request r on r.UF_ROW_ID=a.Номер
--where a.[Канал от источника]<>b.[Канал от источника] and a.[Заем выдан] is not null
--order by a.[Верификация КЦ]  desc

--SELECT DISTINCT [Канал от источника] , [Группа каналов] FROM #tt_end

--declare @recreate_table int=1

--select Номер, [Канал от источника] from #tt_end where номер='23020220693655'
--
----except 
--select Номер, [Канал от источника] from dm_factor_analysis_001  where номер='23020220693655'

--select Номер, [офис ссылка] from #tt_end --where номер='22021820258764'
--
--except 
--select Номер, [офис ссылка] from dm_factor_analysis_001 -- where номер='22021820258764'

--select * from stg._1cCRM.Документ_ЗаявкаНаЗаймПодПТС
--where номер='19121310000330'

--select [Канал от источника], [Канал от источника лид], [Exceptions info], [Место cоздания],  * from #tt_end
--where [Канал от источника]<>[Канал от источника лид]
--order by 4, 3
if @recreate_table = 1  

begin

	drop table if exists _birs.[factor_analysis_light_staging]
	select top(0) * 
	into _birs.[factor_analysis_light_staging]
	from #tt_end
	
	drop table if exists _birs.[factor_analysis_light]
	select top(0) * 
	into _birs.[factor_analysis_light]
	from #tt_end

end


drop table if exists _birs.[factor_analysis_light_to_del]
select top(0) * 
into _birs.[factor_analysis_light_to_del]
from #tt_end
...
begin
	...
	delete from  _birs.[factor_analysis_light_staging] with(tablockx)
	insert into  _birs.[factor_analysis_light_staging] with(tablockx)
	SELECT *
	from #tt_end
...
--	select * from  cte_fa _001
	...
	begin tran
		alter table _birs.[factor_analysis_light]
			switch to _birs.[factor_analysis_light_to_del]
		--delete from reports.[dbo].[dm_Factor_Analysis_001]
		alter table  _birs.[factor_analysis_light_staging] 
			switch  to _birs.[factor_analysis_light]
	commit tran
end
