CREATE   proc [_birs].[kpi_report_mail_1] @mode nvarchar(max)
--ass
as
begin
if @mode = 'update'
...
select f.[Верификация КЦ],f.[Заем выдан],f.[Заем погашен],f.ispts,f.[Вид займа],f.[Номер],f.[Группа каналов],f.[Канал от источника],f.[Сумма Дополнительных Услуг Carmoney Net],f.RBP,f.[Первичная сумма],f.[Сумма одобренная],f.[Выданная сумма],f.[Предварительное одобрение],f.[Контроль данных],f.[Одобрено],f.[Отказ документов клиента],f.[Отказано],f.[Место cоздания],f.Дубль into #dm_Factor_Analysis_1 from reports.dbo.dm_Factor_Analysis_001 f with(nolock)
...
into #product_report from [_birs].[product_report_all_actions] c with(nolock) where Дубль2 = 0  and  (	 [Как создан]='ref' 	or  id is not null )  and isnull(Дубль2, 0)<>1	  
and case 
...
drop table if exists _birs.kpi_report_by_date
select * into _birs.kpi_report_by_date from #report
