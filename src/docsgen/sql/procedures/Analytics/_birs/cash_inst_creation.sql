...
CREATE     
      proc [_birs].[cash_inst_creation] @mode nvarchar(100) = 'update'
as 
begin
...
select aa.Номер, [Дата отправки иска в суд], aa.[вид займа], c.[Маркетинговые расходы], c.[Продажа трафика net] into #fa from reports.dbo.dm_Factor_Analysis_001 aa 
left join #sp b on aa.Номер=b.Код
left join [dbo].[v_Отчет стоимость займа опер]	 c on c.Номер=aa.Номер

 
where aa.isPts=0 and aa.[Заем выдан] is not null--<='20220401'
...
SELECT d
	...
	,max(Сумма) Сумма
	,max([Маркетинговые расходы]) [Маркетинговые расходы]
	,max([Продажа трафика net]) [Продажа трафика net]
...
select '2) Второй запуск' t           , '2022-11-01' date_start, getdate() date_end union all
select '3) Total' t         , '2021-12-01' date_start, getdate() date_end  --union all
...
select 1 term, term_type = '6) 1 месяц' union all
select 1 term, term_type   = '1) 1,3,6,9,12 месяцев' union all
select 3 term, term_type   = '1) 1,3,6,9,12 месяцев' union all
select 6 term, term_type   = '1) 1,3,6,9,12 месяцев' union all
select 9 term, term_type   = '1) 1,3,6,9,12 месяцев' union all
select 12 term, term_type =  '1) 1,3,6,9,12 месяцев' --union all
...
, c.Чек
, r.return_type
,case when  cast(format(b.[Дата выдачи], 'yyyy-MM-01') as date)='20220301' then '20220201' else cast(format(b.[Дата выдачи], 'yyyy-MM-01') as date) end [Дата_выдачи_Месяц]
, t.срок 
, t.Сроки_займа 
, b.Код
, b.Сумма
...
--, ROW_NUMBER()               over (partition by Пилот,  [Период_тип],  case when  cast(format(b.[Дата выдачи], 'yyyy-MM-01') as date)='20220301' then '20220201' else cast(format(b.[Дата выдачи], 'yyyy-MM-01') as date) end, Сроки_займа, c.Чек,r.return_type, Код order by (select 1)) rn
--, sum(b.[сумма поступлений]) over (partition by Пилот,  [Период_тип],  case when  cast(format(b.[Дата выдачи], 'yyyy-MM-01') as date)='20220301' then '20220201' else cast(format(b.[Дата выдачи], 'yyyy-MM-01') as date) end, Сроки_займа, c.Чек,r.return_type, Код order by (select 1)) [Сумма поступлений по поколению за все время]
,  b.od
,  b.od_active
into #t3
from #v_calendar a
...
create nonclustered index clus_indname on 	#t3
(
 Пилот,  [Период_тип], [Дата_выдачи_Месяц], Сроки_займа, Чек, return_type, Код
)
...
select *
, ROW_NUMBER()               over (partition by Пилот,  [Период_тип], [Дата_выдачи_Месяц], Сроки_займа, Чек, return_type, Код order by (select 1)) rn
, sum([сумма поступлений]) over (partition by Пилот,  [Период_тип],  [Дата_выдачи_Месяц], Сроки_займа,  Чек, return_type, Код order by (select 1)) [Сумма поступлений по поколению за все время]
...
FROM  v_risk_rep_monit_IL_plan_agg
...
drop table if exists #plans
   ...
   select   Сроки_займа='1) 1,3,6,9,12 месяцев', 
...
SELECT distinct NULL [Дата_выдачи_Месяц]
	...
	,sum(case when Дата= [Дата выдачи] then Сумма end) OVER (
		...
		,sum(case when Дата= [Дата выдачи] then [Маркетинговые расходы] end) OVER (
		PARTITION BY [Период_тип]
		,Пилот
		,Сроки_займа
		,чек
		,return_type ORDER BY Период
		) [Маркетинговые расходы Накопительно] 
		,sum(case when Дата= [Дата выдачи] then [Продажа трафика net] end) OVER (
		PARTITION BY [Период_тип]
		,Пилот
		,Сроки_займа
		,чек
		,return_type ORDER BY Период
		) [Продажа трафика net Накопительно]
		into 	 #volumes_on_mob
FROM #t3_
...
select [Дата_выдачи_Месяц],[Период_тип], Пилот, Сроки_займа, чек, return_type
,  sum(case when rn=1 then Сумма end) Сумма
,  sum(case when rn=1 then [Маркетинговые расходы] end) [Маркетинговые расходы]
,  sum(case when rn=1 then [Продажа трафика net] end) [Продажа трафика net]
, sum(case when rn=1 then [Сумма поступлений по поколению за все время] end) [Сумма поступлений по поколению за все время]  
...
select null [Поколение выдачи] ,[Период_тип], Пилот, Сроки_займа, чек, return_type
,  sum(case when rn=1 then Сумма end) Сумма
,  sum(case when rn=1 then [Маркетинговые расходы] end) [Маркетинговые расходы]
,  sum(case when rn=1 then [Продажа трафика net] end) [Продажа трафика net]
, sum(case when rn=1 then [Сумма поступлений по поколению за все время] end) [Сумма поступлений по поколению за все время]
...
,vom.[Маркетинговые расходы Накопительно]
,vom.[Продажа трафика net Накопительно]
...
into #final from #t4 a

  --select top 100 * from #t2_

--drop table if exists _birs.cash_inst 
--select * into _birs.cash_inst  from #final


--select top 10000 *, [Маркетинговые расходы Накопительно]- lag([Маркетинговые расходы Накопительно]) over(order by Период) from #volumes_on_mob 
--where Пилот='2) Второй запуск'	and Сроки_займа='1) 3,6,9,12 месяцев'	and Чек='3) Total'	 and return_type='Первичный + Повторный'
--order by Период
--where код='23091221184942'
--order by d
--	  select sum([Маркетинговые расходы]) from (
--select distinct Код,[Дата выдачи], сумма, [Маркетинговые расходы] from #t2_
--where [Дата выдачи]between '20221201' and '20221231'	 ) x
--order by 2, 1

--	  select sum([продажа трафика net]) from (
--select distinct Код,[Дата выдачи], сумма, [Маркетинговые расходы] from #t2_
--where [Дата выдачи]between '20221201' and '20221231'	 ) x
--order by 2, 1

 -- select top 10000 *, [продажа трафика net Накопительно]- lag([продажа трафика net Накопительно]) over(order by Период) from #volumes_on_mob 
 -- where Пилот='2) Второй запуск'	and Сроки_займа='1) 3,6,9,12 месяцев'	and Чек='3) Total'	 and return_type='Первичный + Повторный'
 -- order by Период
 -- where код='23091221184942'
 -- order by d
...
delete from  _birs.cash_inst
insert into _birs.cash_inst
select *  from #final
...
update  config set   cash_inst_report_status	 = getdate()
	  ...
	  */
	  ...
	  if cast(getdate() as time) < '11:00'
	  WAITFOR TIME '11:00';     

 exec exec_python 'send_report_cash_inst(debug=0)'	, 1
