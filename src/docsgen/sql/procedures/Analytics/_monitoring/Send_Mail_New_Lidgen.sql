...
-- =============================================
CREATE PROC [_monitoring].[Send_Mail_New_Lidgen]
AS
BEGIN
...
return
	...
	, b.[Тип-Источник]
	, case when b.created is null then a.UF_TYPE+' - '+a.UF_SOURCE else '' end [Тип-Источник для заполнения]
	, CountAll = sum(CountAll)
	, Заявок = sum(Заявок)
	, Займов = sum(Займов)
	into #Result
	from #t_Result_PARTITION  a
	left join stg.files.leadref2_buffer_stg b on a.UF_TYPE+' - '+a.UF_SOURCE=b.[Тип-Источник]
	 
	group by UF_TYPE, UF_SOURCE
	, b.[Тип-Источник]
	, case when b.created is null then a.UF_TYPE+' - '+a.UF_SOURCE else '' end 
...
DECLARE @tableHTML  NVARCHAR(MAX) =
    ...
    N'<table border="1">' +  
    N'<tr><th>UF_TYPE</th><th>UF_SOURCE</th><th>Для добавления в справочник</th>' +  
    N'<th>Количество с 01.01.2020</th>' +  
    N'<th>Заявок</th>' +  
    N'<th>Займов</th></tr>' +  
    CAST ( ( SELECT td = UF_TYPE,       '',  
                    td = UF_SOURCE, '',  
                    td = [Тип-Источник для заполнения] , '',  
                    td = CountAll, '',
                    td = Заявок, '',
                    td = Займов, ''
				from #Result
				order by
				 ...
				 case when UF_SOURCE  like 'ZGl%'	 and len(UF_SOURCE)		   
	> len('	ZGlyZWN0LnlhbmRleC5ydTs3OTkzMDAwMjsxMjk2ODA1MDU3Mzt5YW5kZXgucnU6cHJlbWl1bQ')  -10		    then 1 end ,
				
				[Тип-Источник], Займов desc, UF_TYPE,UF_SOURCE
