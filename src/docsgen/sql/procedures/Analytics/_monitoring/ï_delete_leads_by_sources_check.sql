...
CREATE       proc [_monitoring].[leads_by_sources_check]
...
select checksum(id), source , appmetrica , channel_group , partner_id, cast(created_at_time as date), marketing_status,channel , type    from v_lead
  where
...
select *,  
important_decreading =  case when case
 when  cnt1_pts>10 or cnt0_pts>10 or cnt1_inst>30 or cnt0_inst>30 then 1  
 ...
 and ([Дельта %]<=0 or [Дельта %] is null)	  then 1 else 0 end
   into #f 
...
delete from  [_monitoring].[leads_by_sources]
--drop table dbo.[Отчет мониторинг наличия лидов от лидгенов2]
insert into  [_monitoring].[leads_by_sources]
select *  
from #f
...
analytics._monitoring.leads_by_sources a
where   a.[important_decreading] =1


--exec select_table 'analytics.[_monitoring].[leads_by_sources]'
