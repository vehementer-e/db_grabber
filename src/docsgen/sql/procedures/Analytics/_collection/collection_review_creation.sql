...
CREATE      proc  [_collection].[collection_review_creation]
@mode nvarchar(max) = ''
, @zero_rows int = 1
as
...
begin
  ...
  select код, min( case when [Тип ИЛ]<>'Обеспечительные меры' then isnull( isnull([Дата ИЛ] , [Дата получения ИЛ]), [Дата принятия ИЛ в работу] ) end) [Дата ИЛ учет] 
  , min([Банкрот подтвержденный дата])		 [Банкрот подтвержденный дата]
...
from #ka  a
 ...
 -- into #t2
  from v_balance a
  join #ils b on a.Код=b.Код and a.d >= b.[Дата ИЛ учет]  and a.d<=isnull([Банкрот подтвержденный дата], getdate())
  left join v on v.External_id=a.Код and a.d between st_date and fact_end_date
  )
 -- select * from (
...
with v as(
	...
	, case when c.CustomerId is not null then 'Банкрот' else 'Не банакрот' end [Признак банкрот]
	...
	where a.rn=1 and a.[Дата смерти] is not null
...
if @mode='Портфель умерших клиентов закрытые договора'
begin




drop table if exists #bunkruptcy_confirmed_for_clossed
--select distinct CustomerState   from _collection_CustomerStatus
select distinct CustomerId into #bunkruptcy_confirmed_for_clossed from  [_collection].[CustomerStatus_view]
where CustomerState ='Банкрот подтверждённый' and IsActive=1


	drop table if exists #смерть_for_clossed
	
  select x.[CustomerId]
  , [Дата смерти]
  , [Дата статуса Смерть подтвержденная]
  into #смерть_for_clossed
  from (
  select [CustomerId] [CustomerId]
  , min(Date) [Дата смерти]
  , max(isnull(UpdateDate,createdate) ) [Дата статуса Смерть подтвержденная]
...
--добавляю платежи по умершим 


	drop table if exists #pay_clocced_clients
	...
	when min(d) over(partition by  c.[d Месяц],  a.Код) =  c.[d Месяц] then
			...
			--,a.[isInstallment]
			--,case when a.[Банкрот подтвержденный дата] is not null then 'Банкрот' else 'Не банкрот' end [Признак банкрот]
			,a.[Остаток ОД на сегодня] 
	from _collection.deals_view  a 
	join #смерть_for_clossed b on a.[id клиента]=b.CustomerId and b.[Дата смерти] is not null
	join v_balance c on c.d>=b.[Дата смерти] and a.Код=c.Код
	where a.rn_Договор=1 and b.[Дата смерти] is not null   and a.[Остаток ОД на сегодня] is  null
	)x
	where 1=1 

	group by [id клиента]--[d Месяц]--,isInstallment,[Признак банкрот]
	--order by [d Месяц]
...
with v as(
	...
	,a.*
	from _collection.deals_view  a 
	LEFT join #смерть_for_clossed b on a.[id клиента]=b.CustomerId and b.[Дата смерти] is not null
	left join #bunkruptcy_confirmed_for_clossed c on c.CustomerId=a.[id клиента]
	--where [Дата погашения] is not null
	)
	, v_rn as (
	select ROW_NUMBER() over(partition by [id клиента] order by [Статус судебной работы] desc) rn,* from v
	...
	left join #pay_clocced_clients pcc on a.[id клиента]=pcc.[id клиента]

	where  a.rn=1 and a.[Дата смерти] is not null and [Остаток ОД на сегодня по клиенту] is null
...
begin
  ...
  , max(isnull(UpdateDate,createdate) ) [Дата статуса Смерть подтвержденная]
	...
	--добавил тип продукта и признак банкрота
	,case when [isInstallment]=0 then 'ПТС'else 'Инстолмент' end [Тип продукта]
	,[Признак банкрот]
	from (
 ...
 and 1=@zero_rows
	...
	group by [d Месяц],isInstallment,[Признак банкрот]
	order by [d Месяц]
