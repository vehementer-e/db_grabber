...
-- EXEC [_LCRM].[load_mobile_from_RMQ] @param1 = <value>, @param2 = <value>;
...
truncate table [_LCRM].[lcrm_queue_mobile_update]
...
insert into [_LCRM].[lcrm_queue_mobile_update]
select 
  client.message_guid
, client.[state]
, client.[publishTime]
...
--into [_LCRM].[lcrm_queue_mobile_update]
...
begin tran
	...
	delete from [_LCRM].[lcrm_queue_mobile] 
	where message_guid in (select message_guid from [_LCRM].[lcrm_queue_mobile_update])

	insert into [_LCRM].[lcrm_queue_mobile]
	select * 
	from [_LCRM].[lcrm_queue_mobile_update]

		select Count(* )
	from [_LCRM].[lcrm_queue_mobile_update]
		select count(*) 
	from [_LCRM].[lcrm_queue_mobile]
