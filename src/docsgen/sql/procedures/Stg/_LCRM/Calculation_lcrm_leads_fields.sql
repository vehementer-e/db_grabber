...
CREATE PROC _LCRM.Calculation_lcrm_leads_fields
...
begin try
	...
	IF @Debug = 1 BEGIN
		SELECT 'Удаление более старых записей в #tmp2BE338B9_lcrm_leads_channel', @row_count, cast(getdate() - @StartDate as time(2))
	END
	--// DWH-1964. Удалить более старые записи
	...
	--IF @Debug = 1 BEGIN
	--	SELECT 'CREATE INDEX ON #tmp2BE338B9_lcrm_leads_channel', @row_count, cast(getdate() - @StartDate as time(2))
	--END
	...
	SELECT @row_count = @@ROWCOUNT
	IF @Debug = 1 BEGIN
		SELECT 'INSERT #t_w_chanals0', @row_count, cast(getdate() - @StartDate as time(2))
	END
		...
		[Канал от источника]=
			CASE 
				...
				WHEN F.UF_APPMECA_TRACKER LIKE 'CPA_Mobishark%' 
					...
					OR F.UF_APPMECA_TRACKER LIKE 'CPA_ZumoAds_Carmoney%' --DWH-2226
                    OR F.UF_APPMECA_TRACKER LIKE 'CPA_ZorkaAgency_CarMoney%' --DWH-2460
                    OR F.UF_APPMECA_TRACKER LIKE 'CPA_SravniruMP_CarMoney%' --DWH-2460
                    OR F.UF_APPMECA_TRACKER LIKE 'CPA_Xpartners_CarMoney%' --DWH-2529
					THEN N'CPA целевой'
	...
	SELECT @row_count = @@ROWCOUNT
	IF @Debug = 1 BEGIN
		SELECT 'INSERT #t_w_chanals1', @row_count, cast(getdate() - @StartDate as time(2))
	END

	SELECT @StartDate = getdate(), @row_count = 0
	--CREATE NONCLUSTERED INDEX [nci_row] ON #t_w_chanals1([Канал от источника] ASC)
	IF @Debug = 1 BEGIN
		SELECT 'CREATE INDEX ON #t_w_chanals1', @row_count, cast(getdate() - @StartDate as time(2))
	END
	...
	create clustered index cix_id on #t_w_chanals2(id, UF_REGISTERED_AT)
	...
	SELECT @row_count = @@ROWCOUNT
	IF @Debug = 1 BEGIN
		SELECT 'INSERT #t_w_chanals2', @row_count, cast(getdate() - @StartDate as time(2))
	END


	-------------------------------------------------------------------------------
	-- добавить записи в _LCRM.lcrm_leads_full_channel_request
	...
	FROM #t_w_chanals2 AS F
		INNER JOIN #tmp2BE338B9_lcrm_leads_channel AS C
			ON F.ID = C.ID
		LEFT JOIN _LCRM.lcrm_leads_full_channel_request AS T WITH(NOLOCK) --DWH-1840
			ON F.ID = T.ID
	...
	SELECT @row_count = @@ROWCOUNT
	IF @Debug = 1 BEGIN
		SELECT 'INSERT #lcrm_leads_request', @row_count, cast(getdate() - @StartDate as time(2))
	END

	SELECT @StartDate = getdate(), @row_count = 0
	CREATE NONCLUSTERED INDEX [nci_row] ON #lcrm_leads_request(ID, DWH_HASH)
	IF @Debug = 1 BEGIN
		SELECT 'CREATE INDEX ON #lcrm_leads_request', @row_count, cast(getdate() - @StartDate as time(2))
	END
	...
	BEGIN TRAN
		...
		FROM #lcrm_leads_request AS S WITH(INDEX=nci_row)
			INNER JOIN _LCRM.lcrm_leads_full_channel_request AS T WITH(INDEX=ci_id)
				ON S.ID = T.ID
				AND S.DWH_HASH = T.DWH_HASH

		SELECT @row_count = @@ROWCOUNT
		IF @Debug = 1 BEGIN
			SELECT 'DELETE FROM #lcrm_leads_request', @row_count, cast(getdate() - @StartDate as time(2))
		END
		...
		BEGIN
			...
			#lcrm_leads_request AS S WITH(INDEX=nci_row)
				INNER JOIN _LCRM.lcrm_leads_full_channel_request AS T WITH(INDEX=ci_id) --, readpast) --похоже, что readpast приводит к дублям по ID
					ON S.ID = T.ID
		END

		SELECT @row_count = @@ROWCOUNT
		IF @Debug = 1 BEGIN
			SELECT 'DELETE FROM lcrm_leads_full_channel_request', @row_count, cast(getdate() - @StartDate as time(2))
		END
		...
		SELECT @StartDate = getdate(), @row_count = 0
		INSERT _LCRM.lcrm_leads_full_channel_request
		--with(tablockX)
		(
		    ID,
		    DWHInsertedDate,
		...
		SELECT @row_count = @@ROWCOUNT
		IF @Debug = 1 BEGIN
			SELECT 'INSERT lcrm_leads_full_channel_request', @row_count, cast(getdate() - @StartDate as time(2))
		END
	COMMIT TRAN
	--// добавить записи в _LCRM.lcrm_leads_full_channel_request
	...
	IF @Debug = 1 BEGIN
		SELECT 'CREATE INDEX ON #t_w_chanals2', @row_count, cast(getdate() - @StartDate as time(2))
	END
		...
		SELECT @row_count = @@ROWCOUNT
		IF @Debug = 1 BEGIN
			SELECT 'DELETE FROM #t_w_chanals2', @row_count, cast(getdate() - @StartDate as time(2))
		END
...
/*
			...
			SELECT @row_count = @@ROWCOUNT
			IF @Debug = 1 BEGIN
				SELECT 'MERGE lcrm_leads_full_calculated', @row_count, cast(getdate() - @StartDate as time(2))
			END
*/
			--DWH-2158 Пакетное удаление

			--select @batch_size = 500
		declare cur_calculated_PartitionId cursor for select distinct 
				$Partition.[pfn_range_right_date_part_lcrm_leads_full_calculated](T.UF_REGISTERED_AT)
			from #t_w_chanals2 t
		declare @calculated_PartitionId int
		OPEN cur_calculated_PartitionId  
		FETCH NEXT FROM cur_calculated_PartitionId INTO @calculated_PartitionId  
		...
		BEGIN  
			...
			BEGIN TRAN
				DELETE 
					--TOP (@batch_size) 
					T
				FROM _LCRM.lcrm_leads_full_calculated AS T  	with(INDEX = [CS_IX_leads_full_calculated])
					where   $Partition.[pfn_range_right_date_part_lcrm_leads_full_calculated](T.UF_REGISTERED_AT) = @calculated_PartitionId
					and EXISTS(sELECT TOP(1) 1
						FROM #t_w_chanals2 AS S-- WITH(INDEX = nci_row)
					WHERE S.ID = T.ID
					and  $Partition.[pfn_range_right_date_part_lcrm_leads_full_calculated](s.UF_REGISTERED_AT) = @calculated_PartitionId
							)
			...
			SELECT @row_count = @@ROWCOUNT
			IF @Debug = 1 BEGIN
				SELECT 'DELETE FROM lcrm_leads_full_calculated', calculated_PartitionId = @calculated_PartitionId, row_count = @row_count, duration = cast(getdate() - @StartDate as time(2))
			END
			...
			SELECT 
				...
				DWHUpdatedDate = getdate()
			FROM #t_w_chanals2 AS F
			where $Partition.[pfn_range_right_date_part_lcrm_leads_full_calculated](f.UF_REGISTERED_AT) = @calculated_PartitionId
			OPTION(USE HINT('ENABLE_PARALLEL_PLAN_PREFERENCE'))

			SELECT @row_count = @@ROWCOUNT
			IF @Debug = 1 BEGIN
				SELECT 'INSERT lcrm_leads_full_calculated',calculated_PartitionId = @calculated_PartitionId, row_count = @row_count, duration = cast(getdate() - @StartDate as time(2))
			END
			COMMIT TRAN
			FETCH NEXT FROM cur_calculated_PartitionId INTO @calculated_PartitionId  
		end --end while
		CLOSE cur_calculated_PartitionId;  
		DEALLOCATE cur_calculated_PartitionId
		END --end if
	...
	BEGIN
		...
		declare cur_partition_leads_full cursor for 
			select distinct PartitionId = $partition.pfn_range_right_date_part_lcrm_leads_full(s.UF_REGISTERED_AT)
			from #tmp2BE338B9_lcrm_leads_channel  s
		declare @leads_full_PartitionId int 
		OPEN cur_partition_leads_full  
			FETCH NEXT FROM cur_partition_leads_full INTO @leads_full_PartitionId  
		WHILE @@FETCH_STATUS = 0  
		BEGIN  
			BEGIN TRAN
				SELECT @StartDate = getdate(), @row_count = 0
				...
				DELETE T
				FROM _LCRM.lcrm_leads_full AS T --! WITH(INDEX=cix_id)
				where $partition.pfn_range_right_date_part_lcrm_leads_full(T.UF_REGISTERED_AT) = @leads_full_PartitionId
				and exists(select top(1) 1 from #tmp2BE338B9_lcrm_leads_channel AS S --! WITH(INDEX=CI_ID)
					where
						S.ID = T.ID
						AND S.UF_REGISTERED_AT = T.UF_REGISTERED_AT
						--and $partition.pfn_range_right_date_part_lcrm_leads_full(S.UF_REGISTERED_AT) = 
						--	$partition.pfn_range_right_date_part_lcrm_leads_full(T.UF_REGISTERED_AT)
						)	
				OPTION(Recompile, QUERYTRACEON 610)

				SELECT @row_count = @@ROWCOUNT
				IF @Debug = 1 BEGIN
					SELECT 'DELETE FROM lcrm_leads_full', leads_full_PartitionId = @leads_full_PartitionId, row_count = @row_count, duration = cast(getdate() - @StartDate as time(2))
				END
			...
			--	with(tablockX)
				...
				SELECT
					...
					UF_CLIENT_ID
				FROM #tmp2BE338B9_lcrm_leads_channel AS F
				where $partition.pfn_range_right_date_part_lcrm_leads_full(F.UF_REGISTERED_AT) =  @leads_full_PartitionId
				
				OPTION(USE HINT('ENABLE_PARALLEL_PLAN_PREFERENCE'))

				SELECT @row_count = @@ROWCOUNT
				IF @Debug = 1 BEGIN
					SELECT 'INSERT lcrm_leads_full', leads_full_PartitionId = @leads_full_PartitionId, row_count = @row_count, duration = cast(getdate() - @StartDate as time(2))
				END
			COMMIT TRAN
		FETCH NEXT FROM cur_partition_leads_full INTO @leads_full_PartitionId  
		end 
		
		CLOSE cur_partition_leads_full;  
		DEALLOCATE cur_partition_leads_full
