...
begin try
	...
	*/
	...
	(
		[Caller] nvarchar (11),
		МобильныйТелефон nvarchar (50),
		Cmclient int,
		CRMClientGUID nvarchar (64),
		CRMRequestGUID nvarchar (64),
		CRMRequestID nvarchar (14),
		fio nvarchar (150),
		RequestType varchar (7),
		CRMRequestDate datetime,
		isActive bit,
		hasContract bit,
		hasRejection bit,
		CRMRequestsLastStatus nvarchar (255),
		CRMRequest_RowVersion binary (8),
		Legal int,
		ExecutionOrder int,
		[Type] nvarchar (255),
		IVRDate date,
	...
	BEGIN TRAN
		...
		THEN UPDATE 
		SET
			[Caller] = isnull(s.[Caller], t.[Caller])
			, МобильныйТелефон = isnull(s.МобильныйТелефон, t.МобильныйТелефон)
			, [Cmclient] = isnull(s.[Cmclient], t.[Cmclient])
			, [CRMClientGUID] = isnull(s.[CRMClientGUID], t.[CRMClientGUID])
			--, [CRMRequestGUID] = isnull(s.[CRMRequestGUID], t.[CRMRequestGUID])
			, [CRMRequestID] = isnull(s.[CRMRequestID], t.[CRMRequestID])
			, [fio] = isnull(s.[fio], t.[fio])
			, [RequestType] = isnull(s.[RequestType], t.[RequestType])
			, [CRMRequestDate] = isnull(s.[CRMRequestDate], t.[CRMRequestDate])
			, [isActive] = isnull(s.[isActive], t.[isActive])
			, [hasContract] = isnull(s.[hasContract], t.[hasContract])
			, [hasRejection] = isnull(s.[hasRejection], t.[hasRejection])
			, [CRMRequestsLastStatus] = isnull(s.[CRMRequestsLastStatus], t.[CRMRequestsLastStatus])
			, [CRMRequest_RowVersion] = isnull(s.[CRMRequest_RowVersion], t.[CRMRequest_RowVersion])
			, Legal = isnull(s.Legal, t.Legal)
			, ExecutionOrder = isnull(s.ExecutionOrder, t.ExecutionOrder)
			--, [Type]
			, [IVRDate] = isnull(s.[IVRDate], t.[IVRDate])
			--, [created] = s.[created]
			, [updated] = isnull(s.[updated], t.[updated])
			, [isHistory] = isnull(s.[isHistory], t.[isHistory])
			--, productType = isnull(s.productType, t.productType)
			, productType = s.productType
			, salesstage = isnull(s.salesstage, t.salesstage)

		OUTPUT 
			Inserted.Caller
			, Inserted.МобильныйТелефон
			, Inserted.[Cmclient]
			, Inserted.[CRMClientGUID] 
			, Inserted.[CRMRequestGUID]
			, Inserted.[CRMRequestID]
			, Inserted.[fio]
			, Inserted.[RequestType]
			, Inserted.[CRMRequestDate]
			, Inserted.[isActive]
			, Inserted.[hasContract]
			, Inserted.[hasRejection]
			, Inserted.[CRMRequestsLastStatus]
			, Inserted.[CRMRequest_RowVersion]
			, Inserted.Legal
			, Inserted.ExecutionOrder
			, Inserted.[Type]
			, Inserted.[IVRDate] 
			, Inserted.[created] 
			, Inserted.[updated]
