...
begin
	...
	select *
		into #t_data
	from (
	select distinct TRIM(UF_PHONE) as PHONE
	...
	if OBJECT_ID('dbo.UnBlockPhone_uat') is null
	begin
		select top(0) *
			into dbo.UnBlockPhone_uat
		from #t_data
	end
	begin try
	begin tran
	delete from dbo.UnBlockPhone_uat
	insert into dbo.UnBlockPhone_uat
