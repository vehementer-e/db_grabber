...
-- EXEC [_yandexmetrica].[CustomReport_postloader_pd] @param1 = <value>, @param2 = <value>;
-- Список и типы параметров смотрите в объявлении процедуры ниже.
CREATE     procedure  [_yandexmetrica].[CustomReport_postloader_pd]
...
*/
    exec logdb.dbo.[LogAndSendMailToAdmin] 'CustomReport_postloader_pd','Info','procedure started',@guid
if @guid is not null
begin
 ...
 rn,
      ...
      ,[ReportDateTo]
     -- ,
 from _yandexmetrica._reportByRow 
   where ReportName      = @ReportName
    and ReportDateFrom  = @ReportDateFrom
    and ReportDateTo    = @ReportDateTo
    and ReportLoadGuid=@guid

    declare @min_dt_of_report datetime
    select @min_dt_of_report= min(reportLoadedAt)
    from  _yandexmetrica._reportByRow 
    where  ReportLoadGuid=@guid
   


    delete from  _yandexmetrica._reportByRow 
   where ReportName      = @ReportName
    and ReportDateFrom  = @ReportDateFrom
    and ReportDateTo    = @ReportDateTo
    and( ReportLoadGuid<>@guid and isnull(ReportLoadedAt,'20200720')< @min_dt_of_report)
...
--drop table if exists [_yandexmetrica].CustomReport
 ...
 begin tran
  ...
  from #dm
  commit tran
   exec logdb.dbo.[LogAndSendMailToAdmin] 'CustomReport_postloader_pd','Info','procedure finished',@guid
  --select * from reports.dbo.dm_yandexCustomReport where ReportDateFrom='20200723'
end
