...
-- EXEC [RMQ].[SaveReceivedMessage_uat]
--      @FromHost = <value>,
...
--OLD:('fedor.requests.2.1','dwh.fedor.requests.2.1','*','RMQ.ReceivedMessages_fedor_request_uat', 'RMQ.SaveReceivedMessage_fedor_request_uat'),
('fedor.requests.2.1','dwh.ivr.requests','*','RMQ.ReceivedMessages_ivr_fedor_request_uat', 'RMQ.SaveReceivedMessage_ivr_fedor_request_uat'),
('interactions','dwh.ivr.requests_event','*','RMQ.ReceivedMessages_ivr_interactions_event_uat', 'RMQ.SaveReceivedMessage_ivr_interactions_event_uat'),

--default table
('*','*','*','RMQ.ReceivedMessages_uat', null)

) t(FromExchange, FromQueue, FromQueueRoutingKey, ReceivedMessages_table,  ReceivedMessages_procedure)
--определение таблицы
SELECT @ReceivedMessages_table = isnull(A.ReceivedMessages_table, 'RMQ.ReceivedMessages_uat')
