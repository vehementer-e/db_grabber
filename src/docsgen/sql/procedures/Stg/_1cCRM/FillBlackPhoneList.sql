...
begin try
	...
	DELETE rm 
	FROM rmq.ReceivedMessages_CRM_Interaction AS rm
	...
	if OBJECT_ID('_1cCRM.BlackPhoneList') is null
	begin
		--drop table if exists _1cCRM.BlackPhoneList
		select top(0)
		*
		into _1cCRM.BlackPhoneList
		from #t_blackPhoneList

	end
	if exists (select top(1) 1 from #t_blackPhoneList)
	begin
		insert into _1cCRM.BlackPhoneList([ReasonAdding_subject], [ReasonAdding_subjectGuid], [ReasonAdding_Comment], [Phone], [create_at])

		select [ReasonAdding_subject], [ReasonAdding_subjectGuid], [ReasonAdding_Comment], [Phone], [create_at] from #t_blackPhoneList s
		where not exists(select top(1) 1 from _1cCRM.BlackPhoneList t where t.Phone = s.Phone
		...
		inner join _1cCRM.BlackPhoneList t on  t.Phone = s.Phone
			and t.ReasonAdding_subjectGuid = s.ReasonAdding_subjectGuid
