...
-- EXEC [_1cCRM].RMQ_CRM_Monitoring_Loader @param1 = <value>, @param2 = <value>;
...
select * 
into #t_messages
from RMQ.ReceivedMessages_CRM_Requests RM
where  
RM.[ReceiveDate] > dateadd(hour, 21, cast(dateadd(day,-3,cast(getdate() as date)) as datetime2) ) 
and 
rm.FromQueue = 'dwh.crm.CRM.Requests_Monitoring.1.1'
...
select distinct rs.[ReceiveDate], rs.publishTime, rs.guid, rs.description, rs.code, number, guidrequest, daterequest, datestuatusrequest,  lastName	        
	...
	,residentialAddress 
into #t_result
from (select [ReceiveDate], publishTime, guid , description, code  from #t where type = 'requestStatus') as rs
...
inner join (select 	publishTime, guid 	,locationOfBirth
	,placeOfIssue
	,dateOfIssue
	,departmentCode
	,registrationAddres
...
begin tran
	...
	delete 
	--select * 
	from [_1cCRM].[RMQ_CRM_Monitoring] 
	where guid in (select guid from #t_result)

	insert into [_1cCRM].[RMQ_CRM_Monitoring] 
