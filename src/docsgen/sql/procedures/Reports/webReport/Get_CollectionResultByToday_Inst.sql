...
CREATE     procedure [webReport].[Get_CollectionResultByToday_Inst]
 WITH EXECUTE AS 'dbo'
as
begin
SET NOCOUNT ON;
begin try
	...
	,[Order] = case MetricName
		...
		when '31-60' then 2
		when '61-90' then 3
		when 'Hard 0-90' then 4
		when '91-360' then 5
		when '361+' then 6
		when 'Итого 1-90' then 7
	...
	(select 
		...
		--,[Итого 1-90]=  cast(t2_5_3 as money) --[Sum(t2_6_3)]
		--,[Итого 91+] = cast(t2_7_3 as money) --[Sum(t2_7_3)]
		,[Итого 1+] = cast(t2_8_3 as money) --[Sum(t2_8_3)]

	from dbo.dm_dashboard_Collection_Installment_v02_new_save_balance
	) t
	UNPIVOT   
