...
-- =============================================
CREATE PROC dbo.ReportDashboard_Collection_Task_IP
as
begin
...
into #ttask_calend
 ...
 --where [Статус задачи] = 'Отменена'
  ...
  --DWH-1764 
  TRUNCATE TABLE dbo.dm_CollectionEnforcementProceeding_Tasks

  INSERT dbo.dm_CollectionEnforcementProceeding_Tasks
  (
      dt,
      Неделя,
      НачалоНедели,
  ...
  Select 
      ...
      [Длительность выполнения задачи],
      [Отличия от плана]
  --into dbo.dm_CollectionEnforcementProceeding_Tasks
  from #ttask_calend
