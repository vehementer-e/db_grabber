...
CREATE  PROCEDURE  [dbo].[report_RequestsOnStatuses_fedor_UAT] 
AS
BEGIN
	SET NOCOUNT ON;
...
select t.[Период_Исх] as [ДатаСтатуса]
      ...
      --,[ЗаявкаДата_Исх]
	  ,'' as [ФИО]
	  --,0 as [ЗапрошеннаяСумма]
	  --,0 as [Сумма]
      ,t.[ИсполнительНаим_След] as [Сотрудник]
      ,t.[СтатусДляСостояния] as [СтатусНаим_Исх]
      --,[СтатусСсылка_Исх]
	  --,z.[Канал]
	  --,z.[Канал2]
	  --,'' as [Канал]
	  ...
	  ,cast(t.[Период_Исх] as datetime) as [Период_Исх]
	  ,cast(isnull(t.[Период_След],getdate()) as datetime) as [Период_След]

	   ,cast(cast(isnull(t.[Период_След],getdate()) as datetime) as decimal(15,10)) - cast(cast(t.[Период_Исх] as datetime) as decimal(15,10)) as [ВремяЗатрачено]
	   ,convert(nvarchar,cast(cast(cast(isnull(t.[Период_След],getdate()) as datetime) as decimal(15,10)) - cast(cast(t.[Период_Исх] as datetime) as decimal(15,10)) as datetime) ,8) as [Продолжительность]
	   ,t.[Период_След] as [ДатаИзменСтатуса]
 
from [dwh_new].[dbo].[mt_requests_transition_fedor_UAT]  t  with (nolock)
--left join (select [ЗаявкаСсылка] ,[ЗаявкаНомер] ,([Фамилия]+' '+[Имя]+' '+[Отчество]) as [ФИО] ,[ПервичнаяСумма] as [ЗапрошеннаяСумма] ,[СуммаЗаявки] 
...
--		   ) z
--on t.[ЗаявкаНомер_Исх]=z.[ЗаявкаНомер]
