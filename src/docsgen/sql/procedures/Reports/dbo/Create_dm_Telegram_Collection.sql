-- exec [dbo].[Create_dm_Telegram_Collection]
CREATE PROC [dbo].[Create_dm_Telegram_Collection]
AS
BEGIN
	SET NOCOUNT ON;
...
-----------------------------------------------------------------------------
  ...
  exec [dbo].[Create_dm_Telegram_Collection_part1] @dt
  -- обновим данные за вчера на случай сбоев системы или учет данных на вечер
 declare  @dt_yesterday date = cast(dateadd(day,-1, getdate()) as date)
 exec [dbo].[Create_dm_Telegram_Collection_part1] @dt_yesterday


-----------------------------------------------------------------------------
--- второй альтернативный расчет на новом алгоритме -------------------------
-----------------------------------------------------------------------------
 --exec [dbo].[Create_dm_Telegram_Collection_part3] @dt
 --exec [dbo].[Create_dm_Telegram_Collection_part3] @dt_yesterday
 EXEC dbo.Create_dm_Telegram_Collection_part3 @DateBegin = @dt_yesterday, @DateEnd = @dt

-------------------------------------------------------------------------------
--- третий альтернативный расчет на новом алгоритме  с учетом перехода 1->0 ---
-------------------------------------------------------------------------------
 exec [dbo].[Create_dm_Telegram_Collection_part3_Alternative] @dt
 exec [dbo].[Create_dm_Telegram_Collection_part3_Alternative] @dt_yesterday
...
CREATE TABLE #Baket(
 ...
 from  [dbo].[dm_CMRStatBalance_2]  balance (nolock) where balance.d = @dt
  ...
  FROM
  (
    select ([основной долг уплачено]) sum_OD, isnull([сумма поступлений],0) sum_all, [Проценты уплачено] sum_procent, ПениУплачено sum_penalty, ПереплатаУплачено sum_over
	 ,case
...
declare  @dt_yesterday date = cast(dateadd(day,-1, getdate()) as date)
  ...
  -- Временная таблица для исключения блокировки в транзакции
 if object_id('tempdb.dbo.#balance_yesterday') is not null drop table #balance_yesterday

 select * into #balance_yesterday
 from  [dbo].[dm_CMRStatBalance_2]  balance (nolock) where balance.d = @dt_yesterday

begin tran

  delete from [dbo].[dm_Telegram_Collection] where Период=@dt_yesterday

  insert into [dbo].[dm_Telegram_Collection] (Период, [Платежи по ОД], [Сумма поступлений], [Проценты], [Пени], [Бакет просрочки])
  Select @dt_yesterday 'Период', Sum(isnull(sum_OD,0)) 'Платежи по ОД', Sum(isnull(sum_all,0)) [Сумма поступлений], Sum(isnull(sum_procent,0)) [Проценты], Sum(isnull(sum_penalty,0)) [Пени], bkt.baket "Бакет просрочки"
  FROM
  (
    select ([основной долг уплачено]) sum_OD, isnull([сумма поступлений],0)  sum_all, [Проценты уплачено] sum_procent, ПениУплачено sum_penalty, ПереплатаУплачено sum_over
	 ,case
