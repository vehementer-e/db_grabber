--select * from  dbo.dm_FedorVerificationRequestsUAT_PDL
-- exec dbo.Create_dm_FedorVerificationRequestsUAT_PDL 366
CREATE   PROC dbo.Create_dm_FedorVerificationRequestsUAT_PDL
  ...
  select cr.* 
    ...
    from stg.[_fedorUAT].[core_ClientRequest] cr 
		INNER JOIN stg.[_fedorUAT].dictionary_ProductType AS T
			...
			AND T.Code = 'pdl'
	WHERE 1=1 --CreatedOn > '2023-11-10' --'20200902' -- дата старта ФЕДОР в проде
     and   dateadd(hour,3,[CreatedOn]) 				  >dateadd(day,-@days,cast(getdate() as date))
	 --AND cr.Type = 5 --PDL

	--TEST !!! удалить после внедрения продукта PDL !!!
	--INSERT #core_ClientRequest
	--SELECT TOP 1000 * 
	--from Stg.[_fedorUAT].[core_ClientRequest] AS cr
	--WHERE 1=1
...
--drop table if exists  dbo.dm_FedorVerificationRequestsUAT_PDL
begin tran
	TRUNCATE TABLE dbo.dm_FedorVerificationRequestsUAT_PDL_STG

	INSERT dbo.dm_FedorVerificationRequestsUAT_PDL_STG
	SELECT *
	FROM dbo.dm_FedorVerificationRequestsUAT_PDL (NOLOCK)

  --delete from  dbo.dm_FedorVerificationRequestsUAT_PDL where   [Дата заведения заявки]						  >=dateadd(day,-@days,cast(getdate() as date))
  DELETE
  FROM dbo.dm_FedorVerificationRequestsUAT_PDL_STG
  WHERE [Дата заведения заявки] >=dateadd(day,-@days,cast(getdate() as date))

  --insert into  dbo.dm_FedorVerificationRequestsUAT_PDL
  insert into  dbo.dm_FedorVerificationRequestsUAT_PDL_STG
  --declare @now datetime=getdate()
 ...
 -- , [Задача предыдущая]
  , iif(isnull(ПризнакИсключенияСотрудника,0) = 1 
	...
	, [Назначен_След]	
		...
		, ТипКлиента
		, IsSkipped
--into dbo.dm_FedorVerificationRequestsUAT_PDL
  ...
  select q.[Дата заведения заявки]
   ...
   --
	...
	--delete 
	----select *
	--from  dbo.dm_FedorVerificationRequestsUAT_PDL
	--where 
...
commit tran
	...
	if OBJECT_ID('dbo.dm_FedorVerificationRequestsUAT_PDL_to_del') is null
	begin
		select top(0)
		*
		into dbo.dm_FedorVerificationRequestsUAT_PDL_to_del
		from dbo.dm_FedorVerificationRequestsUAT_PDL_STG

		CREATE NONCLUSTERED INDEX [ix_Статус_Задача_Дата_статуса] 
		ON dbo.dm_FedorVerificationRequestsUAT_PDL_to_del (
	...
	IF (SELECT count(*) FROM dbo.dm_FedorVerificationRequestsUAT_PDL_STG) > 0
	begin
		
		truncate table dbo.dm_FedorVerificationRequestsUAT_PDL_to_del
		--DWH-2050 Оптимизировать обновление данных в отчете верификации
		BEGIN TRAN
			alter table dbo.dm_FedorVerificationRequestsUAT_PDL
			SWITCH to dbo.dm_FedorVerificationRequestsUAT_PDL_to_del
			with (WAIT_AT_LOW_PRIORITY  ( MAX_DURATION = 1 minutes, ABORT_AFTER_WAIT = SELF ))

			alter table dbo.dm_FedorVerificationRequestsUAT_PDL_STG
			SWITCH to dbo.dm_FedorVerificationRequestsUAT_PDL
			with (WAIT_AT_LOW_PRIORITY  ( MAX_DURATION = 1 minutes, ABORT_AFTER_WAIT = SELF  ))
		COMMIT TRAN
	END
