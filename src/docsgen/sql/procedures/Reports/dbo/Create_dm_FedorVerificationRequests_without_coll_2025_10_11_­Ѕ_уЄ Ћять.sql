...
create   PROC dbo.Create_dm_FedorVerificationRequests_without_coll_2025_10_11_не_удалять
...
--drop table if exists  dbo.dm_FedorVerificationRequests_without_coll
begin tran
  ...
  --var 1
  /*
  --delete from  dbo.dm_FedorVerificationRequests_without_coll where   [Дата заведения заявки]						  >=dateadd(day,-@days,cast(getdate() as date))
  DELETE
  FROM dbo.dm_FedorVerificationRequests_without_coll_STG
  WHERE [Дата заведения заявки] >=dateadd(day,-@days,cast(getdate() as date))

  --insert into  dbo.dm_FedorVerificationRequests_without_coll
  insert into  dbo.dm_FedorVerificationRequests_without_coll_STG
  --declare @now datetime=getdate()
 ...
 -- , [Задача предыдущая]
  , iif(isnull(ПризнакИсключенияСотрудника,0) = 1 
	...
	, [Назначен_След]	
		...
		, ТипКлиента
		, IsSkipped
--into dbo.dm_FedorVerificationRequests_without_coll
  ...
  select q.[Дата заведения заявки]
	...
	*/
	...
	ON #t_dm_FedorVerificationRequests([Номер заявки])
	...
	WHERE [Дата заведения заявки] >=dateadd(day,-@days,cast(getdate() as date))
		AND NOT EXISTS(
			SELECT TOP(1) 1
			FROM #t_dm_FedorVerificationRequests AS N
			WHERE N.[Номер заявки] = D.[Номер заявки]
		)

	/*
	--3. удалить только те заявки, которые будут добавлены из нового расчета
	DELETE D
	FROM dbo.dm_FedorVerificationRequests_without_coll_STG AS D
	WHERE [Дата заведения заявки] >=dateadd(day,-@days,cast(getdate() as date))
		AND EXISTS(
	...
	*/
	...
	--3. удалить заявки в витрине за расчетный период
	DELETE D
	FROM dbo.dm_FedorVerificationRequests_without_coll_STG AS D
	WHERE [Дата заведения заявки] >=dateadd(day,-@days,cast(getdate() as date))
	...
	INSERT dbo.dm_FedorVerificationRequests_without_coll_STG
	SELECT * FROM #t_dm_FedorVerificationRequests AS T
