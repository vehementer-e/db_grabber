...
--             exec [dbo].[ReportDashboard_Collection_Installment_v02_part3]   --1
-- =============================================
CREATE PROC [dbo].[ReportDashboard_Collection_Installment_v02_part3]
	@DateCalculate date	
AS
...
delete from [dbo].[dm_CollectionSavedBalance_Installment]
...
join stg._1cCMR.Справочник_Договоры d on d.Код = b.external_id
...
--and d.IsInstallment = 1
and lower(cmr_ТипыПродуктов.ИдентификаторMDS) = 'installment'
--and b.[Тип Продукта] IN ('Installment', 'Смарт-инстоллмент')
...
------------------------------
	...
	inner join [Stg].[_1cCMR].[Справочник_типыПродуктов] cmr_ТипыПродуктов
			on d.ТипПродукта = cmr_ТипыПродуктов.ссылка	
	where  cmr_ТипыПродуктов.Наименование = 'Installment'

	--select * from [Stg].[_1cCMR].[Справочник_типыПродуктов]
	--
...
-- 19_05_2020 - оставим только данные из CMR stat balance и данные за сегодня (с учетом перехода 1-0)
 ----- для сторонних дашбоардов
 insert into [dbo].[dm_CollectionSavedBalance_Installment]
 ([Дата входа в стадию]
      ,[Корзина Просрочки]
...
--into [dbo].[dm_CollectionSavedBalance_test_2020_05_19]
 ...
 Declare @currentDate datetime = GetDAte()
  ...
  insert into [dbo].[dm_CollectionSavedBalance_Installment_Monitoring]
 ([Тип таблицы],
 [Время записи],
 [Дата входа в стадию]
      ,[Корзина Просрочки]
