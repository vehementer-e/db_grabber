...
create   PROC dbo.CreateDasboardFedorVerification_without_coll_2025_10_27_не_удалять
	@isDebug int = 0,
	@date date = NULL
as
begin
...
-- Верификаторы
  ...
  create table #curr_employee_vr([Employee] nvarchar(255) )
	...
	*/
	...
	select * 
	into #details_KD 
	from dbo.dm_FedorVerificationRequests_Installment  --where [Номер заявки]='20092400036174'
	...
	WHERE ([ФИО сотрудника верификации/чекер] not in (select * from #curr_employee_vr) 
		...
		and( [Дата статуса]>@dt_from and  [Дата статуса]<@dt_to) --or (Статус='Контроль данных' and [Состояние заявки]='Отложена')
	*/
	...
	FROM dbo.dm_FedorVerificationRequests_Installment AS R (NOLOCK)
	...
	SELECT 
		...
		R.ТипКлиента,
		R.isSkipped
	FROM dbo.dm_FedorVerificationRequests_PDL AS R (NOLOCK)
		INNER JOIN Stg._fedor.core_ClientRequest AS CR
			ON CR.Number COLLATE Cyrillic_General_CI_AS = R.[Номер заявки]
	WHERE 1=1
		AND ([ФИО сотрудника верификации/чекер] not in (select * from #curr_employee_vr) 
	...
	*/
	...
	--- количество Доработка и Отложенно, отправленных сегодня LastStage=1
         ...
         ;with 
          rework as (
          select ProductType_Code, 'Доработка' [status], cast([Дата статуса] as date) Дата, [Дата статуса], [ФИО клиента], [Номер заявки] , Сотрудник=СотрудникПоследнегоСтатуса, [ФИО сотрудника верификации/чекер], ВремяЗатрачено, [Время, час:мин:сек]  , 0 LastStage
            from #details_KD
           where Задача='task:Требуется доработка' and [Состояние заявки]='Отложена' and Статус in('Контроль данных') 
          )
          ,rework1 as (
          select ProductType_Code, 'Доработка' [status], cast([Дата статуса] as date) Дата, [Дата статуса], [ФИО клиента], [Номер заявки] , Сотрудник=СотрудникПоследнегоСтатуса, [ФИО сотрудника верификации/чекер], ВремяЗатрачено, [Время, час:мин:сек]  , 1 LastStage
            from #details_KD
          ...
          ,postpone as (
          select ProductType_Code, 'Отложена' [status], cast([Дата статуса] as date) Дата, [Дата статуса] ДатаИВремяСтатуса, [ФИО клиента], [Номер заявки]  , Сотрудник=СотрудникПоследнегоСтатуса, [ФИО сотрудника верификации/чекер], ВремяЗатрачено, [Время, час:мин:сек]  , 0 LastStage
            from #details_KD
           where Задача='task:Отложена' and [Состояние заявки] in('Отложена') and Статус in('Контроль данных')
           )
           ,postpone1 as (
			SELECT ProductType_Code, 'Отложена' [status], cast([Дата статуса] as date) Дата, [Дата статуса] ДатаИВремяСтатуса, [ФИО клиента], [Номер заявки]  , Сотрудник=СотрудникПоследнегоСтатуса, [ФИО сотрудника верификации/чекер], ВремяЗатрачено, [Время, час:мин:сек]  , 1 LastStage
            from #details_KD
           ...
           select N.ProductType_Code, 'Отказано' [status], cast(N.[Дата статуса] as date) Дата, N.[Дата статуса] ДатаИВремяСтатуса, N.[ФИО клиента], N.[Номер заявки], Сотрудник = N.СотрудникПоследнегоСтатуса, N.[ФИО сотрудника верификации/чекер], N.ВремяЗатрачено, N.[Время, час:мин:сек], 0 LastStage
		...
		*/
		...
		--var 2
		UNION
		select A.ProductType_Code, 'ВК' [status], cast(A.[Дата статуса] as date) Дата, A.[Дата статуса] ДатаИВремяСтатуса, A.[ФИО клиента], A.[Номер заявки] , A.СотрудникПоследнегоСтатуса, A.[ФИО сотрудника верификации/чекер], A.ВремяЗатрачено, A.[Время, час:мин:сек], 0 LastStage
		...
		where (
           ...
           union
          select ProductType_Code, 'Новая' [status], cast([Дата статуса] as date) Дата, [Дата статуса] ДатаИВремяСтатуса, [ФИО клиента], [Номер заявки] , СотрудникПоследнегоСтатуса, [ФИО сотрудника верификации/чекер], ВремяЗатрачено, [Время, час:мин:сек]  , 0 LastStage 
            from #details_KD
           where Задача='task:Новая' and Статус in('Контроль данных')
				AND [Задача следующая] <> 'task:Автоматически отложено'
           union 
          select ProductType_Code, 'task:В работе' [status], cast([Дата статуса] as date) Дата, [Дата статуса] ДатаИВремяСтатуса, [ФИО клиента], [Номер заявки] , СотрудникПоследнегоСтатуса, [ФИО сотрудника верификации/чекер], ВремяЗатрачено, [Время, час:мин:сек]  , 0 LastStage
            from #details_KD
           where [Задача]='task:В работе'  and Статус in('Контроль данных')
           union 
           select ProductType_Code, 'Ожидание' [status], cast([Дата статуса] as date) Дата, [Дата статуса] ДатаИВремяСтатуса, [ФИО клиента], [Номер заявки] , СотрудникПоследнегоСтатуса, [ФИО сотрудника верификации/чекер], ВремяЗатрачено, [Время, час:мин:сек]  , 0 LastStage
            from #details_KD
          where Задача in ('task:Вернулась из отложенных','task:Вернулась с доработки','task:Новая','task:Переназначена') and Статус in('Контроль данных')
			...
			UNION
			SELECT ProductType_Code, 'autoapprove' [status], cast([Дата статуса] as date) Дата, [Дата статуса] ДатаИВремяСтатуса, [ФИО клиента], [Номер заявки] , СотрудникПоследнегоСтатуса, [ФИО сотрудника верификации/чекер], ВремяЗатрачено, [Время, час:мин:сек], 0 LastStage
			FROM #details_KD
			WHERE 1=1
			...
			UNION
			SELECT A.ProductType_Code, 'Не вернувшиеся с доработки' [status], cast(A.[Дата статуса] as date) Дата, A.[Дата статуса] ДатаИВремяСтатуса, A.[ФИО клиента], A.[Номер заявки] , A.СотрудникПоследнегоСтатуса, A.[ФИО сотрудника верификации/чекер], A.ВремяЗатрачено, A.[Время, час:мин:сек], 0 LastStage
			FROM (
					SELECT 
						K.*,
	...
	select * 
	into #details_VK
	from dbo.dm_FedorVerificationRequests_Installment/*#details*/
	--where [ФИО сотрудника верификации/чекер] not in (select * from #curr_employee_cd) --and  Статус in ('Верификация клиента') 
	WHERE ([ФИО сотрудника верификации/чекер] not in (select * from #curr_employee_cd) --and  Статус in ('Верификация клиента') 
	...
	*/
	...
	END
       ...
       ,postpone as (
           ...
           -- доработка
          select ProductType_Code, [status],  Дата, [Дата статуса] ДатаИВремяСтатуса, [ФИО клиента], [Номер заявки]  , Сотрудник, [ФИО сотрудника верификации/чекер], ВремяЗатрачено, [Время, час:мин:сек] , 0 LastStage  
            into #fedor_verificator_report_VK
            from rework
           union all 
          select ProductType_Code, [status], Дата, [Дата статуса], [ФИО клиента], [Номер заявки]  , Сотрудник, [ФИО сотрудника верификации/чекер], ВремяЗатрачено, [Время, час:мин:сек] , 1 LastStage   from rework1
           union 
	...
	select duration = avg(datediff(second,[Дата статуса], [Дата след.статуса]))
	into #waitTime
	from r
	where datediff(second,[Дата статуса], [Дата след.статуса]) > 0
	...
	--Дашборд: Уникальное Кол-во заявок на этапе "Контроль данных", (шт.) 
	--Отчет - Блок "КД. Общий отчет в разбивке по месяцам", показатель "БЕЗЗАЛОГ Общее кол-во уникальных заявок на этапе"
  ...
  --                                                               else 0
	--																END as datetime), N'HH:mm:ss')
		, [Ср_время_рассмотрения_КД_день] =
			format(cast(
			CASE 
			WHEN (select count([Номер заявки]) FROM #fedor_verificator_report where status in ('task:В работе'))<>0
			THEN 
			( (select sum([ВремяЗатрачено]) from #fedor_verificator_report where status in ('task:В работе'))
			/ (select count([Номер заявки]) FROM #fedor_verificator_report where status in ('task:В работе'))
			)
			ELSE 0
			END as datetime), N'HH:mm:ss')
  ...
  --                                                           case when	
  --                                                                   (select  count(distinct [Номер заявки])  from  (select * from  #fedor_verificator_report union all  select * from #fedor_verificator_report_VK) q)<>0
  --                                                                then
  --                                                                ( (select sum([ВремяЗатрачено]) from (select * from  #fedor_verificator_report union all  select * from #fedor_verificator_report_VK) q where status in ('task:В работе'))
  --                                                                /(select  count(distinct [Номер заявки])  from  (select * from  #fedor_verificator_report union all  select * from #fedor_verificator_report_VK) q)
  --                                                                )
  --                                                                else 0
  --                                                           end
  --                                                           as datetime), N'HH:mm:ss')
  ...
  --                                                                   (select  count(distinct [Номер заявки])  from  (select * from  #fedor_verificator_report union all  select * from #fedor_verificator_report_VK) q)<>0
  --                                                                then
  --                                                                ( (select sum([ВремяЗатрачено]) from (select * from  #fedor_verificator_report  union all select * from #fedor_verificator_report_VK) q where status in ('Ожидание'))
  --                                                                /(select  count(distinct [Номер заявки])  from  (select * from  #fedor_verificator_report  union all select * from #fedor_verificator_report_VK) q)
  --                                                                )
  --                                                                else 0
  --                                                           end
  --                                                           as datetime), N'HH:mm:ss')
		...
		--[% заявок autoapprove_KD от поступивших на КД]
		--, Заявки_autoapprove_KD_perc
		--= CASE 
		--WHEN (select count(distinct [Номер заявки]) FROM #details_KD where Статус='Контроль данных') <> 0 
		--THEN 1.0* (SELECT count(DISTINCT [Номер заявки]) FROM #fedor_verificator_report AS R WHERE R.status='autoapprove')
		--	/ (select count(distinct [Номер заявки]) FROM #details_KD where Статус='Контроль данных') 
		--ELSE 0.0 
		--END
		...
		--[% заявок autoapprove_VK от поступивших на ВК]
		--, Заявки_autoapprove_VK_perc
		--= CASE 
		--	WHEN (select count(distinct [Номер заявки]) FROM #details_VK where Статус='Верификация клиента') <> 0 
		--	THEN 1.0* (SELECT count(DISTINCT [Номер заявки]) FROM #fedor_verificator_report_VK AS R WHERE R.status='autoapprove')
		--		/ (select count(distinct [Номер заявки]) FROM #details_VK where Статус='Верификация клиента') 
		--	ELSE 0.0 
		--  END
		...
		= CASE 
		  ...
		  --(select count(distinct [Номер заявки] )from #fedor_verificator_report_VK)
	/*
	[Уникальное_количество_autoapprove_KD] [int] NULL,
	[Заявки_autoapprove_KD_perc] [numeric](7, 2) NULL,
	[Уникальное_количество_autoapprove_VK] [int] NULL,
	[Заявки_autoapprove_VK_perc] [numeric](7, 2) NULL
	*/
