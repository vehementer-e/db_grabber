...
create   PROC dbo.Report_verification_fedor_without_coll_2025_10_26_не_удалять
--declare
		...
		BEGIN
			...
			SELECT distinct
			   ...
			   , R.[Время, час:мин:сек]
				...
				, R.Партнер
				, [Проверка дохода] = null
				, [Тип документа подтверждающего доход] = null
	...
	*/
	...
	BEGIN
		...
		values
		   ...
		   , (6, 'TTY  - количество заявок рассмотренных в течение 10 минут на этапе')
		   , (7 ,'TTY  - % заявок рассмотренных в течение 10 минут на этапе')
		   , (10 ,'Среднее время заявки в ожидании очереди на этапе')
		   , (12 ,'Средний Processing time на этапе (время обработки заявки)')
		...
		BEGIN
			...
			*/
			...
			select d.*
				...
				, cast(format(case when [Общее кол-во заявок на этапе]<>0 then 100*tty.cnt*1.0/[Общее кол-во заявок на этапе] else 0 end,'0')+N'%' as nvarchar(50))[TTY  - % заявок рассмотренных в течение 10 минут на этапе]
		...
		BEGIN
			...
			*/
			...
			,  cast(
			  ...
			  , cast(format(case when [Общее кол-во заявок на этапе]<>0 then 100*tty.cnt*1.0/[Общее кол-во заявок на этапе] else 0 end,'0')+N'%' as nvarchar(50)) AS [TTY  - % заявок рассмотренных в течение 10 минут на этапе]
