--select * from  dbo.dm_FedorVerificationRequestsUAT_Installment
-- exec [dbo].[Create_dm_FedorVerificationRequestsUAT_Installment] 366
CREATE   PROC dbo.Create_dm_FedorVerificationRequestsUAT_Installment
...
--declare @days int=10
--select * from dbo.dm_FedorVerificationRequestsUAT_Installment where '20110800049410'=[Номер заявки] [Дата статуса]>'20201113'
...
-- справочники
  ...
  select Id
    ...
    from stg.[_fedorUAT].[dictionary_ClientRequestStatus] 
...
-- select * from  #dictionary_ClientRequestStatus
  ...
  select Id
    ...
    from stg.[_fedorUAT].[dictionary_TaskStatus] 
   where IsDeleted = 0
...
--факты
  ...
  select cr.* 
	...
	--select top 100 * 
    from stg.[_fedorUAT].[core_ClientRequest] cr 
		INNER JOIN stg.[_fedorUAT].dictionary_ProductType AS T
			ON T.Id = cr.ProductTypeId
			AND T.Code = 'installment'
	WHERE CreatedOn>'20200902' -- дата старта ФЕДОР в проде
     and   dateadd(hour,3,[CreatedOn]) 				  >dateadd(day,-@days,cast(getdate() as date))
		...
		--	or isnull(cr.Type,0) in (2,4) -- 'Installment, IsSmartInstallment'
		--)
  ...
  select * 
    ...
    from stg.[_fedorUAT].[core_TaskAndClientRequest]
   where IsDeleted = 0
...
--select * from #core_user  WHERE LASTnAME  like '%Волос%'/*  A96E3624-C23B-4889-A908-3944DB05223E  - Прибылов
  ...
  select * 
    ...
    from stg.[_fedorUAT].[core_TaskAssignUser] 
   where CreatedOn>'20200902' -- дата старта ФЕДОР в проде
     and IsDeleted = 0 
        and   dateadd(hour,3,[CreatedOn]) 				  >dateadd(day,-@days,cast(getdate() as date))
-- select * from #core_TaskAssignUser
  ...
  select *
    ...
    from stg.[_fedorUAT].[core_TaskHistory] 
   where CreatedOn>'20200902' -- дата старта ФЕДОР в проде
     and IsDeleted = 0
        and   dateadd(hour,3,[CreatedOn]) 				  >dateadd(day,-@days,cast(getdate() as date))
...
*/
  ...
  select * 
    ...
    from stg.[_fedorUAT].[core_ClientRequestHistory]
   where CreatedOn>'20200902' -- дата старта ФЕДОР в проде
     and IsDeleted = 0 
       and   dateadd(hour,3,[CreatedOn]) 				  >dateadd(day,-@days,cast(getdate() as date))
...
-- витрина
  ...
  select [ЗаявкаСсылка]           = crh.IdClientRequest
       ...
       , [ФИО клиента] = concat(cr.ClientLastName, ' ', cr.ClientFirstName, ' ', cr.ClientMiddleName)
    ...
    where 1=1
		--AND ClientFirstName not like 'Тест%'  and ClientMiddleName not like 'Тест%' and trim(ClientLastName)<>'ТЕСТОВАЯ'
		AND   dateadd(hour,3,cr.[CreatedOn]) 				  >dateadd(day,-@days,cast(getdate() as date))
  --2021_12_07
  and (cr.IsInstallment = 1
	OR isnull(cr.Type, -1) IN (2, 4) --DWH-1786
  )
  union all
  select [ЗаявкаСсылка]						= cr.Id
   ...
   where  th.IsDeleted = 0 
	   ...
	   and tau.IsDeleted = 0
	  --2021_12_07
	  and (isnull(cr.IsInstallment,-1) = 1
			OR isnull(cr.Type, -1) IN (2, 4) --DWH-1786
		)
...
*/
	...
	DROP TABLE IF EXISTS #t_Звонки
	CREATE TABLE #t_Звонки(
	...
	FROM  
		(
			...
			FROM Stg._fedorUAT.core_CheckListItem AS CheckListItem
				...
				INNER JOIN Stg._fedorUAT.dictionary_CheckListItemType AS CheckListItemType
					ON CheckListItemType.Id = CheckListItem.IdType
				LEFT JOIN Stg._fedorUAT.core_CheckListItemTypeAndCheckListItemStatus AS CH_IT_IS
					ON CH_IT_IS.IdType = CheckListItem.IdType
					AND CH_IT_IS.IdCheckListItemStatus = CheckListItem.IdStatus
				LEFT JOIN Stg._fedorUAT.dictionary_CheckListItemStatus AS CheckListItemStatus
...
--drop table if exists  dbo.dm_FedorVerificationRequestsUAT_Installment
begin tran
	TRUNCATE TABLE dbo.dm_FedorVerificationRequestsUAT_Installment_STG

	INSERT dbo.dm_FedorVerificationRequestsUAT_Installment_STG
	SELECT *
	FROM dbo.dm_FedorVerificationRequestsUAT_Installment (NOLOCK)

  --delete from  dbo.dm_FedorVerificationRequestsUAT_Installment where   [Дата заведения заявки]						  >=dateadd(day,-@days,cast(getdate() as date))
  DELETE
  FROM dbo.dm_FedorVerificationRequestsUAT_Installment_STG
  WHERE [Дата заведения заявки] >=dateadd(day,-@days,cast(getdate() as date))

  --insert into  dbo.dm_FedorVerificationRequestsUAT_Installment
  insert into  dbo.dm_FedorVerificationRequestsUAT_Installment_STG
  --declare @now datetime=getdate()
 ...
 -- , [Задача предыдущая]
  , iif(isnull(ПризнакИсключенияСотрудника,0) = 1 
	...
	, [Назначен_След]	
		...
		, ТипКлиента
		, IsSkipped
--into dbo.dm_FedorVerificationRequestsUAT_Installment
  ...
  select q.[Дата заведения заявки]
   ...
   --
	...
	--delete 
	----select *
	--from  dbo.dm_FedorVerificationRequestsUAT_Installment
	--where 
...
commit tran
	...
	if OBJECT_ID('dbo.dm_FedorVerificationRequestsUAT_Installment_to_del') is null
	begin
		select top(0)
		*
		into dbo.dm_FedorVerificationRequestsUAT_Installment_to_del
		from dbo.dm_FedorVerificationRequestsUAT_Installment_STG

		CREATE NONCLUSTERED INDEX [ix_Статус_Задача_Дата_статуса] 
		ON dbo.dm_FedorVerificationRequestsUAT_Installment_to_del (
	...
	IF (SELECT count(*) FROM dbo.dm_FedorVerificationRequestsUAT_Installment_STG) > 0
	begin
		
		truncate table dbo.dm_FedorVerificationRequestsUAT_Installment_to_del
		--DWH-2050 Оптимизировать обновление данных в отчете верификации
		BEGIN TRAN
			alter table dbo.dm_FedorVerificationRequestsUAT_Installment
			SWITCH to dbo.dm_FedorVerificationRequestsUAT_Installment_to_del
			with (WAIT_AT_LOW_PRIORITY  ( MAX_DURATION = 1 minutes, ABORT_AFTER_WAIT = SELF ))

			alter table dbo.dm_FedorVerificationRequestsUAT_Installment_STG
			SWITCH to dbo.dm_FedorVerificationRequestsUAT_Installment
			with (WAIT_AT_LOW_PRIORITY  ( MAX_DURATION = 1 minutes, ABORT_AFTER_WAIT = SELF  ))
		COMMIT TRAN
	END
