--select * from  dbo.dm_FedorVerificationRequests_AutocredVTB
--exec Reports.dbo.Create_dm_FedorVerificationRequests_AutocredVTB 10
CREATE PROC [dbo].[Create_dm_FedorVerificationRequests_AutocredVTB]
...
begin
	...
	--declare @days int=10
	--select * from dbo.dm_FedorVerificationRequests_AutocredVTB where '20110800049410'=[Номер заявки] [Дата статуса]>'20201113'
	...
	--факты
	  ...
	  ,ФИО_Клиента = TRIM(concat_ws(' '
		...
		and pt.Code in ('autoCredit')-- 'AutocredVTB'
	...
	*/
	...
	--drop table if exists  dbo.dm_FedorVerificationRequests_AutocredVTB

	--DWH-1716 Оптимизировать обновление данных в отчете верификации
	begin tran
		TRUNCATE TABLE dbo.dm_FedorVerificationRequests_AutocredVTB_STG

		SELECT @StartDate = getdate(), @row_count = 0

		INSERT dbo.dm_FedorVerificationRequests_AutocredVTB_STG
		SELECT *
		FROM dbo.dm_FedorVerificationRequests_AutocredVTB (NOLOCK)

		SELECT @row_count = @@ROWCOUNT
		IF @isDebug = 1 BEGIN
			SELECT 'INSERT full table into dbo.dm_FedorVerificationRequests_AutocredVTB_STG', @row_count, datediff(SECOND, @StartDate, getdate())
		END
	  ...
	  DELETE
	  FROM dbo.dm_FedorVerificationRequests_AutocredVTB_STG
	  WHERE [Дата заведения заявки]	>=dateadd(day,-@days,cast(getdate() as date))

	  --DWH-2879. избавляемся от возможных дублей
	  DELETE D
	  FROM dbo.dm_FedorVerificationRequests_AutocredVTB_STG AS D
	  WHERE EXISTS(
			SELECT TOP(1) 1
	  ...
	  insert into dbo.dm_FedorVerificationRequests_AutocredVTB_STG
	  --declare @now datetime=getdate()
	 ...
	 -- , [Задача предыдущая]
	  ...
	  select q.[Дата заведения заявки]
	   ...
	   --
		...
		SELECT @row_count = @@ROWCOUNT
		IF @isDebug = 1 BEGIN
			SELECT 'INSERT increment into dbo.dm_FedorVerificationRequests_AutocredVTB_STG', @row_count, datediff(SECOND, @StartDate, getdate())
		END
		...
		IF @isDebug = 1 BEGIN
			DROP TABLE IF EXISTS ##dm_FedorVerificationRequests_AutocredVTB_STG
			SELECT * INTO ##dm_FedorVerificationRequests_AutocredVTB_STG FROM dbo.dm_FedorVerificationRequests_AutocredVTB_STG
		END
	commit tran

	if OBJECT_ID('dbo.dm_FedorVerificationRequests_AutocredVTB_to_del') is null
	begin
		select top(0)
		*
		into dbo.dm_FedorVerificationRequests_AutocredVTB_to_del
		from dbo.dm_FedorVerificationRequests_AutocredVTB_STG
		create index ix_Статус_Задача_Дата_статуса on dbo.dm_FedorVerificationRequests_AutocredVTB_to_del
		(
			[Статус] ASC,
			[Задача] ASC,
		...
		ON dbo.dm_FedorVerificationRequests_AutocredVTB_to_del
		(
			[Номер заявки] ASC,
			[Дата статуса] ASC,
			[Состояние заявки] ASC,
	...
	IF (SELECT count(*) FROM dbo.dm_FedorVerificationRequests_AutocredVTB_STG) > 0
	begin
		
		truncate table dbo.dm_FedorVerificationRequests_AutocredVTB_to_del
	--DWH-1716 Оптимизировать обновление данных в отчете верификации
		BEGIN TRAN
			alter table dbo.dm_FedorVerificationRequests_AutocredVTB switch  to dbo.dm_FedorVerificationRequests_AutocredVTB_to_del
				with (WAIT_AT_LOW_PRIORITY  ( MAX_DURATION = 1 minutes, ABORT_AFTER_WAIT = SELF ))

			alter table dbo.dm_FedorVerificationRequests_AutocredVTB_STG switch  to dbo.dm_FedorVerificationRequests_AutocredVTB
				with (WAIT_AT_LOW_PRIORITY  ( MAX_DURATION = 1 minutes, ABORT_AFTER_WAIT = SELF  ))
		COMMIT TRAN
	END

	-- select * from dbo.dm_FedorVerificationRequests_AutocredVTB where [Дата заведения заявки]>='20210208' order by [Номер заявки],[Дата статуса]
...
-- ALTER TABLE dbo.dm_FedorVerificationRequests_AutocredVTB  ADD ПризнакИсключенияСотрудника int NULL
