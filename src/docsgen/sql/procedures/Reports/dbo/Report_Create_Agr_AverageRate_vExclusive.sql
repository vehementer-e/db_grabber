...
CREATE  PROCEDURE [dbo].[Report_Create_Agr_AverageRate_vExclusive] 
...
truncate table [dbo].[report_Agreement_AverageRate_vExclusive];

  --create table [dbo].[report_Agreement_AverageRate_vExclusive]
  --        (
  --         [Дата] date null
		--  ,[Время] time null
...
from [dbo].[report_Agreement_InterestRate_vExclusive]
where cast([ДатаВыдачиПолн] as date)>='2019-01-01' --between dateadd(day,-2,dateadd(day,datediff(day,0,getdate()),0)) and getdate()
--order by 1 desc
)
--select distinct max([ДатаВыдачиПолн]) over (partition by [Дата]) from t0 where cast([ДатаВыдачиПолн] as date) < cast(dateadd(day,-2,dateadd(day,datediff(day,0,getdate()),0)) as date) 
,	t_Nd as
(
select distinct [Дата] ,cast(dateadd(SECOND,-1,dateadd(day,datediff(day,0,[ДатаВыдачиПолн]),0)) as time) as [Время] 
...
from t0 where [ДатаВыдачиПолн] in (select distinct max([ДатаВыдачиПолн]) over (partition by [Дата]) from t0 where cast([ДатаВыдачиПолн] as date) < cast(dateadd(day,-2,dateadd(day,datediff(day,0,getdate()),0)) as date)) 
)
--select distinct * from t_Nd
,	t_2d as
(
...
insert into [dbo].[report_Agreement_AverageRate_vExclusive] ([Дата] ,[Время] ,[Сумма] ,[СреднВзвешСтавка])
