...
--             exec [dbo].[ReportDashboard_Collection_Installment_v02_part6_new_saved_balance]  --1
-- =============================================
CREATE PROCEDURE [dbo].[ReportDashboard_Collection_Installment_v02_part6_new_saved_balance]
AS
...
BEGIN
	...
	select sum([остаток од])                         сохбаланс
	,      datepart(hour,ДатаВремяПоследнегоПлатежа) Час
		into #t
	from [dbo].[dm_BucketMigration_Installment]
	...
	-- посчитаем RR
	update [dbo].[dm_dashboard_Collection_Installment_v02_new_save_balance]
	...
	where id = 1
	--select * from [dbo].[dm_dashboard_Collection_Installment_v02_new_save_balance]


	-- посчитаем проценты RR
	update [dbo].[dm_dashboard_Collection_Installment_v02_new_save_balance]
	SET [t1_1_5_rr_to_day_percent] = case when isnull([t1_1_1_plan_to_day],0)<>0 then [t1_1_4_rr_to_day]/[t1_1_1_plan_to_day]
	                                                                             else 0 end
	where id = 1
	update [dbo].[dm_dashboard_Collection_Installment_v02_new_save_balance]
	SET [t1_2_5_rr_to_day_percent] = case when isnull([t1_2_1_plan_to_day],0)<>0 then [t1_2_4_rr_to_day]/[t1_2_1_plan_to_day]
	                                                                             else 0 end
	where id = 1
	update [dbo].[dm_dashboard_Collection_Installment_v02_new_save_balance]
	SET [t1_3_5_rr_to_day_percent] = case when isnull([t1_3_1_plan_to_day],0)<>0 then [t1_3_4_rr_to_day]/[t1_3_1_plan_to_day]
	                                                                             else 0 end
	where id = 1
	update [dbo].[dm_dashboard_Collection_Installment_v02_new_save_balance]
	SET [t1_4_5_rr_to_day_percent] = case when isnull([t1_4_1_plan_to_day],0)<>0 then [t1_4_4_rr_to_day]/[t1_4_1_plan_to_day]
	                                                                             else 0 end
	where id = 1
	update [dbo].[dm_dashboard_Collection_Installment_v02_new_save_balance]
	SET [t1_5_5_rr_to_day_percent] = case when isnull([t1_5_1_plan_to_day],0)<>0 then [t1_5_4_rr_to_day]/[t1_5_1_plan_to_day]
	                                                                             else 0 end
	where id = 1
	--select * from [dbo].[dm_dashboard_Collection_Installment_v02_new_save_balance]
	...
	-- посчитаем проценты RR
	update [dbo].[dm_dashboard_Collection_Installment_v02_new_save_balance]
	SET [t1_1_12_rr_month_percent] = case when isnull([t1_1_10_plan_current_month],0)<>0 then [t1_1_11_rr_month]/[t1_1_10_plan_current_month]
	                                                                                     else 0 end
	where id = 1
	update [dbo].[dm_dashboard_Collection_Installment_v02_new_save_balance]
	SET [t1_2_12_rr_month_percent] = case when isnull([t1_2_10_plan_current_month],0)<>0 then [t1_2_11_rr_month]/[t1_2_10_plan_current_month]
	                                                                                     else 0 end
	where id = 1
	update [dbo].[dm_dashboard_Collection_Installment_v02_new_save_balance]
	SET [t1_3_12_rr_month_percent] = case when isnull([t1_3_10_plan_current_month],0)<>0 then [t1_3_11_rr_month]/[t1_3_10_plan_current_month]
	                                                                                     else 0 end
	where id = 1
	update [dbo].[dm_dashboard_Collection_Installment_v02_new_save_balance]
	SET [t1_4_12_rr_month_percent] = case when isnull([t1_4_10_plan_current_month],0)<>0 then [t1_4_11_rr_month]/[t1_4_10_plan_current_month]
	                                                                                     else 0 end
	where id = 1
	update [dbo].[dm_dashboard_Collection_Installment_v02_new_save_balance]
	SET [t1_5_12_rr_month_percent] = case when isnull([t1_5_10_plan_current_month],0)<>0 then [t1_5_11_rr_month]/[t1_5_10_plan_current_month]
	                                                                                     else 0 end
	where id = 1
	--select * from [dbo].[dm_dashboard_Collection_Installment_v02_new_save_balance]
	...
	-- посчитаем проценты
	update [dbo].[dm_dashboard_Collection_Installment_v02_new_save_balance]
	SET [t1_1_8_fact_rr_to_end_of_month_percent] = case when isnull([t1_1_10_plan_current_month],0)<>0 then [t1_1_7_fact_rr_to_end_of_month]/[t1_1_10_plan_current_month]
	                                                                                                   else 0 end
	where id = 1
	update [dbo].[dm_dashboard_Collection_Installment_v02_new_save_balance]
	SET [t1_2_8_fact_rr_to_end_of_month_percent] = case when isnull([t1_2_10_plan_current_month],0)<>0 then [t1_2_7_fact_rr_to_end_of_month]/[t1_2_10_plan_current_month]
	                                                                                                   else 0 end
	where id = 1
	update [dbo].[dm_dashboard_Collection_Installment_v02_new_save_balance]
	SET [t1_3_8_fact_rr_to_end_of_month_percent] = case when isnull([t1_3_10_plan_current_month],0)<>0 then [t1_3_7_fact_rr_to_end_of_month]/[t1_3_10_plan_current_month]
	                                                                                                   else 0 end
	where id = 1
	update [dbo].[dm_dashboard_Collection_Installment_v02_new_save_balance]
	SET [t1_4_8_fact_rr_to_end_of_month_percent] = case when isnull([t1_4_10_plan_current_month],0)<>0 then [t1_4_7_fact_rr_to_end_of_month]/[t1_4_10_plan_current_month]
	                                                                                                   else 0 end
	where id = 1
	update [dbo].[dm_dashboard_Collection_Installment_v02_new_save_balance]
	SET [t1_5_8_fact_rr_to_end_of_month_percent] = case when isnull([t1_5_10_plan_current_month],0)<>0 then [t1_5_7_fact_rr_to_end_of_month]/[t1_5_10_plan_current_month]
	                                                                                                   else 0 end
	where id = 1
--select * from [dbo].[dm_dashboard_Collection_Installment_v02_new_save_balance]
