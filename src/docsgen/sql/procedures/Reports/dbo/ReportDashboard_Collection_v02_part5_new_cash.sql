...
--             exec [dbo].[ReportDashboard_Collection_v02_part5_new_cash]   --1
--				2020-12-24 - учитываем переход 1-0 в течение дня кэш
-- =============================================
CREATE PROC dbo.ReportDashboard_Collection_v02_part5_new_cash
		@isDebug bit = 0
AS
...
-- part 5 new cash
--22.05.2020
delete from [dbo].[stage_dashboard_Collection_v02_new_cash] 
insert into [dbo].[stage_dashboard_Collection_v02_new_cash] 
...
--- 28.04.2020
	...
	*/
	...
	--var 2 DWH-2442
	SELECT DISTINCT 
		CMRContractNumber = ccs.external_id,
		stageDate = ccs.call_dt, 
		CMRContractStage = ccs.External_Stage
	INTO #hard0_90
	FROM Stg._loginom.Collection_External_Stage_history AS ccs
	...
	-- получим все поступления кэш за текущий и предыдущий месяц
	 drop table if exists #LMSD_tmp
		 ...
		 from   [dbo].dm_Telegram_Collection_Detail_New_Alternative b with(nolock)	  
...
/*
--- 28.04.2020
-- Найдем все договора, у которых был hard
	if object_id('tempdb.dbo.#hard0_90') is not null drop table #hard0_90
	
	select	distinct r.CMRContractNumber
	into #hard0_90
	from dwh_new.[Dialer].[ClientContractStage] ccs
	join dwh_new.staging.CRMClient_references r on r.CMRContractGUID=ccs.CMRContractGUID
	--where CMRContractStage='Hard' and ishistory=0
	-- 02.07.2020 - по просьбе Савра
	where CMRContractStage='Legal' and ishistory=0

	drop table if exists dbo.dm_ClientContractStageNumber
	select r.CMRContractNumber,  CMRContractStage
	into dbo.dm_ClientContractStageNumber
	from dwh_new.[Dialer].[ClientContractStage] ccs
	join dwh_new.staging.CRMClient_references r on r.CMRContractGUID=ccs.CMRContractGUID
	where --CMRContractStage='Hard' and 
	ishistory=0
	group by r.CMRContractNumber,  CMRContractStage

	-- получим все поступления кэш за текущий и предыдущий месяц
	if object_id('tempdb.dbo.#LMSD') is not null drop table #LMSD

	--Select b.external_id
	--	, b.Период 
	--	, b.d 
	--	, isnull(b.[сумма поступлений],0) as суммаплатежей
	--	, isnull(b.[основной долг уплачено],0) [основной долг уплачено]
	--	, isnull(b.[Проценты уплачено],0) [Проценты уплачено]
	--	, isnull(b.[ПениУплачено],0) [ПениУплачено]
	--	--, Dogovor_PDP.dpd PDP
	--	--,  (case when b.[dpd day-1] <= 0   then '(1)_0'
	--	--	when b.[dpd day-1] <= 30  then '(2)_1_30'
	--	--	when b.[dpd day-1] <= 60  then '(3)_31_60'
	--	--	when b.[dpd day-1] <= 90  then '(4)_61_90'
	--	--	when b.[dpd day-1] <= 360 then '(5)_91_360'
	--	--	else '(6)_361+' end) as dpd_bucket
		
	--	, dpd_bucket = case when h.CMRContractNumber is null
	--							then 
	--							(case   when b.[dpd day-1] <= 0   then '(1)_0'
	--									when b.[dpd day-1] <= 30  then '(2)_1_30'
	--									when b.[dpd day-1] <= 60  then '(3)_31_60'
	...
	--							else 
	--							(case   when b.[dpd day-1] <= 0   then '(1)_0'
	--									when b.[dpd day-1] <= 30  then 'hard_0_90'
	--									when b.[dpd day-1] <= 60  then 'hard_0_90'
	--									when b.[dpd day-1] <= 90  then 'hard_0_90'
	--									when b.[dpd day-1] <= 360 then '(5)_91_360'
	--									else '(6)_361+' 
	--							 end)	
	--					end
	--	 , iif(h.CMRContractNumber is null, 0,1) as hard_0_90
	--	into #LMSD
	--FROM [dbo].[dm_CMRStatBalance_2] b (nolock)
	--left join #hard0_90 h on h.CMRContractNumber = b.external_id
	--where b.d>=cast(dateadd(month,-1,dateadd(day,-1,getdate())) as date)  
	--and ( (b.[основной долг уплачено]) is not null or (b.[Проценты уплачено])  is not null or (b.[ПениУплачено]) is not null) 
	----Dogovor_PDP.Период =Format(dateadd(year,-2000,@dt_today),'yyyy-MM-dd') 
	----order by external_id, Dogovor_PDP.Период
		  ...
		  select  b.[ДоговорНомер] external_id
			  ...
			  , dpd_bucket = case when h.CMRContractNumber is null
							then 
							(case   when b.[КоличествоПолныхДнейПросрочки] <= 0   then '(1)_0'
									when b.[КоличествоПолныхДнейПросрочки] <= 30  then '(2)_1_30'
									when b.[КоличествоПолныхДнейПросрочки] <= 60  then '(3)_31_60'
									when b.[КоличествоПолныхДнейПросрочки] <= 90  then '(4)_61_90'
									when b.[КоличествоПолныхДнейПросрочки] <= 360 then '(5)_91_360'
							...
							(case   when b.[КоличествоПолныхДнейПросрочки] <= 0   then 'hard_0_90'
									when b.[КоличествоПолныхДнейПросрочки] <= 30  then 'hard_0_90'
									when b.[КоличествоПолныхДнейПросрочки] <= 60  then 'hard_0_90'
									when b.[КоличествоПолныхДнейПросрочки] <= 90  then 'hard_0_90'
									when b.[КоличествоПолныхДнейПросрочки] <= 360 then '(5)_91_360'
									when b.[КоличествоПолныхДнейПросрочки] > 360 then '(6)_361+' 
									else '(7)_other' 
								end)	
		 ...
		 , iif(h.CMRContractNumber is null, 0,1) as hard_0_90
		 into #LMSD
		 --2020-12-24 новая витрина
		 from   [dbo].dm_Telegram_Collection_Detail_New_Alternative b
	  --from   [dbo].[dm_Telegram_Collection_Detail_New] b
	  left join #hard0_90 h on h.CMRContractNumber = b.ДоговорНомер
	  	where b.датаоперации>=cast(dateadd(month,-1,dateadd(day,-1,getdate())) as date)   -->=cast(dateadd(month,-1, @dt_begin_of_month) as date)  

		*/
		--- создадим витрину для куба по кэшу ----
		--insert into  [dbo].[dm_Telegram_Collection_Cash]
		--		  select  b.[ДоговорНомер] external_id
		--	  , cast(датаоперации as date) Период
		--	  , датаоперации as d
		--	  , isnull([ОДОплачено],0)+isnull([ПроцентыОплачено],0)+isnull([ПениОплачено],0)  суммаплатежей
		--	  , ([ОДОплачено])  [основной долг уплачено]
		--	  , ([ПроцентыОплачено]) [Проценты уплачено]
		--	  , ([ПениОплачено])   [ПениУплачено]
		--	  , dpd_bucket = case when h.CMRContractNumber is null
	 ...
	 -- from   [dbo].[dm_Telegram_Collection_Detail_New] b
	 -- left join #hard0_90 h on h.CMRContractNumber = b.ДоговорНомер
	 -- 	where b.датаоперации>=cast(dateadd(month,-1,dateadd(day,-1,getdate())) as date)
		----------------------------------

	--select top 100 Sum(суммаплатежей) суммаплатежей, external_id,Период,  dpd_bucket  from #LMSD
	--group by external_id, Период,  dpd_bucket
...
--select @summa
update [dbo].[stage_dashboard_Collection_v02_new_cash] 
...
Set @summa = isnull((select sum(суммаплатежей) from #LMSD_report where seg=N'TODAY' 
	and (dpd_bucket =N'(2)_1_30' OR dpd_bucket =N'(3)_31_60' OR dpd_bucket =N'(4)_61_90')),0) --OR dpd_bucket =N'hard_0_90'
update [dbo].[stage_dashboard_Collection_v02_new_cash] 
...
-- 90-360 - за день 
update [dbo].[stage_dashboard_Collection_v02_new_cash] SET  [t1_4_2_fact_to_day] = [t2_4_3] where id = 1 
update [dbo].[stage_dashboard_Collection_v02_new_cash] SET  [t1_4_3_fact_to_day_percent] = case when isnull([t1_4_1_plan_to_day],0)<> 0 then [t1_4_2_fact_to_day]/[t1_4_1_plan_to_day] else 0 end where id = 1
---- 361+ -за день
update [dbo].[stage_dashboard_Collection_v02_new_cash] SET  [t1_5_2_fact_to_day] = [t2_5_3] where id = 1 
update [dbo].[stage_dashboard_Collection_v02_new_cash] SET  [t1_5_3_fact_to_day_percent] = case when isnull([t1_5_1_plan_to_day],0)<>0 then  [t1_5_2_fact_to_day]/[t1_5_1_plan_to_day] else 0 end where id = 1
