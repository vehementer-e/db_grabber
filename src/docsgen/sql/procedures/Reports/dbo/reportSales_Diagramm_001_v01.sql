...
CREATE  PROCEDURE [dbo].[reportSales_Diagramm_001_v01] 
...
--set @ReportMetrika = N'Лиды';
set @R=1000000.00;
set @WeekFrom=case when datepart(dw,getdate())=1 then datepart(ww,dateadd(week,-4,getdate())) else datepart(ww,dateadd(week,-3,getdate())) end;
...
set @dtFrom = case when datepart(dw,getdate())=1 
					then cast(dateadd(week,datediff(week,0,dateadd(week,-4,Getdate())),0) as date) 
					else cast(dateadd(week,datediff(week,0,dateadd(week,-3,Getdate())),0) as date) 
			end;
...
set @MaxValue = (select (count(distinct [Номер]) + 100) as [maxValue] from [dbo].[dm_Factor_Analysis_001]
																 where cast([ДатаЗаявкиПолная] as date) between @dtFrom and @dtFrom 
																		and datepart(yyyy,[ДатаЗаявкиПолная])=datepart(yyyy,getdate())
																		and not [Группа каналов] in (N'Тест')) 

drop table if exists #calendar_week
select created ,datepart(yyyy, created) as yr ,datepart(ww, created) as [wkk] 
		,case when datepart(dw, created)=1 or (datepart(dy, created)=1) 
				then cast(created as date) 
		 end as dt
into #calendar_week
from dwh_new.[dbo].calendar
where cast(created as date) between cast(dateadd(week ,-5 ,getdate()) as date) and cast(dateadd(week ,1 ,getdate()) as date)  
...
select distinct
--cw.dt ,
d.[Метрика] ,d.[ГодУчета] as [Год] ,d.[НеделяУчета] as [Неделя] ,sum(d.[КолвоФакт]) [КолвоФакт] ,d.[Группа каналов] ,d.[ДатаНачалаНедели]
into #SalesDiagramm 
from dwh_new.[dbo].[dashboard_SalesDiagramm001] d with (nolock) 
--right join (select * from #calendar_week where not dt is null) cw on d.[Год]=cw.yr and d.[Неделя]=cw.[wkk]
where [ДатаНачалаНедели] between @dtFrom and @dtTo
--@WeekFrom and @WeekTo and d.[Метрика]=@ReportMetrika
group by d.[Метрика] ,d.[ГодУчета] ,d.[НеделяУчета] ,d.[Группа каналов] ,d.[ДатаНачалаНедели]
...
from (select tp0.[Год] ,tp0.[Неделя] ,tp0.[Группа] ,tp0.[Метрика] ,sum(tp0.[Колво]) as [Колво] ,sum(tp0.[Сумма]) as [Сумма] 
	  from (select datepart(yyyy,[Дата]) as [Год] ,datepart(ww,[Дата]) as [Неделя] ,[Группа] ,[Метрика] ,[Колво] ,[Сумма] ,[created]
			from [Stg].[files].[Sales_DailyPlans] where cast([Дата] as date) between @dtFrom and @dtTo and [Метрика]=@Metrika
			) tp0
...
select cw.dt ,t0.* from t0
right join (select * from #calendar_week where not dt is null) cw on t0.[Год]=cw.yr and t0.[Неделя]=cw.[wkk]
where not [Метрика] is null
order by cw.dt
;
