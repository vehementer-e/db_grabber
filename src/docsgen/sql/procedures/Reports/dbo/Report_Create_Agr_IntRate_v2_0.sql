...
--exec  [dbo].[Report_Create_Agr_IntRate_v2_0] 
--drop table #t1
...
where t1.договорНомер is null or t2.договорНомер is null
  ...
  */
CREATE   PROCEDURE [dbo].[Report_Create_Agr_IntRate_v2_0] 
AS
BEGIN
...
drop table if exists #ssdp
  select  sd.ссылка
	   ...
	   --, SumWLStand = (select s1.Код from [Stg].[_1cCMR].[Справочник_ДоговораПоДопПродуктам] s1 with (nolock) where s1.[Договор]=sd.ссылка  and  
		 ...
		 , [TeleMedicKod] = (select s1.Код from [Stg].[_1cCMR].[Справочник_ДоговораПоДопПродуктам] s1 with (nolock) where s1.[Договор]=sd.ссылка  and  ДопПродукт=0xB81B00155D4D086C11EA69F84A182561)

		 , [CushionKod] = (select s1.Код from [Stg].[_1cCMR].[Справочник_ДоговораПоДопПродуктам] s1 with (nolock) where s1.[Договор]=sd.ссылка  and  ДопПродукт=0xA2C7005056839FE911EA968404C28E69)
...
drop table if exists #lcrm_tbl_full_w_chanals2_end
 ...
 from stg._LCRM.lcrm_leads_full_channel_request ch 
 --from [Stg].[_LCRM].[lcrm_tbl_short_w_channel] ch with (nolock) /*28.02.2022 a.kotelevec*/
...
begin tran
	...
	delete from dbo.report_Agreement_InterestRate_stg where 1=1

	--перемещаем данные в report_Agreement_InterestRate_stg
	insert into report_Agreement_InterestRate_stg
	...
	*/
	...
	begin
	 begin tran
	  delete from dbo.report_Agreement_InterestRate where 1=1
