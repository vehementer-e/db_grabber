...
--             exec [dbo].[ReportDashboard_Collection_v02_part5b_new_cash]  --1
--			   2020-12-24 - учитываем переход 1-0 в течение дня кэш
-- =============================================
CREATE PROC dbo.ReportDashboard_Collection_v02_part5b_new_cash
...
declare  @dt_next_month date = cast(dateadd(month,1, @dt_begin_of_month) as date)
	...
	-- получим все поступления кэш за текущий и предыдущий месяц (теперь суммируем по всему месяцу)
	if object_id('tempdb.dbo.#LMSD') is not null drop table #LMSD


	---29.04.2020
	-- перепишем на общий источник
	-- получим данные из предрасчитанной витрины по кэш
	  select  b.[ДоговорНомер] external_id
			  , cast(датаоперации as date) Период
			  , датаоперации as d
			  , isnull([ОДОплачено],0)+isnull([ПроцентыОплачено],0)+isnull([ПениОплачено],0)+isnull([ПереплатаНачислено],0)-isnull([ПереплатаОплачено],0)  суммаплатежей
	  ...
	  left join #hard0_90 h on h.CMRContractNumber = b.ДоговорНомер
	  	where b.датаоперации>=cast(dateadd(month,-1, @dt_begin_of_month) as date)  
	--and ( (b.[ОДОплачено]) is not null or (b.[ПроцентыОплачено])  is not null or (b.[ПениОплачено]) is not null) 
      --group by датаоперации order by датаоперации
	  --select * from #LMSD
...
update [dbo].stage_dashboard_Collection_v02_new_cash SET  [t1_4_7_fact_rr_to_end_of_month] = [t3_4_2]+[t2_4_3] where id = 1

--update [dbo].stage_dashboard_Collection_v02_new_cash SET  [t1_4_8_fact_rr_to_end_of_month_percent] = [t1_4_7_fact_rr_to_end_of_month]/[t1_4_10_plan_current_month] where id = 1
UPDATE [dbo].stage_dashboard_Collection_v02_new_cash 
...
update [dbo].stage_dashboard_Collection_v02_new_cash SET  [t1_5_7_fact_rr_to_end_of_month] = [t3_5_2]+[t2_5_3] where id = 1

--update [dbo].stage_dashboard_Collection_v02_new_cash SET  [t1_5_8_fact_rr_to_end_of_month_percent] = [t1_5_7_fact_rr_to_end_of_month]/[t1_5_10_plan_current_month] where id = 1
UPDATE [dbo].stage_dashboard_Collection_v02_new_cash 
