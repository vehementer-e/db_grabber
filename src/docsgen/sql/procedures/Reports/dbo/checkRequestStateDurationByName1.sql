...
--exec dbo.checkRequestStateDurationByName1   1200,'Верификация документов клиента','E.Mogilevskaya@carmoney.ru; shubkin_a_n@carmoney.ru','Заявки в статусе "Верификация ВДК" больше 20 минут','Верификация документов клиента'
--exec dbo.checkRequestStateDurationByName1   600,'Верификация документов','E.Mogilevskaya@carmoney.ru; shubkin_a_n@carmoney.ru','Заявки в статусе "Верификация ВД" больше 10 минут','Верификация документов'

CREATE      procedure [dbo].[checkRequestStateDurationByName1]

  @limit int=120
          , @statusName nvarchar(1024)='Верификация КЦ'
          
          , @recipients nvarchar(4000) ='blagoveschenskaya@carmoney.ru; Gustyakov_B_V@carmoney.ru; E.Tigunov@carmoney.ru;V.Shadzhe@carmoney.ru;   prometov@carmoney.ru; ilina_e_v@carmoney.ru; shubkin_a_n@carmoney.ru; kurdin@carmoney.ru ;Vasilev@carmoney.ru'	--; Krivotulov@carmoney.ru
...
set @tsql='
if  not isnull((select count(*) from #t1 where (isnull(['+@fieldName+'],0))>='+format(@limit,'0')+' and lastStatusName in ('''+@statusName+''')),0)=0 
...
begin 
    ...
    from #t1
    where (isnull(['+@fieldName+'],0))>='+format(@limit,'0')+' and lastStatusName in ('''+@statusName+''')
    order by дата
