...
-- =============================================
CREATE   PROCEDURE [collection].[etl_report_collecting_receipt_cmr]
...
into #TempTable_CollectingPayIn
  FROM collection.[mt_payments_receipt_cmr_umfo]
...
from #PayIn_Res pir
left join collection.[mt_requests_loans_mfo] dd
...
TRUNCATE TABLE collection.[report_collecting_receipt_cmr]

--begin tran

insert into collection.[report_collecting_receipt_cmr] ([НаименованиеПараметра] ,[Неделя] ,[Параметр1] ,[Параметр2] ,[Параметр3] ,[Параметр4]
...
delete from collection.[mt_reciept_period_dpd] 
...
insert into collection.[mt_reciept_period_dpd] ([cdate] ,[Indicator] ,[Факт/План] ,[Value] ,[dpd])
...
left join collection.loans_early_repayment e on e.external_id = p.[ДоговорНомер] and e.dt_lastpayment = cast(p.[ДатаОперации] as date)
) td
...
delete from collection.[mt_reciept_period_dpd_early_payments] 
...
insert into collection.[mt_reciept_period_dpd_early_payments] ([cdate] ,[Indicator] ,[Факт/План] ,[Value] ,Qty ,[dpd])
...
where [НаименованиеЛиста] in (N'Платежи по ОД' ,N'Платежи по процентам' ,N'Платежи по пеням') and [ДатаОперации] >= dateadd(month,datediff(month,0,dateadd(month,-1,Getdate())),0)
		and [ДоговорНомер] in (select external_id from collection.[loans_early_repayment]) 
group by [ДатаОперации] ,[НаименованиеЛиста] ,[НаименованиеПараметра]
