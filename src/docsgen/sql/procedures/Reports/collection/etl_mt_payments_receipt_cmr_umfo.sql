...
-- =============================================
create   PROCEDURE collection.[etl_mt_payments_receipt_cmr_umfo]
...
set @DateStart= dateadd(MONTH,datediff(MONTH,0,dateadd(month,-4,Getdate())),0);
set @DateStartmonth= dateadd(MONTH,datediff(MONTH,0,dateadd(month,-4,Getdate())),0);
...
set @DateStartCurr = dateadd(month,-4,dateadd(day,datediff(day,0,Getdate()),0)); --dateadd(MONTH,datediff(MONTH,0,dateadd(month,-43,Getdate())),0); --dateadd(day,-15,dateadd(day,datediff(day,0,Getdate()),0)); --
set @DateStartCurr2000= dateadd(month,-4, dateadd(month,datediff(month,0,dateadd(year,2000,Getdate())),0)); --dateadd(MONTH,datediff(MONTH,0,dateadd(month,-43,dateadd(year,2000,Getdate()))),0); --dateadd(day,-15,dateadd(day,datediff(day,0,dateadd(year,2000,Getdate())),0)); --
...
into #BusinessLoan_UMFO
  ...
  from stg._1cUMFO.[Документ_АЭ_ЗаймПредоставленный] dd
	left join stg._1cUMFO.Справочник_АЭ_ФинансовыеПродукты fp  
	with (nolock) --y
...
select --[Период] ,
      ...
      ,dateadd(day,datediff(day,0,[Период]),0) as [ДатаПлатежа]
...
select --	  [Период] ,[Регистратор_Ссылка] ,
      ...
      ,case when [ВидДвижения]=0 then [Сумма] else 0 end as [СуммаНачислено]
      ,case when [ВидДвижения]=1 then [Сумма] else 0 end as [СуммаОплачено]
from stg._1cUMFO.[РегистрНакопления_АЭ_ВзаиморасчетыПоШтрафамЗаймовПредоставленных]  with (nolock)
where [Займ] in (select distinct [Ссылка] from #BusinessLoan_UMFO)
...
--,	t_resRepayment as
--(
drop table if exists #t_resRepayment
...
where not grp.[ДатаПлатежа] is null --cast(grp.[ПериодУчета] as date)=cast(dateadd(MONTH,datediff(MONTH,0,dateadd(day,-1,Getdate())),0) as date)
 --)
 -- select * from #t_resRepayment where [НомерДоговора] = '18071700601001' /*[ДатаПлатежа] is null*/ order by 2 desc
...
from [Stg].[_1cCMR].[РегистрНакопления_РасчетыПоЗаймам] r  with (nolock) --
		--[prodsql02].[cmr].[dbo].[РегистрНакопления_РасчетыПоЗаймам] r with (nolock)		--[C1-VSR-SQL05].[CMR_NIGHT01].[dbo].[РегистрНакопления_РасчетыПоЗаймам]
left join [Stg].[_1cCMR].[Справочник_Договоры] d  with (nolock) --
		--[prodsql02].[cmr].[dbo].[Справочник_Договоры] d with (nolock)		--[C1-VSR-SQL05].[CMR_NIGHT01].[dbo].[Справочник_Договоры]
...
where r.[Период] between @DateStartCurr2000 and @GetDate2000
	...
	and not r.[ХозяйственнаяОперация] in (0xB81200155D4D085911E944418439AF38 -- сторно по акции
										  ...
										  ,0x814E00155D01BF0711E83DC81768DD69 --продажа
										  ) 
--)
...
begin tran
	delete from collection.[mt_payments_receipt_cmr_umfo]
		where [ДатаОперации] >= @DateStartmonth;

insert into collection.[mt_payments_receipt_cmr_umfo] ([ПериодУчетаЧислом] ,[ПериодУчета] ,[ДатаОперации] 
