...
--				и сохранения данных	по конфигурации лидгенов
--				в таблицу. Исходный пакет так же сохраняется.
--				== DWH-216 ==
-- =============================================
CREATE PROCEDURE Monitoring.SaveJSON4configGeneralizedLeadGenerators
	@json nvarchar(max)
AS
BEGIN
	...
	drop table if exists #t_configGeneralizedLeadGenerators
	select
		id			
		, createdAt		
		, updatedAt		
		...
		, isActive		
		, isDefault		
		, isDefaultForGeneralizedLeadGenerator
	into #t_configGeneralizedLeadGenerators
	...
	with (
		...
		, isDefault		bit							'$.attributes.isActive'
		, isDefaultForGeneralizedLeadGenerator bit	'$.attributes.isDefaultForGeneralizedLeadGenerator'
	)

	BEGIN TRY
	BEGIN TRAN;
		MERGE LogDb.Monitoring.configGeneralizedLeadGenerators	tgt
		USING #t_configGeneralizedLeadGenerators			src
		ON tgt.id = src.id
		WHEN MATCHED THEN
		UPDATE SET
			...
			tgt.isDefaultForGeneralizedLeadGenerator = src.isDefaultForGeneralizedLeadGenerator
		WHEN NOT MATCHED BY TARGET					
		THEN INSERT(
			id, createdAt, updatedAt, orderNumber, [description],
			[code], [name], isActive, isDefault, isDefaultForGeneralizedLeadGenerator
		) VALUES (
			src.id, src.createdAt, src.updatedAt, src.orderNumber,
			src.[description], src.[code], src.[name], src.isActive,
			src.isDefault, src.isDefaultForGeneralizedLeadGenerator
		);
	COMMIT;
	-- Логируем пакет
	...
	BEGIN CATCH
		...
		SELECT  @messageGuid = id FROM #t_configGeneralizedLeadGenerators
