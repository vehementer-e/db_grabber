...
-- =============================================
CREATE PROC Monitoring.ComparisonData_dm_CMRStatBalance_ОстатокПроцентовВсего
	 @isDebug bit = 0
AS
BEGIN  
	...
	declare
		@sourceTable NVARCHAR(255) = 'Stg._1cUMFO.РегистрСведений_СЗД_ПоказателиЗаймовПредоставленных',
		 ...
		 @sourceColumns NVARCHAR(MAX) = 'НомерДоговора, ОстатокПроцентовВсего',
		 @targetColumns NVARCHAR(MAX) = 'external_id, [Остаток % расчетный]',
	...
	SELECT @columnsList = concat_ws(',',
		'Договор',
		'Остаток_Процентов_ЦМР',
		'Остаток_Процентов_УМФО',
		'Разница_ОстатокПроц'
		)
	SELECT @columnsWithAlias = concat_ws(',',
		'stg.НомерДоговора',
		'trg.[Остаток % расчетный]',
		'stg.ОстатокПроцентовВсего',
		'Разница_ОстатокПроц = trg.[Остаток % расчетный] - stg.ОстатокПроцентовВсего'
		)

	SELECT @columnsWithTypes = concat_ws(',', 
		'Договор nvarchar(255)',
		'Остаток_Процентов_ЦМР nvarchar(100)',
		'Остаток_Процентов_УМФО nvarchar(100)',
		'Разница_ОстатокПроц nvarchar(100)'
		)
