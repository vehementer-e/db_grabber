...
--				и сохранения данных	в таблицу. Исходный пакет
--				так же сохраняется. DWH-205
-- =============================================
CREATE PROCEDURE [Monitoring].[SaveJSON4AutoBodyTypes]
	@json nvarchar(max)
AS
BEGIN
	...
	DROP TABLE IF EXISTS #t_autoBodyTypes
	SELECT 
		id,
		createdAt,
		updatedAt,
	...
	BEGIN TRY
	BEGIN TRAN;
		MERGE LogDb.Monitoring.autoBodyTypes	tgt
		USING #t_autoBodyTypes			src
		ON tgt.id = src.id
		WHEN MATCHED THEN
		UPDATE SET
	...
	BEGIN CATCH
		...
		SELECT  @messageGuid = id FROM #t_autoBodyTypes
