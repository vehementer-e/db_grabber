...
--				целям кредитов. Исходный пакет так же сохраняется.
--				== DWH-214 ==
-- =============================================
CREATE PROCEDURE Monitoring.SaveJSON4PurposeFromBankTypes 
	@json nvarchar(max)
AS
BEGIN
	...
	DROP TABLE IF EXISTS #t_purposeFromBankTypes
	SELECT 
		id,
		createdAt,
		updatedAt,
	...
	BEGIN TRY
	BEGIN TRAN;
		MERGE LogDb.Monitoring.purposeFromBankTypes	tgt
		USING #t_purposeFromBankTypes					src
		ON tgt.id = src.id
		WHEN MATCHED THEN
		UPDATE SET
	...
	BEGIN CATCH
		...
		SELECT  @messageGuid = id FROM #t_purposeFromBankTypes
