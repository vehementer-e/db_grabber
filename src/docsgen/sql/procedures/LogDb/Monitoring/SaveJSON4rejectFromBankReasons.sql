...
--				и сохранения данных	по причинам отказа выдачи кредита
--				в таблицу. Исходный пакет так же сохраняется.
--				== DWH-213 ==
-- =============================================
CREATE   PROCEDURE Monitoring.SaveJSON4rejectFromBankReasons 
	@json nvarchar(max)
AS
BEGIN
	...
	drop table if exists #t_rejectFromBankReasons
	select
		id				
		, createdAt			
		, updatedAt			
	...
	with(
		...
		, isTerminalReason  bit					'$.attributes.isTerminalReason'
	)
	
	BEGIN TRY
	BEGIN TRAN;
		MERGE LogDb.Monitoring.rejectFromBankReasons 	tgt
		USING #t_rejectFromBankReasons					src
		ON tgt.id = src.id
		WHEN MATCHED THEN
		UPDATE SET
	...
	BEGIN CATCH
		...
		SELECT  @messageGuid = id FROM #t_rejectFromBankReasons
