...
--				== DWH-214 ==
-- =============================================
CREATE PROCEDURE Monitoring.SaveJSON4ExternalProductTypes 
	@json nvarchar(max)
AS
BEGIN
-- Собираем данные для логирования пакета
	...
	DROP TABLE IF EXISTS #t_externalProductTypes
	SELECT 
		id,
		createdAt,
		updatedAt,
	...
	BEGIN TRY
	BEGIN TRAN;
		MERGE LogDb.Monitoring.externalProductTypes	tgt
		USING #t_externalProductTypes				src
		ON tgt.id = src.id
		WHEN MATCHED THEN
		UPDATE SET
	...
	BEGIN CATCH
		...
		SELECT  @messageGuid = id FROM #t_externalProductTypes
