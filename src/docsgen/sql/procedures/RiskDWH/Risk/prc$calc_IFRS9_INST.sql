/******************************************************************************************************
Расчет резервов МСФО9 для Installment
...
CREATE procedure [Risk].[prc$calc_IFRS9_INST] @rdt date

as


declare @src_name varchar(100) = 'IFRS9 INSTALLMENT'
...
begin try
	...
	exec dbo.prc$set_debug_info @src = @src_name, @info = 'Restuct';
	...
	with base as (
		...
		case 
		when a.external_id = '24071922234372' then 12 --Костыль!!! некорректный срок
		else isnull(a.[Срок договора в месяцах],b.term) end as term,
		a.dpd_prov as dpd,
		a.principal_rest,
		a.percents_rest,
		a.others_rest,
		...
		and (isnull(a.[Финансовый продукт],'n') like '%ПРО100%' --
		and a.external_id <> '24082222363672' --Костыль!!! PDL
			--or isnull(a.[Финансовый продукт],'n') = 'PDL' --pdl отдельно
			)
	...
	exec dbo.prc$set_debug_info @src = @src_name, @info = '#stg2_matrix';

	--агрегат месячных (переход через 1 месяц) данных
	...
	isnull(b.dpd_bucket_analyt,'[06] PAYOFF') as dpd_bucket_to,
	count(*) as cnt
	into #stg2_matrix
	from #THIN a
	left join #THIN b
	on a.external_id = b.external_id
	and a.r_date = eomonth(b.r_date,-1)
	--where not exists (select 1 from dwh2.risk.credit_restructuring r where a.external_id = r.external_id and r.operation_type = 'Кредитные каникулы')
	...
	exec dbo.prc$set_debug_info @src = @src_name, @info = '#stg3_matrix';

	--сборка месячных матриц
	...
	select @info = concat('P2 Проверка: квартальные матрицы, сумма по строке равна 1? (должно быть 0) ',count(*))
	from (
	...
	exec dbo.prc$set_debug_info @src = @src_name, @info = '#quart_matrix';
	--Усредненная месячная матрица
	...
	when a.dpd_bucket_from = '[06] PAYOFF' and a.dpd_bucket_to = '[06] PAYOFF' then 1
	when a.dpd_bucket_from = '[06] PAYOFF' and a.dpd_bucket_to <> '[06] PAYOFF' then 0
	else a.chisl / b.znam 
	end as koef
	into #quart_matrix
	from numerator a
	...
	--поглощающий для PAYOFF
	union all select '[06] PAYOFF', '[01] 0',			null, null, 0 
	union all select '[06] PAYOFF', '[02] 1-30',		null, null, 0 
	union all select '[06] PAYOFF', '[03] 31-60',		null, null, 0 
	union all select '[06] PAYOFF', '[04] 61-90',		null, null, 0 
	union all select '[06] PAYOFF', '[05] 90+',			null, null, 0 
	union all select '[06] PAYOFF', '[06] PAYOFF',		null, null, 1
	;
	...
	select @info = concat('P2 Проверка: усредненная квартальная матрица, сумма по строке равна 1? (должно быть 0) ',count(*))
	from (
	...
	exec dbo.prc$set_debug_info @src = @src_name, @info = '#year_matrix';
	--Годовая матрица
	...
	begin
		...
		set @info = concat('year matrix, iter = ' ,cast(@i as varchar(2)));
		exec dbo.prc$set_debug_info @src = @src_name, @info = @info;
	...
	exec dbo.prc$set_debug_info @src = @src_name, @info = '#TTC_matrix';
	...
	exec dbo.prc$set_debug_info @src = @src_name, @info = '#pd_adjust';

	--Калибровка модельного PD на PD PIT
	----? все дефолты или первые выходы ?
	...
	exec dbo.prc$set_debug_info @src = @src_name, @info = '#PIT_year_matrix';
	...
	select @info = concat('P2 Проверка: годовая матрица PIT, сумма по строке равна 1? (должно быть 0) ',count(*))
	from (
	...
	exec dbo.prc$set_debug_info @src = @src_name, @info = '#PIT_matrix';
	...
	--проверка: сумма по строке равна 1?
	select @info = concat('P2 Проверка: годовые матрицы PIT, сумма по строке равна 1? (должно быть 0) ',count(*))
	from (
	...
	exec dbo.prc$set_debug_info @src = @src_name, @info = '#stg1_pd';
	...
	exec dbo.prc$set_debug_info @src = @src_name, @info = '#stg2_pd';
	...
	order by a.pd_type, a.dpd_bucket, a.mon
	;
	...
	exec dbo.prc$set_debug_info @src = @src_name, @info = '#annuity';
	...
	exec dbo.prc$set_debug_info @src = @src_name, @info = '#virtual_schedule';
	...
	set @info = concat('#virtual_schedule iter = ',cast(@iii as varchar(5)));
	exec dbo.prc$set_debug_info @src = @src_name, @info = @info;
	...
	select @info = concat('P3 Проверка 1: выплат по аналитическому аннуитету не хватило для погашения ОД (должно быть 0) ',count(*))
	from #annuity a
	...
	select @info = concat('P3 Проверка 2: платеж по ОД < 0 (должно быть 0) ',count(distinct a.external_id))
	from #virtual_schedule a
	where a.od_pmt < 0
	;
	exec dbo.prc$set_debug_info @src = @src_name, @info = @info;
	...
	select @info = concat('P3 Проверка 3: дубли по договорам и MoB (должно быть 0) ',count(*))
	from (
	...
	exec dbo.prc$set_debug_info @src = @src_name, @info = '#det_month_year';
	...
	a.yr, 
	case 
	when a.yr = 1 and t.term = 3 then RiskDWH.[CM\A.Borisov].func$greatest(RiskDWH.[CM\A.Borisov].func$least(a.stg_MoB - 3, 1), 1)
	when a.yr = 1 and t.term <> 3 then RiskDWH.[CM\A.Borisov].func$greatest(RiskDWH.[CM\A.Borisov].func$least(a.stg_MoB - 3, 2), 1)
	...
	--a.stg_EAD / power(1.0 + 3.6 / 12.0, a.pd_disc_month) as stg2_EAD --ставка дисконтирования = 360% (кривой ЭПС)
	a.stg_EAD / power(1.0 + a.int_rate, a.pd_disc_month / 12.0) as stg2_EAD --ставка дисконтирования = ставка по договору (кривой ЭПС)
	from base2 a
	;
	...
	select @info = concat('P3 Проверка на дубли по договору и году (должно быть 0) ',count(*))
	from (
	...
	select @info = concat('P3 Проверка: EAD рассчитан по всем договорам (должно быть 0) ',count(*))
	from #FAT a
	...
	select @info = concat('P3 Проверка: EAD > 0? (должно быть 0) ',count(*))
	from #CCF a
	left join #FAT b
	on a.external_id = b.external_id
	...
	exec dbo.prc$set_debug_info @src = @src_name, @info = 'insert and update #CCF';
	...
	update a set 
	a.stg2_EAD = ( b.gross_rest + b.principal_rest * (b.int_rate / 12.0 * 2.0)  ) / power (1.0 + b.int_rate, 2.0 / 12.0)
	from #CCF a
	left join #FAT b
	on a.external_id = b.external_id
	...
	from risk.lgd_gross_cond a --экспертные значения!!!
	where a.r_date = '2023-10-31' 
	...
	--Проверка 1: кол-во месяцев в дефолте для 3-ей корзины должно быть >= 0
	select @info = concat('P5 Проверка: МОД < 0 ? (должно быть 0) ',count(*))
	from #FAT a
	where exists (select 1 from #stg1_provisions b where a.external_id = b.external_id and b.stage = 3 and b.months_in_default < 0)
	;
	exec dbo.prc$set_debug_info @src = @src_name, @info = @info;
	...
	select @info = concat('P5 Проверка 5: для 3-ей корзины должна быть только одна строка (должно быть 0) ',count(*)) from (
		select a.external_id
		from #stg2_provisions a
		where a.stage = 3
		group by a.external_id
