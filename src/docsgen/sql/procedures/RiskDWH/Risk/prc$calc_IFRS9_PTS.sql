...
CREATE procedure [Risk].[prc$calc_IFRS9_PTS] @rdt date

as

declare @src_name varchar(100) = 'IFRS9 PTS'
...
begin try
	...
	with base as (
		...
		where a.r_date = @rdt
		--and isnull(a.product_group,'n') not in ('Installment','Smartinstallment')
	...
	exec dbo.prc$set_debug_info @src = @src_name, @info = 'fixes';
	...
	exec dbo.prc$set_debug_info @src = @src_name, @info = '#CCF insert and update';
	...
	--дополняем до 48 месяцев со значением LGD = 1
	...
	when a.dpd > 1500 and a.dt_last_default is null then 48 
	else RiskDWH.[CM\A.Borisov].func$least( RiskDWH.[CM\A.Borisov].func$greatest( datediff(MM,a.dt_last_default,a.r_date), 0), 48)
	...
	begin transaction;
		...
		--для тех по кому остаток только по %%, пеням и пр резерв 100%
