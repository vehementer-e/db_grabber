...
19/12/23	datsyplakov			v1.1		Исправление ошибок: 
											- Остаток на месяц в дефолте
											- Дисконтирование
											- Механизм сборки LGD
...
begin try
	...
	left join dwh_new.dbo.stat_v_balance2 b
	on a.external_id = b.external_id
	and b.cdate = EOMONTH(a.npl_dt_from,m.months_in_default/*-1*/)
	and b.cdate < @cmr_point 
	left join dwh2.dbo.dm_CMRStatBalance c (nolock)
	on a.external_id = c.external_id
	and c.d = EOMONTH(a.npl_dt_from,m.months_in_default/*-1*/)
	and c.d >= @cmr_point
	;
	...
	begin transaction;
		...
		with base as (
			...
			b.months_in_default as interm_mod,
			sum(b.pay_gross) as pay_gross,
			sum(b.pay_gross  / power(1.0 + c.eff_rate, (b.months_in_default - a.months_in_default + 1)/12.0 )) as pay_gross_disc

			from #default_balance a
			left join #payments b
			on a.external_id = b.external_id
			and a.months_in_default < b.months_in_default
			and b.months_in_default <= 49

			left join #con_rate c
			on a.external_id = c.external_id

			where a.r_date >= @dt_lower_bound
			group by 
			a.months_in_default, 
			b.months_in_default,
			a.npl_dt_from
		), base2 as (
			select a.months_in_default, a.interm_mod, 
			case when sum(a.gross) = 0 then 0 else sum(a.pay_gross) / sum(a.gross) end as interm_rec,
			case when sum(a.gross) = 0 then 0 else sum(a.pay_gross_disc) / sum(a.gross) end as interm_disc_rec
			from base a
			group by a.months_in_default, a.interm_mod
		)
		...
		a.months_in_default, 
		sum(a.interm_rec) as recovery_rate,
		1.0-sum(a.interm_rec) as LGD,

		sum(a.interm_disc_rec) as recovery_rate_disc,
		1.0-sum(a.interm_disc_rec) as LGD_disc
