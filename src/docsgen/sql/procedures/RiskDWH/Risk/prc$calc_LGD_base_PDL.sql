...
23/09/25	agolicyn			v1.0		Создание процедуры. ТОЛЬКО PDL. Основа - [Risk].[prc$calc_LGD_base_bezzalog]
12/12/2025	agolicyn			v3.0		Новый источник цессий - RiskDWH.Risk.Cess_Reestr_UMFO

*****************************************************************************************************/


CREATE procedure [Risk].[prc$calc_LGD_base_PDL] 
...
begin try
	...
	exec dbo.prc$set_debug_info @src = @srcname, @info = 'Temp_LGD_PDL_con_base';

	drop table if exists Temp_LGD_PDL_con_base_mfo;
	with base as (
		select external_id, r_date, overdue_days from Risk.portf_mfo
	)
	select a.external_id, min(a.r_date) as npl_dt_from
	into Temp_LGD_PDL_con_base_mfo
	from base a
	...
	drop table if exists Temp_LGD_PDL_con_base;
	create table Temp_LGD_PDL_con_base (
	...
	begin
		...
		insert into Temp_LGD_PDL_con_base
		select a.external_id, min(a.r_date) as npl_dt_from		
		from base a
		where a.overdue_days > 90
		and a.overdue_days <= 150
	...
	from (	
		...
		from Temp_LGD_PDL_con_base_cmr a
		where a.npl_dt_from >= @cmr_point
		union all
		select * from Temp_LGD_PDL_con_base_mfo b
		where b.npl_dt_from < @cmr_point 
		and not exists (
			select 1 from Temp_LGD_PDL_con_base_cmr c
	...
	with a as (select * from Temp_LGD_PDL_con_base) 
	...
	exec dbo.prc$set_debug_info @src = @srcname, @info = 'Temp_LGD_PDL_stg_con_rate';

	-- эффективная ставка (для тех, по кому есть)
	drop table if exists Temp_LGD_PDL_stg_con_rate;
	select distinct  c.external_id, 
						c.npl_dt_from as generation, 
						max(cast(r.эффективнаяставкапроцента as float)/100) as eff_rate 
	into Temp_LGD_PDL_stg_con_rate
	from Temp_LGD_PDL_con_base c
	...
	exec dbo.prc$set_debug_info @src = @srcname, @info = 'Temp_LGD_PDL_con_rate';


	--по тем, у кого нет ЭПС, средняя по поколению или средняя по всем
	drop table if exists Temp_LGD_PDL_con_rate;

	with avg_generation as 
	(
		select a.generation, avg(a.eff_rate) as eff_rate
		from Temp_LGD_PDL_stg_con_rate a
		group by a.generation
	),
	avg_total as (
		select avg(aa.eff_rate) as eff_rate
		from Temp_LGD_PDL_stg_con_rate aa
	)
	select a.external_id, 
	coalesce( b.eff_rate, ag.eff_rate, t.eff_rate) as eff_rate

	into Temp_LGD_PDL_con_rate

	from Temp_LGD_PDL_con_base a
	left join Temp_LGD_PDL_stg_con_rate b
	...
	exec dbo.prc$set_debug_info @src = @srcname, @info = 'Temp_LGD_PDL_od_before_npl';
	...
	drop table if exists Temp_LGD_PDL_od_before_npl;
	...
	select c.external_id, 
			...
			z.principal_rest, 
			z.total_rest
	into Temp_LGD_PDL_od_before_npl
	from Temp_LGD_PDL_con_base c
	...
	exec dbo.prc$set_debug_info @src = @srcname, @info = 'Temp_LGD_PDL_write_off_umfo';

	
	--собираем все списания (прощения)
	drop table if exists Temp_LGD_PDL_stg1_umfo_writeoff;
	select 
	...
	into Temp_LGD_PDL_stg1_umfo_writeoff
	...
	drop table if exists Temp_LGD_PDL_stg2_umfo_writeoff;
	...
	into Temp_LGD_PDL_stg2_umfo_writeoff
	from Temp_LGD_PDL_stg1_umfo_writeoff a
	where 1=1
	...
	drop table if exists Temp_LGD_PDL_write_off_umfo;
	with base as (
		select a.external_id, a.r_date, a.close_reason,
		...
		from Temp_LGD_PDL_stg2_umfo_writeoff a
	)
	select * 
	into Temp_LGD_PDL_write_off_umfo
	from base a
	...
	--)
	--insert into	Temp_LGD_PDL_write_off_umfo
	--select a.external_id, a.r_date, a.close_reason,
	--case when a.int_wo < 0 then 0 else a.int_wo end as int_wo,
	--case when a.fee_wo < 0 then 0 else a.fee_wo end as fee_wo,
	--case when a.od_forgive < 0 then 0 else a.od_forgive end as od_forgive,
	...
	exec dbo.prc$set_debug_info @src = @srcname, @info = 'Temp_LGD_PDL_write_off__mfo_with_offer';


	drop table if exists Temp_LGD_PDL_write_off__mfo_with_offer;
	...
	where not exists (select 1 from Temp_LGD_PDL_write_off_umfo w
	...
	into Temp_LGD_PDL_write_off__mfo_with_offer
	from base b
	...
	drop table if exists Temp_LGD_PDL_cmr_writeoff;
	with base as (
	...
	select a.*
	into Temp_LGD_PDL_cmr_writeoff
	from base a
	where a.od_wo > 0
	;
	...
	from (
		select a.external_id
		from Temp_LGD_PDL_cmr_writeoff a
		group by a.external_id
		having count(*)>1
	) a
	...
	drop table if exists Temp_LGD_PDL_stg_write_off;
	...
	into Temp_LGD_PDL_stg_write_off
	from Temp_LGD_PDL_pre_write_off a
	left join Temp_LGD_PDL_cmr_writeoff b
	on a.external_id = b.external_id
	...
	exec dbo.prc$set_debug_info @src = @srcname, @info = 'Temp_LGD_PDL_payments';



	--платежи после выхода в npl, группировка по месяцам

	drop table if exists Temp_LGD_PDL_payments;
	...
	into Temp_LGD_PDL_payments

	from Temp_LGD_PDL_od_before_npl a
	...
	left join Temp_LGD_PDL_stg_write_off w_o
	...
	update Temp_LGD_PDL_payments set pay_od = 0  where isnull(pay_od,0) <= 0.01;
	update Temp_LGD_PDL_payments set pay_total = 0  where isnull(pay_total,0) <= 0.01;
	update Temp_LGD_PDL_payments set pay_percents = 0 where isnull(pay_percents,0) <= 0.01;
	update Temp_LGD_PDL_payments set pay_fines = 0 where isnull(pay_fines,0) <= 0.01;
	update Temp_LGD_PDL_payments set pay_straf = 0 where isnull(pay_straf,0) <= 0.01;
	update Temp_LGD_PDL_payments set writeoff_od = 0 where isnull(writeoff_od,0) <= 0.01;
	update Temp_LGD_PDL_payments set writeoff_int = 0 where isnull(writeoff_int,0) <= 0.01;



	exec dbo.prc$set_debug_info @src = @srcname, @info = 'Temp_LGD_PDL_payments_discount';

	--дисконтированные платежи
	drop table if exists Temp_LGD_PDL_payments_discount;
	...
	into Temp_LGD_PDL_payments_discount

	from Temp_LGD_PDL_payments a
	left join Temp_LGD_PDL_con_rate b
	on a.external_id = b.external_id;
	...
	exec dbo.prc$set_debug_info @src = @srcname, @info = 'Temp_LGD_PDL_Legal';

	--Суды

	drop table if exists Temp_LGD_PDL_Legal;
	with src as (
	SELECT DISTINCT d.number as external_id
	,cast(d.Date as date) as startdate
	,case when d.Date<'2022-11-01' then 'PDL-OLD' else 'PDL-NEW' end as Inst_Type
	...
	*/
	...
	exec dbo.prc$set_debug_info @src = @srcname, @info = 'Temp_LGD_PDL_bankrupt';


	drop table if exists Temp_LGD_PDL_bankrupt;

	select distinct a.external_id
	into Temp_LGD_PDL_bankrupt
	...
	*/
	...
	with pmt as (
		...
		from Temp_LGD_PDL_payments_discount a
		left join Temp_LGD_PDL_legal b
		on a.external_id = b.external_id
		left join Temp_LGD_PDL_bankrupt c
	...
	insert into risk.prov_stg_PDL_LGD
