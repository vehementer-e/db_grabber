...
---------------- МЕНЯТЬ: У каждого прогноза продукта-канала своя версия. Файл с версионностью для версии
@vers int = 721,
---------------- МЕНЯТЬ: У каждого продукта-канала своя ставка. Присылает комблок
@avg_rate float = 80.0,
...
-- Информационные логи
@vinfo_start varchar(256) = 'PTS CM Start',
@vinfo_finish varchar(256) = 'PTS CM Finish'
...
begin try
	...
	drop table if exists Temp_PTS_CM_Channel
	...
	select --
		...
		as Product_Channel
		, count(*) as cnt
	into Temp_PTS_CM_Channel--select top 100 *
	...
	drop table if exists Temp_PTS_CM_CMR_M;
	select a.external_id, a.d, a.[остаток од], a.dpd 
	into Temp_PTS_CM_CMR_M
	...
	--select *
	from Temp_PTS_CM_CMR_M a
	where a.[остаток од] < 0
	;

	--итоговый список списаний
	drop table if exists Temp_PTS_CM_stg_write_off;

	select b.external_id, b.r_date, b.od_wo, b.int_wo
	into Temp_PTS_CM_stg_write_off
	...
	print ('timing, Temp_PTS_CM_stg_write_off ' + format(getdate(), 'HH:mm:ss'));
	...
	drop table if exists Temp_PTS_CM_det_current_params;
	with base as (
	...
	into Temp_PTS_CM_det_current_params
	from base a
	where a.rn = 1
	;
	...
	with base as (
		...
		and not exists (select 1 from Temp_PTS_CM_det_current_params c where b.Код = c.external_id)
	)
	insert into Temp_PTS_CM_det_current_params
	select a.external_id, a.cur_rate, a.cur_plan_end_date
	from base a
	where a.rn = 1
	;
	...
	drop table if exists Temp_PTS_CM_det_bucket_360;
	select * 
	into Temp_PTS_CM_det_bucket_360
	from (values
	('[01] 0', 1),
	('[02] 1-30', 2),
	('[03] 31-60', 3),
	...
	drop table if exists Temp_PTS_CM_det_bucket_90;
	select *  
	into Temp_PTS_CM_det_bucket_90
	from (values
	('[01] 0', 1),
	('[02] 1-30', 2),
	('[03] 31-60', 3),
	...
	print ('timing, Temp_PTS_CM_cred_end_date ' + format(getdate(), 'HH:mm:ss'));
	...
	drop table if exists Temp_PTS_CM_cred_CMR_status;
	...
	into Temp_PTS_CM_cred_CMR_status
	...
	delete from Temp_PTS_CM_cred_CMR_status where rown <> 1;


	print ('timing, Temp_PTS_CM_cred_CMR_status ' + format(getdate(), 'HH:mm:ss'));
	...
	drop table if exists Temp_PTS_CM_stg1_zamorozka;
	select a.Договор as Договор, a.Дата as Дата 
	into Temp_PTS_CM_stg1_zamorozka
	...
	drop table if exists Temp_PTS_CM_stg2_zamorozka;
	select b.Код as external_id, eomonth(dateadd(yy,-2000,cast(a.Дата as date))) as freeze_dt
	into Temp_PTS_CM_stg2_zamorozka
	from Temp_PTS_CM_stg1_zamorozka a
	left join stg._1cCMR.Справочник_Договоры b
	on a.Договор = b.Ссылка
	;

	drop table if exists Temp_PTS_CM_zamorozka;
	select a.external_id, 
	min(a.freeze_dt) as freeze_from,
	eomonth(max(a.freeze_dt),3) as freeze_to
	into Temp_PTS_CM_zamorozka
	from Temp_PTS_CM_stg2_zamorozka a
	...
	with a as (select * from Temp_PTS_CM_zamorozka a)
	delete from a
	where exists (select 1 from Temp_PTS_CM_bankrupt b where a.external_id = b.external_id and b.dt <= @rdt)
	;

	print ('timing, Temp_PTS_CM_zamorozka ' + format(getdate(), 'HH:mm:ss'));



	--ЭПС из УМФО

	drop table if exists Temp_PTS_CM_stg2_eps;
	...
	--Перечень договоров
	drop table if exists Temp_PTS_CM_cred_reestr;
	...
	into Temp_PTS_CM_cred_reestr
	--select distinct cred_type
	from dwh2.risk.credits a
	left join Temp_PTS_CM_cred_end_date b
	...
	left join Temp_PTS_CM_bankrupt bkr
	...
	left join Temp_PTS_CM_eps ee
	on a.external_id = ee.external_id

	left join RiskDWH.risk.stg_fcst_fix_eps ffep
	on a.external_id = ffep.external_id

	left join RiskDWH.risk.Temp_PTS_CM_Channel ch
	on a.external_id = ch.external_id

	where cast(a.startdate as date) between cast('2016-01-01' as date) and @rdt
		and not exists (select 1 from Temp_PTS_CM_cred_CMR_status sts where a.external_id = sts.external_id and sts.cred_status = 'Внебаланс')
		and not (a.IsInstallment = 1)
		and not (a.credit_type_init = 'AUTOCREDIT')
		and not (upper(a.credit_type) = upper('bigInstallment'))
	...
	insert into Temp_PTS_CM_cred_reestr 
	...
	--from Temp_PTS_CM_bus_cred_reestr a
	...
	with base as (
		...
		from Temp_PTS_CM_cred_reestr a
	...
	from Temp_PTS_CM_cred_reestr b
	inner join base a
	on a.external_id = b.external_id
	...
	from Temp_PTS_CM_cred_reestr aa
	left join Temp_PTS_CM_det_current_params b
	...
	update Temp_PTS_CM_cred_reestr set eps = 0 where eps is null;


	drop index if exists idx_cred_reestr on Temp_PTS_CM_cred_reestr;
	create clustered index idx_cred_reestr on Temp_PTS_CM_cred_reestr (external_id);



	print ('timing, Temp_PTS_CM_cred_reestr ' + format(getdate(), 'HH:mm:ss'));


	--Историческая просрочка
	drop table if exists Temp_PTS_CM_det_historical_dpd;

	select a.r_date, a.external_id, a.dpd_final
	into Temp_PTS_CM_det_historical_dpd
	...
	--exec dbo.prc$set_debug_info @src = @srcname, @info = 'Temp_PTS_CM_stg_bal';

	--Остатки на различные MOB-ы
	drop table if exists Temp_PTS_CM_stg_bal;
	...
	into Temp_PTS_CM_stg_bal
	from Temp_PTS_CM_cred_reestr a
	inner join Temp_PTS_CM_CMR_M b
	on a.external_id = b.external_id

	left join Temp_PTS_CM_det_historical_dpd h
	...
	--31/08/2021 - Бизнес-займы
	insert into Temp_PTS_CM_stg_bal 
	...
	drop table if exists Temp_PTS_CM_umfo_last_fact;
	select 
	a.external_id, 
	a.r_date,
	a.total_od as od
	into Temp_PTS_CM_umfo_last_fact
	...
	from Temp_PTS_CM_stg_bal a
	left join Temp_PTS_CM_umfo_last_fact b
	on a.external_id = b.external_id
	...
	drop table Temp_PTS_CM_umfo_last_fact;


	--для учета погашенных в нулевом MoB-е

	 merge into Temp_PTS_CM_stg_bal dst
	 using Temp_PTS_CM_cred_reestr src 
	...
	from Temp_PTS_CM_stg_bal a
	inner join Temp_PTS_CM_zamorozka b
	on a.external_id = b.external_id
	...
	print ('timing, Temp_PTS_CM_stg_bal ' + format(getdate(), 'HH:mm:ss'));



	--exec dbo.prc$set_debug_info @src = @srcname, @info = 'Temp_PTS_CM_stg_matrix_detail';

	--сборка переходов подоговорно (присоединение t+1 MOB к t)
	drop table if exists Temp_PTS_CM_stg_matrix_detail; 
	...
	into Temp_PTS_CM_stg_matrix_detail

	from Temp_PTS_CM_stg_bal a
	left join Temp_PTS_CM_stg_bal b
	on a.external_id = b.external_id
	and a.mob = b.mob - 1
	left join Temp_PTS_CM_cred_reestr c
	...
	drop index if exists idx1_stg_matrix_detail on Temp_PTS_CM_stg_matrix_detail;
	create clustered index idx1_stg_matrix_detail on Temp_PTS_CM_stg_matrix_detail (external_id, mob_date_to, generation, term, segment_rbp, flag_kk, mob_from, bucket_90_from, bucket_90_to);

	----------------------------------------------------

	--проверяем переходы на корректность (например, из 0 может перейти в 0 или в 1-30) и учет КК

	--exec dbo.prc$set_debug_info @src = @srcname, @info = 'CORRECTIONS Temp_PTS_CM_stg_matrix_detail';

	--обновление по закрытым договорам
	update Temp_PTS_CM_stg_matrix_detail
	...
	a.dpd_to = 0,
	flag_wrong_migration = 0
	from Temp_PTS_CM_stg_matrix_detail a
	...
	--delete from Temp_PTS_CM_stg_matrix_detail 
	...
	drop table if exists Temp_PTS_CM_wrong_migrations1;
	select * 
	into Temp_PTS_CM_wrong_migrations1
	from Temp_PTS_CM_stg_matrix_detail a
	where exists (select 1 from Temp_PTS_CM_stg_matrix_detail b
	...
	--сохраняем в отдельную таблицу 
	drop table if exists Temp_PTS_CM_wrong_migrations2;
	select * 
	into Temp_PTS_CM_wrong_migrations2
	from Temp_PTS_CM_stg_matrix_detail a
	...
	print ('timing, Temp_PTS_CM_stg_matrix_detail ' + format(getdate(), 'HH:mm:ss'));



	--drop table Temp_PTS_CM_bus_stg_bal;
	drop table Temp_PTS_CM_cred_end_date;
	--drop table Temp_PTS_CM_stg_bal;
	...
	--Агрегат
	drop table if exists Temp_PTS_CM_stg1_agg;
	select 
	...
	print ('timing, Temp_PTS_CM_stg1_agg ' + format(getdate(), 'HH:mm:ss'));



	--Для учета гашений по графику в группе Pay-off
	drop table if exists Temp_PTS_CM_standart_payoff;
	...
	into Temp_PTS_CM_standart_payoff
	from Temp_PTS_CM_stg1_agg a
	...
	print ('timing, Temp_PTS_CM_standart_payoff ' + format(getdate(), 'HH:mm:ss'));


	drop table if exists Temp_PTS_CM_stg2_agg;
	select a.generation, a.term, a.segment_rbp, a.flag_kk,
	...
	into Temp_PTS_CM_stg2_agg
	from Temp_PTS_CM_stg1_agg a
	left join Temp_PTS_CM_standart_payoff b
	...
	insert into Temp_PTS_CM_stg2_agg
	...
	where not exists (select 1 from Temp_PTS_CM_stg1_agg b
	...
	print ('timing, Temp_PTS_CM_stg2_agg ' + format(getdate(), 'HH:mm:ss'));




	drop table if exists Temp_PTS_CM_base_for_stg3_agg;

	with base as (
		select distinct a.generation, a.term, a.segment_rbp, a.flag_kk from Temp_PTS_CM_cred_reestr a	
	), dt as (
		...
		select d.dpd_bucket_90 from Temp_PTS_CM_det_bucket_90 d
		union all
	...
	into Temp_PTS_CM_base_for_stg3_agg
	from base a
	left join dt 
	...
	print ('timing, Temp_PTS_CM_base_for_stg3_agg ' + format(getdate(), 'HH:mm:ss'));



	--полная матрица групп и дат для корректного построени средних значений в Excel
	drop table if exists Temp_PTS_CM_stg3_agg;
	...
	into Temp_PTS_CM_stg3_agg
	from Temp_PTS_CM_base_for_stg3_agg b
	left join Temp_PTS_CM_stg2_agg s
	...
	--drop table Temp_PTS_CM_base_for_stg3_agg;



	--удаляем лишние срезы
	drop table if exists Temp_PTS_CM_for_delete;
	select a.generation, a.term, a.segment_rbp, a.flag_kk
	into Temp_PTS_CM_for_delete
	from Temp_PTS_CM_stg3_agg a
	group by a.generation, a.term, a.segment_rbp, a.flag_kk
	having sum(a.od_from) = 0
	;


	with a as (select * from Temp_PTS_CM_stg3_agg)
	delete from a
	where exists (select 1 from Temp_PTS_CM_for_delete b
				where a.generation = b.generation
				and a.term = b.term
				and a.segment_rbp = b.segment_rbp
				and a.flag_kk = b.flag_kk
	...
	print ('timing, Temp_PTS_CM_stg3_agg ' + format(getdate(), 'HH:mm:ss'));
	...
	drop table if exists Temp_PTS_CM_rest_model;
	...
	--ПТС31
	insert into Temp_PTS_CM_rest_model
	...
	drop table Temp_PTS_CM_stg1_agg;
	drop table Temp_PTS_CM_stg2_agg;
	drop table Temp_PTS_CM_for_delete;
	drop table Temp_PTS_CM_standart_payoff;
	drop table Temp_PTS_CM_det_bucket_360;
	drop table Temp_PTS_CM_det_bucket_90;
	...
	/*
	drop table if exists Temp_PTS_CM_term_generation_cnt;
	select a.term, a.generation, a.segment_rbp, count(*) as cnt, sum(a.amount) as amount
	into Temp_PTS_CM_term_generation_cnt
	from Temp_PTS_CM_cred_reestr a
	group by a.term, a.generation, a.segment_rbp;


	select * from Temp_PTS_CM_term_generation_cnt a
	...
	*/
	...
	from Temp_PTS_CM_stg3_agg a --Temp_PTS_CM_stg4_agg a
	...
	*/
	...
	/*
	select a.generation, a.segment_rbp, a.term, count(*) as cnt, sum(a.amount) as amount, sum(a.int_rate * a.amount) / sum(a.amount) as EIR
	from Temp_PTS_CM_cred_reestr a where year(generation) >= (year(getdate()) - 1)
	group by a.generation, a.segment_rbp, a.term
	order by 1 desc
	;

	select * from Temp_PTS_CM_stg_matrix_detail a
	...
	*/
	...
	drop table if exists Temp_PTS_CM_stg_freeze_mod;
	select a.external_id, 
	--b.mob_date_to, 
	...
	into Temp_PTS_CM_stg_freeze_mod
	from Temp_PTS_CM_zamorozka a
	inner join Temp_PTS_CM_stg_matrix_detail b
	on a.external_id = b.external_id
	...
	drop table if exists Temp_PTS_CM_stg_start_default;

	select a.external_id, 
	min(b.d) as dt_default_from, eomonth(min(b.d)) as month_default_from
	into Temp_PTS_CM_stg_start_default
	from Temp_PTS_CM_cred_reestr a
	...
	and not exists (select 1 from Temp_PTS_CM_zamorozka z where a.external_id = z.external_id and eomonth(@dt_back_test_from,-1) between z.freeze_from and z.freeze_to) --заморозка
	...
	drop table if exists Temp_PTS_CM_npl_cred_with_broken_dpd;
	...
	with base as (
		...
		and not exists (select 1 from risk.stg_fcst_CMR_lite b --dwh2.dbo.dm_CMRStatBalance b (nolock)
						...
						and b.dpd = 91)
		and not exists (select 1 from Temp_PTS_CM_zamorozka z where a.external_id = z.external_id and eomonth(@dt_back_test_from,-1) between z.freeze_from and z.freeze_to) --заморозка
		and not exists (select 1 from Temp_PTS_CM_cred_reestr cr where a.external_id = cr.external_id and cr.flag_kk = 'BUSINESS')
		group by a.external_id
	)
	...
	into Temp_PTS_CM_npl_cred_with_broken_dpd
	from base b
	...
	insert into Temp_PTS_CM_stg_start_default 
	(external_id, dt_default_from, month_default_from)
	select a.external_id, a.dt_default_from, a.month_default_from
	from Temp_PTS_CM_npl_cred_with_broken_dpd a
	where not exists (select 1 from Temp_PTS_CM_stg_start_default b
					where a.external_id = b.external_id)
	;
	...
	--from Temp_PTS_CM_bus_default_from a
	...
	from Temp_PTS_CM_stg_freeze_mod a
	where exists (select 1 from Temp_PTS_CM_stg_matrix_detail s 
				  where a.external_id = s.external_id
				  and s.bucket_90_from = '[07] Freeze' and s.bucket_90_to not in ('[07] Freeze', '[06] Pay-off'))
	and not exists (select 1 from Temp_PTS_CM_stg_start_default d where a.external_id = d.external_id)
	;
	...
	from Temp_PTS_CM_stg_matrix_detail a
	left join Temp_PTS_CM_stg_start_default b
	on a.external_id = b.external_id
	inner join (select a.external_id, min(eomonth(a.freeze_dt)) as freeze_dt from Temp_PTS_CM_stg2_zamorozka a group by a.external_id) z
	on a.external_id = z.external_id
	inner join Temp_PTS_CM_stg_matrix_detail c
	on a.external_id = c.external_id
	...
	and a.dpd_to > 90
	and not exists (select 1 from Temp_PTS_CM_stg_start_default b where a.external_id = b.external_id)

	print ('CNT without default dt = ' + format(@@rowcount,'Temp_PTS_CM_0'));


	print ('timing, Temp_PTS_CM_stg_start_default' + format(getdate(), 'HH:mm:ss'))
	...
	drop table if exists Temp_PTS_CM_fact_due_int;
	...
	into Temp_PTS_CM_fact_due_int
	from RiskDWH.Risk.stg_fcst_umfo a
	inner join Temp_PTS_CM_cred_reestr b
	on a.external_id = b.external_id

	left join Temp_PTS_CM_zamorozka z
	on a.external_id = z.external_id
	and a.r_date >= z.freeze_from --between z.freeze_from and z.freeze_to
	left join Temp_PTS_CM_stg_start_default d
	on a.external_id = d.external_id
	and d.month_default_from <= a.r_date
	left join Temp_PTS_CM_det_historical_dpd hd
	on a.external_id = hd.external_id
	and a.r_date = hd.r_date
	left join Temp_PTS_CM_stg_freeze_mod fm
	on a.external_id = fm.external_id
	and a.r_date = fm.mob_date_to
	left join Temp_PTS_CM_stg_matrix_detail md
	...
	print ('timing, Temp_PTS_CM_fact_due_int ' + format(getdate(), 'HH:mm:ss'))




	drop table if exists Temp_PTS_CM_stg_payment;
	...
	drop table if exists Temp_PTS_CM_stg_fact_od_int_pmt;
	...
	into Temp_PTS_CM_stg_fact_od_int_pmt

	from Temp_PTS_CM_stg_matrix_detail a
	inner join Temp_PTS_CM_stg_payment b
	on a.external_id = b.external_id 
	...
	insert into Temp_PTS_CM_stg_fact_od_int_pmt
	...
	sum(b.fines_cnl) as fines_cnl,
	sum(b.other_cnl) as other_cnl
	from Temp_PTS_CM_stg_payment b
	inner join Temp_PTS_CM_stg_matrix_detail a
	on a.external_id = b.external_id
	and eomonth(b.cdate) = a.mob_date_from
	where not exists (select 1 from Temp_PTS_CM_stg_matrix_detail c where b.external_id = c.external_id and eomonth(b.cdate) = c.mob_date_to)
	...
	sum(b.other_cnl) as other_cnl
	from Temp_PTS_CM_stg_payment b
	inner join Temp_PTS_CM_cred_reestr a
	on a.external_id = b.external_id
	...
	print ('timing, Temp_PTS_CM_stg_fact_od_int_pmt ' + format(getdate(), 'HH:mm:ss'))
	...
	drop table if exists Temp_PTS_CM_stg1_NU_segment_distrib;
	...
	into Temp_PTS_CM_stg1_NU_segment_distrib
	from base a
	...
	---последнее фактическое поколение, т.к. в Temp_PTS_CM_stg_matrix_detail нет записей, где mob_date_to = generation
	with base as (
		select 
		...
		inner join Temp_PTS_CM_cred_reestr cr
	...
	insert into Temp_PTS_CM_stg1_NU_segment_distrib
	...
	drop table if exists Temp_PTS_CM_stg2_NU_segment_distrib;
	...
	into Temp_PTS_CM_stg2_NU_segment_distrib--select top 100 *
	from Temp_PTS_CM_stg1_NU_segment_distrib a
	...
	drop table if exists Temp_PTS_CM_NU_segment_distrib
	select a.generation, a.segment_rbp, a.term, a.flag_kk, a.bucket_90_to, a.NU_segment,
	a.principal_rest_to,
	...
	into Temp_PTS_CM_NU_segment_distrib
	from Temp_PTS_CM_stg2_NU_segment_distrib a
	...
	left join (select distinct b.term, b.segment_rbp, b.flag_kk, b.generation from Temp_PTS_CM_NU_segment_distrib b) b
	on 1=1
	), distbucks as (
	select distinct a.term, a.segment_rbp, a.flag_kk, a.generation, a.bucket_90_to from Temp_PTS_CM_NU_segment_distrib a
	), base as (
	...
	from subportfs a
	left join Temp_PTS_CM_NU_segment_distrib b
	on a.term = b.term
	and a.segment_rbp = b.segment_rbp
	and a.flag_kk = b.flag_kk
	...
	insert into Temp_PTS_CM_NU_segment_distrib
	...
	from Temp_PTS_CM_NU_segment_distrib a
	left join (
	...
	*/
	...
	drop table if exists Temp_PTS_CM_UMFO_NU_segment;
	...
	into Temp_PTS_CM_UMFO_NU_segment
	from RiskDWH.Risk.stg_fcst_umfo a
	...
	drop table if exists Temp_PTS_CM_stg_UMFO_NU_segment;
	with base as (
		select a.external_id, cast(b.dt as date) as rdt
		from (select distinct a.external_id from Temp_PTS_CM_UMFO_NU_segment a) a
		...
		from base a
		left join Temp_PTS_CM_UMFO_NU_segment b
		on a.external_id = b.external_id
		and a.rdt = b.r_date
	)
	select a.external_id, a.r_date, b.NU_segment
	into Temp_PTS_CM_stg_UMFO_NU_segment
	from base2 a
	left join Temp_PTS_CM_UMFO_NU_segment b
	on a.external_id = b.external_id
	...
	merge into Temp_PTS_CM_UMFO_NU_segment dst
	using Temp_PTS_CM_stg_UMFO_NU_segment src
	...
	insert into Temp_PTS_CM_UMFO_NU_segment
	...
	from Temp_PTS_CM_cred_reestr a
	left join Temp_PTS_CM_bankrupt b
	on a.external_id = b.external_id
	...
	where not exists (select 1 from Temp_PTS_CM_UMFO_NU_segment b where a.external_id = b.external_id)
	;



	print ('timing, Temp_PTS_CM_UMFO_NU_segment ' + format(getdate(), 'HH:mm:ss'))




	drop table if exists Temp_PTS_CM_umfo_fact;
	select 
	a.r_date,
	a.external_id, 
	...
	into Temp_PTS_CM_umfo_fact
	from RiskDWH.Risk.stg_fcst_umfo a
	inner join Temp_PTS_CM_cred_reestr b
	on a.external_id = b.external_id
	left join Temp_PTS_CM_det_historical_dpd d
	on a.external_id = d.external_id
	...
	print ('timing, Temp_PTS_CM_umfo_fact ' + format(getdate(), 'HH:mm:ss'))

	--select * from Temp_PTS_CM_det_pieces_L1 order by 1

	--
	--Штуки (кол-во договоров)
	;
	drop table if exists Temp_PTS_CM_det_pieces_L1;
	select 
	RiskDWH.dbo.get_bucket_360_m( a.dpd ) as bucket_360_m, 
	sum(a.due_od) / cast(count(*) as float) as avg_od
	into Temp_PTS_CM_det_pieces_L1
	from Temp_PTS_CM_umfo_fact a
	inner join Temp_PTS_CM_cred_reestr b
	on a.external_id = b.external_id
	where a.r_date = @rdt 
	...
	print ('timing, Temp_PTS_CM_det_pieces_L1 ' + format(getdate(), 'HH:mm:ss'))

	
	--select * from Temp_PTS_CM_det_pieces_L2 order by 1

	drop table if exists Temp_PTS_CM_det_pieces_L2;
	select 
	iif(b.segment_rbp = 'RBP 1', 'RBP 1', 'rest') as segment_rbp_2 ,
	RiskDWH.dbo.get_bucket_360_m( a.dpd ) as bucket_360_m, 
	sum(a.due_od ) / cast(count(*) as float) as avg_od
	into Temp_PTS_CM_det_pieces_L2
	from Temp_PTS_CM_umfo_fact a
	inner join Temp_PTS_CM_cred_reestr b
	on a.external_id = b.external_id
	where a.r_date = @rdt 
	...
	print ('timing, Temp_PTS_CM_det_pieces_L2 ' + format(getdate(), 'HH:mm:ss'))
	...
	/* P 1-2 */
	drop table Temp_PTS_CM_bankrupt;
	--drop table Temp_PTS_CM_CMR_M;
	drop table Temp_PTS_CM_cred_CMR_status;
	--drop table Temp_PTS_CM_cred_reestr;
	--drop table Temp_PTS_CM_det_current_params;
	drop table Temp_PTS_CM_det_historical_dpd;
	--drop table Temp_PTS_CM_det_pieces_L1;
	--drop table Temp_PTS_CM_det_pieces_L2;
	--drop table Temp_PTS_CM_eps;
	--drop table Temp_PTS_CM_fact_due_int;
	drop table Temp_PTS_CM_npl_cred_with_broken_dpd;
	--drop table Temp_PTS_CM_NU_segment_distrib;
	--drop table Temp_PTS_CM_rest_model;
	drop table Temp_PTS_CM_stg_bal;
	drop table Temp_PTS_CM_stg_bankrupt;
	--drop table Temp_PTS_CM_stg_fact_od_int_pmt;
	--drop table Temp_PTS_CM_stg_freeze_mod;
	--drop table Temp_PTS_CM_stg_matrix_detail;
	--drop table Temp_PTS_CM_stg_payment;
	--drop table Temp_PTS_CM_stg_start_default;
	drop table Temp_PTS_CM_stg_UMFO_NU_segment;
	--drop table Temp_PTS_CM_stg_write_off;
	--drop table Temp_PTS_CM_stg1_NU_segment_distrib;
	drop table Temp_PTS_CM_stg1_zamorozka;
	drop table Temp_PTS_CM_stg2_eps;
	drop table Temp_PTS_CM_stg2_NU_segment_distrib;
	drop table Temp_PTS_CM_stg2_zamorozka;
	--drop table Temp_PTS_CM_stg3_agg;
	--drop table Temp_PTS_CM_umfo_fact;
	--drop table Temp_PTS_CM_UMFO_NU_segment;
	drop table Temp_PTS_CM_wrong_migrations1;
	drop table Temp_PTS_CM_wrong_migrations2;
	--drop table Temp_PTS_CM_zamorozka;
	...
	drop table if exists Temp_PTS_CM_bkp_cred_reestr;
	drop table if exists Temp_PTS_CM_bkp_stg3_agg;
	drop table if exists Temp_PTS_CM_bkp_stg_matrix_detail;
	drop table if exists Temp_PTS_CM_bkp_fact_due_int;
	drop table if exists Temp_PTS_CM_bkp_stg_fact_od_int_pmt;

	select * into Temp_PTS_CM_bkp_cred_reestr from Temp_PTS_CM_cred_reestr;
	select * into Temp_PTS_CM_bkp_stg3_agg from Temp_PTS_CM_stg3_agg;
	select * into Temp_PTS_CM_bkp_stg_matrix_detail from Temp_PTS_CM_stg_matrix_detail a
	select * into Temp_PTS_CM_bkp_fact_due_int from Temp_PTS_CM_fact_due_int a
	select * into Temp_PTS_CM_bkp_stg_fact_od_int_pmt from Temp_PTS_CM_stg_fact_od_int_pmt a
	...
	delete from Temp_PTS_CM_cred_reestr; insert into Temp_PTS_CM_cred_reestr select * from Temp_PTS_CM_bkp_cred_reestr;
	delete from Temp_PTS_CM_stg3_agg; insert into Temp_PTS_CM_stg3_agg select * from Temp_PTS_CM_bkp_stg3_agg;
	delete from Temp_PTS_CM_stg_matrix_detail; insert into Temp_PTS_CM_stg_matrix_detail select * from Temp_PTS_CM_bkp_stg_matrix_detail;
	delete from Temp_PTS_CM_fact_due_int; insert into Temp_PTS_CM_fact_due_int select * from Temp_PTS_CM_bkp_fact_due_int;
	delete from Temp_PTS_CM_stg_fact_od_int_pmt; insert into Temp_PTS_CM_stg_fact_od_int_pmt select * from Temp_PTS_CM_bkp_stg_fact_od_int_pmt;
	...
	*/
	...
	delete from Temp_PTS_CM_cred_reestr where flag_kk in ('INSTALLMENT', 'PTS_PSB', 'AUTOCREDIT', 'PTS_Infoseti', 'PTS_TBank');
	delete from Temp_PTS_CM_stg3_agg where flag_kk in ('INSTALLMENT', 'PTS_PSB', 'AUTOCREDIT', 'PTS_Infoseti', 'PTS_TBank');
	delete from Temp_PTS_CM_stg_matrix_detail where flag_kk in ('INSTALLMENT', 'PTS_PSB', 'AUTOCREDIT', 'PTS_Infoseti', 'PTS_TBank');
	delete from Temp_PTS_CM_fact_due_int where flag_kk in ('INSTALLMENT', 'PTS_PSB', 'AUTOCREDIT', 'PTS_Infoseti', 'PTS_TBank');
	delete from Temp_PTS_CM_stg_fact_od_int_pmt where flag_kk in ('INSTALLMENT', 'PTS_PSB', 'AUTOCREDIT', 'PTS_Infoseti', 'PTS_TBank');
	--delete from Temp_PTS_CM_stg_bal where flag_kk in ('INSTALLMENT', 'PTS_PSB', 'AUTOCREDIT', 'PTS_Infoseti', 'PTS_TBank');
	--delete from Temp_PTS_CM_wrong_migrations1 where flag_kk in ('INSTALLMENT', 'PTS_PSB', 'AUTOCREDIT', 'PTS_Infoseti', 'PTS_TBank');
	--delete from Temp_PTS_CM_wrong_migrations2 where flag_kk in ('INSTALLMENT', 'PTS_PSB', 'AUTOCREDIT', 'PTS_Infoseti', 'PTS_TBank');
	--delete from Temp_PTS_CM_stg1_agg where flag_kk in ('INSTALLMENT', 'PTS_PSB', 'AUTOCREDIT', 'PTS_Infoseti', 'PTS_TBank');
	--delete from Temp_PTS_CM_standart_payoff where flag_kk in ('INSTALLMENT', 'PTS_PSB', 'AUTOCREDIT', 'PTS_Infoseti', 'PTS_TBank');
	--delete from Temp_PTS_CM_stg2_agg where flag_kk in ('INSTALLMENT', 'PTS_PSB', 'AUTOCREDIT', 'PTS_Infoseti', 'PTS_TBank');
	delete from Temp_PTS_CM_base_for_stg3_agg where flag_kk in ('INSTALLMENT', 'PTS_PSB', 'AUTOCREDIT', 'PTS_Infoseti', 'PTS_TBank');
	--delete from Temp_PTS_CM_for_delete where flag_kk in ('INSTALLMENT', 'PTS_PSB', 'AUTOCREDIT', 'PTS_Infoseti', 'PTS_TBank');
	delete from Temp_PTS_CM_stg1_NU_segment_distrib where flag_kk in ('INSTALLMENT', 'PTS_PSB', 'AUTOCREDIT', 'PTS_Infoseti', 'PTS_TBank');
	--delete from Temp_PTS_CM_stg2_NU_segment_distrib where flag_kk in ('INSTALLMENT', 'PTS_PSB', 'AUTOCREDIT', 'PTS_Infoseti', 'PTS_TBank');
	delete from Temp_PTS_CM_NU_segment_distrib where flag_kk in ('INSTALLMENT', 'PTS_PSB', 'AUTOCREDIT', 'PTS_Infoseti', 'PTS_TBank');
	--------------------------------------------------------------------------------------------------------



	drop table if exists Temp_PTS_CM_det_pdp_coef;

	select 
	cast(a.segment_rbp as varchar(100)) as segment_rbp,
	cast(a.coef as float) as coef
	into Temp_PTS_CM_det_pdp_coef
	from (values
	...
	--select sum(vl_rub_fact) from Temp_PTS_CM_virt_gens

	--select generation, sum(vl_rub_fact) from Temp_PTS_CM_virt_gens group by generation order by 1 desc

	-------виртуальные поколения для плановых выдач
	drop table if exists Temp_PTS_CM_virt_gens;
	...
	into Temp_PTS_CM_virt_gens
	from (values 


		('other', '2025-11-30', 24, 'USUAL', 1430156.00),
		('other', '2025-11-30', 36, 'USUAL', 1642185.00),
		('other', '2025-11-30', 48, 'USUAL', 31518588.00),
		('PTS31_RBP4', '2025-11-30', 24, 'USUAL', 5571756.00),
		('PTS31_RBP4', '2025-11-30', 36, 'USUAL', 3752157.00),
		('PTS31_RBP4', '2025-11-30', 48, 'USUAL', 22414365.00),
		('RBP 1', '2025-11-30', 24, 'USUAL', 2808428.00),
		('RBP 1', '2025-11-30', 36, 'USUAL', 886400.00),
		('RBP 1', '2025-11-30', 48, 'USUAL', 25096556.00),
		('RBP 2', '2025-11-30', 24, 'USUAL', 777900.00),
		('RBP 2', '2025-11-30', 36, 'USUAL', 0.00),
		('RBP 2', '2025-11-30', 48, 'USUAL', 10051387.00),
		('RBP 3', '2025-11-30', 24, 'USUAL', 4911456.00),
		('RBP 3', '2025-11-30', 36, 'USUAL', 930460.00),
		('RBP 3', '2025-11-30', 48, 'USUAL', 8377428.00),
		('RBP 4', '2025-11-30', 24, 'USUAL', 1371456.00),
		('RBP 4', '2025-11-30', 36, 'USUAL', 352299.00),
		('RBP 4', '2025-11-30', 48, 'USUAL', 6946097.00),




		('other', '2025-12-31', 24, 'USUAL', 2068615.28),
		('other', '2025-12-31', 36, 'USUAL', 2376033.93),
		('other', '2025-12-31', 48, 'USUAL', 36551726.63),
		('PTS31_RBP4', '2025-12-31', 24, 'USUAL', 7758989.54),
		('PTS31_RBP4', '2025-12-31', 36, 'USUAL', 6226145.85),
		('PTS31_RBP4', '2025-12-31', 48, 'USUAL', 32194039.79),
		('RBP 1', '2025-12-31', 24, 'USUAL', 6111607.31),
		('RBP 1', '2025-12-31', 36, 'USUAL', 5006210.34),
		('RBP 1', '2025-12-31', 48, 'USUAL', 45051414.99),
		('RBP 2', '2025-12-31', 24, 'USUAL', 1955856.82),
		('RBP 2', '2025-12-31', 36, 'USUAL', 1508237.96),
		('RBP 2', '2025-12-31', 48, 'USUAL', 18441707.23),
		('RBP 3', '2025-12-31', 24, 'USUAL', 3072357.56),
		('RBP 3', '2025-12-31', 36, 'USUAL', 2337516.20),
		('RBP 3', '2025-12-31', 48, 'USUAL', 16655114.60),
		('RBP 4', '2025-12-31', 24, 'USUAL', 1486530.57),
		('RBP 4', '2025-12-31', 36, 'USUAL', 1147045.36),
		('RBP 4', '2025-12-31', 48, 'USUAL', 10050850.05),




		('other', '2026-01-31', 24, 'USUAL', 1241169.17),
		('other', '2026-01-31', 36, 'USUAL', 1425620.36),
		('other', '2026-01-31', 48, 'USUAL', 21931035.98),
		('PTS31_RBP4', '2026-01-31', 24, 'USUAL', 4655393.72),
		('PTS31_RBP4', '2026-01-31', 36, 'USUAL', 3735687.51),
		('PTS31_RBP4', '2026-01-31', 48, 'USUAL', 19316423.87),
		('RBP 1', '2026-01-31', 24, 'USUAL', 3666964.38),
		('RBP 1', '2026-01-31', 36, 'USUAL', 3003726.21),
		('RBP 1', '2026-01-31', 48, 'USUAL', 27030849.00),
		('RBP 2', '2026-01-31', 24, 'USUAL', 1173514.09),
		('RBP 2', '2026-01-31', 36, 'USUAL', 904942.77),
		('RBP 2', '2026-01-31', 48, 'USUAL', 11065024.34),
		('RBP 3', '2026-01-31', 24, 'USUAL', 1843414.53),
		('RBP 3', '2026-01-31', 36, 'USUAL', 1402509.72),
		('RBP 3', '2026-01-31', 48, 'USUAL', 9993068.76),
		('RBP 4', '2026-01-31', 24, 'USUAL', 891918.34),
		('RBP 4', '2026-01-31', 36, 'USUAL', 688227.21),
		('RBP 4', '2026-01-31', 48, 'USUAL', 6030510.03),




		('other', '2026-02-28', 24, 'USUAL', 1995179.44),
		('other', '2026-02-28', 36, 'USUAL', 2291684.72),
		('other', '2026-02-28', 48, 'USUAL', 35254140.34),
		('PTS31_RBP4', '2026-02-28', 24, 'USUAL', 7483545.41),
		('PTS31_RBP4', '2026-02-28', 36, 'USUAL', 6005117.67),
		('PTS31_RBP4', '2026-02-28', 48, 'USUAL', 31051151.38),
		('RBP 1', '2026-02-28', 24, 'USUAL', 5894645.25),
		('RBP 1', '2026-02-28', 36, 'USUAL', 4828489.87),
		('RBP 1', '2026-02-28', 48, 'USUAL', 43452089.76),
		('RBP 2', '2026-02-28', 24, 'USUAL', 1886423.90),
		('RBP 2', '2026-02-28', 36, 'USUAL', 1454695.51),
		('RBP 2', '2026-02-28', 48, 'USUAL', 17787026.62),
		('RBP 3', '2026-02-28', 24, 'USUAL', 2963288.86),
		('RBP 3', '2026-02-28', 36, 'USUAL', 2254534.38),
		('RBP 3', '2026-02-28', 48, 'USUAL', 16063858.03),
		('RBP 4', '2026-02-28', 24, 'USUAL', 1433758.74),
		('RBP 4', '2026-02-28', 36, 'USUAL', 1106325.25),
		('RBP 4', '2026-02-28', 48, 'USUAL', 9694044.87),




		('other', '2026-03-31', 24, 'USUAL', 1995179.44),
		('other', '2026-03-31', 36, 'USUAL', 2291684.72),
		('other', '2026-03-31', 48, 'USUAL', 35254140.34),
		('PTS31_RBP4', '2026-03-31', 24, 'USUAL', 7483545.41),
		('PTS31_RBP4', '2026-03-31', 36, 'USUAL', 6005117.67),
		('PTS31_RBP4', '2026-03-31', 48, 'USUAL', 31051151.38),
		('RBP 1', '2026-03-31', 24, 'USUAL', 5894645.25),
		('RBP 1', '2026-03-31', 36, 'USUAL', 4828489.87),
		('RBP 1', '2026-03-31', 48, 'USUAL', 43452089.76),
		('RBP 2', '2026-03-31', 24, 'USUAL', 1886423.90),
		('RBP 2', '2026-03-31', 36, 'USUAL', 1454695.51),
		('RBP 2', '2026-03-31', 48, 'USUAL', 17787026.62),
		('RBP 3', '2026-03-31', 24, 'USUAL', 2963288.86),
		('RBP 3', '2026-03-31', 36, 'USUAL', 2254534.38),
		('RBP 3', '2026-03-31', 48, 'USUAL', 16063858.03),
		('RBP 4', '2026-03-31', 24, 'USUAL', 1433758.74),
		('RBP 4', '2026-03-31', 36, 'USUAL', 1106325.25),
		('RBP 4', '2026-03-31', 48, 'USUAL', 9694044.87),




		('other', '2026-04-30', 24, 'USUAL', 1995179.44),
		('other', '2026-04-30', 36, 'USUAL', 2291684.72),
		('other', '2026-04-30', 48, 'USUAL', 35254140.34),
		('PTS31_RBP4', '2026-04-30', 24, 'USUAL', 7483545.41),
		('PTS31_RBP4', '2026-04-30', 36, 'USUAL', 6005117.67),
		('PTS31_RBP4', '2026-04-30', 48, 'USUAL', 31051151.38),
		('RBP 1', '2026-04-30', 24, 'USUAL', 5894645.25),
		('RBP 1', '2026-04-30', 36, 'USUAL', 4828489.87),
		('RBP 1', '2026-04-30', 48, 'USUAL', 43452089.76),
		('RBP 2', '2026-04-30', 24, 'USUAL', 1886423.90),
		('RBP 2', '2026-04-30', 36, 'USUAL', 1454695.51),
		('RBP 2', '2026-04-30', 48, 'USUAL', 17787026.62),
		('RBP 3', '2026-04-30', 24, 'USUAL', 2963288.86),
		('RBP 3', '2026-04-30', 36, 'USUAL', 2254534.38),
		('RBP 3', '2026-04-30', 48, 'USUAL', 16063858.03),
		('RBP 4', '2026-04-30', 24, 'USUAL', 1433758.74),
		('RBP 4', '2026-04-30', 36, 'USUAL', 1106325.25),
		('RBP 4', '2026-04-30', 48, 'USUAL', 9694044.87),




		('other', '2026-05-31', 24, 'USUAL', 1995179.44),
		('other', '2026-05-31', 36, 'USUAL', 2291684.72),
		('other', '2026-05-31', 48, 'USUAL', 35254140.34),
		('PTS31_RBP4', '2026-05-31', 24, 'USUAL', 7483545.41),
		('PTS31_RBP4', '2026-05-31', 36, 'USUAL', 6005117.67),
		('PTS31_RBP4', '2026-05-31', 48, 'USUAL', 31051151.38),
		('RBP 1', '2026-05-31', 24, 'USUAL', 5894645.25),
		('RBP 1', '2026-05-31', 36, 'USUAL', 4828489.87),
		('RBP 1', '2026-05-31', 48, 'USUAL', 43452089.76),
		('RBP 2', '2026-05-31', 24, 'USUAL', 1886423.90),
		('RBP 2', '2026-05-31', 36, 'USUAL', 1454695.51),
		('RBP 2', '2026-05-31', 48, 'USUAL', 17787026.62),
		('RBP 3', '2026-05-31', 24, 'USUAL', 2963288.86),
		('RBP 3', '2026-05-31', 36, 'USUAL', 2254534.38),
		('RBP 3', '2026-05-31', 48, 'USUAL', 16063858.03),
		('RBP 4', '2026-05-31', 24, 'USUAL', 1433758.74),
		('RBP 4', '2026-05-31', 36, 'USUAL', 1106325.25),
		('RBP 4', '2026-05-31', 48, 'USUAL', 9694044.87),




		('other', '2026-06-30', 24, 'USUAL', 1995179.44),
		('other', '2026-06-30', 36, 'USUAL', 2291684.72),
		('other', '2026-06-30', 48, 'USUAL', 35254140.34),
		('PTS31_RBP4', '2026-06-30', 24, 'USUAL', 7483545.41),
		('PTS31_RBP4', '2026-06-30', 36, 'USUAL', 6005117.67),
		('PTS31_RBP4', '2026-06-30', 48, 'USUAL', 31051151.38),
		('RBP 1', '2026-06-30', 24, 'USUAL', 5894645.25),
		('RBP 1', '2026-06-30', 36, 'USUAL', 4828489.87),
		('RBP 1', '2026-06-30', 48, 'USUAL', 43452089.76),
		('RBP 2', '2026-06-30', 24, 'USUAL', 1886423.90),
		('RBP 2', '2026-06-30', 36, 'USUAL', 1454695.51),
		('RBP 2', '2026-06-30', 48, 'USUAL', 17787026.62),
		('RBP 3', '2026-06-30', 24, 'USUAL', 2963288.86),
		('RBP 3', '2026-06-30', 36, 'USUAL', 2254534.38),
		('RBP 3', '2026-06-30', 48, 'USUAL', 16063858.03),
		('RBP 4', '2026-06-30', 24, 'USUAL', 1433758.74),
		('RBP 4', '2026-06-30', 36, 'USUAL', 1106325.25),
		('RBP 4', '2026-06-30', 48, 'USUAL', 9694044.87),




		('other', '2026-07-31', 24, 'USUAL', 1995179.44),
		('other', '2026-07-31', 36, 'USUAL', 2291684.72),
		('other', '2026-07-31', 48, 'USUAL', 35254140.34),
		('PTS31_RBP4', '2026-07-31', 24, 'USUAL', 7483545.41),
		('PTS31_RBP4', '2026-07-31', 36, 'USUAL', 6005117.67),
		('PTS31_RBP4', '2026-07-31', 48, 'USUAL', 31051151.38),
		('RBP 1', '2026-07-31', 24, 'USUAL', 5894645.25),
		('RBP 1', '2026-07-31', 36, 'USUAL', 4828489.87),
		('RBP 1', '2026-07-31', 48, 'USUAL', 43452089.76),
		('RBP 2', '2026-07-31', 24, 'USUAL', 1886423.90),
		('RBP 2', '2026-07-31', 36, 'USUAL', 1454695.51),
		('RBP 2', '2026-07-31', 48, 'USUAL', 17787026.62),
		('RBP 3', '2026-07-31', 24, 'USUAL', 2963288.86),
		('RBP 3', '2026-07-31', 36, 'USUAL', 2254534.38),
		('RBP 3', '2026-07-31', 48, 'USUAL', 16063858.03),
		('RBP 4', '2026-07-31', 24, 'USUAL', 1433758.74),
		('RBP 4', '2026-07-31', 36, 'USUAL', 1106325.25),
		('RBP 4', '2026-07-31', 48, 'USUAL', 9694044.87),




		('other', '2026-08-31', 24, 'USUAL', 1995179.44),
		('other', '2026-08-31', 36, 'USUAL', 2291684.72),
		('other', '2026-08-31', 48, 'USUAL', 35254140.34),
		('PTS31_RBP4', '2026-08-31', 24, 'USUAL', 7483545.41),
		('PTS31_RBP4', '2026-08-31', 36, 'USUAL', 6005117.67),
		('PTS31_RBP4', '2026-08-31', 48, 'USUAL', 31051151.38),
		('RBP 1', '2026-08-31', 24, 'USUAL', 5894645.25),
		('RBP 1', '2026-08-31', 36, 'USUAL', 4828489.87),
		('RBP 1', '2026-08-31', 48, 'USUAL', 43452089.76),
		('RBP 2', '2026-08-31', 24, 'USUAL', 1886423.90),
		('RBP 2', '2026-08-31', 36, 'USUAL', 1454695.51),
		('RBP 2', '2026-08-31', 48, 'USUAL', 17787026.62),
		('RBP 3', '2026-08-31', 24, 'USUAL', 2963288.86),
		('RBP 3', '2026-08-31', 36, 'USUAL', 2254534.38),
		('RBP 3', '2026-08-31', 48, 'USUAL', 16063858.03),
		('RBP 4', '2026-08-31', 24, 'USUAL', 1433758.74),
		('RBP 4', '2026-08-31', 36, 'USUAL', 1106325.25),
		('RBP 4', '2026-08-31', 48, 'USUAL', 9694044.87),




		('other', '2026-09-30', 24, 'USUAL', 1995179.44),
		('other', '2026-09-30', 36, 'USUAL', 2291684.72),
		('other', '2026-09-30', 48, 'USUAL', 35254140.34),
		('PTS31_RBP4', '2026-09-30', 24, 'USUAL', 7483545.41),
		('PTS31_RBP4', '2026-09-30', 36, 'USUAL', 6005117.67),
		('PTS31_RBP4', '2026-09-30', 48, 'USUAL', 31051151.38),
		('RBP 1', '2026-09-30', 24, 'USUAL', 5894645.25),
		('RBP 1', '2026-09-30', 36, 'USUAL', 4828489.87),
		('RBP 1', '2026-09-30', 48, 'USUAL', 43452089.76),
		('RBP 2', '2026-09-30', 24, 'USUAL', 1886423.90),
		('RBP 2', '2026-09-30', 36, 'USUAL', 1454695.51),
		('RBP 2', '2026-09-30', 48, 'USUAL', 17787026.62),
		('RBP 3', '2026-09-30', 24, 'USUAL', 2963288.86),
		('RBP 3', '2026-09-30', 36, 'USUAL', 2254534.38),
		('RBP 3', '2026-09-30', 48, 'USUAL', 16063858.03),
		('RBP 4', '2026-09-30', 24, 'USUAL', 1433758.74),
		('RBP 4', '2026-09-30', 36, 'USUAL', 1106325.25),
		('RBP 4', '2026-09-30', 48, 'USUAL', 9694044.87),




		('other', '2026-10-31', 24, 'USUAL', 1995179.44),
		('other', '2026-10-31', 36, 'USUAL', 2291684.72),
		('other', '2026-10-31', 48, 'USUAL', 35254140.34),
		('PTS31_RBP4', '2026-10-31', 24, 'USUAL', 7483545.41),
		('PTS31_RBP4', '2026-10-31', 36, 'USUAL', 6005117.67),
		('PTS31_RBP4', '2026-10-31', 48, 'USUAL', 31051151.38),
		('RBP 1', '2026-10-31', 24, 'USUAL', 5894645.25),
		('RBP 1', '2026-10-31', 36, 'USUAL', 4828489.87),
		('RBP 1', '2026-10-31', 48, 'USUAL', 43452089.76),
		('RBP 2', '2026-10-31', 24, 'USUAL', 1886423.90),
		('RBP 2', '2026-10-31', 36, 'USUAL', 1454695.51),
		('RBP 2', '2026-10-31', 48, 'USUAL', 17787026.62),
		('RBP 3', '2026-10-31', 24, 'USUAL', 2963288.86),
		('RBP 3', '2026-10-31', 36, 'USUAL', 2254534.38),
		('RBP 3', '2026-10-31', 48, 'USUAL', 16063858.03),
		('RBP 4', '2026-10-31', 24, 'USUAL', 1433758.74),
		('RBP 4', '2026-10-31', 36, 'USUAL', 1106325.25),
		('RBP 4', '2026-10-31', 48, 'USUAL', 9694044.87),




		('other', '2026-11-30', 24, 'USUAL', 1995179.44),
		('other', '2026-11-30', 36, 'USUAL', 2291684.72),
		('other', '2026-11-30', 48, 'USUAL', 35254140.34),
		('PTS31_RBP4', '2026-11-30', 24, 'USUAL', 7483545.41),
		('PTS31_RBP4', '2026-11-30', 36, 'USUAL', 6005117.67),
		('PTS31_RBP4', '2026-11-30', 48, 'USUAL', 31051151.38),
		('RBP 1', '2026-11-30', 24, 'USUAL', 5894645.25),
		('RBP 1', '2026-11-30', 36, 'USUAL', 4828489.87),
		('RBP 1', '2026-11-30', 48, 'USUAL', 43452089.76),
		('RBP 2', '2026-11-30', 24, 'USUAL', 1886423.90),
		('RBP 2', '2026-11-30', 36, 'USUAL', 1454695.51),
		('RBP 2', '2026-11-30', 48, 'USUAL', 17787026.62),
		('RBP 3', '2026-11-30', 24, 'USUAL', 2963288.86),
		('RBP 3', '2026-11-30', 36, 'USUAL', 2254534.38),
		('RBP 3', '2026-11-30', 48, 'USUAL', 16063858.03),
		('RBP 4', '2026-11-30', 24, 'USUAL', 1433758.74),
		('RBP 4', '2026-11-30', 36, 'USUAL', 1106325.25),
		('RBP 4', '2026-11-30', 48, 'USUAL', 9694044.87),




		('other', '2026-12-31', 24, 'USUAL', 1995179.44),
		('other', '2026-12-31', 36, 'USUAL', 2291684.72),
		('other', '2026-12-31', 48, 'USUAL', 35254140.34),
		('PTS31_RBP4', '2026-12-31', 24, 'USUAL', 7483545.41),
		('PTS31_RBP4', '2026-12-31', 36, 'USUAL', 6005117.67),
		('PTS31_RBP4', '2026-12-31', 48, 'USUAL', 31051151.38),
		('RBP 1', '2026-12-31', 24, 'USUAL', 5894645.25),
		('RBP 1', '2026-12-31', 36, 'USUAL', 4828489.87),
		('RBP 1', '2026-12-31', 48, 'USUAL', 43452089.76),
		('RBP 2', '2026-12-31', 24, 'USUAL', 1886423.90),
		('RBP 2', '2026-12-31', 36, 'USUAL', 1454695.51),
		('RBP 2', '2026-12-31', 48, 'USUAL', 17787026.62),
		('RBP 3', '2026-12-31', 24, 'USUAL', 2963288.86),
		('RBP 3', '2026-12-31', 36, 'USUAL', 2254534.38),
		('RBP 3', '2026-12-31', 48, 'USUAL', 16063858.03),
		('RBP 4', '2026-12-31', 24, 'USUAL', 1433758.74),
		('RBP 4', '2026-12-31', 36, 'USUAL', 1106325.25),
		('RBP 4', '2026-12-31', 48, 'USUAL', 9694044.87),




		('other', '2027-01-31', 24, 'USUAL', 2194697.38),
		('other', '2027-01-31', 36, 'USUAL', 2520853.20),
		('other', '2027-01-31', 48, 'USUAL', 38779554.37),
		('PTS31_RBP4', '2027-01-31', 24, 'USUAL', 8231899.95),
		('PTS31_RBP4', '2027-01-31', 36, 'USUAL', 6605629.44),
		('PTS31_RBP4', '2027-01-31', 48, 'USUAL', 34156266.52),
		('RBP 1', '2027-01-31', 24, 'USUAL', 6484109.77),
		('RBP 1', '2027-01-31', 36, 'USUAL', 5311338.86),
		('RBP 1', '2027-01-31', 48, 'USUAL', 47797298.74),
		('RBP 2', '2027-01-31', 24, 'USUAL', 2075066.29),
		('RBP 2', '2027-01-31', 36, 'USUAL', 1600165.06),
		('RBP 2', '2027-01-31', 48, 'USUAL', 19565729.29),
		('RBP 3', '2027-01-31', 24, 'USUAL', 3259617.75),
		('RBP 3', '2027-01-31', 36, 'USUAL', 2479987.82),
		('RBP 3', '2027-01-31', 48, 'USUAL', 17670243.83),
		('RBP 4', '2027-01-31', 24, 'USUAL', 1577134.61),
		('RBP 4', '2027-01-31', 36, 'USUAL', 1216957.77),
		('RBP 4', '2027-01-31', 48, 'USUAL', 10663449.36),




		('other', '2027-02-28', 24, 'USUAL', 2194697.38),
		('other', '2027-02-28', 36, 'USUAL', 2520853.20),
		('other', '2027-02-28', 48, 'USUAL', 38779554.37),
		('PTS31_RBP4', '2027-02-28', 24, 'USUAL', 8231899.95),
		('PTS31_RBP4', '2027-02-28', 36, 'USUAL', 6605629.44),
		('PTS31_RBP4', '2027-02-28', 48, 'USUAL', 34156266.52),
		('RBP 1', '2027-02-28', 24, 'USUAL', 6484109.77),
		('RBP 1', '2027-02-28', 36, 'USUAL', 5311338.86),
		('RBP 1', '2027-02-28', 48, 'USUAL', 47797298.74),
		('RBP 2', '2027-02-28', 24, 'USUAL', 2075066.29),
		('RBP 2', '2027-02-28', 36, 'USUAL', 1600165.06),
		('RBP 2', '2027-02-28', 48, 'USUAL', 19565729.29),
		('RBP 3', '2027-02-28', 24, 'USUAL', 3259617.75),
		('RBP 3', '2027-02-28', 36, 'USUAL', 2479987.82),
		('RBP 3', '2027-02-28', 48, 'USUAL', 17670243.83),
		('RBP 4', '2027-02-28', 24, 'USUAL', 1577134.61),
		('RBP 4', '2027-02-28', 36, 'USUAL', 1216957.77),
		('RBP 4', '2027-02-28', 48, 'USUAL', 10663449.36),




		('other', '2027-03-31', 24, 'USUAL', 2194697.38),
		('other', '2027-03-31', 36, 'USUAL', 2520853.20),
		('other', '2027-03-31', 48, 'USUAL', 38779554.37),
		('PTS31_RBP4', '2027-03-31', 24, 'USUAL', 8231899.95),
		('PTS31_RBP4', '2027-03-31', 36, 'USUAL', 6605629.44),
		('PTS31_RBP4', '2027-03-31', 48, 'USUAL', 34156266.52),
		('RBP 1', '2027-03-31', 24, 'USUAL', 6484109.77),
		('RBP 1', '2027-03-31', 36, 'USUAL', 5311338.86),
		('RBP 1', '2027-03-31', 48, 'USUAL', 47797298.74),
		('RBP 2', '2027-03-31', 24, 'USUAL', 2075066.29),
		('RBP 2', '2027-03-31', 36, 'USUAL', 1600165.06),
		('RBP 2', '2027-03-31', 48, 'USUAL', 19565729.29),
		('RBP 3', '2027-03-31', 24, 'USUAL', 3259617.75),
		('RBP 3', '2027-03-31', 36, 'USUAL', 2479987.82),
		('RBP 3', '2027-03-31', 48, 'USUAL', 17670243.83),
		('RBP 4', '2027-03-31', 24, 'USUAL', 1577134.61),
		('RBP 4', '2027-03-31', 36, 'USUAL', 1216957.77),
		('RBP 4', '2027-03-31', 48, 'USUAL', 10663449.36),




		('other', '2027-04-30', 24, 'USUAL', 2194697.38),
		('other', '2027-04-30', 36, 'USUAL', 2520853.20),
		('other', '2027-04-30', 48, 'USUAL', 38779554.37),
		('PTS31_RBP4', '2027-04-30', 24, 'USUAL', 8231899.95),
		('PTS31_RBP4', '2027-04-30', 36, 'USUAL', 6605629.44),
		('PTS31_RBP4', '2027-04-30', 48, 'USUAL', 34156266.52),
		('RBP 1', '2027-04-30', 24, 'USUAL', 6484109.77),
		('RBP 1', '2027-04-30', 36, 'USUAL', 5311338.86),
		('RBP 1', '2027-04-30', 48, 'USUAL', 47797298.74),
		('RBP 2', '2027-04-30', 24, 'USUAL', 2075066.29),
		('RBP 2', '2027-04-30', 36, 'USUAL', 1600165.06),
		('RBP 2', '2027-04-30', 48, 'USUAL', 19565729.29),
		('RBP 3', '2027-04-30', 24, 'USUAL', 3259617.75),
		('RBP 3', '2027-04-30', 36, 'USUAL', 2479987.82),
		('RBP 3', '2027-04-30', 48, 'USUAL', 17670243.83),
		('RBP 4', '2027-04-30', 24, 'USUAL', 1577134.61),
		('RBP 4', '2027-04-30', 36, 'USUAL', 1216957.77),
		('RBP 4', '2027-04-30', 48, 'USUAL', 10663449.36),




		('other', '2027-05-31', 24, 'USUAL', 2194697.38),
		('other', '2027-05-31', 36, 'USUAL', 2520853.20),
		('other', '2027-05-31', 48, 'USUAL', 38779554.37),
		('PTS31_RBP4', '2027-05-31', 24, 'USUAL', 8231899.95),
		('PTS31_RBP4', '2027-05-31', 36, 'USUAL', 6605629.44),
		('PTS31_RBP4', '2027-05-31', 48, 'USUAL', 34156266.52),
		('RBP 1', '2027-05-31', 24, 'USUAL', 6484109.77),
		('RBP 1', '2027-05-31', 36, 'USUAL', 5311338.86),
		('RBP 1', '2027-05-31', 48, 'USUAL', 47797298.74),
		('RBP 2', '2027-05-31', 24, 'USUAL', 2075066.29),
		('RBP 2', '2027-05-31', 36, 'USUAL', 1600165.06),
		('RBP 2', '2027-05-31', 48, 'USUAL', 19565729.29),
		('RBP 3', '2027-05-31', 24, 'USUAL', 3259617.75),
		('RBP 3', '2027-05-31', 36, 'USUAL', 2479987.82),
		('RBP 3', '2027-05-31', 48, 'USUAL', 17670243.83),
		('RBP 4', '2027-05-31', 24, 'USUAL', 1577134.61),
		('RBP 4', '2027-05-31', 36, 'USUAL', 1216957.77),
		('RBP 4', '2027-05-31', 48, 'USUAL', 10663449.36),




		('other', '2027-06-30', 24, 'USUAL', 2194697.38),
		('other', '2027-06-30', 36, 'USUAL', 2520853.20),
		('other', '2027-06-30', 48, 'USUAL', 38779554.37),
		('PTS31_RBP4', '2027-06-30', 24, 'USUAL', 8231899.95),
		('PTS31_RBP4', '2027-06-30', 36, 'USUAL', 6605629.44),
		('PTS31_RBP4', '2027-06-30', 48, 'USUAL', 34156266.52),
		('RBP 1', '2027-06-30', 24, 'USUAL', 6484109.77),
		('RBP 1', '2027-06-30', 36, 'USUAL', 5311338.86),
		('RBP 1', '2027-06-30', 48, 'USUAL', 47797298.74),
		('RBP 2', '2027-06-30', 24, 'USUAL', 2075066.29),
		('RBP 2', '2027-06-30', 36, 'USUAL', 1600165.06),
		('RBP 2', '2027-06-30', 48, 'USUAL', 19565729.29),
		('RBP 3', '2027-06-30', 24, 'USUAL', 3259617.75),
		('RBP 3', '2027-06-30', 36, 'USUAL', 2479987.82),
		('RBP 3', '2027-06-30', 48, 'USUAL', 17670243.83),
		('RBP 4', '2027-06-30', 24, 'USUAL', 1577134.61),
		('RBP 4', '2027-06-30', 36, 'USUAL', 1216957.77),
		('RBP 4', '2027-06-30', 48, 'USUAL', 10663449.36),




		('other', '2027-07-31', 24, 'USUAL', 2194697.38),
		('other', '2027-07-31', 36, 'USUAL', 2520853.20),
		('other', '2027-07-31', 48, 'USUAL', 38779554.37),
		('PTS31_RBP4', '2027-07-31', 24, 'USUAL', 8231899.95),
		('PTS31_RBP4', '2027-07-31', 36, 'USUAL', 6605629.44),
		('PTS31_RBP4', '2027-07-31', 48, 'USUAL', 34156266.52),
		('RBP 1', '2027-07-31', 24, 'USUAL', 6484109.77),
		('RBP 1', '2027-07-31', 36, 'USUAL', 5311338.86),
		('RBP 1', '2027-07-31', 48, 'USUAL', 47797298.74),
		('RBP 2', '2027-07-31', 24, 'USUAL', 2075066.29),
		('RBP 2', '2027-07-31', 36, 'USUAL', 1600165.06),
		('RBP 2', '2027-07-31', 48, 'USUAL', 19565729.29),
		('RBP 3', '2027-07-31', 24, 'USUAL', 3259617.75),
		('RBP 3', '2027-07-31', 36, 'USUAL', 2479987.82),
		('RBP 3', '2027-07-31', 48, 'USUAL', 17670243.83),
		('RBP 4', '2027-07-31', 24, 'USUAL', 1577134.61),
		('RBP 4', '2027-07-31', 36, 'USUAL', 1216957.77),
		('RBP 4', '2027-07-31', 48, 'USUAL', 10663449.36),




		('other', '2027-08-31', 24, 'USUAL', 2194697.38),
		('other', '2027-08-31', 36, 'USUAL', 2520853.20),
		('other', '2027-08-31', 48, 'USUAL', 38779554.37),
		('PTS31_RBP4', '2027-08-31', 24, 'USUAL', 8231899.95),
		('PTS31_RBP4', '2027-08-31', 36, 'USUAL', 6605629.44),
		('PTS31_RBP4', '2027-08-31', 48, 'USUAL', 34156266.52),
		('RBP 1', '2027-08-31', 24, 'USUAL', 6484109.77),
		('RBP 1', '2027-08-31', 36, 'USUAL', 5311338.86),
		('RBP 1', '2027-08-31', 48, 'USUAL', 47797298.74),
		('RBP 2', '2027-08-31', 24, 'USUAL', 2075066.29),
		('RBP 2', '2027-08-31', 36, 'USUAL', 1600165.06),
		('RBP 2', '2027-08-31', 48, 'USUAL', 19565729.29),
		('RBP 3', '2027-08-31', 24, 'USUAL', 3259617.75),
		('RBP 3', '2027-08-31', 36, 'USUAL', 2479987.82),
		('RBP 3', '2027-08-31', 48, 'USUAL', 17670243.83),
		('RBP 4', '2027-08-31', 24, 'USUAL', 1577134.61),
		('RBP 4', '2027-08-31', 36, 'USUAL', 1216957.77),
		('RBP 4', '2027-08-31', 48, 'USUAL', 10663449.36),




		('other', '2027-09-30', 24, 'USUAL', 2194697.38),
		('other', '2027-09-30', 36, 'USUAL', 2520853.20),
		('other', '2027-09-30', 48, 'USUAL', 38779554.37),
		('PTS31_RBP4', '2027-09-30', 24, 'USUAL', 8231899.95),
		('PTS31_RBP4', '2027-09-30', 36, 'USUAL', 6605629.44),
		('PTS31_RBP4', '2027-09-30', 48, 'USUAL', 34156266.52),
		('RBP 1', '2027-09-30', 24, 'USUAL', 6484109.77),
		('RBP 1', '2027-09-30', 36, 'USUAL', 5311338.86),
		('RBP 1', '2027-09-30', 48, 'USUAL', 47797298.74),
		('RBP 2', '2027-09-30', 24, 'USUAL', 2075066.29),
		('RBP 2', '2027-09-30', 36, 'USUAL', 1600165.06),
		('RBP 2', '2027-09-30', 48, 'USUAL', 19565729.29),
		('RBP 3', '2027-09-30', 24, 'USUAL', 3259617.75),
		('RBP 3', '2027-09-30', 36, 'USUAL', 2479987.82),
		('RBP 3', '2027-09-30', 48, 'USUAL', 17670243.83),
		('RBP 4', '2027-09-30', 24, 'USUAL', 1577134.61),
		('RBP 4', '2027-09-30', 36, 'USUAL', 1216957.77),
		('RBP 4', '2027-09-30', 48, 'USUAL', 10663449.36),




		('other', '2027-10-31', 24, 'USUAL', 2194697.38),
		('other', '2027-10-31', 36, 'USUAL', 2520853.20),
		('other', '2027-10-31', 48, 'USUAL', 38779554.37),
		('PTS31_RBP4', '2027-10-31', 24, 'USUAL', 8231899.95),
		('PTS31_RBP4', '2027-10-31', 36, 'USUAL', 6605629.44),
		('PTS31_RBP4', '2027-10-31', 48, 'USUAL', 34156266.52),
		('RBP 1', '2027-10-31', 24, 'USUAL', 6484109.77),
		('RBP 1', '2027-10-31', 36, 'USUAL', 5311338.86),
		('RBP 1', '2027-10-31', 48, 'USUAL', 47797298.74),
		('RBP 2', '2027-10-31', 24, 'USUAL', 2075066.29),
		('RBP 2', '2027-10-31', 36, 'USUAL', 1600165.06),
		('RBP 2', '2027-10-31', 48, 'USUAL', 19565729.29),
		('RBP 3', '2027-10-31', 24, 'USUAL', 3259617.75),
		('RBP 3', '2027-10-31', 36, 'USUAL', 2479987.82),
		('RBP 3', '2027-10-31', 48, 'USUAL', 17670243.83),
		('RBP 4', '2027-10-31', 24, 'USUAL', 1577134.61),
		('RBP 4', '2027-10-31', 36, 'USUAL', 1216957.77),
		('RBP 4', '2027-10-31', 48, 'USUAL', 10663449.36),




		('other', '2027-11-30', 24, 'USUAL', 2194697.38),
		('other', '2027-11-30', 36, 'USUAL', 2520853.20),
		('other', '2027-11-30', 48, 'USUAL', 38779554.37),
		('PTS31_RBP4', '2027-11-30', 24, 'USUAL', 8231899.95),
		('PTS31_RBP4', '2027-11-30', 36, 'USUAL', 6605629.44),
		('PTS31_RBP4', '2027-11-30', 48, 'USUAL', 34156266.52),
		('RBP 1', '2027-11-30', 24, 'USUAL', 6484109.77),
		('RBP 1', '2027-11-30', 36, 'USUAL', 5311338.86),
		('RBP 1', '2027-11-30', 48, 'USUAL', 47797298.74),
		('RBP 2', '2027-11-30', 24, 'USUAL', 2075066.29),
		('RBP 2', '2027-11-30', 36, 'USUAL', 1600165.06),
		('RBP 2', '2027-11-30', 48, 'USUAL', 19565729.29),
		('RBP 3', '2027-11-30', 24, 'USUAL', 3259617.75),
		('RBP 3', '2027-11-30', 36, 'USUAL', 2479987.82),
		('RBP 3', '2027-11-30', 48, 'USUAL', 17670243.83),
		('RBP 4', '2027-11-30', 24, 'USUAL', 1577134.61),
		('RBP 4', '2027-11-30', 36, 'USUAL', 1216957.77),
		('RBP 4', '2027-11-30', 48, 'USUAL', 10663449.36),




		('other', '2027-12-31', 24, 'USUAL', 2194697.38),
		('other', '2027-12-31', 36, 'USUAL', 2520853.20),
		('other', '2027-12-31', 48, 'USUAL', 38779554.37),
		('PTS31_RBP4', '2027-12-31', 24, 'USUAL', 8231899.95),
		('PTS31_RBP4', '2027-12-31', 36, 'USUAL', 6605629.44),
		('PTS31_RBP4', '2027-12-31', 48, 'USUAL', 34156266.52),
		('RBP 1', '2027-12-31', 24, 'USUAL', 6484109.77),
		('RBP 1', '2027-12-31', 36, 'USUAL', 5311338.86),
		('RBP 1', '2027-12-31', 48, 'USUAL', 47797298.74),
		('RBP 2', '2027-12-31', 24, 'USUAL', 2075066.29),
		('RBP 2', '2027-12-31', 36, 'USUAL', 1600165.06),
		('RBP 2', '2027-12-31', 48, 'USUAL', 19565729.29),
		('RBP 3', '2027-12-31', 24, 'USUAL', 3259617.75),
		('RBP 3', '2027-12-31', 36, 'USUAL', 2479987.82),
		('RBP 3', '2027-12-31', 48, 'USUAL', 17670243.83),
		('RBP 4', '2027-12-31', 24, 'USUAL', 1577134.61),
		('RBP 4', '2027-12-31', 36, 'USUAL', 1216957.77),
		('RBP 4', '2027-12-31', 48, 'USUAL', 10663449.36),




		('other', '2028-01-31', 24, 'USUAL', 2414167.12),
		('other', '2028-01-31', 36, 'USUAL', 2772938.52),
		('other', '2028-01-31', 48, 'USUAL', 42657509.81),
		('PTS31_RBP4', '2028-01-31', 24, 'USUAL', 9055089.95),
		('PTS31_RBP4', '2028-01-31', 36, 'USUAL', 7266192.38),
		('PTS31_RBP4', '2028-01-31', 48, 'USUAL', 37571893.17),
		('RBP 1', '2028-01-31', 24, 'USUAL', 7132520.75),
		('RBP 1', '2028-01-31', 36, 'USUAL', 5842472.75),
		('RBP 1', '2028-01-31', 48, 'USUAL', 52577028.61),
		('RBP 2', '2028-01-31', 24, 'USUAL', 2282572.92),
		('RBP 2', '2028-01-31', 36, 'USUAL', 1760181.56),
		('RBP 2', '2028-01-31', 48, 'USUAL', 21522302.21),
		('RBP 3', '2028-01-31', 24, 'USUAL', 3585579.52),
		('RBP 3', '2028-01-31', 36, 'USUAL', 2727986.60),
		('RBP 3', '2028-01-31', 48, 'USUAL', 19437268.21),
		('RBP 4', '2028-01-31', 24, 'USUAL', 1734848.07),
		('RBP 4', '2028-01-31', 36, 'USUAL', 1338653.55),
		('RBP 4', '2028-01-31', 48, 'USUAL', 11729794.30),




		('other', '2028-02-29', 24, 'USUAL', 2414167.12),
		('other', '2028-02-29', 36, 'USUAL', 2772938.52),
		('other', '2028-02-29', 48, 'USUAL', 42657509.81),
		('PTS31_RBP4', '2028-02-29', 24, 'USUAL', 9055089.95),
		('PTS31_RBP4', '2028-02-29', 36, 'USUAL', 7266192.38),
		('PTS31_RBP4', '2028-02-29', 48, 'USUAL', 37571893.17),
		('RBP 1', '2028-02-29', 24, 'USUAL', 7132520.75),
		('RBP 1', '2028-02-29', 36, 'USUAL', 5842472.75),
		('RBP 1', '2028-02-29', 48, 'USUAL', 52577028.61),
		('RBP 2', '2028-02-29', 24, 'USUAL', 2282572.92),
		('RBP 2', '2028-02-29', 36, 'USUAL', 1760181.56),
		('RBP 2', '2028-02-29', 48, 'USUAL', 21522302.21),
		('RBP 3', '2028-02-29', 24, 'USUAL', 3585579.52),
		('RBP 3', '2028-02-29', 36, 'USUAL', 2727986.60),
		('RBP 3', '2028-02-29', 48, 'USUAL', 19437268.21),
		('RBP 4', '2028-02-29', 24, 'USUAL', 1734848.07),
		('RBP 4', '2028-02-29', 36, 'USUAL', 1338653.55),
		('RBP 4', '2028-02-29', 48, 'USUAL', 11729794.30),




		('other', '2028-03-31', 24, 'USUAL', 2414167.12),
		('other', '2028-03-31', 36, 'USUAL', 2772938.52),
		('other', '2028-03-31', 48, 'USUAL', 42657509.81),
		('PTS31_RBP4', '2028-03-31', 24, 'USUAL', 9055089.95),
		('PTS31_RBP4', '2028-03-31', 36, 'USUAL', 7266192.38),
		('PTS31_RBP4', '2028-03-31', 48, 'USUAL', 37571893.17),
		('RBP 1', '2028-03-31', 24, 'USUAL', 7132520.75),
		('RBP 1', '2028-03-31', 36, 'USUAL', 5842472.75),
		('RBP 1', '2028-03-31', 48, 'USUAL', 52577028.61),
		('RBP 2', '2028-03-31', 24, 'USUAL', 2282572.92),
		('RBP 2', '2028-03-31', 36, 'USUAL', 1760181.56),
		('RBP 2', '2028-03-31', 48, 'USUAL', 21522302.21),
		('RBP 3', '2028-03-31', 24, 'USUAL', 3585579.52),
		('RBP 3', '2028-03-31', 36, 'USUAL', 2727986.60),
		('RBP 3', '2028-03-31', 48, 'USUAL', 19437268.21),
		('RBP 4', '2028-03-31', 24, 'USUAL', 1734848.07),
		('RBP 4', '2028-03-31', 36, 'USUAL', 1338653.55),
		('RBP 4', '2028-03-31', 48, 'USUAL', 11729794.30),




		('other', '2028-04-30', 24, 'USUAL', 2414167.12),
		('other', '2028-04-30', 36, 'USUAL', 2772938.52),
		('other', '2028-04-30', 48, 'USUAL', 42657509.81),
		('PTS31_RBP4', '2028-04-30', 24, 'USUAL', 9055089.95),
		('PTS31_RBP4', '2028-04-30', 36, 'USUAL', 7266192.38),
		('PTS31_RBP4', '2028-04-30', 48, 'USUAL', 37571893.17),
		('RBP 1', '2028-04-30', 24, 'USUAL', 7132520.75),
		('RBP 1', '2028-04-30', 36, 'USUAL', 5842472.75),
		('RBP 1', '2028-04-30', 48, 'USUAL', 52577028.61),
		('RBP 2', '2028-04-30', 24, 'USUAL', 2282572.92),
		('RBP 2', '2028-04-30', 36, 'USUAL', 1760181.56),
		('RBP 2', '2028-04-30', 48, 'USUAL', 21522302.21),
		('RBP 3', '2028-04-30', 24, 'USUAL', 3585579.52),
		('RBP 3', '2028-04-30', 36, 'USUAL', 2727986.60),
		('RBP 3', '2028-04-30', 48, 'USUAL', 19437268.21),
		('RBP 4', '2028-04-30', 24, 'USUAL', 1734848.07),
		('RBP 4', '2028-04-30', 36, 'USUAL', 1338653.55),
		('RBP 4', '2028-04-30', 48, 'USUAL', 11729794.30),




		('other', '2028-05-31', 24, 'USUAL', 2414167.12),
		('other', '2028-05-31', 36, 'USUAL', 2772938.52),
		('other', '2028-05-31', 48, 'USUAL', 42657509.81),
		('PTS31_RBP4', '2028-05-31', 24, 'USUAL', 9055089.95),
		('PTS31_RBP4', '2028-05-31', 36, 'USUAL', 7266192.38),
		('PTS31_RBP4', '2028-05-31', 48, 'USUAL', 37571893.17),
		('RBP 1', '2028-05-31', 24, 'USUAL', 7132520.75),
		('RBP 1', '2028-05-31', 36, 'USUAL', 5842472.75),
		('RBP 1', '2028-05-31', 48, 'USUAL', 52577028.61),
		('RBP 2', '2028-05-31', 24, 'USUAL', 2282572.92),
		('RBP 2', '2028-05-31', 36, 'USUAL', 1760181.56),
		('RBP 2', '2028-05-31', 48, 'USUAL', 21522302.21),
		('RBP 3', '2028-05-31', 24, 'USUAL', 3585579.52),
		('RBP 3', '2028-05-31', 36, 'USUAL', 2727986.60),
		('RBP 3', '2028-05-31', 48, 'USUAL', 19437268.21),
		('RBP 4', '2028-05-31', 24, 'USUAL', 1734848.07),
		('RBP 4', '2028-05-31', 36, 'USUAL', 1338653.55),
		('RBP 4', '2028-05-31', 48, 'USUAL', 11729794.30),




		('other', '2028-06-30', 24, 'USUAL', 2414167.12),
		('other', '2028-06-30', 36, 'USUAL', 2772938.52),
		('other', '2028-06-30', 48, 'USUAL', 42657509.81),
		('PTS31_RBP4', '2028-06-30', 24, 'USUAL', 9055089.95),
		('PTS31_RBP4', '2028-06-30', 36, 'USUAL', 7266192.38),
		('PTS31_RBP4', '2028-06-30', 48, 'USUAL', 37571893.17),
		('RBP 1', '2028-06-30', 24, 'USUAL', 7132520.75),
		('RBP 1', '2028-06-30', 36, 'USUAL', 5842472.75),
		('RBP 1', '2028-06-30', 48, 'USUAL', 52577028.61),
		('RBP 2', '2028-06-30', 24, 'USUAL', 2282572.92),
		('RBP 2', '2028-06-30', 36, 'USUAL', 1760181.56),
		('RBP 2', '2028-06-30', 48, 'USUAL', 21522302.21),
		('RBP 3', '2028-06-30', 24, 'USUAL', 3585579.52),
		('RBP 3', '2028-06-30', 36, 'USUAL', 2727986.60),
		('RBP 3', '2028-06-30', 48, 'USUAL', 19437268.21),
		('RBP 4', '2028-06-30', 24, 'USUAL', 1734848.07),
		('RBP 4', '2028-06-30', 36, 'USUAL', 1338653.55),
		('RBP 4', '2028-06-30', 48, 'USUAL', 11729794.30),




		('other', '2028-07-31', 24, 'USUAL', 2414167.12),
		('other', '2028-07-31', 36, 'USUAL', 2772938.52),
		('other', '2028-07-31', 48, 'USUAL', 42657509.81),
		('PTS31_RBP4', '2028-07-31', 24, 'USUAL', 9055089.95),
		('PTS31_RBP4', '2028-07-31', 36, 'USUAL', 7266192.38),
		('PTS31_RBP4', '2028-07-31', 48, 'USUAL', 37571893.17),
		('RBP 1', '2028-07-31', 24, 'USUAL', 7132520.75),
		('RBP 1', '2028-07-31', 36, 'USUAL', 5842472.75),
		('RBP 1', '2028-07-31', 48, 'USUAL', 52577028.61),
		('RBP 2', '2028-07-31', 24, 'USUAL', 2282572.92),
		('RBP 2', '2028-07-31', 36, 'USUAL', 1760181.56),
		('RBP 2', '2028-07-31', 48, 'USUAL', 21522302.21),
		('RBP 3', '2028-07-31', 24, 'USUAL', 3585579.52),
		('RBP 3', '2028-07-31', 36, 'USUAL', 2727986.60),
		('RBP 3', '2028-07-31', 48, 'USUAL', 19437268.21),
		('RBP 4', '2028-07-31', 24, 'USUAL', 1734848.07),
		('RBP 4', '2028-07-31', 36, 'USUAL', 1338653.55),
		('RBP 4', '2028-07-31', 48, 'USUAL', 11729794.30),




		('other', '2028-08-31', 24, 'USUAL', 2414167.12),
		('other', '2028-08-31', 36, 'USUAL', 2772938.52),
		('other', '2028-08-31', 48, 'USUAL', 42657509.81),
		('PTS31_RBP4', '2028-08-31', 24, 'USUAL', 9055089.95),
		('PTS31_RBP4', '2028-08-31', 36, 'USUAL', 7266192.38),
		('PTS31_RBP4', '2028-08-31', 48, 'USUAL', 37571893.17),
		('RBP 1', '2028-08-31', 24, 'USUAL', 7132520.75),
		('RBP 1', '2028-08-31', 36, 'USUAL', 5842472.75),
		('RBP 1', '2028-08-31', 48, 'USUAL', 52577028.61),
		('RBP 2', '2028-08-31', 24, 'USUAL', 2282572.92),
		('RBP 2', '2028-08-31', 36, 'USUAL', 1760181.56),
		('RBP 2', '2028-08-31', 48, 'USUAL', 21522302.21),
		('RBP 3', '2028-08-31', 24, 'USUAL', 3585579.52),
		('RBP 3', '2028-08-31', 36, 'USUAL', 2727986.60),
		('RBP 3', '2028-08-31', 48, 'USUAL', 19437268.21),
		('RBP 4', '2028-08-31', 24, 'USUAL', 1734848.07),
		('RBP 4', '2028-08-31', 36, 'USUAL', 1338653.55),
		('RBP 4', '2028-08-31', 48, 'USUAL', 11729794.30),




		('other', '2028-09-30', 24, 'USUAL', 2414167.12),
		('other', '2028-09-30', 36, 'USUAL', 2772938.52),
		('other', '2028-09-30', 48, 'USUAL', 42657509.81),
		('PTS31_RBP4', '2028-09-30', 24, 'USUAL', 9055089.95),
		('PTS31_RBP4', '2028-09-30', 36, 'USUAL', 7266192.38),
		('PTS31_RBP4', '2028-09-30', 48, 'USUAL', 37571893.17),
		('RBP 1', '2028-09-30', 24, 'USUAL', 7132520.75),
		('RBP 1', '2028-09-30', 36, 'USUAL', 5842472.75),
		('RBP 1', '2028-09-30', 48, 'USUAL', 52577028.61),
		('RBP 2', '2028-09-30', 24, 'USUAL', 2282572.92),
		('RBP 2', '2028-09-30', 36, 'USUAL', 1760181.56),
		('RBP 2', '2028-09-30', 48, 'USUAL', 21522302.21),
		('RBP 3', '2028-09-30', 24, 'USUAL', 3585579.52),
		('RBP 3', '2028-09-30', 36, 'USUAL', 2727986.60),
		('RBP 3', '2028-09-30', 48, 'USUAL', 19437268.21),
		('RBP 4', '2028-09-30', 24, 'USUAL', 1734848.07),
		('RBP 4', '2028-09-30', 36, 'USUAL', 1338653.55),
		('RBP 4', '2028-09-30', 48, 'USUAL', 11729794.30),




		('other', '2028-10-31', 24, 'USUAL', 2414167.12),
		('other', '2028-10-31', 36, 'USUAL', 2772938.52),
		('other', '2028-10-31', 48, 'USUAL', 42657509.81),
		('PTS31_RBP4', '2028-10-31', 24, 'USUAL', 9055089.95),
		('PTS31_RBP4', '2028-10-31', 36, 'USUAL', 7266192.38),
		('PTS31_RBP4', '2028-10-31', 48, 'USUAL', 37571893.17),
		('RBP 1', '2028-10-31', 24, 'USUAL', 7132520.75),
		('RBP 1', '2028-10-31', 36, 'USUAL', 5842472.75),
		('RBP 1', '2028-10-31', 48, 'USUAL', 52577028.61),
		('RBP 2', '2028-10-31', 24, 'USUAL', 2282572.92),
		('RBP 2', '2028-10-31', 36, 'USUAL', 1760181.56),
		('RBP 2', '2028-10-31', 48, 'USUAL', 21522302.21),
		('RBP 3', '2028-10-31', 24, 'USUAL', 3585579.52),
		('RBP 3', '2028-10-31', 36, 'USUAL', 2727986.60),
		('RBP 3', '2028-10-31', 48, 'USUAL', 19437268.21),
		('RBP 4', '2028-10-31', 24, 'USUAL', 1734848.07),
		('RBP 4', '2028-10-31', 36, 'USUAL', 1338653.55),
		('RBP 4', '2028-10-31', 48, 'USUAL', 11729794.30),




		('other', '2028-11-30', 24, 'USUAL', 2414167.12),
		('other', '2028-11-30', 36, 'USUAL', 2772938.52),
		('other', '2028-11-30', 48, 'USUAL', 42657509.81),
		('PTS31_RBP4', '2028-11-30', 24, 'USUAL', 9055089.95),
		('PTS31_RBP4', '2028-11-30', 36, 'USUAL', 7266192.38),
		('PTS31_RBP4', '2028-11-30', 48, 'USUAL', 37571893.17),
		('RBP 1', '2028-11-30', 24, 'USUAL', 7132520.75),
		('RBP 1', '2028-11-30', 36, 'USUAL', 5842472.75),
		('RBP 1', '2028-11-30', 48, 'USUAL', 52577028.61),
		('RBP 2', '2028-11-30', 24, 'USUAL', 2282572.92),
		('RBP 2', '2028-11-30', 36, 'USUAL', 1760181.56),
		('RBP 2', '2028-11-30', 48, 'USUAL', 21522302.21),
		('RBP 3', '2028-11-30', 24, 'USUAL', 3585579.52),
		('RBP 3', '2028-11-30', 36, 'USUAL', 2727986.60),
		('RBP 3', '2028-11-30', 48, 'USUAL', 19437268.21),
		('RBP 4', '2028-11-30', 24, 'USUAL', 1734848.07),
		('RBP 4', '2028-11-30', 36, 'USUAL', 1338653.55),
		('RBP 4', '2028-11-30', 48, 'USUAL', 11729794.30),




		('other', '2028-12-31', 24, 'USUAL', 2414167.12),
		('other', '2028-12-31', 36, 'USUAL', 2772938.52),
		('other', '2028-12-31', 48, 'USUAL', 42657509.81),
		('PTS31_RBP4', '2028-12-31', 24, 'USUAL', 9055089.95),
		('PTS31_RBP4', '2028-12-31', 36, 'USUAL', 7266192.38),
		('PTS31_RBP4', '2028-12-31', 48, 'USUAL', 37571893.17),
		('RBP 1', '2028-12-31', 24, 'USUAL', 7132520.75),
		('RBP 1', '2028-12-31', 36, 'USUAL', 5842472.75),
		('RBP 1', '2028-12-31', 48, 'USUAL', 52577028.61),
		('RBP 2', '2028-12-31', 24, 'USUAL', 2282572.92),
		('RBP 2', '2028-12-31', 36, 'USUAL', 1760181.56),
		('RBP 2', '2028-12-31', 48, 'USUAL', 21522302.21),
		('RBP 3', '2028-12-31', 24, 'USUAL', 3585579.52),
		('RBP 3', '2028-12-31', 36, 'USUAL', 2727986.60),
		('RBP 3', '2028-12-31', 48, 'USUAL', 19437268.21),
		('RBP 4', '2028-12-31', 24, 'USUAL', 1734848.07),
		('RBP 4', '2028-12-31', 36, 'USUAL', 1338653.55),
		('RBP 4', '2028-12-31', 48, 'USUAL', 11729794.30),




		('other', '2029-01-31', 24, 'USUAL', 2655583.83),
		('other', '2029-01-31', 36, 'USUAL', 3050232.37),
		('other', '2029-01-31', 48, 'USUAL', 46923260.79),
		('PTS31_RBP4', '2029-01-31', 24, 'USUAL', 9960598.94),
		('PTS31_RBP4', '2029-01-31', 36, 'USUAL', 7992811.62),
		('PTS31_RBP4', '2029-01-31', 48, 'USUAL', 41329082.48),
		('RBP 1', '2029-01-31', 24, 'USUAL', 7845772.82),
		('RBP 1', '2029-01-31', 36, 'USUAL', 6426720.02),
		('RBP 1', '2029-01-31', 48, 'USUAL', 57834731.47),
		('RBP 2', '2029-01-31', 24, 'USUAL', 2510830.21),
		('RBP 2', '2029-01-31', 36, 'USUAL', 1936199.72),
		('RBP 2', '2029-01-31', 48, 'USUAL', 23674532.44),
		('RBP 3', '2029-01-31', 24, 'USUAL', 3944137.48),
		('RBP 3', '2029-01-31', 36, 'USUAL', 3000785.26),
		('RBP 3', '2029-01-31', 48, 'USUAL', 21380995.03),
		('RBP 4', '2029-01-31', 24, 'USUAL', 1908332.88),
		('RBP 4', '2029-01-31', 36, 'USUAL', 1472518.90),
		('RBP 4', '2029-01-31', 48, 'USUAL', 12902773.73),




		('other', '2029-02-28', 24, 'USUAL', 2655583.83),
		('other', '2029-02-28', 36, 'USUAL', 3050232.37),
		('other', '2029-02-28', 48, 'USUAL', 46923260.79),
		('PTS31_RBP4', '2029-02-28', 24, 'USUAL', 9960598.94),
		('PTS31_RBP4', '2029-02-28', 36, 'USUAL', 7992811.62),
		('PTS31_RBP4', '2029-02-28', 48, 'USUAL', 41329082.48),
		('RBP 1', '2029-02-28', 24, 'USUAL', 7845772.82),
		('RBP 1', '2029-02-28', 36, 'USUAL', 6426720.02),
		('RBP 1', '2029-02-28', 48, 'USUAL', 57834731.47),
		('RBP 2', '2029-02-28', 24, 'USUAL', 2510830.21),
		('RBP 2', '2029-02-28', 36, 'USUAL', 1936199.72),
		('RBP 2', '2029-02-28', 48, 'USUAL', 23674532.44),
		('RBP 3', '2029-02-28', 24, 'USUAL', 3944137.48),
		('RBP 3', '2029-02-28', 36, 'USUAL', 3000785.26),
		('RBP 3', '2029-02-28', 48, 'USUAL', 21380995.03),
		('RBP 4', '2029-02-28', 24, 'USUAL', 1908332.88),
		('RBP 4', '2029-02-28', 36, 'USUAL', 1472518.90),
		('RBP 4', '2029-02-28', 48, 'USUAL', 12902773.73),




		('other', '2029-03-31', 24, 'USUAL', 2655583.83),
		('other', '2029-03-31', 36, 'USUAL', 3050232.37),
		('other', '2029-03-31', 48, 'USUAL', 46923260.79),
		('PTS31_RBP4', '2029-03-31', 24, 'USUAL', 9960598.94),
		('PTS31_RBP4', '2029-03-31', 36, 'USUAL', 7992811.62),
		('PTS31_RBP4', '2029-03-31', 48, 'USUAL', 41329082.48),
		('RBP 1', '2029-03-31', 24, 'USUAL', 7845772.82),
		('RBP 1', '2029-03-31', 36, 'USUAL', 6426720.02),
		('RBP 1', '2029-03-31', 48, 'USUAL', 57834731.47),
		('RBP 2', '2029-03-31', 24, 'USUAL', 2510830.21),
		('RBP 2', '2029-03-31', 36, 'USUAL', 1936199.72),
		('RBP 2', '2029-03-31', 48, 'USUAL', 23674532.44),
		('RBP 3', '2029-03-31', 24, 'USUAL', 3944137.48),
		('RBP 3', '2029-03-31', 36, 'USUAL', 3000785.26),
		('RBP 3', '2029-03-31', 48, 'USUAL', 21380995.03),
		('RBP 4', '2029-03-31', 24, 'USUAL', 1908332.88),
		('RBP 4', '2029-03-31', 36, 'USUAL', 1472518.90),
		('RBP 4', '2029-03-31', 48, 'USUAL', 12902773.73),




		('other', '2029-04-30', 24, 'USUAL', 2655583.83),
		('other', '2029-04-30', 36, 'USUAL', 3050232.37),
		('other', '2029-04-30', 48, 'USUAL', 46923260.79),
		('PTS31_RBP4', '2029-04-30', 24, 'USUAL', 9960598.94),
		('PTS31_RBP4', '2029-04-30', 36, 'USUAL', 7992811.62),
		('PTS31_RBP4', '2029-04-30', 48, 'USUAL', 41329082.48),
		('RBP 1', '2029-04-30', 24, 'USUAL', 7845772.82),
		('RBP 1', '2029-04-30', 36, 'USUAL', 6426720.02),
		('RBP 1', '2029-04-30', 48, 'USUAL', 57834731.47),
		('RBP 2', '2029-04-30', 24, 'USUAL', 2510830.21),
		('RBP 2', '2029-04-30', 36, 'USUAL', 1936199.72),
		('RBP 2', '2029-04-30', 48, 'USUAL', 23674532.44),
		('RBP 3', '2029-04-30', 24, 'USUAL', 3944137.48),
		('RBP 3', '2029-04-30', 36, 'USUAL', 3000785.26),
		('RBP 3', '2029-04-30', 48, 'USUAL', 21380995.03),
		('RBP 4', '2029-04-30', 24, 'USUAL', 1908332.88),
		('RBP 4', '2029-04-30', 36, 'USUAL', 1472518.90),
		('RBP 4', '2029-04-30', 48, 'USUAL', 12902773.73),




		('other', '2029-05-31', 24, 'USUAL', 2655583.83),
		('other', '2029-05-31', 36, 'USUAL', 3050232.37),
		('other', '2029-05-31', 48, 'USUAL', 46923260.79),
		('PTS31_RBP4', '2029-05-31', 24, 'USUAL', 9960598.94),
		('PTS31_RBP4', '2029-05-31', 36, 'USUAL', 7992811.62),
		('PTS31_RBP4', '2029-05-31', 48, 'USUAL', 41329082.48),
		('RBP 1', '2029-05-31', 24, 'USUAL', 7845772.82),
		('RBP 1', '2029-05-31', 36, 'USUAL', 6426720.02),
		('RBP 1', '2029-05-31', 48, 'USUAL', 57834731.47),
		('RBP 2', '2029-05-31', 24, 'USUAL', 2510830.21),
		('RBP 2', '2029-05-31', 36, 'USUAL', 1936199.72),
		('RBP 2', '2029-05-31', 48, 'USUAL', 23674532.44),
		('RBP 3', '2029-05-31', 24, 'USUAL', 3944137.48),
		('RBP 3', '2029-05-31', 36, 'USUAL', 3000785.26),
		('RBP 3', '2029-05-31', 48, 'USUAL', 21380995.03),
		('RBP 4', '2029-05-31', 24, 'USUAL', 1908332.88),
		('RBP 4', '2029-05-31', 36, 'USUAL', 1472518.90),
		('RBP 4', '2029-05-31', 48, 'USUAL', 12902773.73),




		('other', '2029-06-30', 24, 'USUAL', 2655583.83),
		('other', '2029-06-30', 36, 'USUAL', 3050232.37),
		('other', '2029-06-30', 48, 'USUAL', 46923260.79),
		('PTS31_RBP4', '2029-06-30', 24, 'USUAL', 9960598.94),
		('PTS31_RBP4', '2029-06-30', 36, 'USUAL', 7992811.62),
		('PTS31_RBP4', '2029-06-30', 48, 'USUAL', 41329082.48),
		('RBP 1', '2029-06-30', 24, 'USUAL', 7845772.82),
		('RBP 1', '2029-06-30', 36, 'USUAL', 6426720.02),
		('RBP 1', '2029-06-30', 48, 'USUAL', 57834731.47),
		('RBP 2', '2029-06-30', 24, 'USUAL', 2510830.21),
		('RBP 2', '2029-06-30', 36, 'USUAL', 1936199.72),
		('RBP 2', '2029-06-30', 48, 'USUAL', 23674532.44),
		('RBP 3', '2029-06-30', 24, 'USUAL', 3944137.48),
		('RBP 3', '2029-06-30', 36, 'USUAL', 3000785.26),
		('RBP 3', '2029-06-30', 48, 'USUAL', 21380995.03),
		('RBP 4', '2029-06-30', 24, 'USUAL', 1908332.88),
		('RBP 4', '2029-06-30', 36, 'USUAL', 1472518.90),
		('RBP 4', '2029-06-30', 48, 'USUAL', 12902773.73),




		('other', '2029-07-31', 24, 'USUAL', 2655583.83),
		('other', '2029-07-31', 36, 'USUAL', 3050232.37),
		('other', '2029-07-31', 48, 'USUAL', 46923260.79),
		('PTS31_RBP4', '2029-07-31', 24, 'USUAL', 9960598.94),
		('PTS31_RBP4', '2029-07-31', 36, 'USUAL', 7992811.62),
		('PTS31_RBP4', '2029-07-31', 48, 'USUAL', 41329082.48),
		('RBP 1', '2029-07-31', 24, 'USUAL', 7845772.82),
		('RBP 1', '2029-07-31', 36, 'USUAL', 6426720.02),
		('RBP 1', '2029-07-31', 48, 'USUAL', 57834731.47),
		('RBP 2', '2029-07-31', 24, 'USUAL', 2510830.21),
		('RBP 2', '2029-07-31', 36, 'USUAL', 1936199.72),
		('RBP 2', '2029-07-31', 48, 'USUAL', 23674532.44),
		('RBP 3', '2029-07-31', 24, 'USUAL', 3944137.48),
		('RBP 3', '2029-07-31', 36, 'USUAL', 3000785.26),
		('RBP 3', '2029-07-31', 48, 'USUAL', 21380995.03),
		('RBP 4', '2029-07-31', 24, 'USUAL', 1908332.88),
		('RBP 4', '2029-07-31', 36, 'USUAL', 1472518.90),
		('RBP 4', '2029-07-31', 48, 'USUAL', 12902773.73),




		('other', '2029-08-31', 24, 'USUAL', 2655583.83),
		('other', '2029-08-31', 36, 'USUAL', 3050232.37),
		('other', '2029-08-31', 48, 'USUAL', 46923260.79),
		('PTS31_RBP4', '2029-08-31', 24, 'USUAL', 9960598.94),
		('PTS31_RBP4', '2029-08-31', 36, 'USUAL', 7992811.62),
		('PTS31_RBP4', '2029-08-31', 48, 'USUAL', 41329082.48),
		('RBP 1', '2029-08-31', 24, 'USUAL', 7845772.82),
		('RBP 1', '2029-08-31', 36, 'USUAL', 6426720.02),
		('RBP 1', '2029-08-31', 48, 'USUAL', 57834731.47),
		('RBP 2', '2029-08-31', 24, 'USUAL', 2510830.21),
		('RBP 2', '2029-08-31', 36, 'USUAL', 1936199.72),
		('RBP 2', '2029-08-31', 48, 'USUAL', 23674532.44),
		('RBP 3', '2029-08-31', 24, 'USUAL', 3944137.48),
		('RBP 3', '2029-08-31', 36, 'USUAL', 3000785.26),
		('RBP 3', '2029-08-31', 48, 'USUAL', 21380995.03),
		('RBP 4', '2029-08-31', 24, 'USUAL', 1908332.88),
		('RBP 4', '2029-08-31', 36, 'USUAL', 1472518.90),
		('RBP 4', '2029-08-31', 48, 'USUAL', 12902773.73),




		('other', '2029-09-30', 24, 'USUAL', 2655583.83),
		('other', '2029-09-30', 36, 'USUAL', 3050232.37),
		('other', '2029-09-30', 48, 'USUAL', 46923260.79),
		('PTS31_RBP4', '2029-09-30', 24, 'USUAL', 9960598.94),
		('PTS31_RBP4', '2029-09-30', 36, 'USUAL', 7992811.62),
		('PTS31_RBP4', '2029-09-30', 48, 'USUAL', 41329082.48),
		('RBP 1', '2029-09-30', 24, 'USUAL', 7845772.82),
		('RBP 1', '2029-09-30', 36, 'USUAL', 6426720.02),
		('RBP 1', '2029-09-30', 48, 'USUAL', 57834731.47),
		('RBP 2', '2029-09-30', 24, 'USUAL', 2510830.21),
		('RBP 2', '2029-09-30', 36, 'USUAL', 1936199.72),
		('RBP 2', '2029-09-30', 48, 'USUAL', 23674532.44),
		('RBP 3', '2029-09-30', 24, 'USUAL', 3944137.48),
		('RBP 3', '2029-09-30', 36, 'USUAL', 3000785.26),
		('RBP 3', '2029-09-30', 48, 'USUAL', 21380995.03),
		('RBP 4', '2029-09-30', 24, 'USUAL', 1908332.88),
		('RBP 4', '2029-09-30', 36, 'USUAL', 1472518.90),
		('RBP 4', '2029-09-30', 48, 'USUAL', 12902773.73),




		('other', '2029-10-31', 24, 'USUAL', 2655583.83),
		('other', '2029-10-31', 36, 'USUAL', 3050232.37),
		('other', '2029-10-31', 48, 'USUAL', 46923260.79),
		('PTS31_RBP4', '2029-10-31', 24, 'USUAL', 9960598.94),
		('PTS31_RBP4', '2029-10-31', 36, 'USUAL', 7992811.62),
		('PTS31_RBP4', '2029-10-31', 48, 'USUAL', 41329082.48),
		('RBP 1', '2029-10-31', 24, 'USUAL', 7845772.82),
		('RBP 1', '2029-10-31', 36, 'USUAL', 6426720.02),
		('RBP 1', '2029-10-31', 48, 'USUAL', 57834731.47),
		('RBP 2', '2029-10-31', 24, 'USUAL', 2510830.21),
		('RBP 2', '2029-10-31', 36, 'USUAL', 1936199.72),
		('RBP 2', '2029-10-31', 48, 'USUAL', 23674532.44),
		('RBP 3', '2029-10-31', 24, 'USUAL', 3944137.48),
		('RBP 3', '2029-10-31', 36, 'USUAL', 3000785.26),
		('RBP 3', '2029-10-31', 48, 'USUAL', 21380995.03),
		('RBP 4', '2029-10-31', 24, 'USUAL', 1908332.88),
		('RBP 4', '2029-10-31', 36, 'USUAL', 1472518.90),
		('RBP 4', '2029-10-31', 48, 'USUAL', 12902773.73),




		('other', '2029-11-30', 24, 'USUAL', 2655583.83),
		('other', '2029-11-30', 36, 'USUAL', 3050232.37),
		('other', '2029-11-30', 48, 'USUAL', 46923260.79),
		('PTS31_RBP4', '2029-11-30', 24, 'USUAL', 9960598.94),
		('PTS31_RBP4', '2029-11-30', 36, 'USUAL', 7992811.62),
		('PTS31_RBP4', '2029-11-30', 48, 'USUAL', 41329082.48),
		('RBP 1', '2029-11-30', 24, 'USUAL', 7845772.82),
		('RBP 1', '2029-11-30', 36, 'USUAL', 6426720.02),
		('RBP 1', '2029-11-30', 48, 'USUAL', 57834731.47),
		('RBP 2', '2029-11-30', 24, 'USUAL', 2510830.21),
		('RBP 2', '2029-11-30', 36, 'USUAL', 1936199.72),
		('RBP 2', '2029-11-30', 48, 'USUAL', 23674532.44),
		('RBP 3', '2029-11-30', 24, 'USUAL', 3944137.48),
		('RBP 3', '2029-11-30', 36, 'USUAL', 3000785.26),
		('RBP 3', '2029-11-30', 48, 'USUAL', 21380995.03),
		('RBP 4', '2029-11-30', 24, 'USUAL', 1908332.88),
		('RBP 4', '2029-11-30', 36, 'USUAL', 1472518.90),
		('RBP 4', '2029-11-30', 48, 'USUAL', 12902773.73),




		('other', '2029-12-31', 24, 'USUAL', 2655583.83),
		('other', '2029-12-31', 36, 'USUAL', 3050232.37),
		('other', '2029-12-31', 48, 'USUAL', 46923260.79),
		('PTS31_RBP4', '2029-12-31', 24, 'USUAL', 9960598.94),
		('PTS31_RBP4', '2029-12-31', 36, 'USUAL', 7992811.62),
		('PTS31_RBP4', '2029-12-31', 48, 'USUAL', 41329082.48),
		('RBP 1', '2029-12-31', 24, 'USUAL', 7845772.82),
		('RBP 1', '2029-12-31', 36, 'USUAL', 6426720.02),
		('RBP 1', '2029-12-31', 48, 'USUAL', 57834731.47),
		('RBP 2', '2029-12-31', 24, 'USUAL', 2510830.21),
		('RBP 2', '2029-12-31', 36, 'USUAL', 1936199.72),
		('RBP 2', '2029-12-31', 48, 'USUAL', 23674532.44),
		('RBP 3', '2029-12-31', 24, 'USUAL', 3944137.48),
		('RBP 3', '2029-12-31', 36, 'USUAL', 3000785.26),
		('RBP 3', '2029-12-31', 48, 'USUAL', 21380995.03),
		('RBP 4', '2029-12-31', 24, 'USUAL', 1908332.88),
		('RBP 4', '2029-12-31', 36, 'USUAL', 1472518.90),
		('RBP 4', '2029-12-31', 48, 'USUAL', 12902773.73),




		('other', '2030-01-31', 24, 'USUAL', 2921142.22),
		('other', '2030-01-31', 36, 'USUAL', 3355255.61),
		('other', '2030-01-31', 48, 'USUAL', 51615586.87),
		('PTS31_RBP4', '2030-01-31', 24, 'USUAL', 10956658.84),
		('PTS31_RBP4', '2030-01-31', 36, 'USUAL', 8792092.78),
		('PTS31_RBP4', '2030-01-31', 48, 'USUAL', 45461990.73),
		('RBP 1', '2030-01-31', 24, 'USUAL', 8630350.11),
		('RBP 1', '2030-01-31', 36, 'USUAL', 7069392.03),
		('RBP 1', '2030-01-31', 48, 'USUAL', 63618204.62),
		('RBP 2', '2030-01-31', 24, 'USUAL', 2761913.23),
		('RBP 2', '2030-01-31', 36, 'USUAL', 2129819.69),
		('RBP 2', '2030-01-31', 48, 'USUAL', 26041985.68),
		('RBP 3', '2030-01-31', 24, 'USUAL', 4338551.22),
		('RBP 3', '2030-01-31', 36, 'USUAL', 3300863.78),
		('RBP 3', '2030-01-31', 48, 'USUAL', 23519094.54),
		('RBP 4', '2030-01-31', 24, 'USUAL', 2099166.17),
		('RBP 4', '2030-01-31', 36, 'USUAL', 1619770.79),
		('RBP 4', '2030-01-31', 48, 'USUAL', 14193051.10),




		('other', '2030-02-28', 24, 'USUAL', 2921142.22),
		('other', '2030-02-28', 36, 'USUAL', 3355255.61),
		('other', '2030-02-28', 48, 'USUAL', 51615586.87),
		('PTS31_RBP4', '2030-02-28', 24, 'USUAL', 10956658.84),
		('PTS31_RBP4', '2030-02-28', 36, 'USUAL', 8792092.78),
		('PTS31_RBP4', '2030-02-28', 48, 'USUAL', 45461990.73),
		('RBP 1', '2030-02-28', 24, 'USUAL', 8630350.11),
		('RBP 1', '2030-02-28', 36, 'USUAL', 7069392.03),
		('RBP 1', '2030-02-28', 48, 'USUAL', 63618204.62),
		('RBP 2', '2030-02-28', 24, 'USUAL', 2761913.23),
		('RBP 2', '2030-02-28', 36, 'USUAL', 2129819.69),
		('RBP 2', '2030-02-28', 48, 'USUAL', 26041985.68),
		('RBP 3', '2030-02-28', 24, 'USUAL', 4338551.22),
		('RBP 3', '2030-02-28', 36, 'USUAL', 3300863.78),
		('RBP 3', '2030-02-28', 48, 'USUAL', 23519094.54),
		('RBP 4', '2030-02-28', 24, 'USUAL', 2099166.17),
		('RBP 4', '2030-02-28', 36, 'USUAL', 1619770.79),
		('RBP 4', '2030-02-28', 48, 'USUAL', 14193051.10),




		('other', '2030-03-31', 24, 'USUAL', 2921142.22),
		('other', '2030-03-31', 36, 'USUAL', 3355255.61),
		('other', '2030-03-31', 48, 'USUAL', 51615586.87),
		('PTS31_RBP4', '2030-03-31', 24, 'USUAL', 10956658.84),
		('PTS31_RBP4', '2030-03-31', 36, 'USUAL', 8792092.78),
		('PTS31_RBP4', '2030-03-31', 48, 'USUAL', 45461990.73),
		('RBP 1', '2030-03-31', 24, 'USUAL', 8630350.11),
		('RBP 1', '2030-03-31', 36, 'USUAL', 7069392.03),
		('RBP 1', '2030-03-31', 48, 'USUAL', 63618204.62),
		('RBP 2', '2030-03-31', 24, 'USUAL', 2761913.23),
		('RBP 2', '2030-03-31', 36, 'USUAL', 2129819.69),
		('RBP 2', '2030-03-31', 48, 'USUAL', 26041985.68),
		('RBP 3', '2030-03-31', 24, 'USUAL', 4338551.22),
		('RBP 3', '2030-03-31', 36, 'USUAL', 3300863.78),
		('RBP 3', '2030-03-31', 48, 'USUAL', 23519094.54),
		('RBP 4', '2030-03-31', 24, 'USUAL', 2099166.17),
		('RBP 4', '2030-03-31', 36, 'USUAL', 1619770.79),
		('RBP 4', '2030-03-31', 48, 'USUAL', 14193051.10),




		('other', '2030-04-30', 24, 'USUAL', 2921142.22),
		('other', '2030-04-30', 36, 'USUAL', 3355255.61),
		('other', '2030-04-30', 48, 'USUAL', 51615586.87),
		('PTS31_RBP4', '2030-04-30', 24, 'USUAL', 10956658.84),
		('PTS31_RBP4', '2030-04-30', 36, 'USUAL', 8792092.78),
		('PTS31_RBP4', '2030-04-30', 48, 'USUAL', 45461990.73),
		('RBP 1', '2030-04-30', 24, 'USUAL', 8630350.11),
		('RBP 1', '2030-04-30', 36, 'USUAL', 7069392.03),
		('RBP 1', '2030-04-30', 48, 'USUAL', 63618204.62),
		('RBP 2', '2030-04-30', 24, 'USUAL', 2761913.23),
		('RBP 2', '2030-04-30', 36, 'USUAL', 2129819.69),
		('RBP 2', '2030-04-30', 48, 'USUAL', 26041985.68),
		('RBP 3', '2030-04-30', 24, 'USUAL', 4338551.22),
		('RBP 3', '2030-04-30', 36, 'USUAL', 3300863.78),
		('RBP 3', '2030-04-30', 48, 'USUAL', 23519094.54),
		('RBP 4', '2030-04-30', 24, 'USUAL', 2099166.17),
		('RBP 4', '2030-04-30', 36, 'USUAL', 1619770.79),
		('RBP 4', '2030-04-30', 48, 'USUAL', 14193051.10),




		('other', '2030-05-31', 24, 'USUAL', 2921142.22),
		('other', '2030-05-31', 36, 'USUAL', 3355255.61),
		('other', '2030-05-31', 48, 'USUAL', 51615586.87),
		('PTS31_RBP4', '2030-05-31', 24, 'USUAL', 10956658.84),
		('PTS31_RBP4', '2030-05-31', 36, 'USUAL', 8792092.78),
		('PTS31_RBP4', '2030-05-31', 48, 'USUAL', 45461990.73),
		('RBP 1', '2030-05-31', 24, 'USUAL', 8630350.11),
		('RBP 1', '2030-05-31', 36, 'USUAL', 7069392.03),
		('RBP 1', '2030-05-31', 48, 'USUAL', 63618204.62),
		('RBP 2', '2030-05-31', 24, 'USUAL', 2761913.23),
		('RBP 2', '2030-05-31', 36, 'USUAL', 2129819.69),
		('RBP 2', '2030-05-31', 48, 'USUAL', 26041985.68),
		('RBP 3', '2030-05-31', 24, 'USUAL', 4338551.22),
		('RBP 3', '2030-05-31', 36, 'USUAL', 3300863.78),
		('RBP 3', '2030-05-31', 48, 'USUAL', 23519094.54),
		('RBP 4', '2030-05-31', 24, 'USUAL', 2099166.17),
		('RBP 4', '2030-05-31', 36, 'USUAL', 1619770.79),
		('RBP 4', '2030-05-31', 48, 'USUAL', 14193051.10),




		('other', '2030-06-30', 24, 'USUAL', 2921142.22),
		('other', '2030-06-30', 36, 'USUAL', 3355255.61),
		('other', '2030-06-30', 48, 'USUAL', 51615586.87),
		('PTS31_RBP4', '2030-06-30', 24, 'USUAL', 10956658.84),
		('PTS31_RBP4', '2030-06-30', 36, 'USUAL', 8792092.78),
		('PTS31_RBP4', '2030-06-30', 48, 'USUAL', 45461990.73),
		('RBP 1', '2030-06-30', 24, 'USUAL', 8630350.11),
		('RBP 1', '2030-06-30', 36, 'USUAL', 7069392.03),
		('RBP 1', '2030-06-30', 48, 'USUAL', 63618204.62),
		('RBP 2', '2030-06-30', 24, 'USUAL', 2761913.23),
		('RBP 2', '2030-06-30', 36, 'USUAL', 2129819.69),
		('RBP 2', '2030-06-30', 48, 'USUAL', 26041985.68),
		('RBP 3', '2030-06-30', 24, 'USUAL', 4338551.22),
		('RBP 3', '2030-06-30', 36, 'USUAL', 3300863.78),
		('RBP 3', '2030-06-30', 48, 'USUAL', 23519094.54),
		('RBP 4', '2030-06-30', 24, 'USUAL', 2099166.17),
		('RBP 4', '2030-06-30', 36, 'USUAL', 1619770.79),
		('RBP 4', '2030-06-30', 48, 'USUAL', 14193051.10),




		('other', '2030-07-31', 24, 'USUAL', 2921142.22),
		('other', '2030-07-31', 36, 'USUAL', 3355255.61),
		('other', '2030-07-31', 48, 'USUAL', 51615586.87),
		('PTS31_RBP4', '2030-07-31', 24, 'USUAL', 10956658.84),
		('PTS31_RBP4', '2030-07-31', 36, 'USUAL', 8792092.78),
		('PTS31_RBP4', '2030-07-31', 48, 'USUAL', 45461990.73),
		('RBP 1', '2030-07-31', 24, 'USUAL', 8630350.11),
		('RBP 1', '2030-07-31', 36, 'USUAL', 7069392.03),
		('RBP 1', '2030-07-31', 48, 'USUAL', 63618204.62),
		('RBP 2', '2030-07-31', 24, 'USUAL', 2761913.23),
		('RBP 2', '2030-07-31', 36, 'USUAL', 2129819.69),
		('RBP 2', '2030-07-31', 48, 'USUAL', 26041985.68),
		('RBP 3', '2030-07-31', 24, 'USUAL', 4338551.22),
		('RBP 3', '2030-07-31', 36, 'USUAL', 3300863.78),
		('RBP 3', '2030-07-31', 48, 'USUAL', 23519094.54),
		('RBP 4', '2030-07-31', 24, 'USUAL', 2099166.17),
		('RBP 4', '2030-07-31', 36, 'USUAL', 1619770.79),
		('RBP 4', '2030-07-31', 48, 'USUAL', 14193051.10),




		('other', '2030-08-31', 24, 'USUAL', 2921142.22),
		('other', '2030-08-31', 36, 'USUAL', 3355255.61),
		('other', '2030-08-31', 48, 'USUAL', 51615586.87),
		('PTS31_RBP4', '2030-08-31', 24, 'USUAL', 10956658.84),
		('PTS31_RBP4', '2030-08-31', 36, 'USUAL', 8792092.78),
		('PTS31_RBP4', '2030-08-31', 48, 'USUAL', 45461990.73),
		('RBP 1', '2030-08-31', 24, 'USUAL', 8630350.11),
		('RBP 1', '2030-08-31', 36, 'USUAL', 7069392.03),
		('RBP 1', '2030-08-31', 48, 'USUAL', 63618204.62),
		('RBP 2', '2030-08-31', 24, 'USUAL', 2761913.23),
		('RBP 2', '2030-08-31', 36, 'USUAL', 2129819.69),
		('RBP 2', '2030-08-31', 48, 'USUAL', 26041985.68),
		('RBP 3', '2030-08-31', 24, 'USUAL', 4338551.22),
		('RBP 3', '2030-08-31', 36, 'USUAL', 3300863.78),
		('RBP 3', '2030-08-31', 48, 'USUAL', 23519094.54),
		('RBP 4', '2030-08-31', 24, 'USUAL', 2099166.17),
		('RBP 4', '2030-08-31', 36, 'USUAL', 1619770.79),
		('RBP 4', '2030-08-31', 48, 'USUAL', 14193051.10),




		('other', '2030-09-30', 24, 'USUAL', 2921142.22),
		('other', '2030-09-30', 36, 'USUAL', 3355255.61),
		('other', '2030-09-30', 48, 'USUAL', 51615586.87),
		('PTS31_RBP4', '2030-09-30', 24, 'USUAL', 10956658.84),
		('PTS31_RBP4', '2030-09-30', 36, 'USUAL', 8792092.78),
		('PTS31_RBP4', '2030-09-30', 48, 'USUAL', 45461990.73),
		('RBP 1', '2030-09-30', 24, 'USUAL', 8630350.11),
		('RBP 1', '2030-09-30', 36, 'USUAL', 7069392.03),
		('RBP 1', '2030-09-30', 48, 'USUAL', 63618204.62),
		('RBP 2', '2030-09-30', 24, 'USUAL', 2761913.23),
		('RBP 2', '2030-09-30', 36, 'USUAL', 2129819.69),
		('RBP 2', '2030-09-30', 48, 'USUAL', 26041985.68),
		('RBP 3', '2030-09-30', 24, 'USUAL', 4338551.22),
		('RBP 3', '2030-09-30', 36, 'USUAL', 3300863.78),
		('RBP 3', '2030-09-30', 48, 'USUAL', 23519094.54),
		('RBP 4', '2030-09-30', 24, 'USUAL', 2099166.17),
		('RBP 4', '2030-09-30', 36, 'USUAL', 1619770.79),
		('RBP 4', '2030-09-30', 48, 'USUAL', 14193051.10),




		('other', '2030-10-31', 24, 'USUAL', 2921142.22),
		('other', '2030-10-31', 36, 'USUAL', 3355255.61),
		('other', '2030-10-31', 48, 'USUAL', 51615586.87),
		('PTS31_RBP4', '2030-10-31', 24, 'USUAL', 10956658.84),
		('PTS31_RBP4', '2030-10-31', 36, 'USUAL', 8792092.78),
		('PTS31_RBP4', '2030-10-31', 48, 'USUAL', 45461990.73),
		('RBP 1', '2030-10-31', 24, 'USUAL', 8630350.11),
		('RBP 1', '2030-10-31', 36, 'USUAL', 7069392.03),
		('RBP 1', '2030-10-31', 48, 'USUAL', 63618204.62),
		('RBP 2', '2030-10-31', 24, 'USUAL', 2761913.23),
		('RBP 2', '2030-10-31', 36, 'USUAL', 2129819.69),
		('RBP 2', '2030-10-31', 48, 'USUAL', 26041985.68),
		('RBP 3', '2030-10-31', 24, 'USUAL', 4338551.22),
		('RBP 3', '2030-10-31', 36, 'USUAL', 3300863.78),
		('RBP 3', '2030-10-31', 48, 'USUAL', 23519094.54),
		('RBP 4', '2030-10-31', 24, 'USUAL', 2099166.17),
		('RBP 4', '2030-10-31', 36, 'USUAL', 1619770.79),
		('RBP 4', '2030-10-31', 48, 'USUAL', 14193051.10),




		('other', '2030-11-30', 24, 'USUAL', 2921142.22),
		('other', '2030-11-30', 36, 'USUAL', 3355255.61),
		('other', '2030-11-30', 48, 'USUAL', 51615586.87),
		('PTS31_RBP4', '2030-11-30', 24, 'USUAL', 10956658.84),
		('PTS31_RBP4', '2030-11-30', 36, 'USUAL', 8792092.78),
		('PTS31_RBP4', '2030-11-30', 48, 'USUAL', 45461990.73),
		('RBP 1', '2030-11-30', 24, 'USUAL', 8630350.11),
		('RBP 1', '2030-11-30', 36, 'USUAL', 7069392.03),
		('RBP 1', '2030-11-30', 48, 'USUAL', 63618204.62),
		('RBP 2', '2030-11-30', 24, 'USUAL', 2761913.23),
		('RBP 2', '2030-11-30', 36, 'USUAL', 2129819.69),
		('RBP 2', '2030-11-30', 48, 'USUAL', 26041985.68),
		('RBP 3', '2030-11-30', 24, 'USUAL', 4338551.22),
		('RBP 3', '2030-11-30', 36, 'USUAL', 3300863.78),
		('RBP 3', '2030-11-30', 48, 'USUAL', 23519094.54),
		('RBP 4', '2030-11-30', 24, 'USUAL', 2099166.17),
		('RBP 4', '2030-11-30', 36, 'USUAL', 1619770.79),
		('RBP 4', '2030-11-30', 48, 'USUAL', 14193051.10),




		('other', '2030-12-31', 24, 'USUAL', 2921142.22),
		('other', '2030-12-31', 36, 'USUAL', 3355255.61),
		('other', '2030-12-31', 48, 'USUAL', 51615586.87),
		('PTS31_RBP4', '2030-12-31', 24, 'USUAL', 10956658.84),
		('PTS31_RBP4', '2030-12-31', 36, 'USUAL', 8792092.78),
		('PTS31_RBP4', '2030-12-31', 48, 'USUAL', 45461990.73),
		('RBP 1', '2030-12-31', 24, 'USUAL', 8630350.11),
		('RBP 1', '2030-12-31', 36, 'USUAL', 7069392.03),
		('RBP 1', '2030-12-31', 48, 'USUAL', 63618204.62),
		('RBP 2', '2030-12-31', 24, 'USUAL', 2761913.23),
		('RBP 2', '2030-12-31', 36, 'USUAL', 2129819.69),
		('RBP 2', '2030-12-31', 48, 'USUAL', 26041985.68),
		('RBP 3', '2030-12-31', 24, 'USUAL', 4338551.22),
		('RBP 3', '2030-12-31', 36, 'USUAL', 3300863.78),
		('RBP 3', '2030-12-31', 48, 'USUAL', 23519094.54),
		('RBP 4', '2030-12-31', 24, 'USUAL', 2099166.17),
		('RBP 4', '2030-12-31', 36, 'USUAL', 1619770.79),
		('RBP 4', '2030-12-31', 48, 'USUAL', 14193051.10)
	...
	delete from Temp_PTS_CM_virt_gens where generation <= @rdt;
	--delete from Temp_PTS_CM_virt_gens where flag_kk in ('Installment')
	--delete from Temp_PTS_CM_virt_gens where flag_kk = 'BUSINESS' and generation > '2021-12-31';
	delete from Temp_PTS_CM_virt_gens where generation > '2030-12-31';
	delete from Temp_PTS_CM_virt_gens where generation > eomonth(@dt_back_test_from,@horizon);
	delete from Temp_PTS_CM_virt_gens where vl_rub_fact <= 0;
	delete from Temp_PTS_CM_virt_gens where flag_kk = 'INSTALLMENT' and generation >= '2022-04-30';
	delete from Temp_PTS_CM_virt_gens where flag_kk = 'BUSINESS' and generation >= '2022-08-31';

	--delete from Temp_PTS_CM_virt_gens where generation > '2022-12-31';
	...
	--drop table if exists Temp_PTS_CM_fact_volumes;
	--select 
	--case 
	...
	--insert into Temp_PTS_CM_virt_gens 
	--select a.segment_rbp, a.generation, a.term, a.flag_kk, a.volume 
	--from Temp_PTS_CM_fact_volumes a
	...
	print ('timing, Temp_PTS_CM_virt_gens ' + format(getdate(), 'HH:mm:ss'))
	...
	drop table if exists Temp_PTS_CM_nodpd_payoff_model;
	with base as (
		select 
		case 
	...
	into Temp_PTS_CM_nodpd_payoff_model
	from base b
	left join mm
	on mm.mob <= b.term
	;
	...
	-- Корректировки берём отсюда '[2. Распределяем выдачи по Сегменту и Сроку для SQL PTS_CM.xlsx]Ставки'!$V$25:$V$33
	/*
	select term, sum(cast(amount as float) * cast(eps as float))/sum(cast(amount as float)) as EIR
	from Temp_PTS_CM_cred_reestr
	where eomonth(generation, 0) >= eomonth('2025-10-01', 0)
	...
	*/
	...
	with base as (
		select cast(24 as int) as term, cast((@avg_rate - 7.2) as float) as avg_w_int_rate, 1.0 + cast((@avg_rate - 7.2) as float) / 100.0 / 12.0 as one_plus_r	union all
		...
		select cast(48 as int) as term, cast((@avg_rate - 8.1) as float) as avg_w_int_rate, 1.0 + cast((@avg_rate - 8.1) as float) / 100.0 / 12.0 as one_plus_r
	), mm as (
		select cast(1 as float) as mob
		union all
		select cast(mob + 1 as float)
	...
	insert into Temp_PTS_CM_nodpd_payoff_model
	...
	-- Корректировки берём отсюда '[2. Распределяем выдачи по Сегменту и Сроку для SQL PTS_CM.xlsx]Ставки'!$V$30:$V$32
	...
	*/
	with base as (
		...
		select cast(24 as int) as term, cast((@avg_rate + 23) as float) as avg_w_int_rate, 1.0 + cast((@avg_rate + 23) as float) / 100.0 / 12.0 as one_plus_r	union all
		select cast(36 as int) as term, cast((@avg_rate + 22) as float) as avg_w_int_rate, 1.0 + cast((@avg_rate + 22) as float) / 100.0 / 12.0 as one_plus_r	union all
		select cast(48 as int) as term, cast((@avg_rate + 18) as float) as avg_w_int_rate, 1.0 + cast((@avg_rate + 18) as float) / 100.0 / 12.0 as one_plus_r
	...
	print ('timing, Temp_PTS_CM_nodpd_payoff_model' + format(getdate(), 'HH:mm:ss'))
	...
	drop table if exists Temp_PTS_CM_det_stress;
	select 
	...
	print ('timing, Temp_PTS_CM_rest_model' + format(getdate(), 'HH:mm:ss'))



	------------------------------------------------------------------------------
	---коэффициенты для корректировки pay-down в зависимости от сегмента RBP и срока (term)
	drop table if exists Temp_PTS_CM_model_adjust;
	...
	--07/12/2021 installm
	insert into Temp_PTS_CM_model_adjust
	...
	from Temp_PTS_CM_model_adjust a
	...
	*/
	...
	print ('timing, Temp_PTS_CM_model_adjust' + format(getdate(), 'HH:mm:ss'))

	drop table if exists Temp_PTS_CM_repdates;
	...
	drop table if exists Temp_PTS_CM_stg_back_base;

	with base as (
		select distinct a.term, a.segment_rbp, a.generation, a.flag_kk from Temp_PTS_CM_stg3_agg a
	union
		select distinct b.term, b.segment_rbp, b.generation, b.flag_kk from Temp_PTS_CM_cred_reestr b 
		where b.generation >= @rdt
	union 
		select distinct v.term, v.segment_rbp, v.generation, v.flag_kk from Temp_PTS_CM_virt_gens v 
		...
		from base b
		left join Temp_PTS_CM_repdates d
	...
	select * 
	into Temp_PTS_CM_stg_back_base
	from (
		select * from Temp_PTS_CM_stg3_agg a
	union all
	...
	print ('timing, Temp_PTS_CM_stg_back_base' + format(getdate(), 'HH:mm:ss'))

	--версии кривых для разных продуктов/сегментов


	drop table if exists Temp_PTS_CM_det_curve_version;
	...
	into Temp_PTS_CM_det_curve_version
	from (values 
	...
	drop table if exists Temp_PTS_CM_for_back_test;
	...
	with base as (
		...
		from Temp_PTS_CM_stg_back_base a

		left join Temp_PTS_CM_nodpd_payoff_model n
		on a.term = n.term
		...
		left join Temp_PTS_CM_rest_model r
		on 1=1
		--and a.term = r.term
		...
		left join Temp_PTS_CM_model_adjust adj
		...
		left join Temp_PTS_CM_det_stress sts
		on a.mob_date_to between sts.dt_from and sts.dt_to

		left join Temp_PTS_CM_det_curve_version cv
		on case 
	...
	into Temp_PTS_CM_for_back_test
	from base b
	left join znam z
	on b.term = z.term
	...
	drop table if exists Temp_PTS_CM_season_rr_corr;
	with base as (
		select distinct 
			a.id1,
			--a.term, a.segment_rbp, a.flag_kk, a.generation, 
			a.mob_date_to, a.bucket_90_from
		from Temp_PTS_CM_for_back_test a
		where 1=1
	...
	into Temp_PTS_CM_season_rr_corr
	from Temp_PTS_CM_for_back_test a
	inner join base b
	on 1=1
	and a.id1 = b.id1
	...
	insert into Temp_PTS_CM_season_rr_corr
	...
	merge into Temp_PTS_CM_for_back_test a
	using Temp_PTS_CM_season_rr_corr b
	on (1=1
		--and a.term = b.term
		--and a.segment_rbp = b.segment_rbp
		--and a.flag_kk = b.flag_kk
	...
	update Temp_PTS_CM_for_back_test set model_coef_adj = 0 where bucket_90_from = '[07] Freeze' or bucket_90_to = '[07] Freeze';
	...
	drop table if exists Temp_PTS_CM_pmt_last_gen_0_MoB;
	select 
	...
	into Temp_PTS_CM_pmt_last_gen_0_MoB
	from Temp_PTS_CM_cred_reestr a
	--left join dwh_new.dbo.stat_v_balance2 b
	left join Temp_PTS_CM_stg_payment b
	on a.external_id = b.external_id
	...
	drop table if exists Temp_PTS_CM_fix_matrix;
	with volume as (
		select 
		hashbytes('MD2',concat(a.term, a.segment_rbp, a.generation, a.flag_kk)) as id1,
		a.term, a.segment_rbp, a.generation, a.flag_kk, 
	...
	select a.id1,
		...
		as corr
	into Temp_PTS_CM_fix_matrix
	from Temp_PTS_CM_for_back_test a
	left join Temp_PTS_CM_pmt_last_gen_0_MoB b
	on a.id1 = b.id1
	left join volume c
	on a.id1 = c.id1
	where a.generation = @rdt
	...
	insert into Temp_PTS_CM_fix_matrix
	...
	from Temp_PTS_CM_for_back_test a
	inner join Temp_PTS_CM_fix_matrix b
	on a.id1 = b.id1
	and a.bucket_90_from = b.bucket_90_from
	and a.bucket_90_to = b.bucket_90_to
	and a.mob_date_to = b.mob_date_to
	...
	drop table Temp_PTS_CM_pmt_last_gen_0_MoB;
	...
	select * from Temp_PTS_CM_for_back_test a
	...
	*/
	...
	from Temp_PTS_CM_season_rr_corr a
	...
	*/
	...
	print ('timing, Temp_PTS_CM_for_back_test' + format(getdate(), 'HH:mm:ss'))
	...
	drop table if exists Temp_PTS_CM_back_test_acc_matr;
	create table Temp_PTS_CM_back_test_acc_matr (
	id1 int,
	generation date,
	term int,
	...
	--цикл - шаг 0
	insert into Temp_PTS_CM_back_test_acc_matr
	select 
	a.id1,
	a.generation, a.term, a.segment_rbp, a.flag_kk, 
	...
	drop table if exists Temp_PTS_CM_temp1_back_test_acc_matr;
	select top 0 
	a.id1,
	a.generation, a.term, a.segment_rbp, a.flag_kk, 
	a.mob_to, a.bucket_90_from, a.bucket_90_to, a.fact_coef, a.model_coef_adj
	into Temp_PTS_CM_temp1_back_test_acc_matr
	from Temp_PTS_CM_back_test_acc_matr a;

	drop table if exists Temp_PTS_CM_temp2_back_test_acc_matr;
	select *
	into Temp_PTS_CM_temp2_back_test_acc_matr--select top 5 *
	from Temp_PTS_CM_back_test_acc_matr a;
	...
	print ('timing, Temp_PTS_CM_back_test_acc_matr iter 0 ' + format(getdate(), 'HH:mm:ss'));
	...
	begin
		...
		truncate table Temp_PTS_CM_temp1_back_test_acc_matr
		insert into Temp_PTS_CM_temp1_back_test_acc_matr
		select 
		a.id1,
		a.generation, a.term, a.segment_rbp, a.flag_kk, 
		a.mob_to, a.bucket_90_from, a.bucket_90_to, a.fact_coef, a.model_coef_adj
		from Temp_PTS_CM_temp2_back_test_acc_matr a
		--where a.mob_date_to = @k;


		--умножение на следующую матрицу
		truncate table Temp_PTS_CM_temp2_back_test_acc_matr;
		insert into Temp_PTS_CM_temp2_back_test_acc_matr
		select 
		a.id1,
		...
		from Temp_PTS_CM_temp1_back_test_acc_matr a
		left join Temp_PTS_CM_for_back_test b
		on 1 = 1
		and a.id1 = b.id1
		--and a.term = b.term
		--and a.generation = b.generation
		...
		insert into Temp_PTS_CM_back_test_acc_matr
		select * from Temp_PTS_CM_temp2_back_test_acc_matr;



		print ('timing, Temp_PTS_CM_back_test_acc_matr @k = ' + format(@k, 'dd.MM.yyyy') + ' ' + format(getdate(), 'HH:mm:ss'));

		set @k = EOMONTH(@k,1)

	end;

	drop table Temp_PTS_CM_temp1_back_test_acc_matr;
	drop table Temp_PTS_CM_temp2_back_test_acc_matr;



	print ('timing, Temp_PTS_CM_back_test_acc_matr' + format(getdate(), 'HH:mm:ss'))




	--16/06/2021 - new vers of Temp_PTS_CM_last_fact by mob_date_to



	drop table if exists Temp_PTS_CM_last_fact;
	...
	and a.mob_from = 0
	and a.bucket_90_from = '[01] 0'
	and a.generation > (select max(generation) from Temp_PTS_CM_last_fact)
	and a.generation <= EOMONTH(@rdt, -1)
	), buck as (
	select '[01] 0' as bucket union all
	select '[02] 1-30' as bucket union all
	...
	insert into Temp_PTS_CM_last_fact
	...
	print ('timing, Temp_PTS_CM_last_fact' + format(getdate(), 'HH:mm:ss'))



	--comparison (back-test fact<->model)
	drop table if exists Temp_PTS_CM_back_test;
	...
	into Temp_PTS_CM_back_test
	from Temp_PTS_CM_last_fact a
	left join Temp_PTS_CM_back_test_acc_matr b
	on a.id1 = b.id1
	--and a.generation = b.generation
	--and a.term = b.term
	--and a.segment_rbp = b.segment_rbp
	...
	print ('timing, Temp_PTS_CM_back_test' + format(getdate(), 'HH:mm:ss'))
	...
	drop table if exists Temp_PTS_CM_freeze_last_fact;
	select 
	hashbytes('MD2',concat(a.term, a.segment_rbp, a.generation, a.flag_kk)) as id1,
	...
	into Temp_PTS_CM_freeze_last_fact
	from Temp_PTS_CM_stg_matrix_detail a
	left join Temp_PTS_CM_zamorozka b
	on a.external_id = b.external_id
	and a.mob_date_to >= b.freeze_from
	left join Temp_PTS_CM_stg_start_default dd
	on a.external_id = dd.external_id
	and a.mob_date_to >= dd.month_default_from
	left join Temp_PTS_CM_stg_freeze_mod f
	...
	-- + payoff
	insert into Temp_PTS_CM_freeze_last_fact
	select 
	a.id1,
	...
	print ('timing, Temp_PTS_CM_freeze_last_fact ' + format(getdate(), 'HH:mm:ss'))




	drop table if exists Temp_PTS_CM_stg_freeze_model;

	create table Temp_PTS_CM_stg_freeze_model (
	id1 int,
	term int,
	segment_rbp varchar(100),
	flag_kk varchar(100),
	...
	insert into Temp_PTS_CM_stg_freeze_model
	select 
	a.id1,
	a.term, 
	a.segment_rbp,
	...
	from Temp_PTS_CM_freeze_last_fact a
	where left(a.flag_kk,2) = 'KK'
	...
	print ('timing, Temp_PTS_CM_stg_freeze_model cycle start' + format(getdate(), 'HH:mm:ss'));
	...
	drop table if exists Temp_PTS_CM_tmp1_freeze_model;
	select * 
	into Temp_PTS_CM_tmp1_freeze_model
	from Temp_PTS_CM_stg_freeze_model a
	...
	if @ii < '2021-10-31' begin 
		...
		* case a.bucket_90_to when '[03] 31-60' then 0.75 when '[04] 61-90' then 0.15 end 
		...
		--from Temp_PTS_CM_stg_freeze_model a
		from Temp_PTS_CM_tmp1_freeze_model a
		where left(a.flag_kk,2) = 'KK'
	...
	drop table if exists Temp_PTS_CM_tmp2_freeze_model;
	...
	into Temp_PTS_CM_tmp2_freeze_model
	from Temp_PTS_CM_stg_freeze_model a
	where left(a.flag_kk,2) = 'KK'
	...
	* case when b.bucket_90_from = '[03] 31-60' and a.freeze_from is null and a.mob_date_to < '2021-10-31' then 0.25
	...
	from Temp_PTS_CM_tmp2_freeze_model a
	inner join Temp_PTS_CM_for_back_test b
	on a.id1 = b.id1
	--and a.term = b.term
	--and a.generation = b.generation
	--and a.segment_rbp = b.segment_rbp
	...
	print ('timing, Temp_PTS_CM_stg_freeze_model cycle @ii= ' + format(@ii,'dd.MM.yyyy')+ ' , ' + format(getdate(), 'HH:mm:ss'))
	...
	drop table Temp_PTS_CM_tmp1_freeze_model;
	drop table Temp_PTS_CM_tmp2_freeze_model;

	print ('timing, Temp_PTS_CM_stg_freeze_model ' + format(getdate(), 'HH:mm:ss'))
	...
	drop table if exists Temp_PTS_CM_stg2_freeze_model;
	...
	select m.term, m.segment_rbp, m.flag_kk, m.generation,
		...
		f.fact_od,
		m.model_od
	into Temp_PTS_CM_stg2_freeze_model
	from M
	left join F
	on m.term = f.term
	...
	print ('timing, Temp_PTS_CM_stg2_freeze_model ' + format(getdate(), 'HH:mm:ss'));
	...
	drop table if exists Temp_PTS_CM_stg1_back_test_results;
	...
	with new_volume as (
		...
		and exists (select 1 from Temp_PTS_CM_back_test t
	 ...
	 union all --кредитные каникулы (заморозка)
		select * from Temp_PTS_CM_stg2_freeze_model a	
	)
	...
	into Temp_PTS_CM_stg1_back_test_results
	from u
	;
	...
	insert into Temp_PTS_CM_stg1_back_test_results
	...
	print ('timing, Temp_PTS_CM_stg1_back_test_results' + format(getdate(), 'HH:mm:ss'))

	--добавляем сумму выдачи для расчета GL90
	drop table if exists Temp_PTS_CM_new_volume;

	select * 
	into Temp_PTS_CM_new_volume
	from (
	...
	print ('timing, Temp_PTS_CM_new_volume' + format(getdate(), 'HH:mm:ss'))


	drop table if exists Temp_PTS_CM_stg2_back_test_results;
	...
	into Temp_PTS_CM_stg2_back_test_results

	from Temp_PTS_CM_stg1_back_test_results a
	left join Temp_PTS_CM_new_volume b
	on a.term = b.term
	and a.generation = b.generation
	and a.segment_rbp = b.segment_rbp
	and a.flag_kk = b.flag_kk
	...
	print ('timing, Temp_PTS_CM_stg2_back_test_results' + format(getdate(), 'HH:mm:ss'))
	...
	drop table if exists Temp_PTS_CM_conditional_rec_ALL;
	with base as (
		select a.mod_num, 
		--recovery rates
		a.recov_w, 
	...
	a.* 
	into Temp_PTS_CM_conditional_rec_ALL 
	from un as a
	;


	update Temp_PTS_CM_conditional_rec_ALL set recov_int_rate = recov_int_rate * 1.4;
	update Temp_PTS_CM_conditional_rec_ALL set fee_rec_rate = fee_rec_rate * 1.7;
	update Temp_PTS_CM_conditional_rec_ALL set straf_recovery_rate = straf_recovery_rate * 2.4;


	insert into Temp_PTS_CM_conditional_rec_ALL
	select 
	...
	a.wo_rate,
	a.wo_int_rate
	from Temp_PTS_CM_conditional_rec_ALL a
	;
	...
	print ('timing, Temp_PTS_CM_conditional-s' + format(getdate(), 'HH:mm:ss'))


	--промежуточный расчет накопленных Recovery (для фактических дефолтов - до прогнозной даты)

	drop table if exists Temp_PTS_CM_stg0_default_recovery_fact;
	...
	into Temp_PTS_CM_stg0_default_recovery_fact
	from Temp_PTS_CM_stg_matrix_detail a
	inner join Temp_PTS_CM_stg_start_default b
	on a.external_id = b.external_id
	where 1=1
	and a.mob_date_to = eomonth(@dt_back_test_from,-1)
	and a.bucket_90_to <> '[06] Pay-off'
	...
	drop table if exists Temp_PTS_CM_det_recovery_corr;
	...
	--into Temp_PTS_CM_det_recovery_corr
	--from (values
	...
	print ('timing, Temp_PTS_CM_stg0_default_recovery_fact' + format(getdate(), 'HH:mm:ss'))
	...
	drop table if exists Temp_PTS_CM_stg1_default_recovery_fact;
	create table Temp_PTS_CM_stg1_default_recovery_fact (
		mob_date_to date,
		term int,
		generation date,
		segment_rbp varchar(100),
	...
	print ('timing, Temp_PTS_CM_stg1_default_recovery_fact iter0' + format(getdate(), 'HH:mm:ss'))
	...
	while @cycle_dt <= EOMONTH(@dt_back_test_from, /*12*6*/ @horizon) begin
		...
		left join Temp_PTS_CM_conditional_rec_ALL c
		...
		left join Temp_PTS_CM_det_recovery_corr d
		...
		print ('timing, Temp_PTS_CM_stg1_default_recovery_fact @cycle_dt = ' + format(@cycle_dt, 'dd.MM.yyyy') + ' ' + format(getdate(), 'HH:mm:ss'));
	...
	delete from Temp_PTS_CM_stg1_default_recovery_fact where flag_kk like 'CESS%' and mob_date_to >= cast(substring(flag_kk,6,10) as date);
	...
	from Temp_PTS_CM_stg1_default_recovery_fact a
	where a.flag_kk = 'BANKRUPT';
	*/



	print ('timing, Temp_PTS_CM_stg1_default_recovery_fact' + format(getdate(), 'HH:mm:ss'))



	--накопленные Recovery (для фактических дефолтов - до прогнозной даты)
	drop table if exists Temp_PTS_CM_default_recovery_fact
	...
	into Temp_PTS_CM_default_recovery_fact
	from Temp_PTS_CM_stg1_default_recovery_fact a
	...
	print ('timing, Temp_PTS_CM_default_recovery_fact' + format(getdate(), 'HH:mm:ss'))
	...
	drop table if exists Temp_PTS_CM_stg1_default_recovery_model;
	select 
	--a.*, b.*
	a.id1,
	...
	into Temp_PTS_CM_stg1_default_recovery_model
	from Temp_PTS_CM_last_fact a
	left join Temp_PTS_CM_for_back_test b
	on a.id1 = b.id1
	--and a.generation = b.generation
	...
	insert into Temp_PTS_CM_stg1_default_recovery_model
	select 
	hashbytes('MD2',concat(a.term, a.segment_rbp, a.generation, a.flag_kk)) as id1,
	...
	drop table if exists Temp_PTS_CM_tmp1_default_recovery_model;
	select top 0 
	a.id1,
	a.term, a.segment_rbp, a.flag_kk, a.generation,
	a.mob_to, a.bucket_90_from, a.bucket_90_to, a.mod_num, a.last_MOD, a.model_od
	into Temp_PTS_CM_tmp1_default_recovery_model
	from Temp_PTS_CM_stg1_default_recovery_model a;

	drop table if exists Temp_PTS_CM_tmp2_default_recovery_model;
	select *
	into Temp_PTS_CM_tmp2_default_recovery_model
	from Temp_PTS_CM_stg1_default_recovery_model;





	print ('timing, Temp_PTS_CM_stg1_default_recovery_model' + format(getdate(), 'HH:mm:ss'))
	...
	begin
		...
		truncate table Temp_PTS_CM_tmp1_default_recovery_model;
		insert into Temp_PTS_CM_tmp1_default_recovery_model
		select 
		a.id1,
		a.term, a.segment_rbp, a.flag_kk, a.generation,
		a.mob_to, a.bucket_90_from, a.bucket_90_to, a.mod_num, a.last_MOD, sum(a.model_od) as model_od
		from Temp_PTS_CM_tmp2_default_recovery_model a
		where 1=1
		and a.bucket_90_to <> '[06] Pay-off'
		--and a.model_od > 1
		...
		truncate table Temp_PTS_CM_tmp2_default_recovery_model;
		insert into Temp_PTS_CM_tmp2_default_recovery_model	
		...
		from Temp_PTS_CM_tmp1_default_recovery_model a
		inner join Temp_PTS_CM_for_back_test b
		on a.id1 = b.id1
		--and a.term = b.term
		--and a.segment_rbp = b.segment_rbp
		...
		insert into Temp_PTS_CM_stg1_default_recovery_model
		select * from Temp_PTS_CM_tmp2_default_recovery_model
		;


		print ('timing, Temp_PTS_CM_stg1_default_recovery_model cycle @d = ' + format(@d,'dd.MM.yyyy') + ' , ' + format(getdate(), 'HH:mm:ss'));
	...
	drop table Temp_PTS_CM_tmp1_default_recovery_model;
	drop table Temp_PTS_CM_tmp2_default_recovery_model;



	print ('timing, Temp_PTS_CM_stg1_default_recovery_model cycle' + format(getdate(), 'HH:mm:ss'))




	drop table if exists Temp_PTS_CM_stg2_default_recovery_model;
	...
	sum(a.model_od) as model_od
	into Temp_PTS_CM_stg2_default_recovery_model
	from Temp_PTS_CM_stg1_default_recovery_model a
	...
	insert into Temp_PTS_CM_stg2_default_recovery_model
	...
	-- not exists (select 1 from Temp_PTS_CM_ni_kk b
	...
	--Цессия - обнуляем с момента продажи
	delete from Temp_PTS_CM_stg2_default_recovery_model where flag_kk like 'CESS%' and mob_date_to >= cast(substring(flag_kk,6,10) as date);

	delete from Temp_PTS_CM_stg2_default_recovery_model where isnull(mod_num,-999) < 0;

	print ('timing, Temp_PTS_CM_stg2_default_recovery_model' + format(getdate(), 'HH:mm:ss'))
	...
	drop table if exists Temp_PTS_CM_stg3_default_recovery_model;
	create table Temp_PTS_CM_stg3_default_recovery_model (
		term int,
		segment_rbp varchar(100),
		flag_kk varchar(100),
		generation date,
	...
	while @cycle_dt5 <= eomonth(@dt_back_test_from, @horizon) begin
		...
		insert into Temp_PTS_CM_stg3_default_recovery_model
		select a.term, a.segment_rbp, a.flag_kk, a.generation, a.mob_date_to, a.mob_to, a.mod_num, a.last_MOD, a.freeze_from,
		a.model_od,
		...
		from Temp_PTS_CM_stg2_default_recovery_model a
		left join Temp_PTS_CM_conditional_rec_ALL b
		...
		print ('timing, Temp_PTS_CM_stg3_default_recovery_model @k = ' + format(@cycle_dt5, 'dd.MM.yyyy') + ' ' + format(getdate(), 'HH:mm:ss'));
	...
	print ('timing, Temp_PTS_CM_stg3_default_recovery_model' + format(getdate(), 'HH:mm:ss'))



	--Цессия - обнуляем с момента продажи
	delete from Temp_PTS_CM_stg3_default_recovery_model where flag_kk like 'CESS%' and mob_date_to >= cast(substring(flag_kk,6,10) as date);
	...
	from Temp_PTS_CM_stg3_default_recovery_model a
	where a.flag_kk = 'BANKRUPT';
	*/
	...
	drop table if exists Temp_PTS_CM_default_recovery_model;
	...
	into Temp_PTS_CM_default_recovery_model
	from Temp_PTS_CM_stg3_default_recovery_model b
	...
	print ('timing, Temp_PTS_CM_default_recovery_model' + format(getdate(), 'HH:mm:ss'))


	/******************************************************************************************/
	--корректировка Pay-off и 90+ и добавление списаний и рекавери 90+

	drop table if exists Temp_PTS_CM_stg3_back_test_results;
	...
	into Temp_PTS_CM_stg3_back_test_results

	from Temp_PTS_CM_stg2_back_test_results a

	left join Temp_PTS_CM_default_recovery_fact b
	on a.term = b.term
	and a.generation = b.generation
	and a.segment_rbp = b.segment_rbp
	and a.mob_date_to = b.mob_date_to
	...
	left join Temp_PTS_CM_default_recovery_model c
	on a.term = c.term
	and a.generation = c.generation
	and a.segment_rbp = c.segment_rbp
	...
	print ('timing, Temp_PTS_CM_stg3_back_test_results' + format(getdate(), 'HH:mm:ss'))


	--фактические списания (до прогнозной матрицы)
	drop table if exists Temp_PTS_CM_write_offs_fact;

	select a.term, a.generation, a.segment_rbp, a.flag_kk,
	eomonth(b.r_date) as mob_date_to,
	sum(isnull(b.od_wo,0)) as od_wo
	into Temp_PTS_CM_write_offs_fact
	from Temp_PTS_CM_cred_reestr a
	inner join Temp_PTS_CM_stg_write_off b
	on a.external_id = b.external_id
	where 1=1
	...
	into Temp_PTS_CM_npl_recovery_fact
	from Temp_PTS_CM_stg3_agg a
	left join w
	on a.term = w.term
	...
	drop table if exists Temp_PTS_CM_npl_wo_and_recovery;
	with base as (
		select a.term, a.generation, a.segment_rbp, a.flag_kk, a.mob_date_to,
		...
		from Temp_PTS_CM_default_recovery_fact a
	union all
		select a.term, a.generation, a.segment_rbp, a.flag_kk, a.mob_date_to,
		a.wo_calc_od_payoff - lag(a.wo_calc_od_payoff,1,0) over (partition by a.term, a.generation, a.segment_rbp, a.flag_kk order by a.mob_date_to) as od_wo,
		a.calc_od_payoff - lag(a.calc_od_payoff,1,0) over (partition by a.term, a.generation, a.segment_rbp, a.flag_kk order by a.mob_date_to) as npl_recovery
		from Temp_PTS_CM_default_recovery_model a
	union all
		...
		from Temp_PTS_CM_stg3_back_test_results a
		left join Temp_PTS_CM_write_offs_fact b
		...
		left join Temp_PTS_CM_npl_recovery_fact c
		...
		--30/08/2021 - 90+ не подтянулся на дату @dt_back_test_from по ветке моделирования факта (Temp_PTS_CM_default_recovery_fact)
		...
		from Temp_PTS_CM_stg3_back_test_results a
		left join Temp_PTS_CM_repdates c
		on c.mob_date_to >= @dt_back_test_from
		where not exists (select 1 from Temp_PTS_CM_default_recovery_fact b
							where a.generation = b.generation
							and a.segment_rbp = b.segment_rbp
							and a.term = b.term
							and a.flag_kk = b.flag_kk
	...
	into Temp_PTS_CM_npl_wo_and_recovery
	from total a
	;



	insert into Temp_PTS_CM_npl_wo_and_recovery
	select a.term, a.generation, a.segment_rbp, a.flag_kk, c.mob_date_to, a.od_wo, a.npl_recovery
	from Temp_PTS_CM_npl_wo_and_recovery a
	inner join (
	...
	left join (
		select a.mob_date_to from Temp_PTS_CM_repdates a
		union 
	...
	print ('timing, Temp_PTS_CM_npl_wo_and_recovery' + format(getdate(), 'HH:mm:ss'))


	--добавляем списания и recovery, дельты NL, GL в общую витрину
	drop table if exists Temp_PTS_CM_stg4_back_test_results;
	...
	with base as (
		...
		from Temp_PTS_CM_stg3_back_test_results a
		left join Temp_PTS_CM_npl_wo_and_recovery b
		on a.term = b.term
		and a.generation = b.generation
		and a.segment_rbp = b.segment_rbp
		and a.flag_kk = b.flag_kk
	...
	into Temp_PTS_CM_stg4_back_test_results
	...
	union all -- + бизнес-займы
		...
		from Temp_PTS_CM_cred_reestr a
		--left join Temp_PTS_CM_bus_stg_bal b
	...
	from Temp_PTS_CM_stg4_back_test_results a
	left join base b
	on a.term = b.term
	and a.generation = b.generation
	and a.segment_rbp = b.segment_rbp
	...
	left join Temp_PTS_CM_det_pdp_coef p
	on a.segment_rbp = p.segment_rbp

	where a.mob_to = 0
	;

	print ('timing, Temp_PTS_CM_stg4_back_test_results' + format(getdate(), 'HH:mm:ss'))
	...
	drop table if exists Temp_PTS_CM_cessed_od;
	...
	from Temp_PTS_CM_stg4_back_test_results a
	inner join Temp_PTS_CM_cessed_od b
	on a.term = b.term
	and a.segment_rbp = b.segment_rbp 
	and a.generation = b.generation
	and a.flag_kk = b.flag_kk
	...
	print ('timing, Temp_PTS_CM_cessed_od' + format(getdate(), 'HH:mm:ss'))
	...
	/*
	select a.bucket_90_from, a.bucket_90_to, format(sum(a.model_od),'Temp_PTS_CM_Temp_PTS_CM_Temp_PTS_CM_ Temp_PTS_CM_Temp_PTS_CM_Temp_PTS_CM_ Temp_PTS_CM_Temp_PTS_CM_Temp_PTS_CM_')
	from Temp_PTS_CM_stg_freeze_model a
	...
	*/
	...
	select * from Temp_PTS_CM_stg4_back_test_results a
	...
	*/
	...
	select a.mob_date_to, format(sum(a.model_od),'Temp_PTS_CM_ Temp_PTS_CM_Temp_PTS_CM_Temp_PTS_CM_ Temp_PTS_CM_Temp_PTS_CM_Temp_PTS_CM_ Temp_PTS_CM_Temp_PTS_CM_Temp_PTS_CM_')
	from Temp_PTS_CM_stg4_back_test_results a
	where a.bucket_90_to = '[05] 90+'
	...
	select aa.mob_date_to, format(sum(aa.model_od),'Temp_PTS_CM_ Temp_PTS_CM_Temp_PTS_CM_Temp_PTS_CM_ Temp_PTS_CM_Temp_PTS_CM_Temp_PTS_CM_ Temp_PTS_CM_Temp_PTS_CM_Temp_PTS_CM_')
	from (
	...
	*/
	...
	--drop table Temp_PTS_CM_zamorozka;
	/* P 3 */
	drop table Temp_PTS_CM_back_test;
	drop table Temp_PTS_CM_back_test_acc_matr;
	drop table Temp_PTS_CM_cessed_od;
	--drop table Temp_PTS_CM_conditional_rec_ALL;
	--drop table Temp_PTS_CM_default_recovery_fact;
	--drop table Temp_PTS_CM_default_recovery_model;
	drop table Temp_PTS_CM_det_curve_version;
	--drop table Temp_PTS_CM_det_pdp_coef;
	--drop table Temp_PTS_CM_det_recovery_corr;
	--drop table Temp_PTS_CM_det_stress;
	--drop table Temp_PTS_CM_fix_matrix;
	--drop table Temp_PTS_CM_for_back_test;
	--drop table Temp_PTS_CM_freeze_last_fact;
	--drop table Temp_PTS_CM_last_fact;
	drop table Temp_PTS_CM_model_adjust;
	--drop table Temp_PTS_CM_new_volume;
	drop table Temp_PTS_CM_nodpd_payoff_model;
	drop table Temp_PTS_CM_npl_recovery_fact;
	--drop table Temp_PTS_CM_npl_wo_and_recovery;
	drop table Temp_PTS_CM_repdates;
	drop table Temp_PTS_CM_season_rr_corr;
	drop table Temp_PTS_CM_stg_back_base;
	--drop table Temp_PTS_CM_stg_freeze_model;
	drop table Temp_PTS_CM_stg0_default_recovery_fact;
	drop table Temp_PTS_CM_stg1_back_test_results;
	--drop table Temp_PTS_CM_stg1_default_recovery_fact;
	drop table Temp_PTS_CM_stg1_default_recovery_model;
	drop table Temp_PTS_CM_stg2_back_test_results;
	drop table Temp_PTS_CM_stg2_default_recovery_model;
	drop table Temp_PTS_CM_stg2_freeze_model;
	drop table Temp_PTS_CM_stg3_back_test_results;
	--drop table Temp_PTS_CM_stg3_default_recovery_model;
	--drop table Temp_PTS_CM_stg4_back_test_results;
	--drop table Temp_PTS_CM_virt_gens;
	drop table Temp_PTS_CM_write_offs_fact;
	...
	drop table if exists Temp_PTS_CM_stg1_interest;
	select a.term, a.segment_rbp, a.flag_kk, a.generation, a.mob_date_to, a.mob_to
	...
	into Temp_PTS_CM_stg1_interest--select top 100 *
	from Temp_PTS_CM_for_back_test a
	where 1=1
	...
	print ('timing, Temp_PTS_CM_stg1_interest ' + format(getdate(), 'HH:mm:ss'))
	...
	drop table if exists Temp_PTS_CM_stg2_interest;
	...
	into Temp_PTS_CM_stg2_interest

	from Temp_PTS_CM_stg1_interest a
	left join Temp_PTS_CM_det_stress sts --28.02.2022
	on a.mob_date_to between sts.dt_from and sts.dt_to
	...
	drop table if exists Temp_PTS_CM_upd_stg2_interest;
	with base as (
		select hashbytes('MD2',concat(a.term, a.segment_rbp, a.generation, a.flag_kk)) as id1,
		a.* 
		from Temp_PTS_CM_stg2_interest a
	...
	into Temp_PTS_CM_upd_stg2_interest
	from base a
	left join base2 b
	on a.id1 = b.id1
	and a.bucket_90_from = b.bucket_90_from
	...
	from Temp_PTS_CM_stg2_interest a
	inner join Temp_PTS_CM_upd_stg2_interest b
	...
	print ('timing, Temp_PTS_CM_stg2_interest ' + format(getdate(), 'HH:mm:ss'))
	...
	drop table if exists Temp_PTS_CM_stg_int_rates;
	with rates as (
		select a.term, a.segment_rbp, a.flag_kk, a.generation, 
		...
		select distinct a.term, a.segment_rbp, a.flag_kk, a.generation from Temp_PTS_CM_stg4_back_test_results a
	), base as (
	...
	into Temp_PTS_CM_stg_int_rates
	from base a
	where a.rown = 1
	;
	...
	/*
	Выгрузку вставить в '[2. Распределяем выдачи по Сегменту и Сроку для SQL PTS_CM.xlsx]Распределение по выдачам'!$M$8
	Корректировку забрать из колонки P в блоке с первым прогнозом

	select segment_rbp, flag_kk
		, count(*) as cnt, round(sum(amount * int_rate)/sum(amount), 2) as avg_w_int_rate--select top 100 *
	from Temp_PTS_CM_cred_reestr where year(generation) = 2025 and flag_kk = 'USUAL'
	...
	*/
	...
	update Temp_PTS_CM_stg_int_rates set avg_month_int_rate = (select round(sum(amount * int_rate)/sum(amount), 0) - 2.12 from Temp_PTS_CM_cred_reestr where year(generation) = 2025 and flag_kk = 'USUAL' and segment_rbp = 'other' group by segment_rbp, flag_kk) / 100.0 / 12.0	where segment_rbp = 'other'		and flag_kk = 'USUAL' and eomonth(generation, 0) >= @dt_back_test_from;
	update Temp_PTS_CM_stg_int_rates set avg_month_int_rate = (select round(sum(amount * int_rate)/sum(amount), 0) - 2.12 from Temp_PTS_CM_cred_reestr where year(generation) = 2025 and flag_kk = 'USUAL' and segment_rbp = 'PTS31_RBP4' group by segment_rbp, flag_kk) / 100.0 / 12.0	where segment_rbp = 'PTS31_RBP4'	and flag_kk = 'USUAL' and eomonth(generation, 0) >= @dt_back_test_from;
	update Temp_PTS_CM_stg_int_rates set avg_month_int_rate = (select round(sum(amount * int_rate)/sum(amount), 0) - 2.12 from Temp_PTS_CM_cred_reestr where year(generation) = 2025 and flag_kk = 'USUAL' and segment_rbp = 'RBP 1' group by segment_rbp, flag_kk) / 100.0 / 12.0	where segment_rbp = 'RBP 1'				and flag_kk = 'USUAL' and eomonth(generation, 0) >= @dt_back_test_from;
	update Temp_PTS_CM_stg_int_rates set avg_month_int_rate = (select round(sum(amount * int_rate)/sum(amount), 0) - 2.12 from Temp_PTS_CM_cred_reestr where year(generation) = 2025 and flag_kk = 'USUAL' and segment_rbp = 'RBP 2' group by segment_rbp, flag_kk) / 100.0 / 12.0	where segment_rbp = 'RBP 2'				and flag_kk = 'USUAL' and eomonth(generation, 0) >= @dt_back_test_from;
	update Temp_PTS_CM_stg_int_rates set avg_month_int_rate = (select round(sum(amount * int_rate)/sum(amount), 0) - 2.12 from Temp_PTS_CM_cred_reestr where year(generation) = 2025 and flag_kk = 'USUAL' and segment_rbp = 'RBP 3' group by segment_rbp, flag_kk) / 100.0 / 12.0	where segment_rbp = 'RBP 3'				and flag_kk = 'USUAL' and eomonth(generation, 0) >= @dt_back_test_from;
	update Temp_PTS_CM_stg_int_rates set avg_month_int_rate = (select round(sum(amount * int_rate)/sum(amount), 0) - 2.12 from Temp_PTS_CM_cred_reestr where year(generation) = 2025 and flag_kk = 'USUAL' and segment_rbp = 'RBP 4' group by segment_rbp, flag_kk) / 100.0 / 12.0	where segment_rbp = 'RBP 4'				and flag_kk = 'USUAL' and eomonth(generation, 0) >= @dt_back_test_from;

	--;


	print ('timing, Temp_PTS_CM_stg_int_rates ' + format(getdate(), 'HH:mm:ss'))
	...
	drop table if exists Temp_PTS_CM_stg5_interest;
	...
	into Temp_PTS_CM_stg5_interest--select *
	from Temp_PTS_CM_stg2_interest a
	left join Temp_PTS_CM_stg_int_rates c
	on a.term = c.term
	and a.segment_rbp = c.segment_rbp
	and a.flag_kk = c.flag_kk
	and a.generation = c.generation
	...
	from Temp_PTS_CM_stg5_interest a
	where a.flag_kk in ('BANKRUPT');


	print ('timing, Temp_PTS_CM_stg5_interest ' + format(getdate(), 'HH:mm:ss'))
	...
	----бакеты 0-90
	drop table if exists Temp_PTS_CM_stg_interest_model;
	...
	into Temp_PTS_CM_stg_interest_model

	from Temp_PTS_CM_stg4_back_test_results a
	left join Temp_PTS_CM_stg5_interest b
	on a.term = b.term
	and a.segment_rbp = b.segment_rbp
	and a.flag_kk = b.flag_kk
	and a.generation = b.generation
	...
	print ('timing, Temp_PTS_CM_stg_interest_model USUAL 0-90 ' + format(getdate(), 'HH:mm:ss'));




	--средний портфель в качестве базы начисления %% на NPL
	drop table if exists Temp_PTS_CM_NPL_avg_portf;
	...
	into Temp_PTS_CM_NPL_avg_portf
	from un a
	left join un b
	on a.term = b.term
	and a.segment_rbp = b.segment_rbp
	...
	print ('timing, Temp_PTS_CM_NPL_avg_portf' + format(getdate(), 'HH:mm:ss'));


	--бакеты 90+ - через MOD_NUM - кол-во месяцев в дефолте

	insert into Temp_PTS_CM_stg_interest_model
	...
	--from Temp_PTS_CM_NPL_avg_portf a
	--left join Temp_PTS_CM_stg5_interest b
	--on a.term = b.term
	--and a.segment_rbp = b.segment_rbp
	--and a.flag_kk = b.flag_kk
	...
	print ('timing, Temp_PTS_CM_stg_interest_model USUAL 90+ ' + format(getdate(), 'HH:mm:ss'));
	...
	drop table if exists Temp_PTS_CM_stg_freeze_int_last_fact;
	with base as (
		...
		sum(a.total_od) as total_od,
		sum(a.total_int) as total_int
		from Temp_PTS_CM_fact_due_int a
	...
	into Temp_PTS_CM_stg_freeze_int_last_fact
	from base a
	;
	...
	drop table if exists Temp_PTS_CM_stg22_default_recovery_model;
	...
	eomonth(a.mob_date_to, -1 * a.mod_num) as default_dt,
	sum(a.model_od) as model_od
	into Temp_PTS_CM_stg22_default_recovery_model
	from Temp_PTS_CM_stg_freeze_model a
	where a.bucket_90_to = '[05] 90+'
	and a.mod_num is not null
	and a.mod_num >= 0
	...
	print ('timing, Temp_PTS_CM_stg22_default_recovery_model' + format(getdate(), 'HH:mm:ss'))
	...
	--declare @rdt date = '2024-10-31';declare @dt_back_test_from date = '2024-11-30';declare @horizon int = datediff(MM,@rdt,'2030-12-31') + 1;declare @repeat_share float = 0.62;declare @vers int = 369;
		...
		drop table if exists Temp_PTS_CM_stg33_default_recovery_model;
		...
		into Temp_PTS_CM_stg33_default_recovery_model

		from Temp_PTS_CM_stg22_default_recovery_model a
		left join Temp_PTS_CM_conditional_rec_ALL b
		--on a.mod_num >= b.mod_num
		on b.mod_num between a.mod_num - DATEDIFF(MM, @dt_back_test_from, a.mob_date_to) and a.mod_num
		--and iif(a.flag_kk <> 'Installment', 'PTS', 'Installment') = b.product
	...
	while @cycle_dt3 <= EOMONTH(@dt_back_test_from, @horizon)  begin
		...
		insert into Temp_PTS_CM_stg33_default_recovery_model
		...
		print ('timing, Temp_PTS_CM_stg33_default_recovery_model @cycle_dt3 = ' + format(@cycle_dt3, 'dd.MM.yyyy') + ' ' + format(getdate(), 'HH:mm:ss'));

	set @cycle_dt3 = eomonth(@cycle_dt3,1);
	end 


	print ('timing, Temp_PTS_CM_stg33_default_recovery_model' + format(getdate(), 'HH:mm:ss'));



	---средний портфель в качестве базы начисления %% на NPL - Кредитные каникулы и заморозка
	drop table if exists Temp_PTS_CM_NPL2_avg_portf;
	with base as (
		select a.term, a.segment_rbp, a.flag_kk, a.generation, a.mob_date_to, a.mob_to, 
		a.bucket_90_from,
		cast('[05] 90+' as varchar(100)) as bucket_90_to, 
		...
		from Temp_PTS_CM_stg33_default_recovery_model a
		where a.model_od > 0
		and left(a.flag_kk,2) = 'KK'
	), agg as (
		select a.term, a.segment_rbp, a.flag_kk, a.generation, a.mob_date_to, a.mob_to,
	...
	into Temp_PTS_CM_NPL2_avg_portf
	from base a
	left join agg b
	on a.term = b.term
	and a.segment_rbp = b.segment_rbp 
	...
	with base as (
		...
		from Temp_PTS_CM_stg_freeze_model a
		left join Temp_PTS_CM_stg_int_rates b--Temp_PTS_CM_stg4_interest b
		on a.term = b.term
		and a.segment_rbp = b.segment_rbp
		and a.flag_kk = b.flag_kk 
		and a.generation = b.generation
	...
	union all
		...
		from Temp_PTS_CM_NPL2_avg_portf a --Temp_PTS_CM_stg33_default_recovery_model a
		left join Temp_PTS_CM_stg_int_rates b--Temp_PTS_CM_stg4_interest b
		on a.term = b.term
		and a.segment_rbp = b.segment_rbp
		and a.flag_kk = b.flag_kk 
	...
	from Temp_PTS_CM_stg_interest_model a
	left join Temp_PTS_CM_stg_interest_model b
	on 1=1
	--and a.term = b.term
	--and a.segment_rbp = b.segment_rbp
	--and a.generation = b.generation
	...
	print ('timing, Temp_PTS_CM_stg_interest_model ' + format(getdate(), 'HH:mm:ss'))
	...
	drop table if exists Temp_PTS_CM_stg_int_rec_wo;
	with aa as (
		...
		from (
			...
			from Temp_PTS_CM_stg1_default_recovery_fact a --Temp_PTS_CM_default_recovery_fact a
		) a
		...
		from (
			...
			from Temp_PTS_CM_stg3_default_recovery_model a --Temp_PTS_CM_default_recovery_model a
		) a
	...
	into Temp_PTS_CM_stg_int_rec_wo
	...
	print ('timing, Temp_PTS_CM_stg_int_rec_wo ' + format(getdate(), 'HH:mm:ss'))




	--то же самое для КК с исходящим бакетом
	drop table if exists Temp_PTS_CM_stg2_int_rec_wo;
	...
	with base as (
		...
		from Temp_PTS_CM_stg33_default_recovery_model a --Temp_PTS_CM_default_recovery_model a
	), agg as (
	...
	into Temp_PTS_CM_stg2_int_rec_wo
	from base a
	left join agg b
	on a.term = b.term
	and a.segment_rbp = b.segment_rbp
	...
	drop table if exists Temp_PTS_CM_interest_model;
	...
	into Temp_PTS_CM_interest_model
	from base a
	left join ovr b
	on 1=1
	--and a.term = b.term
	...
	left join Temp_PTS_CM_stg_int_rec_wo c --рекавери и списания 90+
	on 1=1
	--and a.term = c.term
	--and a.segment_rbp = c.segment_rbp 
	--and a.flag_kk = c.flag_kk
	...
	from Temp_PTS_CM_stg_int_rec_wo c
	left join base a 
	on 1=1
	--and a.term = c.term
	--and a.segment_rbp = c.segment_rbp 
	...
	--Расчет для КК
	drop table if exists Temp_PTS_CM_kk_interest;
	...
	into Temp_PTS_CM_kk_interest
	from Temp_PTS_CM_fact_due_int a --Temp_PTS_CM_freeze_last_fact a
	where left(a.flag_kk,2) = 'KK'
	...
	insert into Temp_PTS_CM_kk_interest
	...
	from Temp_PTS_CM_fact_due_int a
	left hash join Temp_PTS_CM_for_back_test b
	on a.term = b.term
	and a.generation = b.generation
	and a.segment_rbp = b.segment_rbp
	and a.flag_kk = b.flag_kk
	...
	from Temp_PTS_CM_stg2_int_rec_wo c
	left join Temp_PTS_CM_kk_interest a --рекавери и списания 90+
	on 1 = 1
	--and a.term = c.term
	--and a.segment_rbp = c.segment_rbp 
	--and a.flag_kk = c.flag_kk
	...
	from Temp_PTS_CM_kk_interest a
	left join ovr b
	on 1 = 1
	--and a.term = b.term
	--and a.segment_rbp = b.segment_rbp 
	...
	and isnull(a.mod_num,-999) = isnull(b.mod_num,-999)
	and isnull(a.last_MOD,-999) = isnull(b.last_MOD,-999)
	left join Temp_PTS_CM_stg2_int_rec_wo c --рекавери и списания 90+
	on 1 = 1
	--and a.term = c.term
	--and a.segment_rbp = c.segment_rbp 
	--and a.flag_kk = c.flag_kk
	...
	insert into Temp_PTS_CM_interest_model
	...
	from Temp_PTS_CM_stg4_back_test_results a
	left join Temp_PTS_CM_stg_int_rates b
	on a.term = b.term
	and a.segment_rbp = b.segment_rbp
	and a.flag_kk = b.flag_kk
	and a.generation = b.generation
	...
	print ('timing, Temp_PTS_CM_interest_model ' + format(getdate(), 'HH:mm:ss'))
	...
	begin
		...
		drop table if exists Temp_PTS_CM_tmp_interest_model;
		select * 
		into Temp_PTS_CM_tmp_interest_model
		from Temp_PTS_CM_interest_model a
		where a.mob_date_to = @jj
		--and a.total_int > 1
		;


		drop table if exists Temp_PTS_CM_tmp_kk_interest;
		select * 
		into Temp_PTS_CM_tmp_kk_interest
		from Temp_PTS_CM_kk_interest a
		where a.mob_date_to = @jj
		--and a.model_int > 1
		;
		...
		with base as ( --Последний факт
			...
			sum(a.total_int * b.model_coef_adj) as total_int
			from Temp_PTS_CM_tmp_interest_model a
			left join Temp_PTS_CM_for_back_test b
			on 1 = 1
			--and a.term = b.term
			--and a.segment_rbp = b.segment_rbp
		...
		from Temp_PTS_CM_stg_int_rec_wo c --рекавери и списания 90+
		left join base a
		on 1=1
		--and a.term = c.term
		--and a.segment_rbp = c.segment_rbp 
		...
		* case a.bucket_90_to when '[03] 31-60' then 0.75 when '[04] 61-90' then 0.15 end 
		...
		from Temp_PTS_CM_tmp_kk_interest a
		where left(a.flag_kk,2) = 'KK'
		and a.bucket_90_to in ('[03] 31-60','[04] 61-90')
		--and a.model_int > 0
		and a.freeze_from is null --без повторных, только продление
		...
		* case when b.bucket_90_from = '[03] 31-60' and a.freeze_from is null and a.mob_date_to < '2021-10-31' then 0.25
		...
		from Temp_PTS_CM_stg2_int_rec_wo c --рекавери и списания 90+

		left join Temp_PTS_CM_kk_interest a
		on 1=1
		--and a.term = c.term
		--and a.segment_rbp = c.segment_rbp 
		--and a.flag_kk = c.flag_kk
	...
	print ('timing, Temp_PTS_CM_interest_model cycle, @jj = ' + format(@jj,'dd.MM.yyyy') + ' ' + format(getdate(), 'HH:mm:ss'));
	...
	drop table Temp_PTS_CM_tmp_interest_model;
	drop table Temp_PTS_CM_tmp_kk_interest;
	...
	*/
	...
	drop table if exists Temp_PTS_CM_model_payment_interest
	...
	into Temp_PTS_CM_model_payment_interest
	from Temp_PTS_CM_stg_interest_model a
	...
	from Temp_PTS_CM_stg_int_rec_wo a
	...
	select a.term, a.segment_rbp, a.flag_kk, a.generation, a.mob_date_to, 
		...
		sum(isnull(a.int_recov,0)) as int_pmt,
		sum(isnull(a.int_wo,0)) as int_wo
	from Temp_PTS_CM_stg2_int_rec_wo a
	where left(a.flag_kk,2) = 'KK'
	group by a.term, a.segment_rbp, a.flag_kk, a.generation, a.mob_date_to
	;
	...
	drop table if exists Temp_PTS_CM_for_update_pmt_int;
	select a.term, a.segment_rbp, a.flag_kk, a.generation, a.mob_date_to, 
	...
	into Temp_PTS_CM_for_update_pmt_int
	from Temp_PTS_CM_kk_interest a
	left join Temp_PTS_CM_stg_interest_model b
	on a.id1 = b.id1
	and a.mob_date_to = b.mob_date_to
	...
	merge into Temp_PTS_CM_model_payment_interest a
	using Temp_PTS_CM_for_update_pmt_int b
	on (a.term = b.term
		and a.segment_rbp = b.segment_rbp
		and a.generation = b.generation
		and a.flag_kk = b.flag_kk
	...
	from Temp_PTS_CM_model_payment_interest a
	left join volume b
	on a.term = b.term
	and a.segment_rbp = b.segment_rbp
	and a.flag_kk = b.flag_kk
	...
	insert into Temp_PTS_CM_model_payment_interest
	...
	print ('timing, Temp_PTS_CM_model_payment_interest ' + format(getdate(), 'HH:mm:ss'))
	;
	...
	--drop table Temp_PTS_CM_virt_gens;
	/* P 5 */
	drop table Temp_PTS_CM_for_update_pmt_int;
	--drop table Temp_PTS_CM_interest_model;
	drop table Temp_PTS_CM_kk_interest;
	--drop table Temp_PTS_CM_model_payment_interest;
	drop table Temp_PTS_CM_NPL_avg_portf;
	drop table Temp_PTS_CM_NPL2_avg_portf;
	drop table Temp_PTS_CM_stg_freeze_int_last_fact;
	--drop table Temp_PTS_CM_stg_int_rates;
	--drop table Temp_PTS_CM_stg_int_rec_wo;
	drop table Temp_PTS_CM_stg_interest_model;
	drop table Temp_PTS_CM_stg1_interest;
	--drop table Temp_PTS_CM_stg2_int_rec_wo;
	drop table Temp_PTS_CM_stg2_interest;
	drop table Temp_PTS_CM_stg22_default_recovery_model;
	drop table Temp_PTS_CM_stg33_default_recovery_model;
	drop table Temp_PTS_CM_stg5_interest;
	drop table Temp_PTS_CM_upd_stg2_interest;
	...
	drop table if exists Temp_PTS_CM_stg_eps_rates;
	with rates as (
		select a.term, a.segment_rbp, a.flag_kk, a.generation, 
	...
	into Temp_PTS_CM_stg_eps_rates
	from base a
	where a.rown = 1;
	...
	--from Temp_PTS_CM_stg_eps_rates a
	--inner join upd b
	--on a.term = b.term
	--and a.segment_rbp = b.segment_rbp
	--and a.generation = b.generation
	...
	print ('timing, Temp_PTS_CM_stg_eps_rates ' + format(getdate(), 'HH:mm:ss'))


	--коэффициент залогового покрытия
	drop table if exists Temp_PTS_CM_pledgecover;
	select a.rep_dt, a.vers,
		max((rr_90_plus_pldg - rr_90_plus) / (1 - rr_90_plus)) as koef
	into Temp_PTS_CM_pledgecover
	from risk.prov_stg_prov_rates_0_90 a
	where 1=1
	group by a.rep_dt, a.vers
	;

	print ('timing, Temp_PTS_CM_pledgecover ' + format(getdate(), 'HH:mm:ss'))
	...
	drop table if exists Temp_PTS_CM_det_BU_prov_rates;
	with base as (
		...
		left join Temp_PTS_CM_pledgecover b
	...
	select 
		...
		as pledge_cover
	into Temp_PTS_CM_det_BU_prov_rates
	from base a 
	;
	...
	print ('timing, Temp_PTS_CM_det_BU_prov_rates ' + format(getdate(), 'HH:mm:ss'))


	--ставки НУ
	drop table if exists Temp_PTS_CM_det_NU_prov_rates;
	...
	into Temp_PTS_CM_det_NU_prov_rates
	from base a
	;
	...
	drop table if exists Temp_PTS_CM_det_NU_prov_rates_PDL;
	...
	into Temp_PTS_CM_det_NU_prov_rates_PDL
	from base a
	...
	--Распределение ПДН для новых выдач
	drop table if exists Temp_PTS_CM_pdn_distrib;

	select 
	cast(a.pdn_group as varchar(100)) as pdn_group,
	cast(a.segment_rbp as varchar(100)) as segment_rbp,
	cast(a.frac as float) as frac
	into Temp_PTS_CM_pdn_distrib
	from (values
	...
	*/
	...
	print ('timing, Temp_PTS_CM_pdn_distrib ' + format(getdate(), 'HH:mm:ss'))



	--аналитические ставки НУ - обратным расчетом
	drop table if exists Temp_PTS_CM_analyt_prov_NU_rates;
	...
	into Temp_PTS_CM_analyt_prov_NU_rates
	from Temp_PTS_CM_stg1_NU_segment_distrib a
	where a.mob_date_to = eomonth(@dt_back_test_from,-1)
	group by a.NU_segment, RiskDWH.dbo.get_bucket_360_m(a.dpd)
	;

	update Temp_PTS_CM_analyt_prov_NU_rates set prov_rate = 1 where prov_rate > 1;
	...
	drop table if exists Temp_PTS_CM_model_od_payment;
	select a.term, a.segment_rbp, a.flag_kk, a.generation, 
	--a.mob_to, 
	--a.mob_date_to,
	b.mob_to, 
	...
	into Temp_PTS_CM_model_od_payment
	from Temp_PTS_CM_stg4_back_test_results a
	left join Temp_PTS_CM_for_back_test b
	on a.term = b.term
	and a.segment_rbp = b.segment_rbp
	...
	from Temp_PTS_CM_model_od_payment a
	left join volume b
	on a.term = b.term 
	and a.segment_rbp = b.segment_rbp
	and a.flag_kk = b.flag_kk
	...
	insert into Temp_PTS_CM_model_od_payment
	select a.term, a.segment_rbp, a.flag_kk, a.generation,
	0 as mob_to,
	...
	print ('timing, Temp_PTS_CM_model_od_payment ' + format(getdate(), 'HH:mm:ss'))




	--платежи ОД и %% по бакетам
	drop table if exists Temp_PTS_CM_model_all_payment;
	with default_pmt as (
	...
	into Temp_PTS_CM_model_all_payment
	from u
	left join Temp_PTS_CM_model_payment_interest m
	on u.term = m.term
	...
	print ('timing, Temp_PTS_CM_model_all_payment ' + format(getdate(), 'HH:mm:ss'))
	...
	select a.term, a.segment_rbp, a.flag_kk, a.generation, a.mob_date_to, a.bucket_90_from
	from Temp_PTS_CM_model_all_payment a
	group by a.term, a.segment_rbp, a.flag_kk, a.generation, a.mob_date_to, a.bucket_90_from
	having count(*)>1
	;
	*/
	...
	--финальная сборка 
	drop table if exists Temp_PTS_CM_final_model;
	...
	with fact_od_payment as (
		...
		from Temp_PTS_CM_stg_fact_od_int_pmt a
	...
	into Temp_PTS_CM_final_model

	from Temp_PTS_CM_stg4_back_test_results a

	left join Temp_PTS_CM_model_all_payment f
	on a.term = f.term
	...
	left join Temp_PTS_CM_stg_int_rates irts
	on a.term = irts.term
	...
	print ('timing, Temp_PTS_CM_final_model ' + format(getdate(), 'HH:mm:ss'))
	...
	a.*
	from Temp_PTS_CM_final_model a 
	;
	...
	*/
	...
	--drop table Temp_PTS_CM_stg2_int_rec_wo;
	/* P 6-7 */
	--drop table Temp_PTS_CM_analyt_prov_NU_rates;
	--drop table Temp_PTS_CM_det_BU_prov_rates;
	--drop table Temp_PTS_CM_det_NU_prov_rates;
	--drop table Temp_PTS_CM_det_NU_prov_rates_PDL;
	--drop table Temp_PTS_CM_final_model;
	drop table Temp_PTS_CM_model_all_payment;
	drop table Temp_PTS_CM_model_od_payment;
	--drop table Temp_PTS_CM_pdn_distrib;
	drop table Temp_PTS_CM_pledgecover;
	--drop table Temp_PTS_CM_stg_eps_rates;
	...
	drop table if exists Temp_PTS_CM_base;
	...
	from total a
	left join Temp_PTS_CM_pdn_distrib b
	...
	print ('timing, Temp_PTS_CM_base ' + format(getdate(), 'HH:mm:ss'));
	...
	drop table if exists Temp_PTS_CM_virt_IFRS_rates_0_90;
	...
	into Temp_PTS_CM_virt_IFRS_rates_0_90
	...
	insert into Temp_PTS_CM_virt_IFRS_rates_0_90
	select 
	cast('INSTALLMENT' as varchar(100)) as product,
	a.dpd_bucket, 
	sum(a.prov_IFRS_TTC) / sum(a.gross) as prov_rate
	...
	drop table if exists Temp_PTS_CM_virt_IFRS_rates_91_plus;

	create table Temp_PTS_CM_virt_IFRS_rates_91_plus (product varchar(100),months_in_default int, prov_rate float);

	declare @mm int = 0;

	while @mm < 400 begin

	insert into Temp_PTS_CM_virt_IFRS_rates_91_plus (product,months_in_default, prov_rate) values ('PTS',@mm, 1.0);
	insert into Temp_PTS_CM_virt_IFRS_rates_91_plus (product,months_in_default, prov_rate) values ('INSTALLMENT',@mm, 1.0);
	...
	merge into Temp_PTS_CM_virt_IFRS_rates_91_plus dst
	...
	drop table if exists Temp_PTS_CM_det_LGD_IL;
	select 
	cast(a.mod_num as int) as mod_num, 
	cast(a.lgd as float) as lgd
	into Temp_PTS_CM_det_LGD_IL
	from (values
	...
	merge into Temp_PTS_CM_virt_IFRS_rates_91_plus dst
	using (select * from Temp_PTS_CM_det_LGD_IL) src
	...
	drop table if exists Temp_PTS_CM_det_BU_IL_DEC23_REC;
	select 
	a.mod_n-1 as mod_n,
	cast(a.recovery_rate as float) as recovery_rate
	into Temp_PTS_CM_det_BU_IL_DEC23_REC
	from (values
	(1 , 0.007951563),
	(2 , 0.0037317878),
	(3 , 0.00506509),
	...
	select @cycle_int1 = max(mod_n)+1 from Temp_PTS_CM_det_BU_IL_DEC23_REC;
	while @cycle_int1 < 500 begin
	insert into Temp_PTS_CM_det_BU_IL_DEC23_REC (mod_n, recovery_rate) values (@cycle_int1, 0)
	...
	drop table if exists Temp_PTS_CM_det_BU_IL_DEC23_PD;
	select 
	cast(a.dpd_bucket_90 as varchar(100)) as dpd_bucket_90,
	cast(a.koef as float) as koef
	into Temp_PTS_CM_det_BU_IL_DEC23_PD
	from (values
	...
	into Temp_PTS_CM_det_BU_IL_DEC23_LGD
	from acc a
	order by 1
	;


	drop table if exists Temp_PTS_CM_det_BU_IL_DEC23_0_90;
	select 
	a.dpd_bucket_90, 
	a.koef as PD, 
	a.koef * b.lgd as prov_rate
	into Temp_PTS_CM_det_BU_IL_DEC23_0_90
	from Temp_PTS_CM_det_BU_IL_DEC23_PD a
	left join Temp_PTS_CM_det_BU_IL_DEC23_LGD b
	on b.mod_n = 0
	;
	...
	--ставки и дисконты для БУ в 2025 году
	drop table if exists Temp_PTS_CM_virt_BU_rates;
	with base as (
		select 
		cast(a.dt_from as date) as dt_from, 
		cast(a.dt_to as date) as dt_to, 
	...
	into Temp_PTS_CM_virt_BU_rates
	from base a
	left join rates b
	...
	drop table if exists Temp_PTS_CM_BU_rates_BUSINESS;
	select 
	cast(a.dpd_bucket as varchar(100)) as dpd_bucket, 
	cast(a.rate as float) as rate
	into Temp_PTS_CM_BU_rates_BUSINESS
	from (values
	('[01] 0' , 0.015),
	('[02] 1-30' , 0.05),
	('[03] 31-60' , 0.3),
	...
	drop table if exists Temp_PTS_CM_for_finance;
	...
	with model as (
		...
		from Temp_PTS_CM_base a

		--13.07.2022
		left join Temp_PTS_CM_det_BU_prov_rates b
		...
		left join Temp_PTS_CM_analyt_prov_NU_rates e
		...
		left join Temp_PTS_CM_BU_rates_BUSINESS bus_bu
		...
		on a.external_id = b.external_id
		--where a.cli_type = 'ФЛ'
		inner join Temp_PTS_CM_UMFO_NU_segment d
		on a.external_id = d.external_id
		and a.r_date = d.r_date
		left join Temp_PTS_CM_zamorozka z
		on a.external_id = z.external_id
	...
	print ('timing, Temp_PTS_CM_for_finance ' + format(getdate(), 'HH:mm:ss'));
	...
	drop table if exists Temp_PTS_CM_pmt_base;
	...
	into Temp_PTS_CM_pmt_base

	from u as a
	inner join Temp_PTS_CM_NU_segment_distrib b
	on a.term = b.term
	and a.segment_rbp = b.segment_rbp
	and a.generation = b.generation
	and a.flag_kk = b.flag_kk
	...
	print ('timing, Temp_PTS_CM_pmt_base (model) ' + format(getdate(), 'HH:mm:ss'));
	...
	with pmt_and_writeoff as (
		...
		from (
			...
			cast(0 as float) as od_wo, 
			cast(0 as float) as int_wo
			from Temp_PTS_CM_stg_payment a
	...
	insert into Temp_PTS_CM_pmt_base
	...
	on a.external_id = b.external_id 
	and a.mob_date_to = eomonth(b.cdate)
	inner join Temp_PTS_CM_UMFO_NU_segment c
	on a.external_id = c.external_id
	...
	merge into Temp_PTS_CM_pmt_base dst
	...
	(not exists (select 1 from Temp_PTS_CM_stg_matrix_detail md where a.external_id = md.external_id) and a.flag_closed_in_month = 1)
	group by a.term, a.generation, a.segment_rbp, a.flag_kk,
	c.NU_segment
	) src
	...
	print ('timing, Temp_PTS_CM_pmt_base (fact) ' + format(getdate(), 'HH:mm:ss'));
	...
	*/
	...
	select 
		...
		--select distinct flag_kk from Temp_PTS_CM_for_finance order by 1
		--select distinct flag_kk from Temp_PTS_CM_pmt_base order by 1
		--select distinct flag_kk from Temp_PTS_CM_stg_int_rates order by 1

	from Temp_PTS_CM_for_finance a
	full join Temp_PTS_CM_pmt_base b
	on a.term = b.term
	and a.segment_rbp = b.segment_rbp
	and a.flag_kk = b.flag_kk 
	and a.generation = b.generation
	...
	*/
	...
	--drop table Temp_PTS_CM_stg_eps_rates;
	--/* P 8 */
	--drop table Temp_PTS_CM_base;
	--drop table Temp_PTS_CM_det_BU_IL_DEC23_0_90;
	--drop table Temp_PTS_CM_det_BU_IL_DEC23_LGD;
	--drop table Temp_PTS_CM_det_BU_IL_DEC23_PD;
	--drop table Temp_PTS_CM_det_BU_IL_DEC23_REC;
	--drop table Temp_PTS_CM_det_LGD_IL;
	--drop table Temp_PTS_CM_for_finance;
	--drop table Temp_PTS_CM_pmt_base;
	--drop table Temp_PTS_CM_virt_BU_rates;
	--drop table Temp_PTS_CM_virt_IFRS_rates_0_90;
	--drop table Temp_PTS_CM_virt_IFRS_rates_91_plus;
	...
	insert into risk.core_fcst_pmt_base select a.*, @vers from Temp_PTS_CM_pmt_base a;

	delete from risk.core_fcst_npl_wo_and_recovery where vers = @vers;
	insert into risk.core_fcst_npl_wo_and_recovery select a.*, @vers from Temp_PTS_CM_npl_wo_and_recovery a;

	delete from risk.core_fcst_int_rates where vers = @vers;
	insert into risk.core_fcst_int_rates select a.*, @vers from Temp_PTS_CM_stg_int_rates a;

	----------------------------------------------------------------

	delete from risk.core_fcst_p8_base where vers = @vers;
	insert into risk.core_fcst_p8_base select a.*, @vers from Temp_PTS_CM_base a;

	delete from risk.core_fcst_eps_rates where vers = @vers;
	insert into risk.core_fcst_eps_rates select a.*, @vers from Temp_PTS_CM_stg_eps_rates a;

	delete from risk.core_fcst_analyt_prov_NU_rates where vers = @vers;
	insert into risk.core_fcst_analyt_prov_NU_rates select a.*, @vers from Temp_PTS_CM_analyt_prov_NU_rates a;

	delete from risk.core_fcst_det_pieces_L1 where vers = @vers;
	insert into risk.core_fcst_det_pieces_L1 select a.*, @vers from Temp_PTS_CM_det_pieces_L1 a;

	delete from risk.core_fcst_det_pieces_L2 where vers = @vers;
	insert into risk.core_fcst_det_pieces_L2 select a.*, @vers from Temp_PTS_CM_det_pieces_L2 a;
	...
	insert into risk.core_fcst_NU_segment_distrib select a.*, @vers from Temp_PTS_CM_NU_segment_distrib a;
	...
	/*
	select * from Temp_PTS_CM_cred_reestr a
	where not exists (select 1 from Temp_PTS_CM_UMFO_NU_segment b where a.external_id = b.external_id)
	;
	...
	*/
	...
	select *
	from (select * from Temp_PTS_CM_for_finance a where a.mob_date_to between '2021-12-31' and '2027-04-30') a
	full join Temp_PTS_CM_pmt_base b
	on a.term = b.term
	and a.segment_rbp = b.segment_rbp
	and a.flag_kk = b.flag_kk 
	...
	*/
	...
	select a.mob_date_to, a.term, a.segment_rbp, a.flag_kk, a.generation, a.NU_segment, a.dpd_bucket
	from Temp_PTS_CM_pmt_base a
	group by a.mob_date_to, a.term, a.segment_rbp, a.flag_kk, a.generation, a.NU_segment, a.dpd_bucket
	having count(*)>1;



	select min(mob_date_to), max(mob_date_to) from Temp_PTS_CM_for_finance;
	select min(mob_date_to), max(mob_date_to) from Temp_PTS_CM_pmt_base;


	select * from Temp_PTS_CM_for_finance a where a.fact_od = 0 and a.fact_pieces > 0;
	select * from Temp_PTS_CM_for_finance a where a.fact_od > 0 and a.fact_pieces = 0;

	select * from Temp_PTS_CM_for_finance a where a.model_od = 0 and a.model_pieces > 0;
	select * from Temp_PTS_CM_for_finance a where a.model_od > 0 and a.model_pieces = 0;
	...
	*/
	...
	--выдачи
	drop table if exists Temp_PTS_CM_stg_volumes;
	...
	--процентная ставка
	drop table if exists Temp_PTS_CM_stg_interest_rates;
	...
	into Temp_PTS_CM_stg_interest_rates
	from Temp_PTS_CM_final_model a
	where a.mob_to = 0
	and a.bucket_90_to = '[01] 0'
	group by a.segment_rbp,
	...
	drop table if exists Temp_PTS_CM_pmt_wo_for_nl;
	select  
		1 as term,
		a.segment_rbp,
		a.generation,
	...
	into Temp_PTS_CM_pmt_wo_for_nl
	from Temp_PTS_CM_npl_wo_and_recovery a
	group by a.segment_rbp,
		a.generation,
		case 
	...
	drop table if exists Temp_PTS_CM_for_demonstration;
	...
	with base as (
		...
		left join Temp_PTS_CM_for_cessed_GL c
		on a.term = c.term
			and a.segment_rbp = c.segment_rbp
			and a.generation = c.generation
			and a.flag_kk = c.flag_kk
	...
	into Temp_PTS_CM_for_demonstration
	from base a
	left join Temp_PTS_CM_stg_volumes b
	on a.term = b.term
		and a.segment_rbp = b.segment_rbp
		and a.generation = b.generation
		and a.subportf = b.subportf

	left join Temp_PTS_CM_stg_interest_rates c
	on a.term = c.term
		and a.segment_rbp = c.segment_rbp
		and a.generation = c.generation
	...
	insert into Temp_PTS_CM_for_demonstration
	select DISTINCT
		a.scenario,
		a.term,
		a.segment_rbp,
		...
		a.avg_int_rate
	from Temp_PTS_CM_for_demonstration a
	left join Temp_PTS_CM_stg_volumes b
	on a.segment_rbp = b.segment_rbp
	and a.generation = b.generation
	and a.subportf = b.subportf
	where 1=1
	--and a.MoB = 1
	--and not exists (select 1 from Temp_PTS_CM_for_demonstration b where a.segment_rbp = b.segment_rbp and a.generation = b.generation and b.MoB = 1 and b.dpd_bucket_90 = '[03] 91-360')
	and not exists (select 1 from Temp_PTS_CM_for_demonstration b 
		where a.segment_rbp = b.segment_rbp
		and a.generation = b.generation 
		and a.subportf = b.subportf
		and a.MoB = b.MoB
	...
	drop table if exists Temp_PTS_CM_for_update_NLGL;
	with a as (
	...
	into Temp_PTS_CM_for_update_NLGL
	from a
	left join Temp_PTS_CM_pmt_wo_for_nl b
		on a.term = b.term
		and a.segment_rbp = b.segment_rbp
		and a.generation = b.generation
		and a.subportf = b.subportf
	...
	from Temp_PTS_CM_for_demonstration a
	left join Temp_PTS_CM_for_update_NLGL b
		on a.term = b.term
		and a.segment_rbp = b.segment_rbp
		and a.generation = b.generation
		and a.subportf = b.subportf
	...
	delete from Temp_PTS_CM_for_demonstration where MoB < 0;
	...
	select
		...
		cash_total,
		avg_int_rate
	from Temp_PTS_CM_for_demonstration
	;
	...
	--Возращаем фактические платежи в нулевом MoB для последнего поколения (чтобы входящая сумма была равна сумме выдачи)
		...
		from Temp_PTS_CM_fix_matrix a
		inner join Temp_PTS_CM_new_volume b
	...
	from Temp_PTS_CM_virt_gens a
	left join Temp_PTS_CM_det_stress st
