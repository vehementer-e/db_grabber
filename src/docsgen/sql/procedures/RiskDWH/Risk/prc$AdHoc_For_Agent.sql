...
/**************************************************************************
Скрипт в агент SQL для AdHoc запросов

Revisions:
dt			user				version		description
24/08/20	golicyn				v1.0		Создание процедуры


*************************************************************************/

CREATE procedure [Risk].[prc$AdHoc_For_Agent]

AS

begin try

	declare @srcname varchar(100) = 'SQL Agent';

	declare @vinfo varchar(1000) = 'AdHoc'
									;
...
declare @gen_mm date = '2012-11-30'--(select eomonth(min(startdate), 0) from dwh2.risk.credits)
		--цикл до винтажа
		, @gen_to date = '2023-01-31'
		-- макс баоансовая дата
		, @rdt date = '2026-01-31'
;
...
BEGIN
	...
	into RiskDWH.Risk.Temp_Person_Deals_stg1--select *
