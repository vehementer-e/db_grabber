...
CREATE procedure [Risk].[prc$calc_Prognoz_PTS_Infoseti]
...
-- Информационные логи
@vinfo_start varchar(256) = 'PTS Infoseti Start',
@vinfo_finish varchar(256) = 'PTS Infoseti Finish'
...
begin try
	...
	--	ставки и дисконты для БУ в 2025 году
		...
		*/
	--	подбираем ставки. В 4 местах по сроку
	...
	drop table if exists Temp_PTS_Infoseti_Channel
	...
	select --
		...
		as Product_Channel
		, count(*) as cnt
	into Temp_PTS_Infoseti_Channel--select top 100 *
	...
	--from Temp_PTS_Infoseti_Channel
	...
	drop table if exists Temp_PTS_Infoseti_CMR_M;
	select a.external_id, a.d, a.[остаток од], a.dpd 
	into Temp_PTS_Infoseti_CMR_M
	...
	--select *
	from Temp_PTS_Infoseti_CMR_M a
	where a.[остаток од] < 0
	;

	--итоговый список списаний
	drop table if exists Temp_PTS_Infoseti_stg_write_off;

	select b.external_id, b.r_date, b.od_wo, b.int_wo
	into Temp_PTS_Infoseti_stg_write_off
	...
	print ('timing, Temp_PTS_Infoseti_stg_write_off ' + format(getdate(), 'HH:mm:ss'));
	...
	drop table if exists Temp_PTS_Infoseti_det_current_params;
	with base as (
	...
	into Temp_PTS_Infoseti_det_current_params
	from base a
	where a.rn = 1
	;
	...
	with base as (
		...
		and not exists (select 1 from Temp_PTS_Infoseti_det_current_params c where b.Код = c.external_id)
	)
	insert into Temp_PTS_Infoseti_det_current_params
	select a.external_id, a.cur_rate, a.cur_plan_end_date
	from base a
	where a.rn = 1
	;
	...
	drop table if exists Temp_PTS_Infoseti_det_bucket_360;
	select * 
	into Temp_PTS_Infoseti_det_bucket_360
	from (values
	('[01] 0', 1),
	('[02] 1-30', 2),
	('[03] 31-60', 3),
	...
	drop table if exists Temp_PTS_Infoseti_det_bucket_90;
	select *  
	into Temp_PTS_Infoseti_det_bucket_90
	from (values
	('[01] 0', 1),
	('[02] 1-30', 2),
	('[03] 31-60', 3),
	...
	drop table if exists Temp_PTS_Infoseti_cred_end_date;
	select d.Код as external_id,
		   cast(dateadd(year,-2000,max(sd.Период)) as date) as end_date
	into Temp_PTS_Infoseti_cred_end_date
	...
	print ('timing, Temp_PTS_Infoseti_cred_end_date ' + format(getdate(), 'HH:mm:ss'));
	...
	drop table if exists Temp_PTS_Infoseti_cred_CMR_status;
	...
	into Temp_PTS_Infoseti_cred_CMR_status
	...
	delete from Temp_PTS_Infoseti_cred_CMR_status where rown <> 1;


	print ('timing, Temp_PTS_Infoseti_cred_CMR_status ' + format(getdate(), 'HH:mm:ss'));
	...
	drop table if exists Temp_PTS_Infoseti_stg1_zamorozka;
	select a.Договор as Договор, a.Дата as Дата 
	into Temp_PTS_Infoseti_stg1_zamorozka
	...
	drop table if exists Temp_PTS_Infoseti_stg2_zamorozka;
	select b.Код as external_id, eomonth(dateadd(yy,-2000,cast(a.Дата as date))) as freeze_dt
	into Temp_PTS_Infoseti_stg2_zamorozka
	from Temp_PTS_Infoseti_stg1_zamorozka a
	left join stg._1cCMR.Справочник_Договоры b
	on a.Договор = b.Ссылка
	;

	drop table if exists Temp_PTS_Infoseti_zamorozka;
	select a.external_id, 
	min(a.freeze_dt) as freeze_from,
	eomonth(max(a.freeze_dt),3) as freeze_to
	into Temp_PTS_Infoseti_zamorozka
	from Temp_PTS_Infoseti_stg2_zamorozka a
	...
	drop table if exists Temp_PTS_Infoseti_stg_bankrupt;
	select a.Контрагент as contragent,
	min(cast(dateadd(yy,-2000,a.дата) as date)) as dt
	into Temp_PTS_Infoseti_stg_bankrupt
	...
	into Temp_PTS_Infoseti_bankrupt
	from base a
	group by a.external_id
	;
	...
	with a as (select * from Temp_PTS_Infoseti_zamorozka a)
	delete from a
	where exists (select 1 from Temp_PTS_Infoseti_bankrupt b where a.external_id = b.external_id and b.dt <= @rdt)
	;

	print ('timing, Temp_PTS_Infoseti_zamorozka ' + format(getdate(), 'HH:mm:ss'));



	--ЭПС из УМФО

	drop table if exists Temp_PTS_Infoseti_stg2_eps;
	...
	into Temp_PTS_Infoseti_stg2_eps
	--select *
	...
	--Перечень договоров
	drop table if exists Temp_PTS_Infoseti_cred_reestr;
	...
	select DISTINCT 
		...
		cast(
			case 
				...
				when ch.Product_Channel = 'PTS_PSB' then 'PTS_PSB'
				when ch.Product_Channel = 'PTS_Infoseti' then 'PTS_Infoseti'
	...
	into Temp_PTS_Infoseti_cred_reestr
	from dwh2.risk.credits a
	left join Temp_PTS_Infoseti_cred_end_date b
	...
	left join Temp_PTS_Infoseti_bankrupt bkr
	...
	left join Temp_PTS_Infoseti_eps ee
	on a.external_id = ee.external_id

	left join RiskDWH.risk.stg_fcst_fix_eps ffep
	on a.external_id = ffep.external_id

	left join RiskDWH.risk.Temp_PTS_Infoseti_Channel ch
	on a.external_id = ch.external_id

	where cast(a.startdate as date) between cast('2016-01-01' as date) and @rdt
		and not exists (select 1 from Temp_PTS_Infoseti_cred_CMR_status sts where a.external_id = sts.external_id and sts.cred_status = 'Внебаланс')
	...
	insert into Temp_PTS_Infoseti_cred_reestr 
	...
	--from Temp_PTS_Infoseti_bus_cred_reestr a
	...
	with base as (
		...
		from Temp_PTS_Infoseti_cred_reestr a
	...
	from Temp_PTS_Infoseti_cred_reestr b
	inner join base a
	on a.external_id = b.external_id
	...
	from Temp_PTS_Infoseti_cred_reestr aa
	left join Temp_PTS_Infoseti_det_current_params b
	...
	update Temp_PTS_Infoseti_cred_reestr set eps = 0 where eps is null;


	drop index if exists idx_cred_reestr on Temp_PTS_Infoseti_cred_reestr;
	create clustered index idx_cred_reestr on Temp_PTS_Infoseti_cred_reestr (external_id);



	print ('timing, Temp_PTS_Infoseti_cred_reestr ' + format(getdate(), 'HH:mm:ss'));


	--Историческая просрочка
	drop table if exists Temp_PTS_Infoseti_det_historical_dpd;

	select a.r_date, a.external_id, a.dpd_final
	into Temp_PTS_Infoseti_det_historical_dpd
	...
	--exec dbo.prc$set_debug_info @src = @srcname, @info = 'Temp_PTS_Infoseti_stg_bal';

	--Остатки на различные MOB-ы
	drop table if exists Temp_PTS_Infoseti_stg_bal;
	...
	into Temp_PTS_Infoseti_stg_bal
	from Temp_PTS_Infoseti_cred_reestr a
	inner join Temp_PTS_Infoseti_CMR_M b
	on a.external_id = b.external_id

	left join Temp_PTS_Infoseti_det_historical_dpd h
	...
	--31/08/2021 - Бизнес-займы
	insert into Temp_PTS_Infoseti_stg_bal 
	...
	drop table if exists Temp_PTS_Infoseti_umfo_last_fact;
	select 
	a.external_id, 
	a.r_date,
	a.total_od as od
	into Temp_PTS_Infoseti_umfo_last_fact
	...
	from Temp_PTS_Infoseti_stg_bal a
	left join Temp_PTS_Infoseti_umfo_last_fact b
	on a.external_id = b.external_id
	...
	drop table Temp_PTS_Infoseti_umfo_last_fact;


	--для учета погашенных в нулевом MoB-е

	 merge into Temp_PTS_Infoseti_stg_bal dst
	 using Temp_PTS_Infoseti_cred_reestr src 
	...
	from Temp_PTS_Infoseti_stg_bal a
	inner join Temp_PTS_Infoseti_zamorozka b
	on a.external_id = b.external_id
	...
	print ('timing, Temp_PTS_Infoseti_stg_bal ' + format(getdate(), 'HH:mm:ss'));



	--exec dbo.prc$set_debug_info @src = @srcname, @info = 'Temp_PTS_Infoseti_stg_matrix_detail';

	--сборка переходов подоговорно (присоединение t+1 MOB к t)
	drop table if exists Temp_PTS_Infoseti_stg_matrix_detail; 
	...
	into Temp_PTS_Infoseti_stg_matrix_detail

	from Temp_PTS_Infoseti_stg_bal a
	left join Temp_PTS_Infoseti_stg_bal b
	on a.external_id = b.external_id
	and a.mob = b.mob - 1
	left join Temp_PTS_Infoseti_cred_reestr c
	...
	drop index if exists idx1_stg_matrix_detail on Temp_PTS_Infoseti_stg_matrix_detail;
	create clustered index idx1_stg_matrix_detail on Temp_PTS_Infoseti_stg_matrix_detail (external_id, mob_date_to, generation, term, segment_rbp, flag_kk, mob_from, bucket_90_from, bucket_90_to);

	----------------------------------------------------

	--проверяем переходы на корректность (например, из 0 может перейти в 0 или в 1-30) и учет КК

	--exec dbo.prc$set_debug_info @src = @srcname, @info = 'CORRECTIONS Temp_PTS_Infoseti_stg_matrix_detail';

	--обновление по закрытым договорам
	update Temp_PTS_Infoseti_stg_matrix_detail
	...
	a.dpd_to = 0,
	flag_wrong_migration = 0
	from Temp_PTS_Infoseti_stg_matrix_detail a
	...
	--delete from Temp_PTS_Infoseti_stg_matrix_detail 
	...
	drop table if exists Temp_PTS_Infoseti_wrong_migrations1;
	select * 
	into Temp_PTS_Infoseti_wrong_migrations1
	from Temp_PTS_Infoseti_stg_matrix_detail a
	where exists (select 1 from Temp_PTS_Infoseti_stg_matrix_detail b
	...
	--сохраняем в отдельную таблицу 
	drop table if exists Temp_PTS_Infoseti_wrong_migrations2;
	select * 
	into Temp_PTS_Infoseti_wrong_migrations2
	from Temp_PTS_Infoseti_stg_matrix_detail a
	...
	print ('timing, Temp_PTS_Infoseti_stg_matrix_detail ' + format(getdate(), 'HH:mm:ss'));



	--drop table Temp_PTS_Infoseti_bus_stg_bal;
	drop table Temp_PTS_Infoseti_cred_end_date;
	--drop table Temp_PTS_Infoseti_stg_bal;
	...
	--Агрегат
	drop table if exists Temp_PTS_Infoseti_stg1_agg;
	select 
	...
	into Temp_PTS_Infoseti_stg1_agg
	from Temp_PTS_Infoseti_stg_matrix_detail a
	...
	print ('timing, Temp_PTS_Infoseti_stg1_agg ' + format(getdate(), 'HH:mm:ss'));



	--Для учета гашений по графику в группе Pay-off
	drop table if exists Temp_PTS_Infoseti_standart_payoff;
	...
	into Temp_PTS_Infoseti_standart_payoff
	from Temp_PTS_Infoseti_stg1_agg a
	...
	print ('timing, Temp_PTS_Infoseti_standart_payoff ' + format(getdate(), 'HH:mm:ss'));


	drop table if exists Temp_PTS_Infoseti_stg2_agg;
	select a.generation, a.term, a.segment_rbp, a.flag_kk,
	...
	into Temp_PTS_Infoseti_stg2_agg
	from Temp_PTS_Infoseti_stg1_agg a
	left join Temp_PTS_Infoseti_standart_payoff b
	...
	insert into Temp_PTS_Infoseti_stg2_agg
	...
	where not exists (select 1 from Temp_PTS_Infoseti_stg1_agg b
	...
	print ('timing, Temp_PTS_Infoseti_stg2_agg ' + format(getdate(), 'HH:mm:ss'));




	drop table if exists Temp_PTS_Infoseti_base_for_stg3_agg;

	with base as (
		select distinct a.generation, a.term, a.segment_rbp, a.flag_kk from Temp_PTS_Infoseti_cred_reestr a	
	), dt as (
		...
		select d.dpd_bucket_90 from Temp_PTS_Infoseti_det_bucket_90 d
		union all
	...
	into Temp_PTS_Infoseti_base_for_stg3_agg
	from base a
	left join dt 
	...
	print ('timing, Temp_PTS_Infoseti_base_for_stg3_agg ' + format(getdate(), 'HH:mm:ss'));



	--полная матрица групп и дат для корректного построени средних значений в Excel
	drop table if exists Temp_PTS_Infoseti_stg3_agg;
	...
	into Temp_PTS_Infoseti_stg3_agg
	from Temp_PTS_Infoseti_base_for_stg3_agg b
	left join Temp_PTS_Infoseti_stg2_agg s
	...
	--drop table Temp_PTS_Infoseti_base_for_stg3_agg;



	--удаляем лишние срезы
	drop table if exists Temp_PTS_Infoseti_for_delete;
	select a.generation, a.term, a.segment_rbp, a.flag_kk
	into Temp_PTS_Infoseti_for_delete
	from Temp_PTS_Infoseti_stg3_agg a
	group by a.generation, a.term, a.segment_rbp, a.flag_kk
	having sum(a.od_from) = 0
	;


	with a as (select * from Temp_PTS_Infoseti_stg3_agg)
	delete from a
	where exists (select 1 from Temp_PTS_Infoseti_for_delete b
				where a.generation = b.generation
				and a.term = b.term
				and a.segment_rbp = b.segment_rbp
				and a.flag_kk = b.flag_kk
	...
	print ('timing, Temp_PTS_Infoseti_stg3_agg ' + format(getdate(), 'HH:mm:ss'));
	...
	drop table if exists Temp_PTS_Infoseti_rest_model;
	...
	into Temp_PTS_Infoseti_rest_model
	from Temp_PTS_Infoseti_stg3_agg a
	...
	--ПТС31
	insert into Temp_PTS_Infoseti_rest_model
	...
	--drop table Temp_PTS_Infoseti_stg1_agg;
	--drop table Temp_PTS_Infoseti_stg2_agg;
	--drop table Temp_PTS_Infoseti_for_delete;
	--drop table Temp_PTS_Infoseti_standart_payoff;
	--drop table Temp_PTS_Infoseti_det_bucket_360;
	--drop table Temp_PTS_Infoseti_det_bucket_90;
	...
	/*
	drop table if exists Temp_PTS_Infoseti_term_generation_cnt;
	select a.term, a.generation, a.segment_rbp, count(*) as cnt, sum(a.amount) as amount
	into Temp_PTS_Infoseti_term_generation_cnt
	from Temp_PTS_Infoseti_cred_reestr a
	group by a.term, a.generation, a.segment_rbp;


	select * from Temp_PTS_Infoseti_term_generation_cnt a
	...
	*/
	...
	from Temp_PTS_Infoseti_stg3_agg a --Temp_PTS_Infoseti_stg4_agg a
	...
	*/
	...
	select * from Temp_PTS_Infoseti_stg_matrix_detail a
	...
	*/
	...
	drop table if exists Temp_PTS_Infoseti_stg_freeze_mod;
	select a.external_id, 
	--b.mob_date_to, 
	...
	into Temp_PTS_Infoseti_stg_freeze_mod
	from Temp_PTS_Infoseti_zamorozka a
	inner join Temp_PTS_Infoseti_stg_matrix_detail b
	on a.external_id = b.external_id
	...
	drop table if exists Temp_PTS_Infoseti_stg_start_default;

	select a.external_id, 
	min(b.d) as dt_default_from, eomonth(min(b.d)) as month_default_from
	into Temp_PTS_Infoseti_stg_start_default
	from Temp_PTS_Infoseti_cred_reestr a
	...
	and not exists (select 1 from Temp_PTS_Infoseti_zamorozka z where a.external_id = z.external_id and eomonth(@dt_back_test_from,-1) between z.freeze_from and z.freeze_to) --заморозка
	...
	drop table if exists Temp_PTS_Infoseti_npl_cred_with_broken_dpd;
	...
	with base as (
		...
		and not exists (select 1 from risk.stg_fcst_CMR_lite b --dwh2.dbo.dm_CMRStatBalance b (nolock)
						...
						and b.dpd = 91)
		and not exists (select 1 from Temp_PTS_Infoseti_zamorozka z where a.external_id = z.external_id and eomonth(@dt_back_test_from,-1) between z.freeze_from and z.freeze_to) --заморозка
		and not exists (select 1 from Temp_PTS_Infoseti_cred_reestr cr where a.external_id = cr.external_id and cr.flag_kk = 'BUSINESS')
		group by a.external_id
	)
	...
	into Temp_PTS_Infoseti_npl_cred_with_broken_dpd
	from base b
	...
	insert into Temp_PTS_Infoseti_stg_start_default 
	(external_id, dt_default_from, month_default_from)
	select a.external_id, a.dt_default_from, a.month_default_from
	from Temp_PTS_Infoseti_npl_cred_with_broken_dpd a
	where not exists (select 1 from Temp_PTS_Infoseti_stg_start_default b
					where a.external_id = b.external_id)
	;
	...
	--from Temp_PTS_Infoseti_bus_default_from a
	...
	from Temp_PTS_Infoseti_stg_freeze_mod a
	where exists (select 1 from Temp_PTS_Infoseti_stg_matrix_detail s 
				  where a.external_id = s.external_id
				  and s.bucket_90_from = '[07] Freeze' and s.bucket_90_to not in ('[07] Freeze', '[06] Pay-off'))
	and not exists (select 1 from Temp_PTS_Infoseti_stg_start_default d where a.external_id = d.external_id)
	;
	...
	from Temp_PTS_Infoseti_stg_matrix_detail a
	left join Temp_PTS_Infoseti_stg_start_default b
	on a.external_id = b.external_id
	inner join (select a.external_id, min(eomonth(a.freeze_dt)) as freeze_dt from Temp_PTS_Infoseti_stg2_zamorozka a group by a.external_id) z
	on a.external_id = z.external_id
	inner join Temp_PTS_Infoseti_stg_matrix_detail c
	on a.external_id = c.external_id
	...
	and a.dpd_to > 90
	and not exists (select 1 from Temp_PTS_Infoseti_stg_start_default b where a.external_id = b.external_id)

	print ('CNT without default dt = ' + format(@@rowcount,'Temp_PTS_Infoseti_0'));


	print ('timing, Temp_PTS_Infoseti_stg_start_default' + format(getdate(), 'HH:mm:ss'))
	...
	drop table if exists Temp_PTS_Infoseti_fact_due_int;
	...
	into Temp_PTS_Infoseti_fact_due_int
	from RiskDWH.Risk.stg_fcst_umfo a
	inner join Temp_PTS_Infoseti_cred_reestr b
	on a.external_id = b.external_id

	left join Temp_PTS_Infoseti_zamorozka z
	on a.external_id = z.external_id
	and a.r_date >= z.freeze_from --between z.freeze_from and z.freeze_to
	left join Temp_PTS_Infoseti_stg_start_default d
	on a.external_id = d.external_id
	and d.month_default_from <= a.r_date
	left join Temp_PTS_Infoseti_det_historical_dpd hd
	on a.external_id = hd.external_id
	and a.r_date = hd.r_date
	left join Temp_PTS_Infoseti_stg_freeze_mod fm
	on a.external_id = fm.external_id
	and a.r_date = fm.mob_date_to
	left join Temp_PTS_Infoseti_stg_matrix_detail md
	...
	print ('timing, Temp_PTS_Infoseti_fact_due_int ' + format(getdate(), 'HH:mm:ss'))




	drop table if exists Temp_PTS_Infoseti_stg_payment;
	...
	a.fines_cnl,
	a.other_cnl
	into Temp_PTS_Infoseti_stg_payment
	...
	drop table if exists Temp_PTS_Infoseti_stg_fact_od_int_pmt;
	...
	into Temp_PTS_Infoseti_stg_fact_od_int_pmt

	from Temp_PTS_Infoseti_stg_matrix_detail a
	inner join Temp_PTS_Infoseti_stg_payment b
	on a.external_id = b.external_id 
	...
	insert into Temp_PTS_Infoseti_stg_fact_od_int_pmt
	...
	sum(b.fines_cnl) as fines_cnl,
	sum(b.other_cnl) as other_cnl
	from Temp_PTS_Infoseti_stg_payment b
	inner join Temp_PTS_Infoseti_stg_matrix_detail a
	on a.external_id = b.external_id
	and eomonth(b.cdate) = a.mob_date_from
	where not exists (select 1 from Temp_PTS_Infoseti_stg_matrix_detail c where b.external_id = c.external_id and eomonth(b.cdate) = c.mob_date_to)
	...
	sum(b.other_cnl) as other_cnl
	from Temp_PTS_Infoseti_stg_payment b
	inner join Temp_PTS_Infoseti_cred_reestr a
	on a.external_id = b.external_id
	...
	print ('timing, Temp_PTS_Infoseti_stg_fact_od_int_pmt ' + format(getdate(), 'HH:mm:ss'))
	...
	drop table if exists Temp_PTS_Infoseti_stg1_NU_segment_distrib;
	...
	into Temp_PTS_Infoseti_stg1_NU_segment_distrib
	from base a
	...
	---последнее фактическое поколение, т.к. в Temp_PTS_Infoseti_stg_matrix_detail нет записей, где mob_date_to = generation
	with base as (
		select 
		...
		inner join Temp_PTS_Infoseti_cred_reestr cr
	...
	insert into Temp_PTS_Infoseti_stg1_NU_segment_distrib
	...
	drop table if exists Temp_PTS_Infoseti_stg2_NU_segment_distrib;
	...
	into Temp_PTS_Infoseti_stg2_NU_segment_distrib
	from Temp_PTS_Infoseti_stg1_NU_segment_distrib a
	...
	drop table if exists Temp_PTS_Infoseti_NU_segment_distrib
	select a.generation, a.segment_rbp, a.term, a.flag_kk, a.bucket_90_to, a.NU_segment,
	a.principal_rest_to,
	...
	into Temp_PTS_Infoseti_NU_segment_distrib
	from Temp_PTS_Infoseti_stg2_NU_segment_distrib a
	...
	left join (select distinct b.term, b.segment_rbp, b.flag_kk, b.generation from Temp_PTS_Infoseti_NU_segment_distrib b) b
	on 1=1
	), distbucks as (
	select distinct a.term, a.segment_rbp, a.flag_kk, a.generation, a.bucket_90_to from Temp_PTS_Infoseti_NU_segment_distrib a
	), base as (
	...
	from subportfs a
	left join Temp_PTS_Infoseti_NU_segment_distrib b
	on a.term = b.term
	and a.segment_rbp = b.segment_rbp
	and a.flag_kk = b.flag_kk
	...
	insert into Temp_PTS_Infoseti_NU_segment_distrib
	...
	from Temp_PTS_Infoseti_NU_segment_distrib a
	left join (
	...
	*/
	...
	drop table if exists Temp_PTS_Infoseti_UMFO_NU_segment;
	...
	into Temp_PTS_Infoseti_UMFO_NU_segment
	from RiskDWH.Risk.stg_fcst_umfo a
	...
	drop table if exists Temp_PTS_Infoseti_stg_UMFO_NU_segment;
	with base as (
		select a.external_id, cast(b.dt as date) as rdt
		from (select distinct a.external_id from Temp_PTS_Infoseti_UMFO_NU_segment a) a
		...
		from base a
		left join Temp_PTS_Infoseti_UMFO_NU_segment b
		on a.external_id = b.external_id
		and a.rdt = b.r_date
	)
	select a.external_id, a.r_date, b.NU_segment
	into Temp_PTS_Infoseti_stg_UMFO_NU_segment
	from base2 a
	left join Temp_PTS_Infoseti_UMFO_NU_segment b
	on a.external_id = b.external_id
	...
	merge into Temp_PTS_Infoseti_UMFO_NU_segment dst
	using Temp_PTS_Infoseti_stg_UMFO_NU_segment src
	...
	insert into Temp_PTS_Infoseti_UMFO_NU_segment
	...
	from Temp_PTS_Infoseti_cred_reestr a
	left join Temp_PTS_Infoseti_bankrupt b
	on a.external_id = b.external_id
	...
	where not exists (select 1 from Temp_PTS_Infoseti_UMFO_NU_segment b where a.external_id = b.external_id)
	;



	print ('timing, Temp_PTS_Infoseti_UMFO_NU_segment ' + format(getdate(), 'HH:mm:ss'))




	drop table if exists Temp_PTS_Infoseti_umfo_fact;
	select 
	a.r_date,
	a.external_id, 
	...
	into Temp_PTS_Infoseti_umfo_fact
	from RiskDWH.Risk.stg_fcst_umfo a
	inner join Temp_PTS_Infoseti_cred_reestr b
	on a.external_id = b.external_id
	left join Temp_PTS_Infoseti_det_historical_dpd d
	on a.external_id = d.external_id
	...
	print ('timing, Temp_PTS_Infoseti_umfo_fact ' + format(getdate(), 'HH:mm:ss'))


	--Штуки (кол-во договоров)
	;
	drop table if exists Temp_PTS_Infoseti_det_pieces_L1;
	select 
	RiskDWH.dbo.get_bucket_360_m( a.dpd ) as bucket_360_m, 
	sum(a.due_od) / cast(count(*) as float) as avg_od
	into Temp_PTS_Infoseti_det_pieces_L1
	from Temp_PTS_Infoseti_umfo_fact a
	inner join Temp_PTS_Infoseti_cred_reestr b
	on a.external_id = b.external_id
	where a.r_date = @rdt 
	...
	print ('timing, Temp_PTS_Infoseti_det_pieces_L1 ' + format(getdate(), 'HH:mm:ss'))



	drop table if exists Temp_PTS_Infoseti_det_pieces_L2;
	select 
	iif(b.segment_rbp = 'RBP 1', 'RBP 1', 'rest') as segment_rbp_2 ,
	RiskDWH.dbo.get_bucket_360_m( a.dpd ) as bucket_360_m, 
	sum(a.due_od ) / cast(count(*) as float) as avg_od
	into Temp_PTS_Infoseti_det_pieces_L2
	from Temp_PTS_Infoseti_umfo_fact a
	inner join Temp_PTS_Infoseti_cred_reestr b
	on a.external_id = b.external_id
	where a.r_date = @rdt 
	...
	print ('timing, Temp_PTS_Infoseti_det_pieces_L2 ' + format(getdate(), 'HH:mm:ss'))
	...
	--/* P 1-2 */
	--drop table Temp_PTS_Infoseti_bankrupt;
	----drop table Temp_PTS_Infoseti_CMR_M;
	--drop table Temp_PTS_Infoseti_cred_CMR_status;
	----drop table Temp_PTS_Infoseti_cred_reestr;
	----drop table Temp_PTS_Infoseti_det_current_params;
	--drop table Temp_PTS_Infoseti_det_historical_dpd;
	----drop table Temp_PTS_Infoseti_det_pieces_L1;
	----drop table Temp_PTS_Infoseti_det_pieces_L2;
	----drop table Temp_PTS_Infoseti_eps;
	----drop table Temp_PTS_Infoseti_fact_due_int;
	--drop table Temp_PTS_Infoseti_npl_cred_with_broken_dpd;
	----drop table Temp_PTS_Infoseti_NU_segment_distrib;
	----drop table Temp_PTS_Infoseti_rest_model;
	--drop table Temp_PTS_Infoseti_stg_bal;
	--drop table Temp_PTS_Infoseti_stg_bankrupt;
	----drop table Temp_PTS_Infoseti_stg_fact_od_int_pmt;
	----drop table Temp_PTS_Infoseti_stg_freeze_mod;
	----drop table Temp_PTS_Infoseti_stg_matrix_detail;
	----drop table Temp_PTS_Infoseti_stg_payment;
	----drop table Temp_PTS_Infoseti_stg_start_default;
	--drop table Temp_PTS_Infoseti_stg_UMFO_NU_segment;
	----drop table Temp_PTS_Infoseti_stg_write_off;
	----drop table Temp_PTS_Infoseti_stg1_NU_segment_distrib;
	--drop table Temp_PTS_Infoseti_stg1_zamorozka;
	--drop table Temp_PTS_Infoseti_stg2_eps;
	--drop table Temp_PTS_Infoseti_stg2_NU_segment_distrib;
	--drop table Temp_PTS_Infoseti_stg2_zamorozka;
	----drop table Temp_PTS_Infoseti_stg3_agg;
	----drop table Temp_PTS_Infoseti_umfo_fact;
	----drop table Temp_PTS_Infoseti_UMFO_NU_segment;
	--drop table Temp_PTS_Infoseti_wrong_migrations1;
	--drop table Temp_PTS_Infoseti_wrong_migrations2;
	----drop table Temp_PTS_Infoseti_zamorozka;
	...
	drop table if exists Temp_PTS_Infoseti_bkp_cred_reestr;
	drop table if exists Temp_PTS_Infoseti_bkp_stg3_agg;
	drop table if exists Temp_PTS_Infoseti_bkp_stg_matrix_detail;
	drop table if exists Temp_PTS_Infoseti_bkp_fact_due_int;
	drop table if exists Temp_PTS_Infoseti_bkp_stg_fact_od_int_pmt;

	select * into Temp_PTS_Infoseti_bkp_cred_reestr from Temp_PTS_Infoseti_cred_reestr;
	select * into Temp_PTS_Infoseti_bkp_stg3_agg from Temp_PTS_Infoseti_stg3_agg;
	select * into Temp_PTS_Infoseti_bkp_stg_matrix_detail from Temp_PTS_Infoseti_stg_matrix_detail a
	select * into Temp_PTS_Infoseti_bkp_fact_due_int from Temp_PTS_Infoseti_fact_due_int a
	select * into Temp_PTS_Infoseti_bkp_stg_fact_od_int_pmt from Temp_PTS_Infoseti_stg_fact_od_int_pmt a
	...
	delete from Temp_PTS_Infoseti_cred_reestr; insert into Temp_PTS_Infoseti_cred_reestr select * from Temp_PTS_Infoseti_bkp_cred_reestr;
	delete from Temp_PTS_Infoseti_stg3_agg; insert into Temp_PTS_Infoseti_stg3_agg select * from Temp_PTS_Infoseti_bkp_stg3_agg;
	delete from Temp_PTS_Infoseti_stg_matrix_detail; insert into Temp_PTS_Infoseti_stg_matrix_detail select * from Temp_PTS_Infoseti_bkp_stg_matrix_detail;
	delete from Temp_PTS_Infoseti_fact_due_int; insert into Temp_PTS_Infoseti_fact_due_int select * from Temp_PTS_Infoseti_bkp_fact_due_int;
	delete from Temp_PTS_Infoseti_stg_fact_od_int_pmt; insert into Temp_PTS_Infoseti_stg_fact_od_int_pmt select * from Temp_PTS_Infoseti_bkp_stg_fact_od_int_pmt;
	...
	*/
	...
	delete from Temp_PTS_Infoseti_cred_reestr where not (flag_kk = 'PTS_Infoseti');
	delete from Temp_PTS_Infoseti_stg_bal where not (flag_kk = 'PTS_Infoseti');
	delete from Temp_PTS_Infoseti_stg_matrix_detail where not (flag_kk = 'PTS_Infoseti');
	delete from Temp_PTS_Infoseti_wrong_migrations1 where not (flag_kk = 'PTS_Infoseti');
	delete from Temp_PTS_Infoseti_wrong_migrations2 where not (flag_kk = 'PTS_Infoseti');
	delete from Temp_PTS_Infoseti_stg1_agg where not (flag_kk = 'PTS_Infoseti');
	delete from Temp_PTS_Infoseti_standart_payoff where not (flag_kk = 'PTS_Infoseti');
	delete from Temp_PTS_Infoseti_stg2_agg where not (flag_kk = 'PTS_Infoseti');
	delete from Temp_PTS_Infoseti_base_for_stg3_agg where not (flag_kk = 'PTS_Infoseti');
	delete from Temp_PTS_Infoseti_stg3_agg where not (flag_kk = 'PTS_Infoseti');
	delete from Temp_PTS_Infoseti_for_delete where not (flag_kk = 'PTS_Infoseti');
	delete from Temp_PTS_Infoseti_fact_due_int where not (flag_kk = 'PTS_Infoseti');
	delete from Temp_PTS_Infoseti_stg_fact_od_int_pmt where not (flag_kk = 'PTS_Infoseti');
	delete from Temp_PTS_Infoseti_stg1_NU_segment_distrib where not (flag_kk = 'PTS_Infoseti');
	delete from Temp_PTS_Infoseti_stg2_NU_segment_distrib where not (flag_kk = 'PTS_Infoseti');
	delete from Temp_PTS_Infoseti_NU_segment_distrib where not (flag_kk = 'PTS_Infoseti');
	;

	-- Возвращаем им значение из справочника, которое моделируется в скрипте 
	--
	update Temp_PTS_Infoseti_cred_reestr set flag_kk = 'USUAL';
	update Temp_PTS_Infoseti_stg_bal set flag_kk = 'USUAL';
	update Temp_PTS_Infoseti_stg_matrix_detail  set flag_kk = 'USUAL';
	update Temp_PTS_Infoseti_wrong_migrations1 set flag_kk = 'USUAL';
	update Temp_PTS_Infoseti_wrong_migrations2 set flag_kk = 'USUAL';
	update Temp_PTS_Infoseti_stg1_agg set flag_kk = 'USUAL';
	update Temp_PTS_Infoseti_standart_payoff set flag_kk = 'USUAL';
	update Temp_PTS_Infoseti_stg2_agg set flag_kk = 'USUAL';
	update Temp_PTS_Infoseti_base_for_stg3_agg set flag_kk = 'USUAL';
	update Temp_PTS_Infoseti_stg3_agg set flag_kk = 'USUAL';
	update Temp_PTS_Infoseti_for_delete set flag_kk = 'USUAL';
	update Temp_PTS_Infoseti_fact_due_int set flag_kk = 'USUAL';
	update Temp_PTS_Infoseti_stg_fact_od_int_pmt set flag_kk = 'USUAL';
	update Temp_PTS_Infoseti_stg1_NU_segment_distrib set flag_kk = 'USUAL';
	update Temp_PTS_Infoseti_stg2_NU_segment_distrib set flag_kk = 'USUAL';
	update Temp_PTS_Infoseti_NU_segment_distrib set flag_kk = 'USUAL';
	--------------------------------------------------------------------------------------------------------



	drop table if exists Temp_PTS_Infoseti_det_pdp_coef;

	select 
	cast(a.segment_rbp as varchar(100)) as segment_rbp,
	cast(a.coef as float) as coef
	into Temp_PTS_Infoseti_det_pdp_coef
	from (values
	...
	--select generation, sum(vl_rub_fact) from Temp_PTS_Infoseti_virt_gens group by generation order by 1
	--select sum(vl_rub_fact) from Temp_PTS_Infoseti_virt_gens

	-------виртуальные поколения для плановых выдач
	drop table if exists Temp_PTS_Infoseti_virt_gens;
	...
	into Temp_PTS_Infoseti_virt_gens
	from (values 


('other', '2025-11-30', 24, 'USUAL', 0.00),
('other', '2025-11-30', 36, 'USUAL', 0.00),
('other', '2025-11-30', 48, 'USUAL', 3787651.00),
('PTS31_RBP4', '2025-11-30', 24, 'USUAL', 0.00),
('PTS31_RBP4', '2025-11-30', 36, 'USUAL', 0.00),
('PTS31_RBP4', '2025-11-30', 48, 'USUAL', 2980972.00),
('RBP 1', '2025-11-30', 24, 'USUAL', 320000.00),
('RBP 1', '2025-11-30', 36, 'USUAL', 0.00),
('RBP 1', '2025-11-30', 48, 'USUAL', 8574216.00),
('RBP 2', '2025-11-30', 24, 'USUAL', 0.00),
('RBP 2', '2025-11-30', 36, 'USUAL', 0.00),
('RBP 2', '2025-11-30', 48, 'USUAL', 649700.00),
('RBP 3', '2025-11-30', 24, 'USUAL', 0.00),
('RBP 3', '2025-11-30', 36, 'USUAL', 850000.00),
('RBP 3', '2025-11-30', 48, 'USUAL', 0.00),
('RBP 4', '2025-11-30', 24, 'USUAL', 0.00),
('RBP 4', '2025-11-30', 36, 'USUAL', 0.00),
('RBP 4', '2025-11-30', 48, 'USUAL', 866500.00),




('other', '2025-12-31', 24, 'USUAL', 693478.95),
('other', '2025-12-31', 36, 'USUAL', 83223.80),
('other', '2025-12-31', 48, 'USUAL', 3663706.76),
('PTS31_RBP4', '2025-12-31', 24, 'USUAL', 243353.60),
('PTS31_RBP4', '2025-12-31', 36, 'USUAL', 330304.16),
('PTS31_RBP4', '2025-12-31', 48, 'USUAL', 2550806.74),
('RBP 1', '2025-12-31', 24, 'USUAL', 816747.12),
('RBP 1', '2025-12-31', 36, 'USUAL', 609122.32),
('RBP 1', '2025-12-31', 48, 'USUAL', 12620438.85),
('RBP 2', '2025-12-31', 24, 'USUAL', 117946.48),
('RBP 2', '2025-12-31', 36, 'USUAL', 0.00),
('RBP 2', '2025-12-31', 48, 'USUAL', 943940.69),
('RBP 3', '2025-12-31', 24, 'USUAL', 208625.91),
('RBP 3', '2025-12-31', 36, 'USUAL', 446310.33),
('RBP 3', '2025-12-31', 48, 'USUAL', 1319208.62),
('RBP 4', '2025-12-31', 24, 'USUAL', 0.00),
('RBP 4', '2025-12-31', 36, 'USUAL', 39517.47),
('RBP 4', '2025-12-31', 48, 'USUAL', 313268.18),




('other', '2026-01-31', 24, 'USUAL', 693478.95),
('other', '2026-01-31', 36, 'USUAL', 83223.80),
('other', '2026-01-31', 48, 'USUAL', 3663706.76),
('PTS31_RBP4', '2026-01-31', 24, 'USUAL', 243353.60),
('PTS31_RBP4', '2026-01-31', 36, 'USUAL', 330304.16),
('PTS31_RBP4', '2026-01-31', 48, 'USUAL', 2550806.74),
('RBP 1', '2026-01-31', 24, 'USUAL', 816747.12),
('RBP 1', '2026-01-31', 36, 'USUAL', 609122.32),
('RBP 1', '2026-01-31', 48, 'USUAL', 12620438.85),
('RBP 2', '2026-01-31', 24, 'USUAL', 117946.48),
('RBP 2', '2026-01-31', 36, 'USUAL', 0.00),
('RBP 2', '2026-01-31', 48, 'USUAL', 943940.69),
('RBP 3', '2026-01-31', 24, 'USUAL', 208625.91),
('RBP 3', '2026-01-31', 36, 'USUAL', 446310.33),
('RBP 3', '2026-01-31', 48, 'USUAL', 1319208.62),
('RBP 4', '2026-01-31', 24, 'USUAL', 0.00),
('RBP 4', '2026-01-31', 36, 'USUAL', 39517.47),
('RBP 4', '2026-01-31', 48, 'USUAL', 313268.18),




('other', '2026-02-28', 24, 'USUAL', 693478.95),
('other', '2026-02-28', 36, 'USUAL', 83223.80),
('other', '2026-02-28', 48, 'USUAL', 3663706.76),
('PTS31_RBP4', '2026-02-28', 24, 'USUAL', 243353.60),
('PTS31_RBP4', '2026-02-28', 36, 'USUAL', 330304.16),
('PTS31_RBP4', '2026-02-28', 48, 'USUAL', 2550806.74),
('RBP 1', '2026-02-28', 24, 'USUAL', 816747.12),
('RBP 1', '2026-02-28', 36, 'USUAL', 609122.32),
('RBP 1', '2026-02-28', 48, 'USUAL', 12620438.85),
('RBP 2', '2026-02-28', 24, 'USUAL', 117946.48),
('RBP 2', '2026-02-28', 36, 'USUAL', 0.00),
('RBP 2', '2026-02-28', 48, 'USUAL', 943940.69),
('RBP 3', '2026-02-28', 24, 'USUAL', 208625.91),
('RBP 3', '2026-02-28', 36, 'USUAL', 446310.33),
('RBP 3', '2026-02-28', 48, 'USUAL', 1319208.62),
('RBP 4', '2026-02-28', 24, 'USUAL', 0.00),
('RBP 4', '2026-02-28', 36, 'USUAL', 39517.47),
('RBP 4', '2026-02-28', 48, 'USUAL', 313268.18),




('other', '2026-03-31', 24, 'USUAL', 693478.95),
('other', '2026-03-31', 36, 'USUAL', 83223.80),
('other', '2026-03-31', 48, 'USUAL', 3663706.76),
('PTS31_RBP4', '2026-03-31', 24, 'USUAL', 243353.60),
('PTS31_RBP4', '2026-03-31', 36, 'USUAL', 330304.16),
('PTS31_RBP4', '2026-03-31', 48, 'USUAL', 2550806.74),
('RBP 1', '2026-03-31', 24, 'USUAL', 816747.12),
('RBP 1', '2026-03-31', 36, 'USUAL', 609122.32),
('RBP 1', '2026-03-31', 48, 'USUAL', 12620438.85),
('RBP 2', '2026-03-31', 24, 'USUAL', 117946.48),
('RBP 2', '2026-03-31', 36, 'USUAL', 0.00),
('RBP 2', '2026-03-31', 48, 'USUAL', 943940.69),
('RBP 3', '2026-03-31', 24, 'USUAL', 208625.91),
('RBP 3', '2026-03-31', 36, 'USUAL', 446310.33),
('RBP 3', '2026-03-31', 48, 'USUAL', 1319208.62),
('RBP 4', '2026-03-31', 24, 'USUAL', 0.00),
('RBP 4', '2026-03-31', 36, 'USUAL', 39517.47),
('RBP 4', '2026-03-31', 48, 'USUAL', 313268.18),




('other', '2026-04-30', 24, 'USUAL', 693478.95),
('other', '2026-04-30', 36, 'USUAL', 83223.80),
('other', '2026-04-30', 48, 'USUAL', 3663706.76),
('PTS31_RBP4', '2026-04-30', 24, 'USUAL', 243353.60),
('PTS31_RBP4', '2026-04-30', 36, 'USUAL', 330304.16),
('PTS31_RBP4', '2026-04-30', 48, 'USUAL', 2550806.74),
('RBP 1', '2026-04-30', 24, 'USUAL', 816747.12),
('RBP 1', '2026-04-30', 36, 'USUAL', 609122.32),
('RBP 1', '2026-04-30', 48, 'USUAL', 12620438.85),
('RBP 2', '2026-04-30', 24, 'USUAL', 117946.48),
('RBP 2', '2026-04-30', 36, 'USUAL', 0.00),
('RBP 2', '2026-04-30', 48, 'USUAL', 943940.69),
('RBP 3', '2026-04-30', 24, 'USUAL', 208625.91),
('RBP 3', '2026-04-30', 36, 'USUAL', 446310.33),
('RBP 3', '2026-04-30', 48, 'USUAL', 1319208.62),
('RBP 4', '2026-04-30', 24, 'USUAL', 0.00),
('RBP 4', '2026-04-30', 36, 'USUAL', 39517.47),
('RBP 4', '2026-04-30', 48, 'USUAL', 313268.18),




('other', '2026-05-31', 24, 'USUAL', 693478.95),
('other', '2026-05-31', 36, 'USUAL', 83223.80),
('other', '2026-05-31', 48, 'USUAL', 3663706.76),
('PTS31_RBP4', '2026-05-31', 24, 'USUAL', 243353.60),
('PTS31_RBP4', '2026-05-31', 36, 'USUAL', 330304.16),
('PTS31_RBP4', '2026-05-31', 48, 'USUAL', 2550806.74),
('RBP 1', '2026-05-31', 24, 'USUAL', 816747.12),
('RBP 1', '2026-05-31', 36, 'USUAL', 609122.32),
('RBP 1', '2026-05-31', 48, 'USUAL', 12620438.85),
('RBP 2', '2026-05-31', 24, 'USUAL', 117946.48),
('RBP 2', '2026-05-31', 36, 'USUAL', 0.00),
('RBP 2', '2026-05-31', 48, 'USUAL', 943940.69),
('RBP 3', '2026-05-31', 24, 'USUAL', 208625.91),
('RBP 3', '2026-05-31', 36, 'USUAL', 446310.33),
('RBP 3', '2026-05-31', 48, 'USUAL', 1319208.62),
('RBP 4', '2026-05-31', 24, 'USUAL', 0.00),
('RBP 4', '2026-05-31', 36, 'USUAL', 39517.47),
('RBP 4', '2026-05-31', 48, 'USUAL', 313268.18),




('other', '2026-06-30', 24, 'USUAL', 693478.95),
('other', '2026-06-30', 36, 'USUAL', 83223.80),
('other', '2026-06-30', 48, 'USUAL', 3663706.76),
('PTS31_RBP4', '2026-06-30', 24, 'USUAL', 243353.60),
('PTS31_RBP4', '2026-06-30', 36, 'USUAL', 330304.16),
('PTS31_RBP4', '2026-06-30', 48, 'USUAL', 2550806.74),
('RBP 1', '2026-06-30', 24, 'USUAL', 816747.12),
('RBP 1', '2026-06-30', 36, 'USUAL', 609122.32),
('RBP 1', '2026-06-30', 48, 'USUAL', 12620438.85),
('RBP 2', '2026-06-30', 24, 'USUAL', 117946.48),
('RBP 2', '2026-06-30', 36, 'USUAL', 0.00),
('RBP 2', '2026-06-30', 48, 'USUAL', 943940.69),
('RBP 3', '2026-06-30', 24, 'USUAL', 208625.91),
('RBP 3', '2026-06-30', 36, 'USUAL', 446310.33),
('RBP 3', '2026-06-30', 48, 'USUAL', 1319208.62),
('RBP 4', '2026-06-30', 24, 'USUAL', 0.00),
('RBP 4', '2026-06-30', 36, 'USUAL', 39517.47),
('RBP 4', '2026-06-30', 48, 'USUAL', 313268.18),




('other', '2026-07-31', 24, 'USUAL', 693478.95),
('other', '2026-07-31', 36, 'USUAL', 83223.80),
('other', '2026-07-31', 48, 'USUAL', 3663706.76),
('PTS31_RBP4', '2026-07-31', 24, 'USUAL', 243353.60),
('PTS31_RBP4', '2026-07-31', 36, 'USUAL', 330304.16),
('PTS31_RBP4', '2026-07-31', 48, 'USUAL', 2550806.74),
('RBP 1', '2026-07-31', 24, 'USUAL', 816747.12),
('RBP 1', '2026-07-31', 36, 'USUAL', 609122.32),
('RBP 1', '2026-07-31', 48, 'USUAL', 12620438.85),
('RBP 2', '2026-07-31', 24, 'USUAL', 117946.48),
('RBP 2', '2026-07-31', 36, 'USUAL', 0.00),
('RBP 2', '2026-07-31', 48, 'USUAL', 943940.69),
('RBP 3', '2026-07-31', 24, 'USUAL', 208625.91),
('RBP 3', '2026-07-31', 36, 'USUAL', 446310.33),
('RBP 3', '2026-07-31', 48, 'USUAL', 1319208.62),
('RBP 4', '2026-07-31', 24, 'USUAL', 0.00),
('RBP 4', '2026-07-31', 36, 'USUAL', 39517.47),
('RBP 4', '2026-07-31', 48, 'USUAL', 313268.18),




('other', '2026-08-31', 24, 'USUAL', 693478.95),
('other', '2026-08-31', 36, 'USUAL', 83223.80),
('other', '2026-08-31', 48, 'USUAL', 3663706.76),
('PTS31_RBP4', '2026-08-31', 24, 'USUAL', 243353.60),
('PTS31_RBP4', '2026-08-31', 36, 'USUAL', 330304.16),
('PTS31_RBP4', '2026-08-31', 48, 'USUAL', 2550806.74),
('RBP 1', '2026-08-31', 24, 'USUAL', 816747.12),
('RBP 1', '2026-08-31', 36, 'USUAL', 609122.32),
('RBP 1', '2026-08-31', 48, 'USUAL', 12620438.85),
('RBP 2', '2026-08-31', 24, 'USUAL', 117946.48),
('RBP 2', '2026-08-31', 36, 'USUAL', 0.00),
('RBP 2', '2026-08-31', 48, 'USUAL', 943940.69),
('RBP 3', '2026-08-31', 24, 'USUAL', 208625.91),
('RBP 3', '2026-08-31', 36, 'USUAL', 446310.33),
('RBP 3', '2026-08-31', 48, 'USUAL', 1319208.62),
('RBP 4', '2026-08-31', 24, 'USUAL', 0.00),
('RBP 4', '2026-08-31', 36, 'USUAL', 39517.47),
('RBP 4', '2026-08-31', 48, 'USUAL', 313268.18),




('other', '2026-09-30', 24, 'USUAL', 693478.95),
('other', '2026-09-30', 36, 'USUAL', 83223.80),
('other', '2026-09-30', 48, 'USUAL', 3663706.76),
('PTS31_RBP4', '2026-09-30', 24, 'USUAL', 243353.60),
('PTS31_RBP4', '2026-09-30', 36, 'USUAL', 330304.16),
('PTS31_RBP4', '2026-09-30', 48, 'USUAL', 2550806.74),
('RBP 1', '2026-09-30', 24, 'USUAL', 816747.12),
('RBP 1', '2026-09-30', 36, 'USUAL', 609122.32),
('RBP 1', '2026-09-30', 48, 'USUAL', 12620438.85),
('RBP 2', '2026-09-30', 24, 'USUAL', 117946.48),
('RBP 2', '2026-09-30', 36, 'USUAL', 0.00),
('RBP 2', '2026-09-30', 48, 'USUAL', 943940.69),
('RBP 3', '2026-09-30', 24, 'USUAL', 208625.91),
('RBP 3', '2026-09-30', 36, 'USUAL', 446310.33),
('RBP 3', '2026-09-30', 48, 'USUAL', 1319208.62),
('RBP 4', '2026-09-30', 24, 'USUAL', 0.00),
('RBP 4', '2026-09-30', 36, 'USUAL', 39517.47),
('RBP 4', '2026-09-30', 48, 'USUAL', 313268.18),




('other', '2026-10-31', 24, 'USUAL', 693478.95),
('other', '2026-10-31', 36, 'USUAL', 83223.80),
('other', '2026-10-31', 48, 'USUAL', 3663706.76),
('PTS31_RBP4', '2026-10-31', 24, 'USUAL', 243353.60),
('PTS31_RBP4', '2026-10-31', 36, 'USUAL', 330304.16),
('PTS31_RBP4', '2026-10-31', 48, 'USUAL', 2550806.74),
('RBP 1', '2026-10-31', 24, 'USUAL', 816747.12),
('RBP 1', '2026-10-31', 36, 'USUAL', 609122.32),
('RBP 1', '2026-10-31', 48, 'USUAL', 12620438.85),
('RBP 2', '2026-10-31', 24, 'USUAL', 117946.48),
('RBP 2', '2026-10-31', 36, 'USUAL', 0.00),
('RBP 2', '2026-10-31', 48, 'USUAL', 943940.69),
('RBP 3', '2026-10-31', 24, 'USUAL', 208625.91),
('RBP 3', '2026-10-31', 36, 'USUAL', 446310.33),
('RBP 3', '2026-10-31', 48, 'USUAL', 1319208.62),
('RBP 4', '2026-10-31', 24, 'USUAL', 0.00),
('RBP 4', '2026-10-31', 36, 'USUAL', 39517.47),
('RBP 4', '2026-10-31', 48, 'USUAL', 313268.18),




('other', '2026-11-30', 24, 'USUAL', 693478.95),
('other', '2026-11-30', 36, 'USUAL', 83223.80),
('other', '2026-11-30', 48, 'USUAL', 3663706.76),
('PTS31_RBP4', '2026-11-30', 24, 'USUAL', 243353.60),
('PTS31_RBP4', '2026-11-30', 36, 'USUAL', 330304.16),
('PTS31_RBP4', '2026-11-30', 48, 'USUAL', 2550806.74),
('RBP 1', '2026-11-30', 24, 'USUAL', 816747.12),
('RBP 1', '2026-11-30', 36, 'USUAL', 609122.32),
('RBP 1', '2026-11-30', 48, 'USUAL', 12620438.85),
('RBP 2', '2026-11-30', 24, 'USUAL', 117946.48),
('RBP 2', '2026-11-30', 36, 'USUAL', 0.00),
('RBP 2', '2026-11-30', 48, 'USUAL', 943940.69),
('RBP 3', '2026-11-30', 24, 'USUAL', 208625.91),
('RBP 3', '2026-11-30', 36, 'USUAL', 446310.33),
('RBP 3', '2026-11-30', 48, 'USUAL', 1319208.62),
('RBP 4', '2026-11-30', 24, 'USUAL', 0.00),
('RBP 4', '2026-11-30', 36, 'USUAL', 39517.47),
('RBP 4', '2026-11-30', 48, 'USUAL', 313268.18),




('other', '2026-12-31', 24, 'USUAL', 693478.95),
('other', '2026-12-31', 36, 'USUAL', 83223.80),
('other', '2026-12-31', 48, 'USUAL', 3663706.76),
('PTS31_RBP4', '2026-12-31', 24, 'USUAL', 243353.60),
('PTS31_RBP4', '2026-12-31', 36, 'USUAL', 330304.16),
('PTS31_RBP4', '2026-12-31', 48, 'USUAL', 2550806.74),
('RBP 1', '2026-12-31', 24, 'USUAL', 816747.12),
('RBP 1', '2026-12-31', 36, 'USUAL', 609122.32),
('RBP 1', '2026-12-31', 48, 'USUAL', 12620438.85),
('RBP 2', '2026-12-31', 24, 'USUAL', 117946.48),
('RBP 2', '2026-12-31', 36, 'USUAL', 0.00),
('RBP 2', '2026-12-31', 48, 'USUAL', 943940.69),
('RBP 3', '2026-12-31', 24, 'USUAL', 208625.91),
('RBP 3', '2026-12-31', 36, 'USUAL', 446310.33),
('RBP 3', '2026-12-31', 48, 'USUAL', 1319208.62),
('RBP 4', '2026-12-31', 24, 'USUAL', 0.00),
('RBP 4', '2026-12-31', 36, 'USUAL', 39517.47),
('RBP 4', '2026-12-31', 48, 'USUAL', 313268.18),




('other', '2027-01-31', 24, 'USUAL', 762826.85),
('other', '2027-01-31', 36, 'USUAL', 91546.18),
('other', '2027-01-31', 48, 'USUAL', 4030077.44),
('PTS31_RBP4', '2027-01-31', 24, 'USUAL', 267688.96),
('PTS31_RBP4', '2027-01-31', 36, 'USUAL', 363334.58),
('PTS31_RBP4', '2027-01-31', 48, 'USUAL', 2805887.42),
('RBP 1', '2027-01-31', 24, 'USUAL', 898421.83),
('RBP 1', '2027-01-31', 36, 'USUAL', 670034.55),
('RBP 1', '2027-01-31', 48, 'USUAL', 13882482.73),
('RBP 2', '2027-01-31', 24, 'USUAL', 129741.13),
('RBP 2', '2027-01-31', 36, 'USUAL', 0.00),
('RBP 2', '2027-01-31', 48, 'USUAL', 1038334.76),
('RBP 3', '2027-01-31', 24, 'USUAL', 229488.50),
('RBP 3', '2027-01-31', 36, 'USUAL', 490941.37),
('RBP 3', '2027-01-31', 48, 'USUAL', 1451129.48),
('RBP 4', '2027-01-31', 24, 'USUAL', 0.00),
('RBP 4', '2027-01-31', 36, 'USUAL', 43469.22),
('RBP 4', '2027-01-31', 48, 'USUAL', 344594.99),




('other', '2027-02-28', 24, 'USUAL', 762826.85),
('other', '2027-02-28', 36, 'USUAL', 91546.18),
('other', '2027-02-28', 48, 'USUAL', 4030077.44),
('PTS31_RBP4', '2027-02-28', 24, 'USUAL', 267688.96),
('PTS31_RBP4', '2027-02-28', 36, 'USUAL', 363334.58),
('PTS31_RBP4', '2027-02-28', 48, 'USUAL', 2805887.42),
('RBP 1', '2027-02-28', 24, 'USUAL', 898421.83),
('RBP 1', '2027-02-28', 36, 'USUAL', 670034.55),
('RBP 1', '2027-02-28', 48, 'USUAL', 13882482.73),
('RBP 2', '2027-02-28', 24, 'USUAL', 129741.13),
('RBP 2', '2027-02-28', 36, 'USUAL', 0.00),
('RBP 2', '2027-02-28', 48, 'USUAL', 1038334.76),
('RBP 3', '2027-02-28', 24, 'USUAL', 229488.50),
('RBP 3', '2027-02-28', 36, 'USUAL', 490941.37),
('RBP 3', '2027-02-28', 48, 'USUAL', 1451129.48),
('RBP 4', '2027-02-28', 24, 'USUAL', 0.00),
('RBP 4', '2027-02-28', 36, 'USUAL', 43469.22),
('RBP 4', '2027-02-28', 48, 'USUAL', 344594.99),




('other', '2027-03-31', 24, 'USUAL', 762826.85),
('other', '2027-03-31', 36, 'USUAL', 91546.18),
('other', '2027-03-31', 48, 'USUAL', 4030077.44),
('PTS31_RBP4', '2027-03-31', 24, 'USUAL', 267688.96),
('PTS31_RBP4', '2027-03-31', 36, 'USUAL', 363334.58),
('PTS31_RBP4', '2027-03-31', 48, 'USUAL', 2805887.42),
('RBP 1', '2027-03-31', 24, 'USUAL', 898421.83),
('RBP 1', '2027-03-31', 36, 'USUAL', 670034.55),
('RBP 1', '2027-03-31', 48, 'USUAL', 13882482.73),
('RBP 2', '2027-03-31', 24, 'USUAL', 129741.13),
('RBP 2', '2027-03-31', 36, 'USUAL', 0.00),
('RBP 2', '2027-03-31', 48, 'USUAL', 1038334.76),
('RBP 3', '2027-03-31', 24, 'USUAL', 229488.50),
('RBP 3', '2027-03-31', 36, 'USUAL', 490941.37),
('RBP 3', '2027-03-31', 48, 'USUAL', 1451129.48),
('RBP 4', '2027-03-31', 24, 'USUAL', 0.00),
('RBP 4', '2027-03-31', 36, 'USUAL', 43469.22),
('RBP 4', '2027-03-31', 48, 'USUAL', 344594.99),




('other', '2027-04-30', 24, 'USUAL', 762826.85),
('other', '2027-04-30', 36, 'USUAL', 91546.18),
('other', '2027-04-30', 48, 'USUAL', 4030077.44),
('PTS31_RBP4', '2027-04-30', 24, 'USUAL', 267688.96),
('PTS31_RBP4', '2027-04-30', 36, 'USUAL', 363334.58),
('PTS31_RBP4', '2027-04-30', 48, 'USUAL', 2805887.42),
('RBP 1', '2027-04-30', 24, 'USUAL', 898421.83),
('RBP 1', '2027-04-30', 36, 'USUAL', 670034.55),
('RBP 1', '2027-04-30', 48, 'USUAL', 13882482.73),
('RBP 2', '2027-04-30', 24, 'USUAL', 129741.13),
('RBP 2', '2027-04-30', 36, 'USUAL', 0.00),
('RBP 2', '2027-04-30', 48, 'USUAL', 1038334.76),
('RBP 3', '2027-04-30', 24, 'USUAL', 229488.50),
('RBP 3', '2027-04-30', 36, 'USUAL', 490941.37),
('RBP 3', '2027-04-30', 48, 'USUAL', 1451129.48),
('RBP 4', '2027-04-30', 24, 'USUAL', 0.00),
('RBP 4', '2027-04-30', 36, 'USUAL', 43469.22),
('RBP 4', '2027-04-30', 48, 'USUAL', 344594.99),




('other', '2027-05-31', 24, 'USUAL', 762826.85),
('other', '2027-05-31', 36, 'USUAL', 91546.18),
('other', '2027-05-31', 48, 'USUAL', 4030077.44),
('PTS31_RBP4', '2027-05-31', 24, 'USUAL', 267688.96),
('PTS31_RBP4', '2027-05-31', 36, 'USUAL', 363334.58),
('PTS31_RBP4', '2027-05-31', 48, 'USUAL', 2805887.42),
('RBP 1', '2027-05-31', 24, 'USUAL', 898421.83),
('RBP 1', '2027-05-31', 36, 'USUAL', 670034.55),
('RBP 1', '2027-05-31', 48, 'USUAL', 13882482.73),
('RBP 2', '2027-05-31', 24, 'USUAL', 129741.13),
('RBP 2', '2027-05-31', 36, 'USUAL', 0.00),
('RBP 2', '2027-05-31', 48, 'USUAL', 1038334.76),
('RBP 3', '2027-05-31', 24, 'USUAL', 229488.50),
('RBP 3', '2027-05-31', 36, 'USUAL', 490941.37),
('RBP 3', '2027-05-31', 48, 'USUAL', 1451129.48),
('RBP 4', '2027-05-31', 24, 'USUAL', 0.00),
('RBP 4', '2027-05-31', 36, 'USUAL', 43469.22),
('RBP 4', '2027-05-31', 48, 'USUAL', 344594.99),




('other', '2027-06-30', 24, 'USUAL', 762826.85),
('other', '2027-06-30', 36, 'USUAL', 91546.18),
('other', '2027-06-30', 48, 'USUAL', 4030077.44),
('PTS31_RBP4', '2027-06-30', 24, 'USUAL', 267688.96),
('PTS31_RBP4', '2027-06-30', 36, 'USUAL', 363334.58),
('PTS31_RBP4', '2027-06-30', 48, 'USUAL', 2805887.42),
('RBP 1', '2027-06-30', 24, 'USUAL', 898421.83),
('RBP 1', '2027-06-30', 36, 'USUAL', 670034.55),
('RBP 1', '2027-06-30', 48, 'USUAL', 13882482.73),
('RBP 2', '2027-06-30', 24, 'USUAL', 129741.13),
('RBP 2', '2027-06-30', 36, 'USUAL', 0.00),
('RBP 2', '2027-06-30', 48, 'USUAL', 1038334.76),
('RBP 3', '2027-06-30', 24, 'USUAL', 229488.50),
('RBP 3', '2027-06-30', 36, 'USUAL', 490941.37),
('RBP 3', '2027-06-30', 48, 'USUAL', 1451129.48),
('RBP 4', '2027-06-30', 24, 'USUAL', 0.00),
('RBP 4', '2027-06-30', 36, 'USUAL', 43469.22),
('RBP 4', '2027-06-30', 48, 'USUAL', 344594.99),




('other', '2027-07-31', 24, 'USUAL', 762826.85),
('other', '2027-07-31', 36, 'USUAL', 91546.18),
('other', '2027-07-31', 48, 'USUAL', 4030077.44),
('PTS31_RBP4', '2027-07-31', 24, 'USUAL', 267688.96),
('PTS31_RBP4', '2027-07-31', 36, 'USUAL', 363334.58),
('PTS31_RBP4', '2027-07-31', 48, 'USUAL', 2805887.42),
('RBP 1', '2027-07-31', 24, 'USUAL', 898421.83),
('RBP 1', '2027-07-31', 36, 'USUAL', 670034.55),
('RBP 1', '2027-07-31', 48, 'USUAL', 13882482.73),
('RBP 2', '2027-07-31', 24, 'USUAL', 129741.13),
('RBP 2', '2027-07-31', 36, 'USUAL', 0.00),
('RBP 2', '2027-07-31', 48, 'USUAL', 1038334.76),
('RBP 3', '2027-07-31', 24, 'USUAL', 229488.50),
('RBP 3', '2027-07-31', 36, 'USUAL', 490941.37),
('RBP 3', '2027-07-31', 48, 'USUAL', 1451129.48),
('RBP 4', '2027-07-31', 24, 'USUAL', 0.00),
('RBP 4', '2027-07-31', 36, 'USUAL', 43469.22),
('RBP 4', '2027-07-31', 48, 'USUAL', 344594.99),




('other', '2027-08-31', 24, 'USUAL', 762826.85),
('other', '2027-08-31', 36, 'USUAL', 91546.18),
('other', '2027-08-31', 48, 'USUAL', 4030077.44),
('PTS31_RBP4', '2027-08-31', 24, 'USUAL', 267688.96),
('PTS31_RBP4', '2027-08-31', 36, 'USUAL', 363334.58),
('PTS31_RBP4', '2027-08-31', 48, 'USUAL', 2805887.42),
('RBP 1', '2027-08-31', 24, 'USUAL', 898421.83),
('RBP 1', '2027-08-31', 36, 'USUAL', 670034.55),
('RBP 1', '2027-08-31', 48, 'USUAL', 13882482.73),
('RBP 2', '2027-08-31', 24, 'USUAL', 129741.13),
('RBP 2', '2027-08-31', 36, 'USUAL', 0.00),
('RBP 2', '2027-08-31', 48, 'USUAL', 1038334.76),
('RBP 3', '2027-08-31', 24, 'USUAL', 229488.50),
('RBP 3', '2027-08-31', 36, 'USUAL', 490941.37),
('RBP 3', '2027-08-31', 48, 'USUAL', 1451129.48),
('RBP 4', '2027-08-31', 24, 'USUAL', 0.00),
('RBP 4', '2027-08-31', 36, 'USUAL', 43469.22),
('RBP 4', '2027-08-31', 48, 'USUAL', 344594.99),




('other', '2027-09-30', 24, 'USUAL', 762826.85),
('other', '2027-09-30', 36, 'USUAL', 91546.18),
('other', '2027-09-30', 48, 'USUAL', 4030077.44),
('PTS31_RBP4', '2027-09-30', 24, 'USUAL', 267688.96),
('PTS31_RBP4', '2027-09-30', 36, 'USUAL', 363334.58),
('PTS31_RBP4', '2027-09-30', 48, 'USUAL', 2805887.42),
('RBP 1', '2027-09-30', 24, 'USUAL', 898421.83),
('RBP 1', '2027-09-30', 36, 'USUAL', 670034.55),
('RBP 1', '2027-09-30', 48, 'USUAL', 13882482.73),
('RBP 2', '2027-09-30', 24, 'USUAL', 129741.13),
('RBP 2', '2027-09-30', 36, 'USUAL', 0.00),
('RBP 2', '2027-09-30', 48, 'USUAL', 1038334.76),
('RBP 3', '2027-09-30', 24, 'USUAL', 229488.50),
('RBP 3', '2027-09-30', 36, 'USUAL', 490941.37),
('RBP 3', '2027-09-30', 48, 'USUAL', 1451129.48),
('RBP 4', '2027-09-30', 24, 'USUAL', 0.00),
('RBP 4', '2027-09-30', 36, 'USUAL', 43469.22),
('RBP 4', '2027-09-30', 48, 'USUAL', 344594.99),




('other', '2027-10-31', 24, 'USUAL', 762826.85),
('other', '2027-10-31', 36, 'USUAL', 91546.18),
('other', '2027-10-31', 48, 'USUAL', 4030077.44),
('PTS31_RBP4', '2027-10-31', 24, 'USUAL', 267688.96),
('PTS31_RBP4', '2027-10-31', 36, 'USUAL', 363334.58),
('PTS31_RBP4', '2027-10-31', 48, 'USUAL', 2805887.42),
('RBP 1', '2027-10-31', 24, 'USUAL', 898421.83),
('RBP 1', '2027-10-31', 36, 'USUAL', 670034.55),
('RBP 1', '2027-10-31', 48, 'USUAL', 13882482.73),
('RBP 2', '2027-10-31', 24, 'USUAL', 129741.13),
('RBP 2', '2027-10-31', 36, 'USUAL', 0.00),
('RBP 2', '2027-10-31', 48, 'USUAL', 1038334.76),
('RBP 3', '2027-10-31', 24, 'USUAL', 229488.50),
('RBP 3', '2027-10-31', 36, 'USUAL', 490941.37),
('RBP 3', '2027-10-31', 48, 'USUAL', 1451129.48),
('RBP 4', '2027-10-31', 24, 'USUAL', 0.00),
('RBP 4', '2027-10-31', 36, 'USUAL', 43469.22),
('RBP 4', '2027-10-31', 48, 'USUAL', 344594.99),




('other', '2027-11-30', 24, 'USUAL', 762826.85),
('other', '2027-11-30', 36, 'USUAL', 91546.18),
('other', '2027-11-30', 48, 'USUAL', 4030077.44),
('PTS31_RBP4', '2027-11-30', 24, 'USUAL', 267688.96),
('PTS31_RBP4', '2027-11-30', 36, 'USUAL', 363334.58),
('PTS31_RBP4', '2027-11-30', 48, 'USUAL', 2805887.42),
('RBP 1', '2027-11-30', 24, 'USUAL', 898421.83),
('RBP 1', '2027-11-30', 36, 'USUAL', 670034.55),
('RBP 1', '2027-11-30', 48, 'USUAL', 13882482.73),
('RBP 2', '2027-11-30', 24, 'USUAL', 129741.13),
('RBP 2', '2027-11-30', 36, 'USUAL', 0.00),
('RBP 2', '2027-11-30', 48, 'USUAL', 1038334.76),
('RBP 3', '2027-11-30', 24, 'USUAL', 229488.50),
('RBP 3', '2027-11-30', 36, 'USUAL', 490941.37),
('RBP 3', '2027-11-30', 48, 'USUAL', 1451129.48),
('RBP 4', '2027-11-30', 24, 'USUAL', 0.00),
('RBP 4', '2027-11-30', 36, 'USUAL', 43469.22),
('RBP 4', '2027-11-30', 48, 'USUAL', 344594.99),




('other', '2027-12-31', 24, 'USUAL', 762826.85),
('other', '2027-12-31', 36, 'USUAL', 91546.18),
('other', '2027-12-31', 48, 'USUAL', 4030077.44),
('PTS31_RBP4', '2027-12-31', 24, 'USUAL', 267688.96),
('PTS31_RBP4', '2027-12-31', 36, 'USUAL', 363334.58),
('PTS31_RBP4', '2027-12-31', 48, 'USUAL', 2805887.42),
('RBP 1', '2027-12-31', 24, 'USUAL', 898421.83),
('RBP 1', '2027-12-31', 36, 'USUAL', 670034.55),
('RBP 1', '2027-12-31', 48, 'USUAL', 13882482.73),
('RBP 2', '2027-12-31', 24, 'USUAL', 129741.13),
('RBP 2', '2027-12-31', 36, 'USUAL', 0.00),
('RBP 2', '2027-12-31', 48, 'USUAL', 1038334.76),
('RBP 3', '2027-12-31', 24, 'USUAL', 229488.50),
('RBP 3', '2027-12-31', 36, 'USUAL', 490941.37),
('RBP 3', '2027-12-31', 48, 'USUAL', 1451129.48),
('RBP 4', '2027-12-31', 24, 'USUAL', 0.00),
('RBP 4', '2027-12-31', 36, 'USUAL', 43469.22),
('RBP 4', '2027-12-31', 48, 'USUAL', 344594.99),




('other', '2028-01-31', 24, 'USUAL', 839109.53),
('other', '2028-01-31', 36, 'USUAL', 100700.79),
('other', '2028-01-31', 48, 'USUAL', 4433085.18),
('PTS31_RBP4', '2028-01-31', 24, 'USUAL', 294457.86),
('PTS31_RBP4', '2028-01-31', 36, 'USUAL', 399668.03),
('PTS31_RBP4', '2028-01-31', 48, 'USUAL', 3086476.16),
('RBP 1', '2028-01-31', 24, 'USUAL', 988264.02),
('RBP 1', '2028-01-31', 36, 'USUAL', 737038.01),
('RBP 1', '2028-01-31', 48, 'USUAL', 15270731.01),
('RBP 2', '2028-01-31', 24, 'USUAL', 142715.24),
('RBP 2', '2028-01-31', 36, 'USUAL', 0.00),
('RBP 2', '2028-01-31', 48, 'USUAL', 1142168.24),
('RBP 3', '2028-01-31', 24, 'USUAL', 252437.35),
('RBP 3', '2028-01-31', 36, 'USUAL', 540035.50),
('RBP 3', '2028-01-31', 48, 'USUAL', 1596242.43),
('RBP 4', '2028-01-31', 24, 'USUAL', 0.00),
('RBP 4', '2028-01-31', 36, 'USUAL', 47816.14),
('RBP 4', '2028-01-31', 48, 'USUAL', 379054.49),




('other', '2028-02-29', 24, 'USUAL', 839109.53),
('other', '2028-02-29', 36, 'USUAL', 100700.79),
('other', '2028-02-29', 48, 'USUAL', 4433085.18),
('PTS31_RBP4', '2028-02-29', 24, 'USUAL', 294457.86),
('PTS31_RBP4', '2028-02-29', 36, 'USUAL', 399668.03),
('PTS31_RBP4', '2028-02-29', 48, 'USUAL', 3086476.16),
('RBP 1', '2028-02-29', 24, 'USUAL', 988264.02),
('RBP 1', '2028-02-29', 36, 'USUAL', 737038.01),
('RBP 1', '2028-02-29', 48, 'USUAL', 15270731.01),
('RBP 2', '2028-02-29', 24, 'USUAL', 142715.24),
('RBP 2', '2028-02-29', 36, 'USUAL', 0.00),
('RBP 2', '2028-02-29', 48, 'USUAL', 1142168.24),
('RBP 3', '2028-02-29', 24, 'USUAL', 252437.35),
('RBP 3', '2028-02-29', 36, 'USUAL', 540035.50),
('RBP 3', '2028-02-29', 48, 'USUAL', 1596242.43),
('RBP 4', '2028-02-29', 24, 'USUAL', 0.00),
('RBP 4', '2028-02-29', 36, 'USUAL', 47816.14),
('RBP 4', '2028-02-29', 48, 'USUAL', 379054.49),




('other', '2028-03-31', 24, 'USUAL', 839109.53),
('other', '2028-03-31', 36, 'USUAL', 100700.79),
('other', '2028-03-31', 48, 'USUAL', 4433085.18),
('PTS31_RBP4', '2028-03-31', 24, 'USUAL', 294457.86),
('PTS31_RBP4', '2028-03-31', 36, 'USUAL', 399668.03),
('PTS31_RBP4', '2028-03-31', 48, 'USUAL', 3086476.16),
('RBP 1', '2028-03-31', 24, 'USUAL', 988264.02),
('RBP 1', '2028-03-31', 36, 'USUAL', 737038.01),
('RBP 1', '2028-03-31', 48, 'USUAL', 15270731.01),
('RBP 2', '2028-03-31', 24, 'USUAL', 142715.24),
('RBP 2', '2028-03-31', 36, 'USUAL', 0.00),
('RBP 2', '2028-03-31', 48, 'USUAL', 1142168.24),
('RBP 3', '2028-03-31', 24, 'USUAL', 252437.35),
('RBP 3', '2028-03-31', 36, 'USUAL', 540035.50),
('RBP 3', '2028-03-31', 48, 'USUAL', 1596242.43),
('RBP 4', '2028-03-31', 24, 'USUAL', 0.00),
('RBP 4', '2028-03-31', 36, 'USUAL', 47816.14),
('RBP 4', '2028-03-31', 48, 'USUAL', 379054.49),




('other', '2028-04-30', 24, 'USUAL', 839109.53),
('other', '2028-04-30', 36, 'USUAL', 100700.79),
('other', '2028-04-30', 48, 'USUAL', 4433085.18),
('PTS31_RBP4', '2028-04-30', 24, 'USUAL', 294457.86),
('PTS31_RBP4', '2028-04-30', 36, 'USUAL', 399668.03),
('PTS31_RBP4', '2028-04-30', 48, 'USUAL', 3086476.16),
('RBP 1', '2028-04-30', 24, 'USUAL', 988264.02),
('RBP 1', '2028-04-30', 36, 'USUAL', 737038.01),
('RBP 1', '2028-04-30', 48, 'USUAL', 15270731.01),
('RBP 2', '2028-04-30', 24, 'USUAL', 142715.24),
('RBP 2', '2028-04-30', 36, 'USUAL', 0.00),
('RBP 2', '2028-04-30', 48, 'USUAL', 1142168.24),
('RBP 3', '2028-04-30', 24, 'USUAL', 252437.35),
('RBP 3', '2028-04-30', 36, 'USUAL', 540035.50),
('RBP 3', '2028-04-30', 48, 'USUAL', 1596242.43),
('RBP 4', '2028-04-30', 24, 'USUAL', 0.00),
('RBP 4', '2028-04-30', 36, 'USUAL', 47816.14),
('RBP 4', '2028-04-30', 48, 'USUAL', 379054.49),




('other', '2028-05-31', 24, 'USUAL', 839109.53),
('other', '2028-05-31', 36, 'USUAL', 100700.79),
('other', '2028-05-31', 48, 'USUAL', 4433085.18),
('PTS31_RBP4', '2028-05-31', 24, 'USUAL', 294457.86),
('PTS31_RBP4', '2028-05-31', 36, 'USUAL', 399668.03),
('PTS31_RBP4', '2028-05-31', 48, 'USUAL', 3086476.16),
('RBP 1', '2028-05-31', 24, 'USUAL', 988264.02),
('RBP 1', '2028-05-31', 36, 'USUAL', 737038.01),
('RBP 1', '2028-05-31', 48, 'USUAL', 15270731.01),
('RBP 2', '2028-05-31', 24, 'USUAL', 142715.24),
('RBP 2', '2028-05-31', 36, 'USUAL', 0.00),
('RBP 2', '2028-05-31', 48, 'USUAL', 1142168.24),
('RBP 3', '2028-05-31', 24, 'USUAL', 252437.35),
('RBP 3', '2028-05-31', 36, 'USUAL', 540035.50),
('RBP 3', '2028-05-31', 48, 'USUAL', 1596242.43),
('RBP 4', '2028-05-31', 24, 'USUAL', 0.00),
('RBP 4', '2028-05-31', 36, 'USUAL', 47816.14),
('RBP 4', '2028-05-31', 48, 'USUAL', 379054.49),




('other', '2028-06-30', 24, 'USUAL', 839109.53),
('other', '2028-06-30', 36, 'USUAL', 100700.79),
('other', '2028-06-30', 48, 'USUAL', 4433085.18),
('PTS31_RBP4', '2028-06-30', 24, 'USUAL', 294457.86),
('PTS31_RBP4', '2028-06-30', 36, 'USUAL', 399668.03),
('PTS31_RBP4', '2028-06-30', 48, 'USUAL', 3086476.16),
('RBP 1', '2028-06-30', 24, 'USUAL', 988264.02),
('RBP 1', '2028-06-30', 36, 'USUAL', 737038.01),
('RBP 1', '2028-06-30', 48, 'USUAL', 15270731.01),
('RBP 2', '2028-06-30', 24, 'USUAL', 142715.24),
('RBP 2', '2028-06-30', 36, 'USUAL', 0.00),
('RBP 2', '2028-06-30', 48, 'USUAL', 1142168.24),
('RBP 3', '2028-06-30', 24, 'USUAL', 252437.35),
('RBP 3', '2028-06-30', 36, 'USUAL', 540035.50),
('RBP 3', '2028-06-30', 48, 'USUAL', 1596242.43),
('RBP 4', '2028-06-30', 24, 'USUAL', 0.00),
('RBP 4', '2028-06-30', 36, 'USUAL', 47816.14),
('RBP 4', '2028-06-30', 48, 'USUAL', 379054.49),




('other', '2028-07-31', 24, 'USUAL', 839109.53),
('other', '2028-07-31', 36, 'USUAL', 100700.79),
('other', '2028-07-31', 48, 'USUAL', 4433085.18),
('PTS31_RBP4', '2028-07-31', 24, 'USUAL', 294457.86),
('PTS31_RBP4', '2028-07-31', 36, 'USUAL', 399668.03),
('PTS31_RBP4', '2028-07-31', 48, 'USUAL', 3086476.16),
('RBP 1', '2028-07-31', 24, 'USUAL', 988264.02),
('RBP 1', '2028-07-31', 36, 'USUAL', 737038.01),
('RBP 1', '2028-07-31', 48, 'USUAL', 15270731.01),
('RBP 2', '2028-07-31', 24, 'USUAL', 142715.24),
('RBP 2', '2028-07-31', 36, 'USUAL', 0.00),
('RBP 2', '2028-07-31', 48, 'USUAL', 1142168.24),
('RBP 3', '2028-07-31', 24, 'USUAL', 252437.35),
('RBP 3', '2028-07-31', 36, 'USUAL', 540035.50),
('RBP 3', '2028-07-31', 48, 'USUAL', 1596242.43),
('RBP 4', '2028-07-31', 24, 'USUAL', 0.00),
('RBP 4', '2028-07-31', 36, 'USUAL', 47816.14),
('RBP 4', '2028-07-31', 48, 'USUAL', 379054.49),




('other', '2028-08-31', 24, 'USUAL', 839109.53),
('other', '2028-08-31', 36, 'USUAL', 100700.79),
('other', '2028-08-31', 48, 'USUAL', 4433085.18),
('PTS31_RBP4', '2028-08-31', 24, 'USUAL', 294457.86),
('PTS31_RBP4', '2028-08-31', 36, 'USUAL', 399668.03),
('PTS31_RBP4', '2028-08-31', 48, 'USUAL', 3086476.16),
('RBP 1', '2028-08-31', 24, 'USUAL', 988264.02),
('RBP 1', '2028-08-31', 36, 'USUAL', 737038.01),
('RBP 1', '2028-08-31', 48, 'USUAL', 15270731.01),
('RBP 2', '2028-08-31', 24, 'USUAL', 142715.24),
('RBP 2', '2028-08-31', 36, 'USUAL', 0.00),
('RBP 2', '2028-08-31', 48, 'USUAL', 1142168.24),
('RBP 3', '2028-08-31', 24, 'USUAL', 252437.35),
('RBP 3', '2028-08-31', 36, 'USUAL', 540035.50),
('RBP 3', '2028-08-31', 48, 'USUAL', 1596242.43),
('RBP 4', '2028-08-31', 24, 'USUAL', 0.00),
('RBP 4', '2028-08-31', 36, 'USUAL', 47816.14),
('RBP 4', '2028-08-31', 48, 'USUAL', 379054.49),




('other', '2028-09-30', 24, 'USUAL', 839109.53),
('other', '2028-09-30', 36, 'USUAL', 100700.79),
('other', '2028-09-30', 48, 'USUAL', 4433085.18),
('PTS31_RBP4', '2028-09-30', 24, 'USUAL', 294457.86),
('PTS31_RBP4', '2028-09-30', 36, 'USUAL', 399668.03),
('PTS31_RBP4', '2028-09-30', 48, 'USUAL', 3086476.16),
('RBP 1', '2028-09-30', 24, 'USUAL', 988264.02),
('RBP 1', '2028-09-30', 36, 'USUAL', 737038.01),
('RBP 1', '2028-09-30', 48, 'USUAL', 15270731.01),
('RBP 2', '2028-09-30', 24, 'USUAL', 142715.24),
('RBP 2', '2028-09-30', 36, 'USUAL', 0.00),
('RBP 2', '2028-09-30', 48, 'USUAL', 1142168.24),
('RBP 3', '2028-09-30', 24, 'USUAL', 252437.35),
('RBP 3', '2028-09-30', 36, 'USUAL', 540035.50),
('RBP 3', '2028-09-30', 48, 'USUAL', 1596242.43),
('RBP 4', '2028-09-30', 24, 'USUAL', 0.00),
('RBP 4', '2028-09-30', 36, 'USUAL', 47816.14),
('RBP 4', '2028-09-30', 48, 'USUAL', 379054.49),




('other', '2028-10-31', 24, 'USUAL', 839109.53),
('other', '2028-10-31', 36, 'USUAL', 100700.79),
('other', '2028-10-31', 48, 'USUAL', 4433085.18),
('PTS31_RBP4', '2028-10-31', 24, 'USUAL', 294457.86),
('PTS31_RBP4', '2028-10-31', 36, 'USUAL', 399668.03),
('PTS31_RBP4', '2028-10-31', 48, 'USUAL', 3086476.16),
('RBP 1', '2028-10-31', 24, 'USUAL', 988264.02),
('RBP 1', '2028-10-31', 36, 'USUAL', 737038.01),
('RBP 1', '2028-10-31', 48, 'USUAL', 15270731.01),
('RBP 2', '2028-10-31', 24, 'USUAL', 142715.24),
('RBP 2', '2028-10-31', 36, 'USUAL', 0.00),
('RBP 2', '2028-10-31', 48, 'USUAL', 1142168.24),
('RBP 3', '2028-10-31', 24, 'USUAL', 252437.35),
('RBP 3', '2028-10-31', 36, 'USUAL', 540035.50),
('RBP 3', '2028-10-31', 48, 'USUAL', 1596242.43),
('RBP 4', '2028-10-31', 24, 'USUAL', 0.00),
('RBP 4', '2028-10-31', 36, 'USUAL', 47816.14),
('RBP 4', '2028-10-31', 48, 'USUAL', 379054.49),




('other', '2028-11-30', 24, 'USUAL', 839109.53),
('other', '2028-11-30', 36, 'USUAL', 100700.79),
('other', '2028-11-30', 48, 'USUAL', 4433085.18),
('PTS31_RBP4', '2028-11-30', 24, 'USUAL', 294457.86),
('PTS31_RBP4', '2028-11-30', 36, 'USUAL', 399668.03),
('PTS31_RBP4', '2028-11-30', 48, 'USUAL', 3086476.16),
('RBP 1', '2028-11-30', 24, 'USUAL', 988264.02),
('RBP 1', '2028-11-30', 36, 'USUAL', 737038.01),
('RBP 1', '2028-11-30', 48, 'USUAL', 15270731.01),
('RBP 2', '2028-11-30', 24, 'USUAL', 142715.24),
('RBP 2', '2028-11-30', 36, 'USUAL', 0.00),
('RBP 2', '2028-11-30', 48, 'USUAL', 1142168.24),
('RBP 3', '2028-11-30', 24, 'USUAL', 252437.35),
('RBP 3', '2028-11-30', 36, 'USUAL', 540035.50),
('RBP 3', '2028-11-30', 48, 'USUAL', 1596242.43),
('RBP 4', '2028-11-30', 24, 'USUAL', 0.00),
('RBP 4', '2028-11-30', 36, 'USUAL', 47816.14),
('RBP 4', '2028-11-30', 48, 'USUAL', 379054.49),




('other', '2028-12-31', 24, 'USUAL', 839109.53),
('other', '2028-12-31', 36, 'USUAL', 100700.79),
('other', '2028-12-31', 48, 'USUAL', 4433085.18),
('PTS31_RBP4', '2028-12-31', 24, 'USUAL', 294457.86),
('PTS31_RBP4', '2028-12-31', 36, 'USUAL', 399668.03),
('PTS31_RBP4', '2028-12-31', 48, 'USUAL', 3086476.16),
('RBP 1', '2028-12-31', 24, 'USUAL', 988264.02),
('RBP 1', '2028-12-31', 36, 'USUAL', 737038.01),
('RBP 1', '2028-12-31', 48, 'USUAL', 15270731.01),
('RBP 2', '2028-12-31', 24, 'USUAL', 142715.24),
('RBP 2', '2028-12-31', 36, 'USUAL', 0.00),
('RBP 2', '2028-12-31', 48, 'USUAL', 1142168.24),
('RBP 3', '2028-12-31', 24, 'USUAL', 252437.35),
('RBP 3', '2028-12-31', 36, 'USUAL', 540035.50),
('RBP 3', '2028-12-31', 48, 'USUAL', 1596242.43),
('RBP 4', '2028-12-31', 24, 'USUAL', 0.00),
('RBP 4', '2028-12-31', 36, 'USUAL', 47816.14),
('RBP 4', '2028-12-31', 48, 'USUAL', 379054.49),




('other', '2029-01-31', 24, 'USUAL', 923020.49),
('other', '2029-01-31', 36, 'USUAL', 110770.87),
('other', '2029-01-31', 48, 'USUAL', 4876393.70),
('PTS31_RBP4', '2029-01-31', 24, 'USUAL', 323903.64),
('PTS31_RBP4', '2029-01-31', 36, 'USUAL', 439634.84),
('PTS31_RBP4', '2029-01-31', 48, 'USUAL', 3395123.78),
('RBP 1', '2029-01-31', 24, 'USUAL', 1087090.42),
('RBP 1', '2029-01-31', 36, 'USUAL', 810741.81),
('RBP 1', '2029-01-31', 48, 'USUAL', 16797804.11),
('RBP 2', '2029-01-31', 24, 'USUAL', 156986.77),
('RBP 2', '2029-01-31', 36, 'USUAL', 0.00),
('RBP 2', '2029-01-31', 48, 'USUAL', 1256385.06),
('RBP 3', '2029-01-31', 24, 'USUAL', 277681.09),
('RBP 3', '2029-01-31', 36, 'USUAL', 594039.05),
('RBP 3', '2029-01-31', 48, 'USUAL', 1755866.68),
('RBP 4', '2029-01-31', 24, 'USUAL', 0.00),
('RBP 4', '2029-01-31', 36, 'USUAL', 52597.76),
('RBP 4', '2029-01-31', 48, 'USUAL', 416959.94),




('other', '2029-02-28', 24, 'USUAL', 923020.49),
('other', '2029-02-28', 36, 'USUAL', 110770.87),
('other', '2029-02-28', 48, 'USUAL', 4876393.70),
('PTS31_RBP4', '2029-02-28', 24, 'USUAL', 323903.64),
('PTS31_RBP4', '2029-02-28', 36, 'USUAL', 439634.84),
('PTS31_RBP4', '2029-02-28', 48, 'USUAL', 3395123.78),
('RBP 1', '2029-02-28', 24, 'USUAL', 1087090.42),
('RBP 1', '2029-02-28', 36, 'USUAL', 810741.81),
('RBP 1', '2029-02-28', 48, 'USUAL', 16797804.11),
('RBP 2', '2029-02-28', 24, 'USUAL', 156986.77),
('RBP 2', '2029-02-28', 36, 'USUAL', 0.00),
('RBP 2', '2029-02-28', 48, 'USUAL', 1256385.06),
('RBP 3', '2029-02-28', 24, 'USUAL', 277681.09),
('RBP 3', '2029-02-28', 36, 'USUAL', 594039.05),
('RBP 3', '2029-02-28', 48, 'USUAL', 1755866.68),
('RBP 4', '2029-02-28', 24, 'USUAL', 0.00),
('RBP 4', '2029-02-28', 36, 'USUAL', 52597.76),
('RBP 4', '2029-02-28', 48, 'USUAL', 416959.94),




('other', '2029-03-31', 24, 'USUAL', 923020.49),
('other', '2029-03-31', 36, 'USUAL', 110770.87),
('other', '2029-03-31', 48, 'USUAL', 4876393.70),
('PTS31_RBP4', '2029-03-31', 24, 'USUAL', 323903.64),
('PTS31_RBP4', '2029-03-31', 36, 'USUAL', 439634.84),
('PTS31_RBP4', '2029-03-31', 48, 'USUAL', 3395123.78),
('RBP 1', '2029-03-31', 24, 'USUAL', 1087090.42),
('RBP 1', '2029-03-31', 36, 'USUAL', 810741.81),
('RBP 1', '2029-03-31', 48, 'USUAL', 16797804.11),
('RBP 2', '2029-03-31', 24, 'USUAL', 156986.77),
('RBP 2', '2029-03-31', 36, 'USUAL', 0.00),
('RBP 2', '2029-03-31', 48, 'USUAL', 1256385.06),
('RBP 3', '2029-03-31', 24, 'USUAL', 277681.09),
('RBP 3', '2029-03-31', 36, 'USUAL', 594039.05),
('RBP 3', '2029-03-31', 48, 'USUAL', 1755866.68),
('RBP 4', '2029-03-31', 24, 'USUAL', 0.00),
('RBP 4', '2029-03-31', 36, 'USUAL', 52597.76),
('RBP 4', '2029-03-31', 48, 'USUAL', 416959.94),




('other', '2029-04-30', 24, 'USUAL', 923020.49),
('other', '2029-04-30', 36, 'USUAL', 110770.87),
('other', '2029-04-30', 48, 'USUAL', 4876393.70),
('PTS31_RBP4', '2029-04-30', 24, 'USUAL', 323903.64),
('PTS31_RBP4', '2029-04-30', 36, 'USUAL', 439634.84),
('PTS31_RBP4', '2029-04-30', 48, 'USUAL', 3395123.78),
('RBP 1', '2029-04-30', 24, 'USUAL', 1087090.42),
('RBP 1', '2029-04-30', 36, 'USUAL', 810741.81),
('RBP 1', '2029-04-30', 48, 'USUAL', 16797804.11),
('RBP 2', '2029-04-30', 24, 'USUAL', 156986.77),
('RBP 2', '2029-04-30', 36, 'USUAL', 0.00),
('RBP 2', '2029-04-30', 48, 'USUAL', 1256385.06),
('RBP 3', '2029-04-30', 24, 'USUAL', 277681.09),
('RBP 3', '2029-04-30', 36, 'USUAL', 594039.05),
('RBP 3', '2029-04-30', 48, 'USUAL', 1755866.68),
('RBP 4', '2029-04-30', 24, 'USUAL', 0.00),
('RBP 4', '2029-04-30', 36, 'USUAL', 52597.76),
('RBP 4', '2029-04-30', 48, 'USUAL', 416959.94),




('other', '2029-05-31', 24, 'USUAL', 923020.49),
('other', '2029-05-31', 36, 'USUAL', 110770.87),
('other', '2029-05-31', 48, 'USUAL', 4876393.70),
('PTS31_RBP4', '2029-05-31', 24, 'USUAL', 323903.64),
('PTS31_RBP4', '2029-05-31', 36, 'USUAL', 439634.84),
('PTS31_RBP4', '2029-05-31', 48, 'USUAL', 3395123.78),
('RBP 1', '2029-05-31', 24, 'USUAL', 1087090.42),
('RBP 1', '2029-05-31', 36, 'USUAL', 810741.81),
('RBP 1', '2029-05-31', 48, 'USUAL', 16797804.11),
('RBP 2', '2029-05-31', 24, 'USUAL', 156986.77),
('RBP 2', '2029-05-31', 36, 'USUAL', 0.00),
('RBP 2', '2029-05-31', 48, 'USUAL', 1256385.06),
('RBP 3', '2029-05-31', 24, 'USUAL', 277681.09),
('RBP 3', '2029-05-31', 36, 'USUAL', 594039.05),
('RBP 3', '2029-05-31', 48, 'USUAL', 1755866.68),
('RBP 4', '2029-05-31', 24, 'USUAL', 0.00),
('RBP 4', '2029-05-31', 36, 'USUAL', 52597.76),
('RBP 4', '2029-05-31', 48, 'USUAL', 416959.94),




('other', '2029-06-30', 24, 'USUAL', 923020.49),
('other', '2029-06-30', 36, 'USUAL', 110770.87),
('other', '2029-06-30', 48, 'USUAL', 4876393.70),
('PTS31_RBP4', '2029-06-30', 24, 'USUAL', 323903.64),
('PTS31_RBP4', '2029-06-30', 36, 'USUAL', 439634.84),
('PTS31_RBP4', '2029-06-30', 48, 'USUAL', 3395123.78),
('RBP 1', '2029-06-30', 24, 'USUAL', 1087090.42),
('RBP 1', '2029-06-30', 36, 'USUAL', 810741.81),
('RBP 1', '2029-06-30', 48, 'USUAL', 16797804.11),
('RBP 2', '2029-06-30', 24, 'USUAL', 156986.77),
('RBP 2', '2029-06-30', 36, 'USUAL', 0.00),
('RBP 2', '2029-06-30', 48, 'USUAL', 1256385.06),
('RBP 3', '2029-06-30', 24, 'USUAL', 277681.09),
('RBP 3', '2029-06-30', 36, 'USUAL', 594039.05),
('RBP 3', '2029-06-30', 48, 'USUAL', 1755866.68),
('RBP 4', '2029-06-30', 24, 'USUAL', 0.00),
('RBP 4', '2029-06-30', 36, 'USUAL', 52597.76),
('RBP 4', '2029-06-30', 48, 'USUAL', 416959.94),




('other', '2029-07-31', 24, 'USUAL', 923020.49),
('other', '2029-07-31', 36, 'USUAL', 110770.87),
('other', '2029-07-31', 48, 'USUAL', 4876393.70),
('PTS31_RBP4', '2029-07-31', 24, 'USUAL', 323903.64),
('PTS31_RBP4', '2029-07-31', 36, 'USUAL', 439634.84),
('PTS31_RBP4', '2029-07-31', 48, 'USUAL', 3395123.78),
('RBP 1', '2029-07-31', 24, 'USUAL', 1087090.42),
('RBP 1', '2029-07-31', 36, 'USUAL', 810741.81),
('RBP 1', '2029-07-31', 48, 'USUAL', 16797804.11),
('RBP 2', '2029-07-31', 24, 'USUAL', 156986.77),
('RBP 2', '2029-07-31', 36, 'USUAL', 0.00),
('RBP 2', '2029-07-31', 48, 'USUAL', 1256385.06),
('RBP 3', '2029-07-31', 24, 'USUAL', 277681.09),
('RBP 3', '2029-07-31', 36, 'USUAL', 594039.05),
('RBP 3', '2029-07-31', 48, 'USUAL', 1755866.68),
('RBP 4', '2029-07-31', 24, 'USUAL', 0.00),
('RBP 4', '2029-07-31', 36, 'USUAL', 52597.76),
('RBP 4', '2029-07-31', 48, 'USUAL', 416959.94),




('other', '2029-08-31', 24, 'USUAL', 923020.49),
('other', '2029-08-31', 36, 'USUAL', 110770.87),
('other', '2029-08-31', 48, 'USUAL', 4876393.70),
('PTS31_RBP4', '2029-08-31', 24, 'USUAL', 323903.64),
('PTS31_RBP4', '2029-08-31', 36, 'USUAL', 439634.84),
('PTS31_RBP4', '2029-08-31', 48, 'USUAL', 3395123.78),
('RBP 1', '2029-08-31', 24, 'USUAL', 1087090.42),
('RBP 1', '2029-08-31', 36, 'USUAL', 810741.81),
('RBP 1', '2029-08-31', 48, 'USUAL', 16797804.11),
('RBP 2', '2029-08-31', 24, 'USUAL', 156986.77),
('RBP 2', '2029-08-31', 36, 'USUAL', 0.00),
('RBP 2', '2029-08-31', 48, 'USUAL', 1256385.06),
('RBP 3', '2029-08-31', 24, 'USUAL', 277681.09),
('RBP 3', '2029-08-31', 36, 'USUAL', 594039.05),
('RBP 3', '2029-08-31', 48, 'USUAL', 1755866.68),
('RBP 4', '2029-08-31', 24, 'USUAL', 0.00),
('RBP 4', '2029-08-31', 36, 'USUAL', 52597.76),
('RBP 4', '2029-08-31', 48, 'USUAL', 416959.94),




('other', '2029-09-30', 24, 'USUAL', 923020.49),
('other', '2029-09-30', 36, 'USUAL', 110770.87),
('other', '2029-09-30', 48, 'USUAL', 4876393.70),
('PTS31_RBP4', '2029-09-30', 24, 'USUAL', 323903.64),
('PTS31_RBP4', '2029-09-30', 36, 'USUAL', 439634.84),
('PTS31_RBP4', '2029-09-30', 48, 'USUAL', 3395123.78),
('RBP 1', '2029-09-30', 24, 'USUAL', 1087090.42),
('RBP 1', '2029-09-30', 36, 'USUAL', 810741.81),
('RBP 1', '2029-09-30', 48, 'USUAL', 16797804.11),
('RBP 2', '2029-09-30', 24, 'USUAL', 156986.77),
('RBP 2', '2029-09-30', 36, 'USUAL', 0.00),
('RBP 2', '2029-09-30', 48, 'USUAL', 1256385.06),
('RBP 3', '2029-09-30', 24, 'USUAL', 277681.09),
('RBP 3', '2029-09-30', 36, 'USUAL', 594039.05),
('RBP 3', '2029-09-30', 48, 'USUAL', 1755866.68),
('RBP 4', '2029-09-30', 24, 'USUAL', 0.00),
('RBP 4', '2029-09-30', 36, 'USUAL', 52597.76),
('RBP 4', '2029-09-30', 48, 'USUAL', 416959.94),




('other', '2029-10-31', 24, 'USUAL', 923020.49),
('other', '2029-10-31', 36, 'USUAL', 110770.87),
('other', '2029-10-31', 48, 'USUAL', 4876393.70),
('PTS31_RBP4', '2029-10-31', 24, 'USUAL', 323903.64),
('PTS31_RBP4', '2029-10-31', 36, 'USUAL', 439634.84),
('PTS31_RBP4', '2029-10-31', 48, 'USUAL', 3395123.78),
('RBP 1', '2029-10-31', 24, 'USUAL', 1087090.42),
('RBP 1', '2029-10-31', 36, 'USUAL', 810741.81),
('RBP 1', '2029-10-31', 48, 'USUAL', 16797804.11),
('RBP 2', '2029-10-31', 24, 'USUAL', 156986.77),
('RBP 2', '2029-10-31', 36, 'USUAL', 0.00),
('RBP 2', '2029-10-31', 48, 'USUAL', 1256385.06),
('RBP 3', '2029-10-31', 24, 'USUAL', 277681.09),
('RBP 3', '2029-10-31', 36, 'USUAL', 594039.05),
('RBP 3', '2029-10-31', 48, 'USUAL', 1755866.68),
('RBP 4', '2029-10-31', 24, 'USUAL', 0.00),
('RBP 4', '2029-10-31', 36, 'USUAL', 52597.76),
('RBP 4', '2029-10-31', 48, 'USUAL', 416959.94),




('other', '2029-11-30', 24, 'USUAL', 923020.49),
('other', '2029-11-30', 36, 'USUAL', 110770.87),
('other', '2029-11-30', 48, 'USUAL', 4876393.70),
('PTS31_RBP4', '2029-11-30', 24, 'USUAL', 323903.64),
('PTS31_RBP4', '2029-11-30', 36, 'USUAL', 439634.84),
('PTS31_RBP4', '2029-11-30', 48, 'USUAL', 3395123.78),
('RBP 1', '2029-11-30', 24, 'USUAL', 1087090.42),
('RBP 1', '2029-11-30', 36, 'USUAL', 810741.81),
('RBP 1', '2029-11-30', 48, 'USUAL', 16797804.11),
('RBP 2', '2029-11-30', 24, 'USUAL', 156986.77),
('RBP 2', '2029-11-30', 36, 'USUAL', 0.00),
('RBP 2', '2029-11-30', 48, 'USUAL', 1256385.06),
('RBP 3', '2029-11-30', 24, 'USUAL', 277681.09),
('RBP 3', '2029-11-30', 36, 'USUAL', 594039.05),
('RBP 3', '2029-11-30', 48, 'USUAL', 1755866.68),
('RBP 4', '2029-11-30', 24, 'USUAL', 0.00),
('RBP 4', '2029-11-30', 36, 'USUAL', 52597.76),
('RBP 4', '2029-11-30', 48, 'USUAL', 416959.94),




('other', '2029-12-31', 24, 'USUAL', 923020.49),
('other', '2029-12-31', 36, 'USUAL', 110770.87),
('other', '2029-12-31', 48, 'USUAL', 4876393.70),
('PTS31_RBP4', '2029-12-31', 24, 'USUAL', 323903.64),
('PTS31_RBP4', '2029-12-31', 36, 'USUAL', 439634.84),
('PTS31_RBP4', '2029-12-31', 48, 'USUAL', 3395123.78),
('RBP 1', '2029-12-31', 24, 'USUAL', 1087090.42),
('RBP 1', '2029-12-31', 36, 'USUAL', 810741.81),
('RBP 1', '2029-12-31', 48, 'USUAL', 16797804.11),
('RBP 2', '2029-12-31', 24, 'USUAL', 156986.77),
('RBP 2', '2029-12-31', 36, 'USUAL', 0.00),
('RBP 2', '2029-12-31', 48, 'USUAL', 1256385.06),
('RBP 3', '2029-12-31', 24, 'USUAL', 277681.09),
('RBP 3', '2029-12-31', 36, 'USUAL', 594039.05),
('RBP 3', '2029-12-31', 48, 'USUAL', 1755866.68),
('RBP 4', '2029-12-31', 24, 'USUAL', 0.00),
('RBP 4', '2029-12-31', 36, 'USUAL', 52597.76),
('RBP 4', '2029-12-31', 48, 'USUAL', 416959.94),




('other', '2030-01-31', 24, 'USUAL', 1015322.53),
('other', '2030-01-31', 36, 'USUAL', 121847.96),
('other', '2030-01-31', 48, 'USUAL', 5364033.07),
('PTS31_RBP4', '2030-01-31', 24, 'USUAL', 356294.01),
('PTS31_RBP4', '2030-01-31', 36, 'USUAL', 483598.32),
('PTS31_RBP4', '2030-01-31', 48, 'USUAL', 3734636.16),
('RBP 1', '2030-01-31', 24, 'USUAL', 1195799.46),
('RBP 1', '2030-01-31', 36, 'USUAL', 891815.99),
('RBP 1', '2030-01-31', 48, 'USUAL', 18477584.52),
('RBP 2', '2030-01-31', 24, 'USUAL', 172685.45),
('RBP 2', '2030-01-31', 36, 'USUAL', 0.00),
('RBP 2', '2030-01-31', 48, 'USUAL', 1382023.57),
('RBP 3', '2030-01-31', 24, 'USUAL', 305449.19),
('RBP 3', '2030-01-31', 36, 'USUAL', 653442.96),
('RBP 3', '2030-01-31', 48, 'USUAL', 1931453.34),
('RBP 4', '2030-01-31', 24, 'USUAL', 0.00),
('RBP 4', '2030-01-31', 36, 'USUAL', 57857.53),
('RBP 4', '2030-01-31', 48, 'USUAL', 458655.94),




('other', '2030-02-28', 24, 'USUAL', 1015322.53),
('other', '2030-02-28', 36, 'USUAL', 121847.96),
('other', '2030-02-28', 48, 'USUAL', 5364033.07),
('PTS31_RBP4', '2030-02-28', 24, 'USUAL', 356294.01),
('PTS31_RBP4', '2030-02-28', 36, 'USUAL', 483598.32),
('PTS31_RBP4', '2030-02-28', 48, 'USUAL', 3734636.16),
('RBP 1', '2030-02-28', 24, 'USUAL', 1195799.46),
('RBP 1', '2030-02-28', 36, 'USUAL', 891815.99),
('RBP 1', '2030-02-28', 48, 'USUAL', 18477584.52),
('RBP 2', '2030-02-28', 24, 'USUAL', 172685.45),
('RBP 2', '2030-02-28', 36, 'USUAL', 0.00),
('RBP 2', '2030-02-28', 48, 'USUAL', 1382023.57),
('RBP 3', '2030-02-28', 24, 'USUAL', 305449.19),
('RBP 3', '2030-02-28', 36, 'USUAL', 653442.96),
('RBP 3', '2030-02-28', 48, 'USUAL', 1931453.34),
('RBP 4', '2030-02-28', 24, 'USUAL', 0.00),
('RBP 4', '2030-02-28', 36, 'USUAL', 57857.53),
('RBP 4', '2030-02-28', 48, 'USUAL', 458655.94),




('other', '2030-03-31', 24, 'USUAL', 1015322.53),
('other', '2030-03-31', 36, 'USUAL', 121847.96),
('other', '2030-03-31', 48, 'USUAL', 5364033.07),
('PTS31_RBP4', '2030-03-31', 24, 'USUAL', 356294.01),
('PTS31_RBP4', '2030-03-31', 36, 'USUAL', 483598.32),
('PTS31_RBP4', '2030-03-31', 48, 'USUAL', 3734636.16),
('RBP 1', '2030-03-31', 24, 'USUAL', 1195799.46),
('RBP 1', '2030-03-31', 36, 'USUAL', 891815.99),
('RBP 1', '2030-03-31', 48, 'USUAL', 18477584.52),
('RBP 2', '2030-03-31', 24, 'USUAL', 172685.45),
('RBP 2', '2030-03-31', 36, 'USUAL', 0.00),
('RBP 2', '2030-03-31', 48, 'USUAL', 1382023.57),
('RBP 3', '2030-03-31', 24, 'USUAL', 305449.19),
('RBP 3', '2030-03-31', 36, 'USUAL', 653442.96),
('RBP 3', '2030-03-31', 48, 'USUAL', 1931453.34),
('RBP 4', '2030-03-31', 24, 'USUAL', 0.00),
('RBP 4', '2030-03-31', 36, 'USUAL', 57857.53),
('RBP 4', '2030-03-31', 48, 'USUAL', 458655.94),




('other', '2030-04-30', 24, 'USUAL', 1015322.53),
('other', '2030-04-30', 36, 'USUAL', 121847.96),
('other', '2030-04-30', 48, 'USUAL', 5364033.07),
('PTS31_RBP4', '2030-04-30', 24, 'USUAL', 356294.01),
('PTS31_RBP4', '2030-04-30', 36, 'USUAL', 483598.32),
('PTS31_RBP4', '2030-04-30', 48, 'USUAL', 3734636.16),
('RBP 1', '2030-04-30', 24, 'USUAL', 1195799.46),
('RBP 1', '2030-04-30', 36, 'USUAL', 891815.99),
('RBP 1', '2030-04-30', 48, 'USUAL', 18477584.52),
('RBP 2', '2030-04-30', 24, 'USUAL', 172685.45),
('RBP 2', '2030-04-30', 36, 'USUAL', 0.00),
('RBP 2', '2030-04-30', 48, 'USUAL', 1382023.57),
('RBP 3', '2030-04-30', 24, 'USUAL', 305449.19),
('RBP 3', '2030-04-30', 36, 'USUAL', 653442.96),
('RBP 3', '2030-04-30', 48, 'USUAL', 1931453.34),
('RBP 4', '2030-04-30', 24, 'USUAL', 0.00),
('RBP 4', '2030-04-30', 36, 'USUAL', 57857.53),
('RBP 4', '2030-04-30', 48, 'USUAL', 458655.94),




('other', '2030-05-31', 24, 'USUAL', 1015322.53),
('other', '2030-05-31', 36, 'USUAL', 121847.96),
('other', '2030-05-31', 48, 'USUAL', 5364033.07),
('PTS31_RBP4', '2030-05-31', 24, 'USUAL', 356294.01),
('PTS31_RBP4', '2030-05-31', 36, 'USUAL', 483598.32),
('PTS31_RBP4', '2030-05-31', 48, 'USUAL', 3734636.16),
('RBP 1', '2030-05-31', 24, 'USUAL', 1195799.46),
('RBP 1', '2030-05-31', 36, 'USUAL', 891815.99),
('RBP 1', '2030-05-31', 48, 'USUAL', 18477584.52),
('RBP 2', '2030-05-31', 24, 'USUAL', 172685.45),
('RBP 2', '2030-05-31', 36, 'USUAL', 0.00),
('RBP 2', '2030-05-31', 48, 'USUAL', 1382023.57),
('RBP 3', '2030-05-31', 24, 'USUAL', 305449.19),
('RBP 3', '2030-05-31', 36, 'USUAL', 653442.96),
('RBP 3', '2030-05-31', 48, 'USUAL', 1931453.34),
('RBP 4', '2030-05-31', 24, 'USUAL', 0.00),
('RBP 4', '2030-05-31', 36, 'USUAL', 57857.53),
('RBP 4', '2030-05-31', 48, 'USUAL', 458655.94),




('other', '2030-06-30', 24, 'USUAL', 1015322.53),
('other', '2030-06-30', 36, 'USUAL', 121847.96),
('other', '2030-06-30', 48, 'USUAL', 5364033.07),
('PTS31_RBP4', '2030-06-30', 24, 'USUAL', 356294.01),
('PTS31_RBP4', '2030-06-30', 36, 'USUAL', 483598.32),
('PTS31_RBP4', '2030-06-30', 48, 'USUAL', 3734636.16),
('RBP 1', '2030-06-30', 24, 'USUAL', 1195799.46),
('RBP 1', '2030-06-30', 36, 'USUAL', 891815.99),
('RBP 1', '2030-06-30', 48, 'USUAL', 18477584.52),
('RBP 2', '2030-06-30', 24, 'USUAL', 172685.45),
('RBP 2', '2030-06-30', 36, 'USUAL', 0.00),
('RBP 2', '2030-06-30', 48, 'USUAL', 1382023.57),
('RBP 3', '2030-06-30', 24, 'USUAL', 305449.19),
('RBP 3', '2030-06-30', 36, 'USUAL', 653442.96),
('RBP 3', '2030-06-30', 48, 'USUAL', 1931453.34),
('RBP 4', '2030-06-30', 24, 'USUAL', 0.00),
('RBP 4', '2030-06-30', 36, 'USUAL', 57857.53),
('RBP 4', '2030-06-30', 48, 'USUAL', 458655.94),




('other', '2030-07-31', 24, 'USUAL', 1015322.53),
('other', '2030-07-31', 36, 'USUAL', 121847.96),
('other', '2030-07-31', 48, 'USUAL', 5364033.07),
('PTS31_RBP4', '2030-07-31', 24, 'USUAL', 356294.01),
('PTS31_RBP4', '2030-07-31', 36, 'USUAL', 483598.32),
('PTS31_RBP4', '2030-07-31', 48, 'USUAL', 3734636.16),
('RBP 1', '2030-07-31', 24, 'USUAL', 1195799.46),
('RBP 1', '2030-07-31', 36, 'USUAL', 891815.99),
('RBP 1', '2030-07-31', 48, 'USUAL', 18477584.52),
('RBP 2', '2030-07-31', 24, 'USUAL', 172685.45),
('RBP 2', '2030-07-31', 36, 'USUAL', 0.00),
('RBP 2', '2030-07-31', 48, 'USUAL', 1382023.57),
('RBP 3', '2030-07-31', 24, 'USUAL', 305449.19),
('RBP 3', '2030-07-31', 36, 'USUAL', 653442.96),
('RBP 3', '2030-07-31', 48, 'USUAL', 1931453.34),
('RBP 4', '2030-07-31', 24, 'USUAL', 0.00),
('RBP 4', '2030-07-31', 36, 'USUAL', 57857.53),
('RBP 4', '2030-07-31', 48, 'USUAL', 458655.94),




('other', '2030-08-31', 24, 'USUAL', 1015322.53),
('other', '2030-08-31', 36, 'USUAL', 121847.96),
('other', '2030-08-31', 48, 'USUAL', 5364033.07),
('PTS31_RBP4', '2030-08-31', 24, 'USUAL', 356294.01),
('PTS31_RBP4', '2030-08-31', 36, 'USUAL', 483598.32),
('PTS31_RBP4', '2030-08-31', 48, 'USUAL', 3734636.16),
('RBP 1', '2030-08-31', 24, 'USUAL', 1195799.46),
('RBP 1', '2030-08-31', 36, 'USUAL', 891815.99),
('RBP 1', '2030-08-31', 48, 'USUAL', 18477584.52),
('RBP 2', '2030-08-31', 24, 'USUAL', 172685.45),
('RBP 2', '2030-08-31', 36, 'USUAL', 0.00),
('RBP 2', '2030-08-31', 48, 'USUAL', 1382023.57),
('RBP 3', '2030-08-31', 24, 'USUAL', 305449.19),
('RBP 3', '2030-08-31', 36, 'USUAL', 653442.96),
('RBP 3', '2030-08-31', 48, 'USUAL', 1931453.34),
('RBP 4', '2030-08-31', 24, 'USUAL', 0.00),
('RBP 4', '2030-08-31', 36, 'USUAL', 57857.53),
('RBP 4', '2030-08-31', 48, 'USUAL', 458655.94),




('other', '2030-09-30', 24, 'USUAL', 1015322.53),
('other', '2030-09-30', 36, 'USUAL', 121847.96),
('other', '2030-09-30', 48, 'USUAL', 5364033.07),
('PTS31_RBP4', '2030-09-30', 24, 'USUAL', 356294.01),
('PTS31_RBP4', '2030-09-30', 36, 'USUAL', 483598.32),
('PTS31_RBP4', '2030-09-30', 48, 'USUAL', 3734636.16),
('RBP 1', '2030-09-30', 24, 'USUAL', 1195799.46),
('RBP 1', '2030-09-30', 36, 'USUAL', 891815.99),
('RBP 1', '2030-09-30', 48, 'USUAL', 18477584.52),
('RBP 2', '2030-09-30', 24, 'USUAL', 172685.45),
('RBP 2', '2030-09-30', 36, 'USUAL', 0.00),
('RBP 2', '2030-09-30', 48, 'USUAL', 1382023.57),
('RBP 3', '2030-09-30', 24, 'USUAL', 305449.19),
('RBP 3', '2030-09-30', 36, 'USUAL', 653442.96),
('RBP 3', '2030-09-30', 48, 'USUAL', 1931453.34),
('RBP 4', '2030-09-30', 24, 'USUAL', 0.00),
('RBP 4', '2030-09-30', 36, 'USUAL', 57857.53),
('RBP 4', '2030-09-30', 48, 'USUAL', 458655.94),




('other', '2030-10-31', 24, 'USUAL', 1015322.53),
('other', '2030-10-31', 36, 'USUAL', 121847.96),
('other', '2030-10-31', 48, 'USUAL', 5364033.07),
('PTS31_RBP4', '2030-10-31', 24, 'USUAL', 356294.01),
('PTS31_RBP4', '2030-10-31', 36, 'USUAL', 483598.32),
('PTS31_RBP4', '2030-10-31', 48, 'USUAL', 3734636.16),
('RBP 1', '2030-10-31', 24, 'USUAL', 1195799.46),
('RBP 1', '2030-10-31', 36, 'USUAL', 891815.99),
('RBP 1', '2030-10-31', 48, 'USUAL', 18477584.52),
('RBP 2', '2030-10-31', 24, 'USUAL', 172685.45),
('RBP 2', '2030-10-31', 36, 'USUAL', 0.00),
('RBP 2', '2030-10-31', 48, 'USUAL', 1382023.57),
('RBP 3', '2030-10-31', 24, 'USUAL', 305449.19),
('RBP 3', '2030-10-31', 36, 'USUAL', 653442.96),
('RBP 3', '2030-10-31', 48, 'USUAL', 1931453.34),
('RBP 4', '2030-10-31', 24, 'USUAL', 0.00),
('RBP 4', '2030-10-31', 36, 'USUAL', 57857.53),
('RBP 4', '2030-10-31', 48, 'USUAL', 458655.94),




('other', '2030-11-30', 24, 'USUAL', 1015322.53),
('other', '2030-11-30', 36, 'USUAL', 121847.96),
('other', '2030-11-30', 48, 'USUAL', 5364033.07),
('PTS31_RBP4', '2030-11-30', 24, 'USUAL', 356294.01),
('PTS31_RBP4', '2030-11-30', 36, 'USUAL', 483598.32),
('PTS31_RBP4', '2030-11-30', 48, 'USUAL', 3734636.16),
('RBP 1', '2030-11-30', 24, 'USUAL', 1195799.46),
('RBP 1', '2030-11-30', 36, 'USUAL', 891815.99),
('RBP 1', '2030-11-30', 48, 'USUAL', 18477584.52),
('RBP 2', '2030-11-30', 24, 'USUAL', 172685.45),
('RBP 2', '2030-11-30', 36, 'USUAL', 0.00),
('RBP 2', '2030-11-30', 48, 'USUAL', 1382023.57),
('RBP 3', '2030-11-30', 24, 'USUAL', 305449.19),
('RBP 3', '2030-11-30', 36, 'USUAL', 653442.96),
('RBP 3', '2030-11-30', 48, 'USUAL', 1931453.34),
('RBP 4', '2030-11-30', 24, 'USUAL', 0.00),
('RBP 4', '2030-11-30', 36, 'USUAL', 57857.53),
('RBP 4', '2030-11-30', 48, 'USUAL', 458655.94),




('other', '2030-12-31', 24, 'USUAL', 1015322.53),
('other', '2030-12-31', 36, 'USUAL', 121847.96),
('other', '2030-12-31', 48, 'USUAL', 5364033.07),
('PTS31_RBP4', '2030-12-31', 24, 'USUAL', 356294.01),
('PTS31_RBP4', '2030-12-31', 36, 'USUAL', 483598.32),
('PTS31_RBP4', '2030-12-31', 48, 'USUAL', 3734636.16),
('RBP 1', '2030-12-31', 24, 'USUAL', 1195799.46),
('RBP 1', '2030-12-31', 36, 'USUAL', 891815.99),
('RBP 1', '2030-12-31', 48, 'USUAL', 18477584.52),
('RBP 2', '2030-12-31', 24, 'USUAL', 172685.45),
('RBP 2', '2030-12-31', 36, 'USUAL', 0.00),
('RBP 2', '2030-12-31', 48, 'USUAL', 1382023.57),
('RBP 3', '2030-12-31', 24, 'USUAL', 305449.19),
('RBP 3', '2030-12-31', 36, 'USUAL', 653442.96),
('RBP 3', '2030-12-31', 48, 'USUAL', 1931453.34),
('RBP 4', '2030-12-31', 24, 'USUAL', 0.00),
('RBP 4', '2030-12-31', 36, 'USUAL', 57857.53),
('RBP 4', '2030-12-31', 48, 'USUAL', 458655.94)
	...
	delete from Temp_PTS_Infoseti_virt_gens where generation <= @rdt;
	--delete from Temp_PTS_Infoseti_virt_gens where flag_kk in ('Installment')
	--delete from Temp_PTS_Infoseti_virt_gens where flag_kk = 'BUSINESS' and generation > '2021-12-31';
	delete from Temp_PTS_Infoseti_virt_gens where generation > '2030-12-31';
	delete from Temp_PTS_Infoseti_virt_gens where generation > eomonth(@dt_back_test_from,@horizon);
	delete from Temp_PTS_Infoseti_virt_gens where vl_rub_fact <= 0;
	delete from Temp_PTS_Infoseti_virt_gens where flag_kk = 'INSTALLMENT' and generation >= '2022-04-30';
	delete from Temp_PTS_Infoseti_virt_gens where flag_kk = 'BUSINESS' and generation >= '2022-08-31';

	--delete from Temp_PTS_Infoseti_virt_gens where generation > '2022-12-31';
	...
	--drop table if exists Temp_PTS_Infoseti_fact_volumes;
	--select 
	--case 
	...
	--into Temp_PTS_Infoseti_fact_volumes
	--from dwh2.risk.credits a
	--left join risk.stg_fcst_fix_rbp_2 fr
	--on a.external_id = fr.external_id
	...
	--insert into Temp_PTS_Infoseti_virt_gens 
	--select a.segment_rbp, a.generation, a.term, a.flag_kk, a.volume 
	--from Temp_PTS_Infoseti_fact_volumes a
	...
	print ('timing, Temp_PTS_Infoseti_virt_gens ' + format(getdate(), 'HH:mm:ss'))
	...
	drop table if exists Temp_PTS_Infoseti_nodpd_payoff_model;
	with base as (
		select 
		case 
	...
	into Temp_PTS_Infoseti_nodpd_payoff_model
	from base b
	left join mm
	on mm.mob <= b.term
	;
	...
	select term
		...
		, round(100*sum(cast(amount as float) * cast(eps as float))/sum(cast(amount as float)) - 74, 0) as EIR_DIFF
	from Temp_PTS_Infoseti_cred_reestr
	where eomonth(generation, 0) >= eomonth('2025-11-01', 0)
	...
	select 100
		, round(100*sum(cast(amount as float) * cast(eps as float))/sum(cast(amount as float)), 0) as EIR
		, round(100*sum(cast(amount as float) * cast(eps as float))/sum(cast(amount as float)) - 68.1, 0) as EIR_DIFF
	from Temp_PTS_Infoseti_cred_reestr
	where eomonth(generation, 0) >= eomonth('2025-11-01', 0)
		--and segment_rbp != 'PTS31_RBP4'
	order by 1
	*/
	...
	with base as (
		select cast(24 as int) as term, (@avg_rate + 0) as avg_w_int_rate, 1.0 + (@avg_rate + 0) / 100.0 / 12.0 as one_plus_r	union all
		select cast(36 as int) as term, (@avg_rate + 0) as avg_w_int_rate, 1.0 + (@avg_rate + 0) / 100.0 / 12.0 as one_plus_r	union all
		select cast(48 as int) as term, (@avg_rate + 0) as avg_w_int_rate, 1.0 + (@avg_rate + 0) / 100.0 / 12.0 as one_plus_r
	), mm as (
		select cast(1 as float) as mob
		union all
		select cast(mob + 1 as float)
	...
	insert into Temp_PTS_Infoseti_nodpd_payoff_model
	...
	select term
		, round(100*sum(cast(amount as float) * cast(eps as float))/sum(cast(amount as float)), 0) as EIR
		, round(100*sum(cast(amount as float) * cast(eps as float))/sum(cast(amount as float)) - 68, 0) as EIR_DIFF
	from Temp_PTS_Infoseti_cred_reestr
	where eomonth(generation, 0) >= eomonth('2025-11-01', 0)
		and segment_rbp = 'PTS31_RBP4'
	group by term
	...
	select 100
		, round(100*sum(cast(amount as float) * cast(eps as float))/sum(cast(amount as float)), 0) as EIR
		, round(100*sum(cast(amount as float) * cast(eps as float))/sum(cast(amount as float)) - 79, 0) as EIR_DIFF
	from Temp_PTS_Infoseti_cred_reestr
	where eomonth(generation, 0) >= eomonth('2025-11-01', 0)
		and segment_rbp = 'PTS31_RBP4'
	order by 1
	*/
	...
	with base as (
		select cast(24 as int) as term, (@avg_rate + 33) as avg_w_int_rate, 1.0 + (@avg_rate + 33) / 100.0 / 12.0 as one_plus_r	union all
		select cast(36 as int) as term, (@avg_rate + 33) as avg_w_int_rate, 1.0 + (@avg_rate + 33) / 100.0 / 12.0 as one_plus_r	union all
		select cast(48 as int) as term, (@avg_rate + 33) as avg_w_int_rate, 1.0 + (@avg_rate + 33) / 100.0 / 12.0 as one_plus_r
	), mm as (
		select cast(1 as float) as mob
		union all
		select cast(mob + 1 as float)
	...
	print ('timing, Temp_PTS_Infoseti_nodpd_payoff_model' + format(getdate(), 'HH:mm:ss'))
	...
	drop table if exists Temp_PTS_Infoseti_det_stress;
	select 
	...
	into Temp_PTS_Infoseti_det_stress
	from (values
	...
	print ('timing, Temp_PTS_Infoseti_rest_model' + format(getdate(), 'HH:mm:ss'))



	------------------------------------------------------------------------------
	---коэффициенты для корректировки pay-down в зависимости от сегмента RBP и срока (term)
	drop table if exists Temp_PTS_Infoseti_model_adjust;
	...
	cast(a.coef as float) as coef
	into Temp_PTS_Infoseti_model_adjust
	from (
	--pay-off
	...
	--07/12/2021 installm
	insert into Temp_PTS_Infoseti_model_adjust
	...
	from Temp_PTS_Infoseti_model_adjust a
	...
	*/
	...
	print ('timing, Temp_PTS_Infoseti_model_adjust' + format(getdate(), 'HH:mm:ss'))

	drop table if exists Temp_PTS_Infoseti_repdates;
	...
	into Temp_PTS_Infoseti_repdates
	from increment i
	...
	drop table if exists Temp_PTS_Infoseti_stg_back_base;

	with base as (
		select distinct a.term, a.segment_rbp, a.generation, a.flag_kk from Temp_PTS_Infoseti_stg3_agg a
	union
		select distinct b.term, b.segment_rbp, b.generation, b.flag_kk from Temp_PTS_Infoseti_cred_reestr b 
		where b.generation >= @rdt
	union 
		select distinct v.term, v.segment_rbp, v.generation, v.flag_kk from Temp_PTS_Infoseti_virt_gens v 
		...
		from base b
		left join Temp_PTS_Infoseti_repdates d
	...
	select * 
	into Temp_PTS_Infoseti_stg_back_base
	from (
		select * from Temp_PTS_Infoseti_stg3_agg a
	union all
	...
	print ('timing, Temp_PTS_Infoseti_stg_back_base' + format(getdate(), 'HH:mm:ss'))

	--версии кривых для разных продуктов/сегментов


	drop table if exists Temp_PTS_Infoseti_det_curve_version;
	...
	into Temp_PTS_Infoseti_det_curve_version
	from (values 
	...
	drop table if exists Temp_PTS_Infoseti_for_back_test;
	...
	with base as (
		...
		from Temp_PTS_Infoseti_stg_back_base a

		left join Temp_PTS_Infoseti_nodpd_payoff_model n
		on a.term = n.term
		...
		left join Temp_PTS_Infoseti_rest_model r
		on 1=1
		--and a.term = r.term
		...
		left join Temp_PTS_Infoseti_model_adjust adj
		...
		left join Temp_PTS_Infoseti_det_stress sts
		on a.mob_date_to between sts.dt_from and sts.dt_to

		left join Temp_PTS_Infoseti_det_curve_version cv
		on case 
	...
	into Temp_PTS_Infoseti_for_back_test
	from base b
	left join znam z
	on b.term = z.term
	...
	drop table if exists Temp_PTS_Infoseti_season_rr_corr;
	with base as (
		select distinct 
			a.id1,
			--a.term, a.segment_rbp, a.flag_kk, a.generation, 
			a.mob_date_to, a.bucket_90_from
		from Temp_PTS_Infoseti_for_back_test a
		where 1=1
	...
	into Temp_PTS_Infoseti_season_rr_corr
	from Temp_PTS_Infoseti_for_back_test a
	inner join base b
	on 1=1
	and a.id1 = b.id1
	...
	insert into Temp_PTS_Infoseti_season_rr_corr
	...
	merge into Temp_PTS_Infoseti_for_back_test a
	using Temp_PTS_Infoseti_season_rr_corr b
	on (1=1
		--and a.term = b.term
		--and a.segment_rbp = b.segment_rbp
		--and a.flag_kk = b.flag_kk
	...
	update Temp_PTS_Infoseti_for_back_test set model_coef_adj = 0 where bucket_90_from = '[07] Freeze' or bucket_90_to = '[07] Freeze';
	...
	drop table if exists Temp_PTS_Infoseti_pmt_last_gen_0_MoB;
	select 
	...
	into Temp_PTS_Infoseti_pmt_last_gen_0_MoB
	from Temp_PTS_Infoseti_cred_reestr a
	--left join dwh_new.dbo.stat_v_balance2 b
	left join Temp_PTS_Infoseti_stg_payment b
	on a.external_id = b.external_id
	...
	drop table if exists Temp_PTS_Infoseti_fix_matrix;
	with volume as (
		select 
		hashbytes('MD2',concat(a.term, a.segment_rbp, a.generation, a.flag_kk)) as id1,
		a.term, a.segment_rbp, a.generation, a.flag_kk, 
	...
	select a.id1,
		...
		as corr
	into Temp_PTS_Infoseti_fix_matrix
	from Temp_PTS_Infoseti_for_back_test a
	left join Temp_PTS_Infoseti_pmt_last_gen_0_MoB b
	on a.id1 = b.id1
	left join volume c
	on a.id1 = c.id1
	where a.generation = @rdt
	...
	insert into Temp_PTS_Infoseti_fix_matrix
	...
	from Temp_PTS_Infoseti_for_back_test a
	inner join Temp_PTS_Infoseti_fix_matrix b
	on a.id1 = b.id1
	and a.bucket_90_from = b.bucket_90_from
	and a.bucket_90_to = b.bucket_90_to
	and a.mob_date_to = b.mob_date_to
	...
	drop table Temp_PTS_Infoseti_pmt_last_gen_0_MoB;
	...
	select * from Temp_PTS_Infoseti_for_back_test a
	...
	*/
	...
	from Temp_PTS_Infoseti_season_rr_corr a
	...
	*/
	...
	print ('timing, Temp_PTS_Infoseti_for_back_test' + format(getdate(), 'HH:mm:ss'))
	...
	drop table if exists Temp_PTS_Infoseti_back_test_acc_matr;
	create table Temp_PTS_Infoseti_back_test_acc_matr (
	id1 int,
	generation date,
	term int,
	...
	--цикл - шаг 0
	insert into Temp_PTS_Infoseti_back_test_acc_matr
	select 
	a.id1,
	a.generation, a.term, a.segment_rbp, a.flag_kk, 
	...
	drop table if exists Temp_PTS_Infoseti_temp1_back_test_acc_matr;
	select top 0 
	a.id1,
	a.generation, a.term, a.segment_rbp, a.flag_kk, 
	a.mob_to, a.bucket_90_from, a.bucket_90_to, a.fact_coef, a.model_coef_adj
	into Temp_PTS_Infoseti_temp1_back_test_acc_matr
	from Temp_PTS_Infoseti_back_test_acc_matr a;

	drop table if exists Temp_PTS_Infoseti_temp2_back_test_acc_matr;
	select *
	into Temp_PTS_Infoseti_temp2_back_test_acc_matr--select top 5 *
	from Temp_PTS_Infoseti_back_test_acc_matr a;
	...
	print ('timing, Temp_PTS_Infoseti_back_test_acc_matr iter 0 ' + format(getdate(), 'HH:mm:ss'));
	...
	begin
		...
		truncate table Temp_PTS_Infoseti_temp1_back_test_acc_matr
		insert into Temp_PTS_Infoseti_temp1_back_test_acc_matr
		select 
		a.id1,
		a.generation, a.term, a.segment_rbp, a.flag_kk, 
		a.mob_to, a.bucket_90_from, a.bucket_90_to, a.fact_coef, a.model_coef_adj
		from Temp_PTS_Infoseti_temp2_back_test_acc_matr a
		--where a.mob_date_to = @k;


		--умножение на следующую матрицу
		truncate table Temp_PTS_Infoseti_temp2_back_test_acc_matr;
		insert into Temp_PTS_Infoseti_temp2_back_test_acc_matr
		select 
		a.id1,
		...
		from Temp_PTS_Infoseti_temp1_back_test_acc_matr a
		left join Temp_PTS_Infoseti_for_back_test b
		on 1 = 1
		and a.id1 = b.id1
		--and a.term = b.term
		--and a.generation = b.generation
		...
		insert into Temp_PTS_Infoseti_back_test_acc_matr
		select * from Temp_PTS_Infoseti_temp2_back_test_acc_matr;



		print ('timing, Temp_PTS_Infoseti_back_test_acc_matr @k = ' + format(@k, 'dd.MM.yyyy') + ' ' + format(getdate(), 'HH:mm:ss'));

		set @k = EOMONTH(@k,1)

	end;

	drop table Temp_PTS_Infoseti_temp1_back_test_acc_matr;
	drop table Temp_PTS_Infoseti_temp2_back_test_acc_matr;



	print ('timing, Temp_PTS_Infoseti_back_test_acc_matr' + format(getdate(), 'HH:mm:ss'))




	--16/06/2021 - new vers of Temp_PTS_Infoseti_last_fact by mob_date_to



	drop table if exists Temp_PTS_Infoseti_last_fact;
	...
	into Temp_PTS_Infoseti_last_fact
	from Temp_PTS_Infoseti_stg3_agg a
	where a.mob_date_to = eomonth(@dt_back_test_from,-1)
	...
	and a.mob_from = 0
	and a.bucket_90_from = '[01] 0'
	and a.generation > (select max(generation) from Temp_PTS_Infoseti_last_fact)
	and a.generation <= EOMONTH(@rdt, -1)
	), buck as (
	select '[01] 0' as bucket union all
	select '[02] 1-30' as bucket union all
	...
	insert into Temp_PTS_Infoseti_last_fact
	...
	from Temp_PTS_Infoseti_virt_gens a
	left join buck b
	on 1=1
	group by 
	hashbytes('MD2',concat(a.term, a.segment_rbp, a.generation, a.flag_kk)),
	...
	print ('timing, Temp_PTS_Infoseti_last_fact' + format(getdate(), 'HH:mm:ss'))



	--comparison (back-test fact<->model)
	drop table if exists Temp_PTS_Infoseti_back_test;
	...
	into Temp_PTS_Infoseti_back_test
	from Temp_PTS_Infoseti_last_fact a
	left join Temp_PTS_Infoseti_back_test_acc_matr b
	on a.id1 = b.id1
	--and a.generation = b.generation
	--and a.term = b.term
	--and a.segment_rbp = b.segment_rbp
	...
	print ('timing, Temp_PTS_Infoseti_back_test' + format(getdate(), 'HH:mm:ss'))
	...
	drop table if exists Temp_PTS_Infoseti_freeze_last_fact;
	select 
	hashbytes('MD2',concat(a.term, a.segment_rbp, a.generation, a.flag_kk)) as id1,
	...
	into Temp_PTS_Infoseti_freeze_last_fact
	from Temp_PTS_Infoseti_stg_matrix_detail a
	left join Temp_PTS_Infoseti_zamorozka b
	on a.external_id = b.external_id
	and a.mob_date_to >= b.freeze_from
	left join Temp_PTS_Infoseti_stg_start_default dd
	on a.external_id = dd.external_id
	and a.mob_date_to >= dd.month_default_from
	left join Temp_PTS_Infoseti_stg_freeze_mod f
	...
	-- + payoff
	insert into Temp_PTS_Infoseti_freeze_last_fact
	select 
	a.id1,
	...
	print ('timing, Temp_PTS_Infoseti_freeze_last_fact ' + format(getdate(), 'HH:mm:ss'))




	drop table if exists Temp_PTS_Infoseti_stg_freeze_model;

	create table Temp_PTS_Infoseti_stg_freeze_model (
	id1 int,
	term int,
	segment_rbp varchar(100),
	flag_kk varchar(100),
	...
	insert into Temp_PTS_Infoseti_stg_freeze_model
	select 
	a.id1,
	a.term, 
	a.segment_rbp,
	...
	from Temp_PTS_Infoseti_freeze_last_fact a
	where left(a.flag_kk,2) = 'KK'
	...
	print ('timing, Temp_PTS_Infoseti_stg_freeze_model cycle start' + format(getdate(), 'HH:mm:ss'));
	...
	drop table if exists Temp_PTS_Infoseti_tmp1_freeze_model;
	select * 
	into Temp_PTS_Infoseti_tmp1_freeze_model
	from Temp_PTS_Infoseti_stg_freeze_model a
	...
	if @ii < '2021-10-31' begin 
		...
		* case a.bucket_90_to when '[03] 31-60' then 0.75 when '[04] 61-90' then 0.15 end 
		...
		--from Temp_PTS_Infoseti_stg_freeze_model a
		from Temp_PTS_Infoseti_tmp1_freeze_model a
		where left(a.flag_kk,2) = 'KK'
	...
	drop table if exists Temp_PTS_Infoseti_tmp2_freeze_model;
	...
	into Temp_PTS_Infoseti_tmp2_freeze_model
	from Temp_PTS_Infoseti_stg_freeze_model a
	where left(a.flag_kk,2) = 'KK'
	...
	* case when b.bucket_90_from = '[03] 31-60' and a.freeze_from is null and a.mob_date_to < '2021-10-31' then 0.25
	...
	from Temp_PTS_Infoseti_tmp2_freeze_model a
	inner join Temp_PTS_Infoseti_for_back_test b
	on a.id1 = b.id1
	--and a.term = b.term
	--and a.generation = b.generation
	--and a.segment_rbp = b.segment_rbp
	...
	print ('timing, Temp_PTS_Infoseti_stg_freeze_model cycle @ii= ' + format(@ii,'dd.MM.yyyy')+ ' , ' + format(getdate(), 'HH:mm:ss'))
	...
	drop table Temp_PTS_Infoseti_tmp1_freeze_model;
	drop table Temp_PTS_Infoseti_tmp2_freeze_model;

	print ('timing, Temp_PTS_Infoseti_stg_freeze_model ' + format(getdate(), 'HH:mm:ss'))
	...
	drop table if exists Temp_PTS_Infoseti_stg2_freeze_model;
	...
	with M as (
		...
		from Temp_PTS_Infoseti_back_test a 
		where left(a.flag_kk,2) = 'KK'
	)
	select m.term, m.segment_rbp, m.flag_kk, m.generation,
		...
		f.fact_od,
		m.model_od
	into Temp_PTS_Infoseti_stg2_freeze_model
	from M
	left join F
	on m.term = f.term
	...
	print ('timing, Temp_PTS_Infoseti_stg2_freeze_model ' + format(getdate(), 'HH:mm:ss'));
	...
	drop table if exists Temp_PTS_Infoseti_stg1_back_test_results;
	...
	with new_volume as (
		...
		and exists (select 1 from Temp_PTS_Infoseti_back_test t
	 ...
	 union all --кредитные каникулы (заморозка)
		select * from Temp_PTS_Infoseti_stg2_freeze_model a	
	)
	...
	into Temp_PTS_Infoseti_stg1_back_test_results
	from u
	;
	...
	insert into Temp_PTS_Infoseti_stg1_back_test_results
	...
	print ('timing, Temp_PTS_Infoseti_stg1_back_test_results' + format(getdate(), 'HH:mm:ss'))

	--добавляем сумму выдачи для расчета GL90
	drop table if exists Temp_PTS_Infoseti_new_volume;

	select * 
	into Temp_PTS_Infoseti_new_volume
	from (
	...
	print ('timing, Temp_PTS_Infoseti_new_volume' + format(getdate(), 'HH:mm:ss'))


	drop table if exists Temp_PTS_Infoseti_stg2_back_test_results;
	...
	into Temp_PTS_Infoseti_stg2_back_test_results

	from Temp_PTS_Infoseti_stg1_back_test_results a
	left join Temp_PTS_Infoseti_new_volume b
	on a.term = b.term
	and a.generation = b.generation
	and a.segment_rbp = b.segment_rbp
	and a.flag_kk = b.flag_kk
	...
	print ('timing, Temp_PTS_Infoseti_stg2_back_test_results' + format(getdate(), 'HH:mm:ss'))
	...
	drop table if exists Temp_PTS_Infoseti_conditional_rec_ALL;
	with base as (
		select a.mod_num, 
		--recovery rates
		a.recov_w, 
	...
	a.* 
	into Temp_PTS_Infoseti_conditional_rec_ALL 
	from un as a
	;


	update Temp_PTS_Infoseti_conditional_rec_ALL set recov_int_rate = recov_int_rate * 1.4;
	update Temp_PTS_Infoseti_conditional_rec_ALL set fee_rec_rate = fee_rec_rate * 1.7;
	update Temp_PTS_Infoseti_conditional_rec_ALL set straf_recovery_rate = straf_recovery_rate * 2.4;


	insert into Temp_PTS_Infoseti_conditional_rec_ALL
	select 
	...
	a.wo_rate,
	a.wo_int_rate
	from Temp_PTS_Infoseti_conditional_rec_ALL a
	;
	...
	print ('timing, Temp_PTS_Infoseti_conditional-s' + format(getdate(), 'HH:mm:ss'))


	--промежуточный расчет накопленных Recovery (для фактических дефолтов - до прогнозной даты)

	drop table if exists Temp_PTS_Infoseti_stg0_default_recovery_fact;
	...
	into Temp_PTS_Infoseti_stg0_default_recovery_fact
	from Temp_PTS_Infoseti_stg_matrix_detail a
	inner join Temp_PTS_Infoseti_stg_start_default b
	on a.external_id = b.external_id
	where 1=1
	and a.mob_date_to = eomonth(@dt_back_test_from,-1)
	and a.bucket_90_to <> '[06] Pay-off'
	...
	drop table if exists Temp_PTS_Infoseti_det_recovery_corr;
	...
	--into Temp_PTS_Infoseti_det_recovery_corr
	--from (values
	...
	print ('timing, Temp_PTS_Infoseti_stg0_default_recovery_fact' + format(getdate(), 'HH:mm:ss'))
	...
	drop table if exists Temp_PTS_Infoseti_stg1_default_recovery_fact;
	create table Temp_PTS_Infoseti_stg1_default_recovery_fact (
		mob_date_to date,
		term int,
		generation date,
		segment_rbp varchar(100),
	...
	print ('timing, Temp_PTS_Infoseti_stg1_default_recovery_fact iter0' + format(getdate(), 'HH:mm:ss'))
	...
	while @cycle_dt <= EOMONTH(@dt_back_test_from, /*12*6*/ @horizon) begin
		...
		with agg as (
			...
			count(*) as cnt
			from Temp_PTS_Infoseti_stg0_default_recovery_fact b
			where 1=1
			group by b.mob_date_to, b.term, b.generation, b.segment_rbp, b.flag_kk, b.months_in_default, b.last_MOD
		)
		insert into Temp_PTS_Infoseti_stg1_default_recovery_fact
		select 
		...
		left join Temp_PTS_Infoseti_conditional_rec_ALL c
		...
		left join Temp_PTS_Infoseti_det_recovery_corr d
		...
		print ('timing, Temp_PTS_Infoseti_stg1_default_recovery_fact @cycle_dt = ' + format(@cycle_dt, 'dd.MM.yyyy') + ' ' + format(getdate(), 'HH:mm:ss'));
	...
	delete from Temp_PTS_Infoseti_stg1_default_recovery_fact where flag_kk like 'CESS%' and mob_date_to >= cast(substring(flag_kk,6,10) as date);
	...
	from Temp_PTS_Infoseti_stg1_default_recovery_fact a
	where a.flag_kk = 'BANKRUPT';
	*/



	print ('timing, Temp_PTS_Infoseti_stg1_default_recovery_fact' + format(getdate(), 'HH:mm:ss'))



	--накопленные Recovery (для фактических дефолтов - до прогнозной даты)
	drop table if exists Temp_PTS_Infoseti_default_recovery_fact
	...
	into Temp_PTS_Infoseti_default_recovery_fact
	from Temp_PTS_Infoseti_stg1_default_recovery_fact a
	...
	print ('timing, Temp_PTS_Infoseti_default_recovery_fact' + format(getdate(), 'HH:mm:ss'))
	...
	drop table if exists Temp_PTS_Infoseti_stg1_default_recovery_model;
	select 
	--a.*, b.*
	a.id1,
	...
	into Temp_PTS_Infoseti_stg1_default_recovery_model
	from Temp_PTS_Infoseti_last_fact a
	left join Temp_PTS_Infoseti_for_back_test b
	on a.id1 = b.id1
	--and a.generation = b.generation
	...
	insert into Temp_PTS_Infoseti_stg1_default_recovery_model
	select 
	hashbytes('MD2',concat(a.term, a.segment_rbp, a.generation, a.flag_kk)) as id1,
	...
	drop table if exists Temp_PTS_Infoseti_tmp1_default_recovery_model;
	select top 0 
	a.id1,
	a.term, a.segment_rbp, a.flag_kk, a.generation,
	a.mob_to, a.bucket_90_from, a.bucket_90_to, a.mod_num, a.last_MOD, a.model_od
	into Temp_PTS_Infoseti_tmp1_default_recovery_model
	from Temp_PTS_Infoseti_stg1_default_recovery_model a;

	drop table if exists Temp_PTS_Infoseti_tmp2_default_recovery_model;
	select *
	into Temp_PTS_Infoseti_tmp2_default_recovery_model
	from Temp_PTS_Infoseti_stg1_default_recovery_model;





	print ('timing, Temp_PTS_Infoseti_stg1_default_recovery_model' + format(getdate(), 'HH:mm:ss'))
	...
	begin
		...
		truncate table Temp_PTS_Infoseti_tmp1_default_recovery_model;
		insert into Temp_PTS_Infoseti_tmp1_default_recovery_model
		select 
		a.id1,
		a.term, a.segment_rbp, a.flag_kk, a.generation,
		a.mob_to, a.bucket_90_from, a.bucket_90_to, a.mod_num, a.last_MOD, sum(a.model_od) as model_od
		from Temp_PTS_Infoseti_tmp2_default_recovery_model a
		where 1=1
		and a.bucket_90_to <> '[06] Pay-off'
		--and a.model_od > 1
		...
		truncate table Temp_PTS_Infoseti_tmp2_default_recovery_model;
		insert into Temp_PTS_Infoseti_tmp2_default_recovery_model	
		...
		from Temp_PTS_Infoseti_tmp1_default_recovery_model a
		inner join Temp_PTS_Infoseti_for_back_test b
		on a.id1 = b.id1
		--and a.term = b.term
		--and a.segment_rbp = b.segment_rbp
		...
		insert into Temp_PTS_Infoseti_stg1_default_recovery_model
		select * from Temp_PTS_Infoseti_tmp2_default_recovery_model
		;


		print ('timing, Temp_PTS_Infoseti_stg1_default_recovery_model cycle @d = ' + format(@d,'dd.MM.yyyy') + ' , ' + format(getdate(), 'HH:mm:ss'));
	...
	drop table Temp_PTS_Infoseti_tmp1_default_recovery_model;
	drop table Temp_PTS_Infoseti_tmp2_default_recovery_model;



	print ('timing, Temp_PTS_Infoseti_stg1_default_recovery_model cycle' + format(getdate(), 'HH:mm:ss'))




	drop table if exists Temp_PTS_Infoseti_stg2_default_recovery_model;
	...
	sum(a.model_od) as model_od
	into Temp_PTS_Infoseti_stg2_default_recovery_model
	from Temp_PTS_Infoseti_stg1_default_recovery_model a
	...
	insert into Temp_PTS_Infoseti_stg2_default_recovery_model
	...
	-- not exists (select 1 from Temp_PTS_Infoseti_ni_kk b
	...
	--Цессия - обнуляем с момента продажи
	delete from Temp_PTS_Infoseti_stg2_default_recovery_model where flag_kk like 'CESS%' and mob_date_to >= cast(substring(flag_kk,6,10) as date);

	delete from Temp_PTS_Infoseti_stg2_default_recovery_model where isnull(mod_num,-999) < 0;

	print ('timing, Temp_PTS_Infoseti_stg2_default_recovery_model' + format(getdate(), 'HH:mm:ss'))
	...
	drop table if exists Temp_PTS_Infoseti_stg3_default_recovery_model;
	create table Temp_PTS_Infoseti_stg3_default_recovery_model (
		term int,
		segment_rbp varchar(100),
		flag_kk varchar(100),
		generation date,
	...
	while @cycle_dt5 <= eomonth(@dt_back_test_from, @horizon) begin
		...
		insert into Temp_PTS_Infoseti_stg3_default_recovery_model
		select a.term, a.segment_rbp, a.flag_kk, a.generation, a.mob_date_to, a.mob_to, a.mod_num, a.last_MOD, a.freeze_from,
		a.model_od,
		...
		from Temp_PTS_Infoseti_stg2_default_recovery_model a
		left join Temp_PTS_Infoseti_conditional_rec_ALL b
		...
		print ('timing, Temp_PTS_Infoseti_stg3_default_recovery_model @k = ' + format(@cycle_dt5, 'dd.MM.yyyy') + ' ' + format(getdate(), 'HH:mm:ss'));
	...
	print ('timing, Temp_PTS_Infoseti_stg3_default_recovery_model' + format(getdate(), 'HH:mm:ss'))



	--Цессия - обнуляем с момента продажи
	delete from Temp_PTS_Infoseti_stg3_default_recovery_model where flag_kk like 'CESS%' and mob_date_to >= cast(substring(flag_kk,6,10) as date);
	...
	from Temp_PTS_Infoseti_stg3_default_recovery_model a
	where a.flag_kk = 'BANKRUPT';
	*/
	...
	drop table if exists Temp_PTS_Infoseti_default_recovery_model;
	...
	into Temp_PTS_Infoseti_default_recovery_model
	from Temp_PTS_Infoseti_stg3_default_recovery_model b
	...
	print ('timing, Temp_PTS_Infoseti_default_recovery_model' + format(getdate(), 'HH:mm:ss'))


	/******************************************************************************************/
	--корректировка Pay-off и 90+ и добавление списаний и рекавери 90+

	drop table if exists Temp_PTS_Infoseti_stg3_back_test_results;
	...
	into Temp_PTS_Infoseti_stg3_back_test_results

	from Temp_PTS_Infoseti_stg2_back_test_results a

	left join Temp_PTS_Infoseti_default_recovery_fact b
	on a.term = b.term
	and a.generation = b.generation
	and a.segment_rbp = b.segment_rbp
	and a.mob_date_to = b.mob_date_to
	...
	left join Temp_PTS_Infoseti_default_recovery_model c
	on a.term = c.term
	and a.generation = c.generation
	and a.segment_rbp = c.segment_rbp
	...
	print ('timing, Temp_PTS_Infoseti_stg3_back_test_results' + format(getdate(), 'HH:mm:ss'))


	--фактические списания (до прогнозной матрицы)
	drop table if exists Temp_PTS_Infoseti_write_offs_fact;

	select a.term, a.generation, a.segment_rbp, a.flag_kk,
	eomonth(b.r_date) as mob_date_to,
	sum(isnull(b.od_wo,0)) as od_wo
	into Temp_PTS_Infoseti_write_offs_fact
	from Temp_PTS_Infoseti_cred_reestr a
	inner join Temp_PTS_Infoseti_stg_write_off b
	on a.external_id = b.external_id
	where 1=1
	...
	into Temp_PTS_Infoseti_npl_recovery_fact
	from Temp_PTS_Infoseti_stg3_agg a
	left join w
	on a.term = w.term
	...
	drop table if exists Temp_PTS_Infoseti_npl_wo_and_recovery;
	with base as (
		select a.term, a.generation, a.segment_rbp, a.flag_kk, a.mob_date_to,
		...
		from Temp_PTS_Infoseti_default_recovery_fact a
	union all
		select a.term, a.generation, a.segment_rbp, a.flag_kk, a.mob_date_to,
		a.wo_calc_od_payoff - lag(a.wo_calc_od_payoff,1,0) over (partition by a.term, a.generation, a.segment_rbp, a.flag_kk order by a.mob_date_to) as od_wo,
		a.calc_od_payoff - lag(a.calc_od_payoff,1,0) over (partition by a.term, a.generation, a.segment_rbp, a.flag_kk order by a.mob_date_to) as npl_recovery
		from Temp_PTS_Infoseti_default_recovery_model a
	union all
		...
		from Temp_PTS_Infoseti_stg3_back_test_results a
		left join Temp_PTS_Infoseti_write_offs_fact b
		...
		left join Temp_PTS_Infoseti_npl_recovery_fact c
		...
		--30/08/2021 - 90+ не подтянулся на дату @dt_back_test_from по ветке моделирования факта (Temp_PTS_Infoseti_default_recovery_fact)
		...
		from Temp_PTS_Infoseti_stg3_back_test_results a
		left join Temp_PTS_Infoseti_repdates c
		on c.mob_date_to >= @dt_back_test_from
		where not exists (select 1 from Temp_PTS_Infoseti_default_recovery_fact b
							where a.generation = b.generation
							and a.segment_rbp = b.segment_rbp
							and a.term = b.term
							and a.flag_kk = b.flag_kk
	...
	into Temp_PTS_Infoseti_npl_wo_and_recovery
	from total a
	;



	insert into Temp_PTS_Infoseti_npl_wo_and_recovery
	select a.term, a.generation, a.segment_rbp, a.flag_kk, c.mob_date_to, a.od_wo, a.npl_recovery
	from Temp_PTS_Infoseti_npl_wo_and_recovery a
	inner join (
	...
	left join (
		select a.mob_date_to from Temp_PTS_Infoseti_repdates a
		union 
	...
	print ('timing, Temp_PTS_Infoseti_npl_wo_and_recovery' + format(getdate(), 'HH:mm:ss'))


	--добавляем списания и recovery, дельты NL, GL в общую витрину
	drop table if exists Temp_PTS_Infoseti_stg4_back_test_results;
	...
	with base as (
		...
		from Temp_PTS_Infoseti_stg3_back_test_results a
		left join Temp_PTS_Infoseti_npl_wo_and_recovery b
		on a.term = b.term
		and a.generation = b.generation
		and a.segment_rbp = b.segment_rbp
		and a.flag_kk = b.flag_kk
	...
	into Temp_PTS_Infoseti_stg4_back_test_results
	...
	with base as (
		...
		from Temp_PTS_Infoseti_cred_reestr a
		left join Temp_PTS_Infoseti_CMR_M b
		on a.external_id = b.external_id
	...
	union all -- + бизнес-займы
		...
		from Temp_PTS_Infoseti_cred_reestr a
		--left join Temp_PTS_Infoseti_bus_stg_bal b
	...
	from Temp_PTS_Infoseti_stg4_back_test_results a
	left join base b
	on a.term = b.term
	and a.generation = b.generation
	and a.segment_rbp = b.segment_rbp
	...
	left join Temp_PTS_Infoseti_det_pdp_coef p
	on a.segment_rbp = p.segment_rbp

	where a.mob_to = 0
	;

	print ('timing, Temp_PTS_Infoseti_stg4_back_test_results' + format(getdate(), 'HH:mm:ss'))
	...
	drop table if exists Temp_PTS_Infoseti_cessed_od;
	...
	into Temp_PTS_Infoseti_cessed_od
	from Temp_PTS_Infoseti_stg4_back_test_results a
	where a.flag_kk like 'CESS %'
	...
	from Temp_PTS_Infoseti_stg4_back_test_results a
	inner join Temp_PTS_Infoseti_cessed_od b
	on a.term = b.term
	and a.segment_rbp = b.segment_rbp 
	and a.generation = b.generation
	and a.flag_kk = b.flag_kk
	...
	print ('timing, Temp_PTS_Infoseti_cessed_od' + format(getdate(), 'HH:mm:ss'))
	...
	/*
	select a.bucket_90_from, a.bucket_90_to, format(sum(a.model_od),'Temp_PTS_Infoseti_Temp_PTS_Infoseti_Temp_PTS_Infoseti_ Temp_PTS_Infoseti_Temp_PTS_Infoseti_Temp_PTS_Infoseti_ Temp_PTS_Infoseti_Temp_PTS_Infoseti_Temp_PTS_Infoseti_')
	from Temp_PTS_Infoseti_stg_freeze_model a
	...
	*/
	...
	select * from Temp_PTS_Infoseti_stg4_back_test_results a
	...
	*/
	...
	select a.mob_date_to, format(sum(a.model_od),'Temp_PTS_Infoseti_ Temp_PTS_Infoseti_Temp_PTS_Infoseti_Temp_PTS_Infoseti_ Temp_PTS_Infoseti_Temp_PTS_Infoseti_Temp_PTS_Infoseti_ Temp_PTS_Infoseti_Temp_PTS_Infoseti_Temp_PTS_Infoseti_')
	from Temp_PTS_Infoseti_stg4_back_test_results a
	where a.bucket_90_to = '[05] 90+'
	...
	select aa.mob_date_to, format(sum(aa.model_od),'Temp_PTS_Infoseti_ Temp_PTS_Infoseti_Temp_PTS_Infoseti_Temp_PTS_Infoseti_ Temp_PTS_Infoseti_Temp_PTS_Infoseti_Temp_PTS_Infoseti_ Temp_PTS_Infoseti_Temp_PTS_Infoseti_Temp_PTS_Infoseti_')
	from (
	...
	*/
	...
	----drop table Temp_PTS_Infoseti_zamorozka;
	--/* P 3 */
	--drop table Temp_PTS_Infoseti_back_test;
	--drop table Temp_PTS_Infoseti_back_test_acc_matr;
	--drop table Temp_PTS_Infoseti_cessed_od;
	----drop table Temp_PTS_Infoseti_conditional_rec_ALL;
	----drop table Temp_PTS_Infoseti_default_recovery_fact;
	----drop table Temp_PTS_Infoseti_default_recovery_model;
	--drop table Temp_PTS_Infoseti_det_curve_version;
	----drop table Temp_PTS_Infoseti_det_pdp_coef;
	----drop table Temp_PTS_Infoseti_det_recovery_corr;
	----drop table Temp_PTS_Infoseti_det_stress;
	----drop table Temp_PTS_Infoseti_fix_matrix;
	----drop table Temp_PTS_Infoseti_for_back_test;
	----drop table Temp_PTS_Infoseti_freeze_last_fact;
	----drop table Temp_PTS_Infoseti_last_fact;
	--drop table Temp_PTS_Infoseti_model_adjust;
	----drop table Temp_PTS_Infoseti_new_volume;
	--drop table Temp_PTS_Infoseti_nodpd_payoff_model;
	--drop table Temp_PTS_Infoseti_npl_recovery_fact;
	----drop table Temp_PTS_Infoseti_npl_wo_and_recovery;
	--drop table Temp_PTS_Infoseti_repdates;
	--drop table Temp_PTS_Infoseti_season_rr_corr;
	--drop table Temp_PTS_Infoseti_stg_back_base;
	----drop table Temp_PTS_Infoseti_stg_freeze_model;
	--drop table Temp_PTS_Infoseti_stg0_default_recovery_fact;
	--drop table Temp_PTS_Infoseti_stg1_back_test_results;
	----drop table Temp_PTS_Infoseti_stg1_default_recovery_fact;
	--drop table Temp_PTS_Infoseti_stg1_default_recovery_model;
	--drop table Temp_PTS_Infoseti_stg2_back_test_results;
	--drop table Temp_PTS_Infoseti_stg2_default_recovery_model;
	--drop table Temp_PTS_Infoseti_stg2_freeze_model;
	--drop table Temp_PTS_Infoseti_stg3_back_test_results;
	----drop table Temp_PTS_Infoseti_stg3_default_recovery_model;
	----drop table Temp_PTS_Infoseti_stg4_back_test_results;
	----drop table Temp_PTS_Infoseti_virt_gens;
	--drop table Temp_PTS_Infoseti_write_offs_fact;
	...
	drop table if exists Temp_PTS_Infoseti_stg1_interest;
	select a.term, a.segment_rbp, a.flag_kk, a.generation, a.mob_date_to, a.mob_to
	...
	into Temp_PTS_Infoseti_stg1_interest--select top 100 *
	from Temp_PTS_Infoseti_for_back_test a
	where 1=1
	...
	print ('timing, Temp_PTS_Infoseti_stg1_interest ' + format(getdate(), 'HH:mm:ss'))
	...
	drop table if exists Temp_PTS_Infoseti_stg2_interest;
	...
	into Temp_PTS_Infoseti_stg2_interest

	from Temp_PTS_Infoseti_stg1_interest a
	left join Temp_PTS_Infoseti_det_stress sts --28.02.2022
	on a.mob_date_to between sts.dt_from and sts.dt_to
	...
	drop table if exists Temp_PTS_Infoseti_upd_stg2_interest;
	with base as (
		select hashbytes('MD2',concat(a.term, a.segment_rbp, a.generation, a.flag_kk)) as id1,
		a.* 
		from Temp_PTS_Infoseti_stg2_interest a
	...
	into Temp_PTS_Infoseti_upd_stg2_interest
	from base a
	left join base2 b
	on a.id1 = b.id1
	and a.bucket_90_from = b.bucket_90_from
	...
	from Temp_PTS_Infoseti_stg2_interest a
	inner join Temp_PTS_Infoseti_upd_stg2_interest b
	...
	print ('timing, Temp_PTS_Infoseti_stg2_interest ' + format(getdate(), 'HH:mm:ss'))
	...
	drop table if exists Temp_PTS_Infoseti_stg_int_rates;
	with rates as (
		select a.term, a.segment_rbp, a.flag_kk, a.generation, 
		...
		select distinct a.term, a.segment_rbp, a.flag_kk, a.generation from Temp_PTS_Infoseti_stg4_back_test_results a
	), base as (
	...
	into Temp_PTS_Infoseti_stg_int_rates
	from base a
	where a.rown = 1
	...
	from Temp_PTS_Infoseti_cred_reestr where year(generation) = 2025 and flag_kk = 'USUAL'
	...
	*/
	...
	update Temp_PTS_Infoseti_stg_int_rates set avg_month_int_rate = (select round(sum(amount * int_rate)/sum(amount), 0) - 0.66 from Temp_PTS_Infoseti_cred_reestr where year(generation) = 2025 and flag_kk = 'USUAL' and segment_rbp = 'other' group by segment_rbp, flag_kk) / 100.0 / 12.0	where segment_rbp = 'other'		and flag_kk = 'USUAL' and eomonth(generation, 0) >= @dt_back_test_from;
	update Temp_PTS_Infoseti_stg_int_rates set avg_month_int_rate = (select round(sum(amount * int_rate)/sum(amount), 0) - 0.66 from Temp_PTS_Infoseti_cred_reestr where year(generation) = 2025 and flag_kk = 'USUAL' and segment_rbp = 'PTS31_RBP4' group by segment_rbp, flag_kk) / 100.0 / 12.0	where segment_rbp = 'PTS31_RBP4'	and flag_kk = 'USUAL' and eomonth(generation, 0) >= @dt_back_test_from;
	update Temp_PTS_Infoseti_stg_int_rates set avg_month_int_rate = (select round(sum(amount * int_rate)/sum(amount), 0) - 0.66 from Temp_PTS_Infoseti_cred_reestr where year(generation) = 2025 and flag_kk = 'USUAL' and segment_rbp = 'RBP 1' group by segment_rbp, flag_kk) / 100.0 / 12.0	where segment_rbp = 'RBP 1'				and flag_kk = 'USUAL' and eomonth(generation, 0) >= @dt_back_test_from;
	update Temp_PTS_Infoseti_stg_int_rates set avg_month_int_rate = (select round(sum(amount * int_rate)/sum(amount), 0) - 0.66 from Temp_PTS_Infoseti_cred_reestr where year(generation) = 2025 and flag_kk = 'USUAL' and segment_rbp = 'RBP 2' group by segment_rbp, flag_kk) / 100.0 / 12.0	where segment_rbp = 'RBP 2'				and flag_kk = 'USUAL' and eomonth(generation, 0) >= @dt_back_test_from;
	update Temp_PTS_Infoseti_stg_int_rates set avg_month_int_rate = (select round(sum(amount * int_rate)/sum(amount), 0) - 0.66 from Temp_PTS_Infoseti_cred_reestr where year(generation) = 2025 and flag_kk = 'USUAL' and segment_rbp = 'RBP 3' group by segment_rbp, flag_kk) / 100.0 / 12.0	where segment_rbp = 'RBP 3'				and flag_kk = 'USUAL' and eomonth(generation, 0) >= @dt_back_test_from;
	update Temp_PTS_Infoseti_stg_int_rates set avg_month_int_rate = (select round(sum(amount * int_rate)/sum(amount), 0) - 0.66 from Temp_PTS_Infoseti_cred_reestr where year(generation) = 2025 and flag_kk = 'USUAL' and segment_rbp = 'RBP 4' group by segment_rbp, flag_kk) / 100.0 / 12.0	where segment_rbp = 'RBP 4'				and flag_kk = 'USUAL' and eomonth(generation, 0) >= @dt_back_test_from;

	;

	print ('timing, Temp_PTS_Infoseti_stg_int_rates ' + format(getdate(), 'HH:mm:ss'))
	...
	drop table if exists Temp_PTS_Infoseti_stg5_interest;
	...
	into Temp_PTS_Infoseti_stg5_interest--select *
	from Temp_PTS_Infoseti_stg2_interest a
	left join Temp_PTS_Infoseti_stg_int_rates c
	on a.term = c.term
	and a.segment_rbp = c.segment_rbp
	and a.flag_kk = c.flag_kk
	and a.generation = c.generation
	...
	from Temp_PTS_Infoseti_stg5_interest a
	where a.flag_kk in ('BANKRUPT');


	print ('timing, Temp_PTS_Infoseti_stg5_interest ' + format(getdate(), 'HH:mm:ss'))
	...
	----бакеты 0-90
	drop table if exists Temp_PTS_Infoseti_stg_interest_model;
	...
	into Temp_PTS_Infoseti_stg_interest_model

	from Temp_PTS_Infoseti_stg4_back_test_results a
	left join Temp_PTS_Infoseti_stg5_interest b
	on a.term = b.term
	and a.segment_rbp = b.segment_rbp
	and a.flag_kk = b.flag_kk
	and a.generation = b.generation
	...
	print ('timing, Temp_PTS_Infoseti_stg_interest_model USUAL 0-90 ' + format(getdate(), 'HH:mm:ss'));




	--средний портфель в качестве базы начисления %% на NPL
	drop table if exists Temp_PTS_Infoseti_NPL_avg_portf;
	...
	into Temp_PTS_Infoseti_NPL_avg_portf
	from un a
	left join un b
	on a.term = b.term
	and a.segment_rbp = b.segment_rbp
	...
	print ('timing, Temp_PTS_Infoseti_NPL_avg_portf' + format(getdate(), 'HH:mm:ss'));


	--бакеты 90+ - через MOD_NUM - кол-во месяцев в дефолте

	insert into Temp_PTS_Infoseti_stg_interest_model
	...
	--from Temp_PTS_Infoseti_NPL_avg_portf a
	--left join Temp_PTS_Infoseti_stg5_interest b
	--on a.term = b.term
	--and a.segment_rbp = b.segment_rbp
	--and a.flag_kk = b.flag_kk
	...
	print ('timing, Temp_PTS_Infoseti_stg_interest_model USUAL 90+ ' + format(getdate(), 'HH:mm:ss'));
	...
	drop table if exists Temp_PTS_Infoseti_stg_freeze_int_last_fact;
	with base as (
		...
		sum(a.total_od) as total_od,
		sum(a.total_int) as total_int
		from Temp_PTS_Infoseti_fact_due_int a
	...
	into Temp_PTS_Infoseti_stg_freeze_int_last_fact
	from base a
	;
	...
	drop table if exists Temp_PTS_Infoseti_stg22_default_recovery_model;
	...
	eomonth(a.mob_date_to, -1 * a.mod_num) as default_dt,
	sum(a.model_od) as model_od
	into Temp_PTS_Infoseti_stg22_default_recovery_model
	from Temp_PTS_Infoseti_stg_freeze_model a
	where a.bucket_90_to = '[05] 90+'
	and a.mod_num is not null
	and a.mod_num >= 0
	...
	print ('timing, Temp_PTS_Infoseti_stg22_default_recovery_model' + format(getdate(), 'HH:mm:ss'))
	...
	--declare @rdt date = '2024-10-31';declare @dt_back_test_from date = '2024-11-30';declare @horizon int = datediff(MM,@rdt,'2030-12-31') + 1;declare @repeat_share float = 0.62;declare @vers int = 369;
		...
		drop table if exists Temp_PTS_Infoseti_stg33_default_recovery_model;
		...
		into Temp_PTS_Infoseti_stg33_default_recovery_model

		from Temp_PTS_Infoseti_stg22_default_recovery_model a
		left join Temp_PTS_Infoseti_conditional_rec_ALL b
		--on a.mod_num >= b.mod_num
		on b.mod_num between a.mod_num - DATEDIFF(MM, @dt_back_test_from, a.mob_date_to) and a.mod_num
		--and iif(a.flag_kk <> 'Installment', 'PTS', 'Installment') = b.product
	...
	while @cycle_dt3 <= EOMONTH(@dt_back_test_from, @horizon)  begin
		...
		insert into Temp_PTS_Infoseti_stg33_default_recovery_model
		...
		print ('timing, Temp_PTS_Infoseti_stg33_default_recovery_model @cycle_dt3 = ' + format(@cycle_dt3, 'dd.MM.yyyy') + ' ' + format(getdate(), 'HH:mm:ss'));

	set @cycle_dt3 = eomonth(@cycle_dt3,1);
	end 


	print ('timing, Temp_PTS_Infoseti_stg33_default_recovery_model' + format(getdate(), 'HH:mm:ss'));



	---средний портфель в качестве базы начисления %% на NPL - Кредитные каникулы и заморозка
	drop table if exists Temp_PTS_Infoseti_NPL2_avg_portf;
	with base as (
		select a.term, a.segment_rbp, a.flag_kk, a.generation, a.mob_date_to, a.mob_to, 
		a.bucket_90_from,
		cast('[05] 90+' as varchar(100)) as bucket_90_to, 
		...
		from Temp_PTS_Infoseti_stg33_default_recovery_model a
		where a.model_od > 0
		and left(a.flag_kk,2) = 'KK'
	), agg as (
		select a.term, a.segment_rbp, a.flag_kk, a.generation, a.mob_date_to, a.mob_to,
	...
	into Temp_PTS_Infoseti_NPL2_avg_portf
	from base a
	left join agg b
	on a.term = b.term
	and a.segment_rbp = b.segment_rbp 
	...
	with base as (
		...
		from Temp_PTS_Infoseti_stg_freeze_model a
		left join Temp_PTS_Infoseti_stg_int_rates b--Temp_PTS_Infoseti_stg4_interest b
		on a.term = b.term
		and a.segment_rbp = b.segment_rbp
		and a.flag_kk = b.flag_kk 
		and a.generation = b.generation
	...
	union all
		...
		from Temp_PTS_Infoseti_NPL2_avg_portf a --Temp_PTS_Infoseti_stg33_default_recovery_model a
		left join Temp_PTS_Infoseti_stg_int_rates b--Temp_PTS_Infoseti_stg4_interest b
		on a.term = b.term
		and a.segment_rbp = b.segment_rbp
		and a.flag_kk = b.flag_kk 
	...
	from Temp_PTS_Infoseti_stg_interest_model a
	left join Temp_PTS_Infoseti_stg_interest_model b
	on 1=1
	--and a.term = b.term
	--and a.segment_rbp = b.segment_rbp
	--and a.generation = b.generation
	...
	print ('timing, Temp_PTS_Infoseti_stg_interest_model ' + format(getdate(), 'HH:mm:ss'))
	...
	drop table if exists Temp_PTS_Infoseti_stg_int_rec_wo;
	with aa as (
		...
		from (
			...
			from Temp_PTS_Infoseti_stg1_default_recovery_fact a --Temp_PTS_Infoseti_default_recovery_fact a
		) a
		...
		from (
			...
			from Temp_PTS_Infoseti_stg3_default_recovery_model a --Temp_PTS_Infoseti_default_recovery_model a
		) a
	...
	into Temp_PTS_Infoseti_stg_int_rec_wo
	...
	print ('timing, Temp_PTS_Infoseti_stg_int_rec_wo ' + format(getdate(), 'HH:mm:ss'))




	--то же самое для КК с исходящим бакетом
	drop table if exists Temp_PTS_Infoseti_stg2_int_rec_wo;
	...
	with base as (
		...
		from Temp_PTS_Infoseti_stg33_default_recovery_model a --Temp_PTS_Infoseti_default_recovery_model a
	), agg as (
	...
	into Temp_PTS_Infoseti_stg2_int_rec_wo
	from base a
	left join agg b
	on a.term = b.term
	and a.segment_rbp = b.segment_rbp
	...
	drop table if exists Temp_PTS_Infoseti_interest_model;
	...
	into Temp_PTS_Infoseti_interest_model
	from base a
	left join ovr b
	on 1=1
	--and a.term = b.term
	...
	left join Temp_PTS_Infoseti_stg_int_rec_wo c --рекавери и списания 90+
	on 1=1
	--and a.term = c.term
	--and a.segment_rbp = c.segment_rbp 
	--and a.flag_kk = c.flag_kk
	...
	from Temp_PTS_Infoseti_stg_int_rec_wo c
	left join base a 
	on 1=1
	--and a.term = c.term
	--and a.segment_rbp = c.segment_rbp 
	...
	--Расчет для КК
	drop table if exists Temp_PTS_Infoseti_kk_interest;
	...
	into Temp_PTS_Infoseti_kk_interest
	from Temp_PTS_Infoseti_fact_due_int a --Temp_PTS_Infoseti_freeze_last_fact a
	where left(a.flag_kk,2) = 'KK'
	...
	insert into Temp_PTS_Infoseti_kk_interest
	...
	from Temp_PTS_Infoseti_fact_due_int a
	left hash join Temp_PTS_Infoseti_for_back_test b
	on a.term = b.term
	and a.generation = b.generation
	and a.segment_rbp = b.segment_rbp
	and a.flag_kk = b.flag_kk
	...
	from Temp_PTS_Infoseti_stg2_int_rec_wo c
	left join Temp_PTS_Infoseti_kk_interest a --рекавери и списания 90+
	on 1 = 1
	--and a.term = c.term
	--and a.segment_rbp = c.segment_rbp 
	--and a.flag_kk = c.flag_kk
	...
	from Temp_PTS_Infoseti_kk_interest a
	left join ovr b
	on 1 = 1
	--and a.term = b.term
	--and a.segment_rbp = b.segment_rbp 
	...
	and isnull(a.mod_num,-999) = isnull(b.mod_num,-999)
	and isnull(a.last_MOD,-999) = isnull(b.last_MOD,-999)
	left join Temp_PTS_Infoseti_stg2_int_rec_wo c --рекавери и списания 90+
	on 1 = 1
	--and a.term = c.term
	--and a.segment_rbp = c.segment_rbp 
	--and a.flag_kk = c.flag_kk
	...
	insert into Temp_PTS_Infoseti_interest_model
	...
	from Temp_PTS_Infoseti_stg4_back_test_results a
	left join Temp_PTS_Infoseti_stg_int_rates b
	on a.term = b.term
	and a.segment_rbp = b.segment_rbp
	and a.flag_kk = b.flag_kk
	and a.generation = b.generation
	...
	print ('timing, Temp_PTS_Infoseti_interest_model ' + format(getdate(), 'HH:mm:ss'))
	...
	begin
		...
		drop table if exists Temp_PTS_Infoseti_tmp_interest_model;
		select * 
		into Temp_PTS_Infoseti_tmp_interest_model
		from Temp_PTS_Infoseti_interest_model a
		where a.mob_date_to = @jj
		--and a.total_int > 1
		;


		drop table if exists Temp_PTS_Infoseti_tmp_kk_interest;
		select * 
		into Temp_PTS_Infoseti_tmp_kk_interest
		from Temp_PTS_Infoseti_kk_interest a
		where a.mob_date_to = @jj
		--and a.model_int > 1
		;
		...
		with base as ( --Последний факт
			...
			sum(a.total_int * b.model_coef_adj) as total_int
			from Temp_PTS_Infoseti_tmp_interest_model a
			left join Temp_PTS_Infoseti_for_back_test b
			on 1 = 1
			--and a.term = b.term
			--and a.segment_rbp = b.segment_rbp
		...
		from Temp_PTS_Infoseti_stg_int_rec_wo c --рекавери и списания 90+
		left join base a
		on 1=1
		--and a.term = c.term
		--and a.segment_rbp = c.segment_rbp 
		...
		* case a.bucket_90_to when '[03] 31-60' then 0.75 when '[04] 61-90' then 0.15 end 
		...
		from Temp_PTS_Infoseti_tmp_kk_interest a
		where left(a.flag_kk,2) = 'KK'
		and a.bucket_90_to in ('[03] 31-60','[04] 61-90')
		--and a.model_int > 0
		and a.freeze_from is null --без повторных, только продление
		...
		* case when b.bucket_90_from = '[03] 31-60' and a.freeze_from is null and a.mob_date_to < '2021-10-31' then 0.25
		...
		from Temp_PTS_Infoseti_stg2_int_rec_wo c --рекавери и списания 90+

		left join Temp_PTS_Infoseti_kk_interest a
		on 1=1
		--and a.term = c.term
		--and a.segment_rbp = c.segment_rbp 
		--and a.flag_kk = c.flag_kk
	...
	print ('timing, Temp_PTS_Infoseti_interest_model cycle, @jj = ' + format(@jj,'dd.MM.yyyy') + ' ' + format(getdate(), 'HH:mm:ss'));
	...
	drop table Temp_PTS_Infoseti_tmp_interest_model;
	drop table Temp_PTS_Infoseti_tmp_kk_interest;
	...
	*/
	...
	drop table if exists Temp_PTS_Infoseti_model_payment_interest
	...
	into Temp_PTS_Infoseti_model_payment_interest
	from Temp_PTS_Infoseti_stg_interest_model a
	...
	from Temp_PTS_Infoseti_stg_int_rec_wo a
	...
	select a.term, a.segment_rbp, a.flag_kk, a.generation, a.mob_date_to, 
		...
		sum(isnull(a.int_recov,0)) as int_pmt,
		sum(isnull(a.int_wo,0)) as int_wo
	from Temp_PTS_Infoseti_stg2_int_rec_wo a
	where left(a.flag_kk,2) = 'KK'
	group by a.term, a.segment_rbp, a.flag_kk, a.generation, a.mob_date_to
	;
	...
	drop table if exists Temp_PTS_Infoseti_for_update_pmt_int;
	select a.term, a.segment_rbp, a.flag_kk, a.generation, a.mob_date_to, 
	...
	into Temp_PTS_Infoseti_for_update_pmt_int
	from Temp_PTS_Infoseti_kk_interest a
	left join Temp_PTS_Infoseti_stg_interest_model b
	on a.id1 = b.id1
	and a.mob_date_to = b.mob_date_to
	...
	merge into Temp_PTS_Infoseti_model_payment_interest a
	using Temp_PTS_Infoseti_for_update_pmt_int b
	on (a.term = b.term
		and a.segment_rbp = b.segment_rbp
		and a.generation = b.generation
		and a.flag_kk = b.flag_kk
	...
	from Temp_PTS_Infoseti_model_payment_interest a
	left join volume b
	on a.term = b.term
	and a.segment_rbp = b.segment_rbp
	and a.flag_kk = b.flag_kk
	...
	insert into Temp_PTS_Infoseti_model_payment_interest
	...
	print ('timing, Temp_PTS_Infoseti_model_payment_interest ' + format(getdate(), 'HH:mm:ss'))
	;
	...
	----drop table Temp_PTS_Infoseti_virt_gens;
	--/* P 5 */
	--drop table Temp_PTS_Infoseti_for_update_pmt_int;
	----drop table Temp_PTS_Infoseti_interest_model;
	--drop table Temp_PTS_Infoseti_kk_interest;
	----drop table Temp_PTS_Infoseti_model_payment_interest;
	--drop table Temp_PTS_Infoseti_NPL_avg_portf;
	--drop table Temp_PTS_Infoseti_NPL2_avg_portf;
	--drop table Temp_PTS_Infoseti_stg_freeze_int_last_fact;
	----drop table Temp_PTS_Infoseti_stg_int_rates;
	----drop table Temp_PTS_Infoseti_stg_int_rec_wo;
	--drop table Temp_PTS_Infoseti_stg_interest_model;
	--drop table Temp_PTS_Infoseti_stg1_interest;
	----drop table Temp_PTS_Infoseti_stg2_int_rec_wo;
	--drop table Temp_PTS_Infoseti_stg2_interest;
	--drop table Temp_PTS_Infoseti_stg22_default_recovery_model;
	--drop table Temp_PTS_Infoseti_stg33_default_recovery_model;
	--drop table Temp_PTS_Infoseti_stg5_interest;
	--drop table Temp_PTS_Infoseti_upd_stg2_interest;
	...
	drop table if exists Temp_PTS_Infoseti_stg_eps_rates;
	with rates as (
		select a.term, a.segment_rbp, a.flag_kk, a.generation, 
		...
		--on a.external_id = b.external_id
		--and b.r_date = @rdt
		inner join Temp_PTS_Infoseti_eps b
		on a.external_id = b.external_id
	...
	into Temp_PTS_Infoseti_stg_eps_rates
	from base a
	where a.rown = 1;
	...
	--	from dwh2.risk.credits a
	--	left join Temp_PTS_Infoseti_eps b
	--	on a.external_id = b.external_id
	--	left join risk.stg_fcst_fix_rbp_2 fr
	--	on a.external_id = fr.external_id
	--	where a.generation between '2023-04-01' and '2023-04-30'
	...
	--from Temp_PTS_Infoseti_stg_eps_rates a
	--inner join upd b
	--on a.term = b.term
	--and a.segment_rbp = b.segment_rbp
	--and a.generation = b.generation
	...
	print ('timing, Temp_PTS_Infoseti_stg_eps_rates ' + format(getdate(), 'HH:mm:ss'))


	--коэффициент залогового покрытия
	drop table if exists Temp_PTS_Infoseti_pledgecover;
	select a.rep_dt, a.vers,
		max((rr_90_plus_pldg - rr_90_plus) / (1 - rr_90_plus)) as koef
	into Temp_PTS_Infoseti_pledgecover
	from risk.prov_stg_prov_rates_0_90 a
	where 1=1
	group by a.rep_dt, a.vers
	;

	print ('timing, Temp_PTS_Infoseti_pledgecover ' + format(getdate(), 'HH:mm:ss'))
	...
	drop table if exists Temp_PTS_Infoseti_det_BU_prov_rates;
	with base as (
		...
		left join Temp_PTS_Infoseti_pledgecover b
	...
	select 
		...
		as pledge_cover
	into Temp_PTS_Infoseti_det_BU_prov_rates
	from base a 
	;
	...
	print ('timing, Temp_PTS_Infoseti_det_BU_prov_rates ' + format(getdate(), 'HH:mm:ss'))


	--ставки НУ
	drop table if exists Temp_PTS_Infoseti_det_NU_prov_rates;
	...
	into Temp_PTS_Infoseti_det_NU_prov_rates
	from base a
	;
	...
	drop table if exists Temp_PTS_Infoseti_det_NU_prov_rates_PDL;
	...
	into Temp_PTS_Infoseti_det_NU_prov_rates_PDL
	from base a
	...
	--Распределение ПДН для новых выдач
	drop table if exists Temp_PTS_Infoseti_pdn_distrib;

	select 
	cast(a.pdn_group as varchar(100)) as pdn_group,
	cast(a.segment_rbp as varchar(100)) as segment_rbp,
	cast(a.frac as float) as frac
	into Temp_PTS_Infoseti_pdn_distrib
	from (values
	...
	*/
	...
	print ('timing, Temp_PTS_Infoseti_pdn_distrib ' + format(getdate(), 'HH:mm:ss'))


	--аналитические ставки НУ - обратным расчетом
	drop table if exists Temp_PTS_Infoseti_analyt_prov_NU_rates;
	...
	into Temp_PTS_Infoseti_analyt_prov_NU_rates
	from Temp_PTS_Infoseti_stg1_NU_segment_distrib a
	...
	update Temp_PTS_Infoseti_analyt_prov_NU_rates set prov_rate = 1 where prov_rate > 1;
	...
	drop table if exists Temp_PTS_Infoseti_model_od_payment;
	select a.term, a.segment_rbp, a.flag_kk, a.generation, 
	--a.mob_to, 
	--a.mob_date_to,
	b.mob_to, 
	...
	into Temp_PTS_Infoseti_model_od_payment
	from Temp_PTS_Infoseti_stg4_back_test_results a
	left join Temp_PTS_Infoseti_for_back_test b
	on a.term = b.term
	and a.segment_rbp = b.segment_rbp
	...
	from Temp_PTS_Infoseti_model_od_payment a
	left join volume b
	on a.term = b.term 
	and a.segment_rbp = b.segment_rbp
	and a.flag_kk = b.flag_kk
	...
	insert into Temp_PTS_Infoseti_model_od_payment
	select a.term, a.segment_rbp, a.flag_kk, a.generation,
	0 as mob_to,
	...
	print ('timing, Temp_PTS_Infoseti_model_od_payment ' + format(getdate(), 'HH:mm:ss'))




	--платежи ОД и %% по бакетам
	drop table if exists Temp_PTS_Infoseti_model_all_payment;
	with default_pmt as (
	...
	into Temp_PTS_Infoseti_model_all_payment
	from u
	left join Temp_PTS_Infoseti_model_payment_interest m
	on u.term = m.term
	...
	print ('timing, Temp_PTS_Infoseti_model_all_payment ' + format(getdate(), 'HH:mm:ss'))
	...
	select a.term, a.segment_rbp, a.flag_kk, a.generation, a.mob_date_to, a.bucket_90_from
	from Temp_PTS_Infoseti_model_all_payment a
	group by a.term, a.segment_rbp, a.flag_kk, a.generation, a.mob_date_to, a.bucket_90_from
	having count(*)>1
	;
	*/
	...
	--финальная сборка 
	drop table if exists Temp_PTS_Infoseti_final_model;
	...
	with fact_od_payment as (
		...
		from Temp_PTS_Infoseti_stg_fact_od_int_pmt a
	...
	into Temp_PTS_Infoseti_final_model

	from Temp_PTS_Infoseti_stg4_back_test_results a

	left join Temp_PTS_Infoseti_model_all_payment f
	on a.term = f.term
	...
	left join Temp_PTS_Infoseti_stg_int_rates irts
	on a.term = irts.term
	...
	print ('timing, Temp_PTS_Infoseti_final_model ' + format(getdate(), 'HH:mm:ss'))
	...
	a.*
	from Temp_PTS_Infoseti_final_model a 
	;
	...
	*/
	...
	----drop table Temp_PTS_Infoseti_stg2_int_rec_wo;
	--/* P 6-7 */
	----drop table Temp_PTS_Infoseti_analyt_prov_NU_rates;
	----drop table Temp_PTS_Infoseti_det_BU_prov_rates;
	----drop table Temp_PTS_Infoseti_det_NU_prov_rates;
	----drop table Temp_PTS_Infoseti_det_NU_prov_rates_PDL;
	----drop table Temp_PTS_Infoseti_final_model;
	--drop table Temp_PTS_Infoseti_model_all_payment;
	--drop table Temp_PTS_Infoseti_model_od_payment;
	----drop table Temp_PTS_Infoseti_pdn_distrib;
	--drop table Temp_PTS_Infoseti_pledgecover;
	----drop table Temp_PTS_Infoseti_stg_eps_rates;
	...
	drop table if exists Temp_PTS_Infoseti_base;
	...
	from total a
	left join Temp_PTS_Infoseti_pdn_distrib b
	...
	print ('timing, Temp_PTS_Infoseti_base ' + format(getdate(), 'HH:mm:ss'));
	...
	drop table if exists Temp_PTS_Infoseti_virt_IFRS_rates_0_90;
	...
	into Temp_PTS_Infoseti_virt_IFRS_rates_0_90
	...
	insert into Temp_PTS_Infoseti_virt_IFRS_rates_0_90
	select 
	cast('INSTALLMENT' as varchar(100)) as product,
	a.dpd_bucket, 
	sum(a.prov_IFRS_TTC) / sum(a.gross) as prov_rate
	...
	drop table if exists Temp_PTS_Infoseti_virt_IFRS_rates_91_plus;

	create table Temp_PTS_Infoseti_virt_IFRS_rates_91_plus (product varchar(100),months_in_default int, prov_rate float);

	declare @mm int = 0;

	while @mm < 400 begin

	insert into Temp_PTS_Infoseti_virt_IFRS_rates_91_plus (product,months_in_default, prov_rate) values ('PTS',@mm, 1.0);
	insert into Temp_PTS_Infoseti_virt_IFRS_rates_91_plus (product,months_in_default, prov_rate) values ('INSTALLMENT',@mm, 1.0);
	...
	merge into Temp_PTS_Infoseti_virt_IFRS_rates_91_plus dst
	...
	drop table if exists Temp_PTS_Infoseti_det_LGD_IL;
	select 
	cast(a.mod_num as int) as mod_num, 
	cast(a.lgd as float) as lgd
	into Temp_PTS_Infoseti_det_LGD_IL
	from (values
	...
	merge into Temp_PTS_Infoseti_virt_IFRS_rates_91_plus dst
	using (select * from Temp_PTS_Infoseti_det_LGD_IL) src
	...
	drop table if exists Temp_PTS_Infoseti_det_BU_IL_DEC23_REC;
	select 
	a.mod_n-1 as mod_n,
	cast(a.recovery_rate as float) as recovery_rate
	into Temp_PTS_Infoseti_det_BU_IL_DEC23_REC
	from (values
	(1 , 0.007951563),
	(2 , 0.0037317878),
	(3 , 0.00506509),
	...
	select @cycle_int1 = max(mod_n)+1 from Temp_PTS_Infoseti_det_BU_IL_DEC23_REC;
	while @cycle_int1 < 500 begin
	insert into Temp_PTS_Infoseti_det_BU_IL_DEC23_REC (mod_n, recovery_rate) values (@cycle_int1, 0)
	...
	drop table if exists Temp_PTS_Infoseti_det_BU_IL_DEC23_PD;
	select 
	cast(a.dpd_bucket_90 as varchar(100)) as dpd_bucket_90,
	cast(a.koef as float) as koef
	into Temp_PTS_Infoseti_det_BU_IL_DEC23_PD
	from (values
	...
	into Temp_PTS_Infoseti_det_BU_IL_DEC23_LGD
	from acc a
	order by 1
	;


	drop table if exists Temp_PTS_Infoseti_det_BU_IL_DEC23_0_90;
	select 
	a.dpd_bucket_90, 
	a.koef as PD, 
	a.koef * b.lgd as prov_rate
	into Temp_PTS_Infoseti_det_BU_IL_DEC23_0_90
	from Temp_PTS_Infoseti_det_BU_IL_DEC23_PD a
	left join Temp_PTS_Infoseti_det_BU_IL_DEC23_LGD b
	on b.mod_n = 0
	;
	...
	--ставки и дисконты для БУ в 2025 году
	drop table if exists Temp_PTS_Infoseti_virt_BU_rates;
	with base as (
		select 
		cast(a.dt_from as date) as dt_from, 
		cast(a.dt_to as date) as dt_to, 
	...
	into Temp_PTS_Infoseti_virt_BU_rates
	from base a
	left join rates b
	...
	drop table if exists Temp_PTS_Infoseti_BU_rates_BUSINESS;
	select 
	cast(a.dpd_bucket as varchar(100)) as dpd_bucket, 
	cast(a.rate as float) as rate
	into Temp_PTS_Infoseti_BU_rates_BUSINESS
	from (values
	('[01] 0' , 0.015),
	('[02] 1-30' , 0.05),
	('[03] 31-60' , 0.3),
	...
	drop table if exists Temp_PTS_Infoseti_for_finance;
	...
	with model as (
		...
		from Temp_PTS_Infoseti_base a

		--13.07.2022
		left join Temp_PTS_Infoseti_det_BU_prov_rates b
		...
		left join Temp_PTS_Infoseti_stg_eps_rates ep
		on a.term = ep.term
		and a.segment_rbp = ep.segment_rbp
		and a.generation = ep.generation
		and a.flag_kk = ep.flag_kk

		left join Temp_PTS_Infoseti_det_NU_prov_rates d
		...
		left join Temp_PTS_Infoseti_analyt_prov_NU_rates e
		...
		left join Temp_PTS_Infoseti_det_NU_prov_rates_PDL nupdl --с октября 2024 по кредитам со ставкой от 250% применяются коэф-ты PDL
		on a.bucket_90 = nupdl.dpd_bucket_90
		and a.flag_kk = 'INSTALLMENT'
		and a.mob_date_to >= '2024-10-01'
		and a.term in (3,6)

		left join Temp_PTS_Infoseti_det_pieces_L1 d1
		on a.bucket_360 = d1.bucket_360_m

		left join Temp_PTS_Infoseti_det_pieces_L2 d2
		on a.bucket_360 = d2.bucket_360_m
		and iif(a.segment_rbp = 'RBP 1','RBP 1','rest') = d2.segment_rbp_2

		--3.10.2022
		left join Temp_PTS_Infoseti_virt_IFRS_rates_0_90 ifrs1
		on a.bucket_360 = ifrs1.dpd_bucket
		and iif(a.flag_kk = 'INSTALLMENT', 'INSTALLMENT', 'PTS') = ifrs1.product

		left join Temp_PTS_Infoseti_virt_IFRS_rates_91_plus ifrs2
		on a.months_in_default = ifrs2.months_in_default
		and iif(a.flag_kk = 'INSTALLMENT', 'INSTALLMENT', 'PTS') = ifrs2.product

		--23.11.2023
		left join Temp_PTS_Infoseti_det_BU_IL_DEC23_0_90 buil090
		on a.bucket_90 = buil090.dpd_bucket_90
		left join Temp_PTS_Infoseti_det_BU_IL_DEC23_LGD buillgd
		on a.months_in_default = buillgd.mod_n

		--27.12.2023
		left join Temp_PTS_Infoseti_virt_BU_rates bu24
		...
		left join Temp_PTS_Infoseti_BU_rates_BUSINESS bus_bu
		...
		on a.external_id = b.external_id
		--where a.cli_type = 'ФЛ'
		inner join Temp_PTS_Infoseti_UMFO_NU_segment d
		on a.external_id = d.external_id
		and a.r_date = d.r_date
		left join Temp_PTS_Infoseti_zamorozka z
		on a.external_id = z.external_id
	...
	into Temp_PTS_Infoseti_for_finance
	...
	print ('timing, Temp_PTS_Infoseti_for_finance ' + format(getdate(), 'HH:mm:ss'));
	...
	drop table if exists Temp_PTS_Infoseti_pmt_base;
	...
	into Temp_PTS_Infoseti_pmt_base

	from u as a
	inner join Temp_PTS_Infoseti_NU_segment_distrib b
	on a.term = b.term
	and a.segment_rbp = b.segment_rbp
	and a.generation = b.generation
	and a.flag_kk = b.flag_kk
	...
	print ('timing, Temp_PTS_Infoseti_pmt_base (model) ' + format(getdate(), 'HH:mm:ss'));
	...
	with pmt_and_writeoff as (
		...
		from (
			...
			cast(0 as float) as od_wo, 
			cast(0 as float) as int_wo
			from Temp_PTS_Infoseti_stg_payment a
	...
	insert into Temp_PTS_Infoseti_pmt_base
	...
	on a.external_id = b.external_id 
	and a.mob_date_to = eomonth(b.cdate)
	inner join Temp_PTS_Infoseti_UMFO_NU_segment c
	on a.external_id = c.external_id
	...
	merge into Temp_PTS_Infoseti_pmt_base dst
	...
	(not exists (select 1 from Temp_PTS_Infoseti_stg_matrix_detail md where a.external_id = md.external_id) and a.flag_closed_in_month = 1)
	group by a.term, a.generation, a.segment_rbp, a.flag_kk,
	c.NU_segment
	) src
	...
	print ('timing, Temp_PTS_Infoseti_pmt_base (fact) ' + format(getdate(), 'HH:mm:ss'));
	...
	*/
	...
	from Temp_PTS_Infoseti_for_finance a
	full join Temp_PTS_Infoseti_pmt_base b
	on a.term = b.term
	and a.segment_rbp = b.segment_rbp
	and a.flag_kk = b.flag_kk 
	and a.generation = b.generation
	...
	*/
	...
	----drop table Temp_PTS_Infoseti_stg_eps_rates;
	--/* P 8 */
	----drop table Temp_PTS_Infoseti_base;
	--drop table Temp_PTS_Infoseti_det_BU_IL_DEC23_0_90;
	--drop table Temp_PTS_Infoseti_det_BU_IL_DEC23_LGD;
	--drop table Temp_PTS_Infoseti_det_BU_IL_DEC23_PD;
	--drop table Temp_PTS_Infoseti_det_BU_IL_DEC23_REC;
	--drop table Temp_PTS_Infoseti_det_LGD_IL;
	----drop table Temp_PTS_Infoseti_for_finance;
	----drop table Temp_PTS_Infoseti_pmt_base;
	--drop table Temp_PTS_Infoseti_virt_BU_rates;
	--drop table Temp_PTS_Infoseti_virt_IFRS_rates_0_90;
	--drop table Temp_PTS_Infoseti_virt_IFRS_rates_91_plus;
	...
	insert into risk.core_fcst_pmt_base select a.*, @vers from Temp_PTS_Infoseti_pmt_base a;

	delete from risk.core_fcst_npl_wo_and_recovery where vers = @vers;
	insert into risk.core_fcst_npl_wo_and_recovery select a.*, @vers from Temp_PTS_Infoseti_npl_wo_and_recovery a;

	delete from risk.core_fcst_int_rates where vers = @vers;
	insert into risk.core_fcst_int_rates select a.*, @vers from Temp_PTS_Infoseti_stg_int_rates a;

	----------------------------------------------------------------

	delete from risk.core_fcst_p8_base where vers = @vers;
	insert into risk.core_fcst_p8_base select a.*, @vers from Temp_PTS_Infoseti_base a;

	delete from risk.core_fcst_eps_rates where vers = @vers;
	insert into risk.core_fcst_eps_rates select a.*, @vers from Temp_PTS_Infoseti_stg_eps_rates a;

	delete from risk.core_fcst_analyt_prov_NU_rates where vers = @vers;
	insert into risk.core_fcst_analyt_prov_NU_rates select a.*, @vers from Temp_PTS_Infoseti_analyt_prov_NU_rates a;

	delete from risk.core_fcst_det_pieces_L1 where vers = @vers;
	insert into risk.core_fcst_det_pieces_L1 select a.*, @vers from Temp_PTS_Infoseti_det_pieces_L1 a;

	delete from risk.core_fcst_det_pieces_L2 where vers = @vers;
	insert into risk.core_fcst_det_pieces_L2 select a.*, @vers from Temp_PTS_Infoseti_det_pieces_L2 a;
	...
	insert into risk.core_fcst_NU_segment_distrib select a.*, @vers from Temp_PTS_Infoseti_NU_segment_distrib a;
	...
	/*
	select * from Temp_PTS_Infoseti_cred_reestr a
	where not exists (select 1 from Temp_PTS_Infoseti_UMFO_NU_segment b where a.external_id = b.external_id)
	;
	...
	*/
	...
	select *
	from (select * from Temp_PTS_Infoseti_for_finance a where a.mob_date_to between '2021-12-31' and '2027-04-30') a
	full join Temp_PTS_Infoseti_pmt_base b
	on a.term = b.term
	and a.segment_rbp = b.segment_rbp
	and a.flag_kk = b.flag_kk 
	...
	*/
	...
	select a.mob_date_to, a.term, a.segment_rbp, a.flag_kk, a.generation, a.NU_segment, a.dpd_bucket
	from Temp_PTS_Infoseti_pmt_base a
	group by a.mob_date_to, a.term, a.segment_rbp, a.flag_kk, a.generation, a.NU_segment, a.dpd_bucket
	having count(*)>1;



	select min(mob_date_to), max(mob_date_to) from Temp_PTS_Infoseti_for_finance;
	select min(mob_date_to), max(mob_date_to) from Temp_PTS_Infoseti_pmt_base;


	select * from Temp_PTS_Infoseti_for_finance a where a.fact_od = 0 and a.fact_pieces > 0;
	select * from Temp_PTS_Infoseti_for_finance a where a.fact_od > 0 and a.fact_pieces = 0;

	select * from Temp_PTS_Infoseti_for_finance a where a.model_od = 0 and a.model_pieces > 0;
	select * from Temp_PTS_Infoseti_for_finance a where a.model_od > 0 and a.model_pieces = 0;
	...
	*/
	...
	--выдачи
	drop table if exists Temp_PTS_Infoseti_stg_volumes;
	...
	into Temp_PTS_Infoseti_stg_volumes
	from Temp_PTS_Infoseti_final_model a
	where a.mob_to = 0
		and a.bucket_90_to = '[01] 0'
	...
	--процентная ставка
	drop table if exists Temp_PTS_Infoseti_stg_interest_rates;
	...
	into Temp_PTS_Infoseti_stg_interest_rates
	from Temp_PTS_Infoseti_final_model a
	where a.mob_to = 0
	and a.bucket_90_to = '[01] 0'
	group by a.segment_rbp,
	...
	drop table if exists Temp_PTS_Infoseti_pmt_wo_for_nl;
	select  
		1 as term,
		a.segment_rbp,
		a.generation,
	...
	into Temp_PTS_Infoseti_pmt_wo_for_nl
	from Temp_PTS_Infoseti_npl_wo_and_recovery a
	group by a.segment_rbp,
		a.generation,
		case 
	...
	drop table if exists Temp_PTS_Infoseti_for_demonstration;
	...
	with base as (
		...
		left join Temp_PTS_Infoseti_for_cessed_GL c
		on a.term = c.term
			and a.segment_rbp = c.segment_rbp
			and a.generation = c.generation
			and a.flag_kk = c.flag_kk
	...
	into Temp_PTS_Infoseti_for_demonstration
	from base a
	left join Temp_PTS_Infoseti_stg_volumes b
	on a.term = b.term
		and a.segment_rbp = b.segment_rbp
		and a.generation = b.generation
		and a.subportf = b.subportf

	left join Temp_PTS_Infoseti_stg_interest_rates c
	on a.term = c.term
		and a.segment_rbp = c.segment_rbp
		and a.generation = c.generation
	...
	insert into Temp_PTS_Infoseti_for_demonstration
	select DISTINCT
		a.scenario,
		a.term,
		a.segment_rbp,
		...
		a.avg_int_rate
	from Temp_PTS_Infoseti_for_demonstration a
	left join Temp_PTS_Infoseti_stg_volumes b
	on a.segment_rbp = b.segment_rbp
	and a.generation = b.generation
	and a.subportf = b.subportf
	where 1=1
	--and a.MoB = 1
	--and not exists (select 1 from Temp_PTS_Infoseti_for_demonstration b where a.segment_rbp = b.segment_rbp and a.generation = b.generation and b.MoB = 1 and b.dpd_bucket_90 = '[03] 91-360')
	and not exists (select 1 from Temp_PTS_Infoseti_for_demonstration b 
		where a.segment_rbp = b.segment_rbp
		and a.generation = b.generation 
		and a.subportf = b.subportf
		and a.MoB = b.MoB
	...
	drop table if exists Temp_PTS_Infoseti_for_update_NLGL;
	with a as (
	...
	into Temp_PTS_Infoseti_for_update_NLGL
	from a
	left join Temp_PTS_Infoseti_pmt_wo_for_nl b
		on a.term = b.term
		and a.segment_rbp = b.segment_rbp
		and a.generation = b.generation
		and a.subportf = b.subportf
	...
	from Temp_PTS_Infoseti_for_demonstration a
	left join Temp_PTS_Infoseti_for_update_NLGL b
		on a.term = b.term
		and a.segment_rbp = b.segment_rbp
		and a.generation = b.generation
		and a.subportf = b.subportf
	...
	delete from Temp_PTS_Infoseti_for_demonstration where MoB < 0;
	...
	select
		...
		cash_total,
		avg_int_rate
	from Temp_PTS_Infoseti_for_demonstration
	;
	...
	--Возращаем фактические платежи в нулевом MoB для последнего поколения (чтобы входящая сумма была равна сумме выдачи)
		...
		from Temp_PTS_Infoseti_fix_matrix a
		inner join Temp_PTS_Infoseti_new_volume b
	...
	from Temp_PTS_Infoseti_virt_gens a
	left join Temp_PTS_Infoseti_det_stress st
