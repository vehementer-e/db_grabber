CREATE PROC dbo.prc$for_check_svd_bal_dashboard

as

declare @srcname varchar(100) = N'FOR CHECK DASHBOARD';

----- найдем договора, которые ушли и вернулись из просрочки в течении дня
declare  @DateCalculate date = cast(dateadd(day,-1, dateadd(year,2000,getdate())) as date)
...
------------------------------
  ...
  if object_id('tempdb.dbo.#change_bucket') is not null drop table #change_bucket
	  ...
	  from    [Stg].[_1cCMR].[РегистрСведений_АналитическиеПоказателиМФО] ap 
	   ...
	   where   ap.Период >= dateadd(day,-1 /*0*/ ,@dt_begin_of_month ) and ap.Период < @dt_next_month
...
left join (
		 ...
		 -- добавим ОД
		  ...
		  delete from dbo.zzz_dashboard_svd_bal 
		  where Период = Format(dateadd(year,-2000,@dt_begin_of_month),'yyyy-MM')


		  insert into dbo.zzz_dashboard_svd_bal
		  ...
		  ---into dbo.zzz_dashboard_svd_bal

		  from #bucket_transfer tr
		  left join [Reports].[dbo].[dm_CMRStatBalance_2] b
		  on tr.external_id = b.external_id 
		  and b.d /*!!!!*/ = --tr.[Дата перехода бакета]
		  
		  --13/08/2020 Datsyplakov - учитываем сумму од, которая была до перехода, а не после перехода
		  case when tr.[Дата перехода бакета] = tr.Дата then dateadd(dd,-1,tr.[Дата перехода бакета])
		  else tr.[Дата перехода бакета] end
		  ...
		  drop table #bucket_transfer;
		  drop table #change_bucket;
		  drop table #change_bucket_by_day;
		  drop table #change_bucket_by_day_and_tomorrow;
		  drop table #change_bucket_final;
		  drop table #deals;	
