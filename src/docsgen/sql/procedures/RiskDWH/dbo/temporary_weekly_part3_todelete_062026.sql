...
create PROC [dbo].[temporary_weekly_part3]
...
--все платежи 91+ и ИП
DROP TABLE IF EXISTS #stg_pmt_91_plus;
	SELECT a.r_date
		,a.external_id
		,a.dpd_bucket_p
		,a.overdue_days_p
...
DROP TABLE IF EXISTS #creds_npl
	SELECT aa.external_id
		,aa.r_date
...
drop table if exists #rep_coll_weekly_npl_portf;
...
into #rep_coll_weekly_npl_portf
FROM #_t00_new a
left JOIN #creds_npl c ON a.external_id = c.external_id
	AND a.r_date = c.r_date
left join #stg_pmt_91_plus pl on a.external_id = pl.external_id and a.r_date = pl.r_date
GROUP BY a.seg3
	,c.seg_ip_hard_agent
	,pl.seg_ip_hard_agent
	,CASE 
...
truncate table RiskDWH.[CM\a.kurikalov].[sndbx_rep_coll_weekly_npl_portf];
if OBJECT_ID('RiskDWH.[CM\a.kurikalov].[sndbx_rep_coll_weekly_npl_portf]') is null
begin
	select  top(0) * into RiskDWH.[CM\a.kurikalov].[sndbx_rep_coll_weekly_npl_portf]
	from #rep_coll_weekly_npl_portf
end;

INSERT INTO RiskDWH.[CM\a.kurikalov].[sndbx_rep_coll_weekly_npl_portf]
SELECT * FROM #rep_coll_weekly_npl_portf


drop table if exists #rep_coll_weekly_npl_portf_det;

SELECT a.external_id , @rdt AS rep_dt, a.r_date as a_r_date 
	,cast(SYSDATETIME() AS DATETIME) AS dt_dml
	,CONCAT (
		a.seg3
		,'#'
	...
	,CASE 
		...
		ELSE @flag_kk_total
		END AS flag_exclude_kk --15.02.2022
into #rep_coll_weekly_npl_portf_det
FROM #_t00_new a
left JOIN #creds_npl c ON a.external_id = c.external_id
	AND a.r_date = c.r_date
left join #stg_pmt_91_plus pl on a.external_id = pl.external_id and a.r_date = pl.r_date;
...
truncate table RiskDWH.[CM\a.kurikalov].[sndbx_rep_coll_weekly_npl_portf_det];
if OBJECT_ID('RiskDWH.[CM\a.kurikalov].[sndbx_rep_coll_weekly_npl_portf_det]') is null
begin
	select  top(0) * into RiskDWH.[CM\a.kurikalov].[sndbx_rep_coll_weekly_npl_portf_det]
	from #rep_coll_weekly_npl_portf_det
end;
INSERT INTO RiskDWH.[CM\a.kurikalov].[sndbx_rep_coll_weekly_npl_portf_det]
SELECT * FROM #rep_coll_weekly_npl_portf_det;

truncate table RiskDWH.[CM\a.kurikalov].[sndbx_stg_pmt_91_plus];
if OBJECT_ID('RiskDWH.[CM\a.kurikalov].[sndbx_stg_pmt_91_plus]') is null
begin
	select  top(0) * into RiskDWH.[CM\a.kurikalov].[sndbx_stg_pmt_91_plus]
	from #stg_pmt_91_plus
end;

truncate table RiskDWH.[CM\a.kurikalov].[sndbx_stg_pmt_91_plus];
INSERT INTO RiskDWH.[CM\a.kurikalov].[sndbx_stg_pmt_91_plus]
SELECT * FROM #stg_pmt_91_plus;


if OBJECT_ID('RiskDWH.[CM\a.kurikalov].[sndbx_stg_ip_0_90]') is null
begin
	select  top(0) * into RiskDWH.[CM\a.kurikalov].[sndbx_stg_ip_0_90]
	from #stg_ip_0_90
end;

truncate table RiskDWH.[CM\a.kurikalov].[sndbx_stg_ip_0_90];
INSERT INTO RiskDWH.[CM\a.kurikalov].[sndbx_stg_ip_0_90]
SELECT * FROM #stg_ip_0_90;
...
DROP TABLE #CMR;
DROP TABLE #rep_coll_weekly_npl_portf;
DROP TABLE #rep_coll_weekly_npl_portf_det;
