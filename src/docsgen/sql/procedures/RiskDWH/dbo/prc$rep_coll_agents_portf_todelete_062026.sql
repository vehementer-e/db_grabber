...
CREATE procedure [dbo].[prc$rep_coll_agents_portf] 
...
exec dbo.prc$set_debug_info @src = @srcname, @info = '#pre_application';
...
select 
	...
	where exists (select * from dwh2.dbo.dm_overdueindicators t
				where z.Номер = t.number);


exec dbo.prc$set_debug_info @src = @srcname, @info = '#stg_application';
...
exec dbo.prc$set_debug_info @src = @srcname, @info = '#credreg1';
...
left join RiskDWH.dbo.det_region_pattern b
on upper(trim(a.region)) like b.pattern;


exec dbo.prc$set_debug_info @src = @srcname, @info = '#credreg2';
...
exec dbo.prc$set_debug_info @src = @srcname, @info = '#stg_final_app';
...
left join RiskDWH.dbo.det_region_pattern z
...
----для разметки 0-90 hard
	...
	exec dbo.prc$set_debug_info @src = @srcname, @info = '#stg_hard90_1';
...
exec dbo.prc$set_debug_info @src = @srcname, @info = '#stg_hard90_2';
...
--3) проверяем, что во время периодов между 91+ не было выхода в нулевой бакет
	...
	exec dbo.prc$set_debug_info @src = @srcname, @info = '#stg_hard90_31';
	...
	exec dbo.prc$set_debug_info @src = @srcname, @info = '#stg_hard90_32';
...
--4) проверяем, чтобы были платежи во время периодов между 91+
	...
	exec dbo.prc$set_debug_info @src = @srcname, @info = '#stg_hard90_4';
...
exec dbo.prc$set_debug_info @src = @srcname, @info = '#stg_hard90_5';
...
exec dbo.prc$set_debug_info @src = @srcname, @info = '#mfo_base';
	...
	drop table if exists #mfo_base;
		...
		where b.r_date between  cast('2019-12-01' as date) and @rdt
...
exec dbo.prc$set_debug_info @src = @srcname, @info = 'merge1 into #mfo_base (kk)';
...
exec dbo.prc$set_debug_info @src = @srcname, @info = '#stg_sb_0_90';
...
exec dbo.prc$set_debug_info @src = @srcname, @info = 'merge2 into #mfo_base (SB)';
...
exec dbo.prc$set_debug_info @src = @srcname, @info = '#reestr_0_90_hard';
...
exec dbo.prc$set_debug_info @src = @srcname, @info = 'merge3 into #mfo_base (0-90 hard)';
...
--база для отчета - дополняем отметками дат: конец месяца и конец недели, и инфо по агентам
	...
	on a.external_id = b.external_id
	and a.r_date between b.st_date and b.end_date
	where a.dpd_bucket_p_ in ('(5)_91_360','(6)_361+','[09] 0-90 hard')
	;


   	  
exec RiskDWH.dbo.prc$set_debug_info @src = @srcname, @info = 'REP WEEKLY';
...
exec RiskDWH.dbo.prc$set_debug_info @src = @srcname, @info = 'REP MONTHLY';
