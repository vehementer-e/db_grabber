...
CREATE procedure [dbo].[prc$update_rep_coll_weekly_part2_2] 
...
begin
	...
	declare @date_from date = CAST(  DATEADD(yy, DATEDIFF(yy,0,getdate()) - 1, 0) AS DATE); --начало прошлого года
	...
	*/
	...
	---------забирвем мартовские данные
	--drop table if exists #migr_MAR20;
	--select * into #migr_MAR20 from dbo.rep_coll_weekly_migr
	--where seg3 = 'MAR20'
	...
	;
	--insert into dbo.rep_coll_weekly_migr select * from #migr_MAR20;


	exec RiskDWH.dbo.prc$set_debug_info @src = @src_name, @info = 'rep_coll_weekly_recov_alt';


	--drop table if exists #recov_alt_MAR20;
	--select * into #recov_alt_MAR20 from dbo.rep_coll_weekly_recov_alt
	--where seg3 = 'MAR20'
	...
	aa.pay_total,
	aa.recovery_rate,
	case when aa.product in ('INSTALLMENT', 'PDL') then 2 else @flag_kk_total end as flag_exclude_kk /*15.02.2022*/
	...
	(
		...
		case when sum(a.principal_rest) = 0 then 0 else
		sum(a.pay_total) / sum(a.principal_rest)  end as recovery_rate,
		a.product /*15.02.2022*/

		from #t00_new a
		where a.dpd_bucket <> '(1)_0'
		group by a.seg3, replace(a.dpd_bucket, ')', '##)'), a.product
	...
	union all
		...
		from #t00_new a
		where a.dpd_bucket <> '(1)_0'
		group by a.seg3, case when a.dpd_bucket in ('(2)_1_30','(3)_31_60','(4)_61_90') then '(4###)_1_90'
		when a.dpd_bucket in ('(5)_91_120','(6)_121_150','(7)_151_180','(8)_181_360','(9)_361+') then '(9###)_91+'
		end,
		a.product /*15.02.2022*/
	...
	--insert into dbo.rep_coll_weekly_recov_alt select * from #recov_alt_MAR20;
	...
	----забираем мартовские данные
	--drop table if exists #svd_bal_MAR20;
	--select * into #svd_bal_MAR20 from dbo.rep_coll_weekly_svd_bal
	--where seg = 'MAR20'
	...
	--insert into dbo.rep_coll_weekly_svd_bal select * from  #svd_bal_MAR20;
