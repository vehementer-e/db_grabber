...
-- =============================================
CREATE PROCEDURE [etl].[base_etl_mt_credit_portfolio_mfo]
...
/*
  ...
  begin
	-- выполняем скрипт на старом связанном сервере
		exec [etl].[base_etl_mt_credit_portfolio_mfo_OldDPC]
  end
...
into #BusinessLoan_UMFO
  ...
  where dd.[ПометкаУдаления]=0x00 AND dd.[Проведен]=0x01 and fp.[Родитель] = 0x810800155D01C00511E86A1E934E0BAE --and fp.[Наименование] like N'Бизнес%займ%'
--)
--select * from #BusinessLoan_UMFO order by [Дата] desc 
...
into #Repayment_UMFO
from [Stg].[_1cUMFO].[РегистрБухгалтерии_БНФОБанковский] rgpln  with (nolock)
	left join  [Stg].[_1cUMFO].[ПланСчетов_БНФОБанковский] pln1 with (nolock)
		on rgpln.[СчетДт]=pln1.[Ссылка]
	left join  [Stg].[_1cUMFO].[ПланСчетов_БНФОБанковский] pln2 with (nolock)
...
--		on rgpln.[Регистратор_Ссылка]=doc1.[Ссылка]	
	left join (	select zp.[Регистратор_Ссылка] ,zp.[Займ] ,zp.[Контрагент] 
				from  [Stg].[_1cUMFO].[РегистрНакопления_АЭ_ВзаиморасчетыПоЗаймамПредоставленным] zp with (nolock)
		...
		left join  [Stg].[_1cUMFO].[Документ_АЭ_ЗаймПредоставленный] dd0 with (nolock)
...
select 
dateadd(year,-2000,dateadd(MONTH,datediff(MONTH,0,ras.[Период]),0)) as [ПериодУчета] 
...
--	  ,cl.[Наименование] as [КонтрагентНаим]
	  ...
	  ,dateadd(year,-2000,dateadd(MONTH,datediff(MONTH,0,cast([ДатаПлатежа] as datetime2)),0)) as [ПериодУчетаДатаПлатежа]
...
--from Prodsql01.[UMFO].[dbo].[РегистрНакопления_АЭ_ВзаиморасчетыПоГрафикуЗаймовПредоставленных] ras with (nolock)
from [Stg].[_1cUMFO].[РегистрНакопления_АЭ_ВзаиморасчетыПоГрафикуЗаймовПредоставленных] ras with (nolock)
...
where ras.[Займ] in (select [Ссылка] from #BusinessLoan_UMFO) 
		and ras.[ВидНачисления]<>0x9B7650E549564EF611E720526D97FF6F 
		and ras.[ДатаПлатежа]>='4016-01-01 00:00:00' 
		and ras.[ДатаПлатежа]<='4035-01-01 00:00:00' 
		--and ras.[ВидДвижения]=1

-- 2021+02_12 -- комментируем до восстановления
/*
union all
...
--from Prodsql01.umfo.[dbo].[РегистрНакопления_АЭ_ВзаиморасчетыПоШтрафамЗаймовПредоставленных] vsh with (nolock)
  from [Stg].[_1cUMFO].[РегистрНакопления_АЭ_ВзаиморасчетыПоШтрафамЗаймовПредоставленных] vsh with (nolock)
...
delete from dwh_new.dbo.mt_GetDefaultCreditsAndBalanceByParam where  [start_date] >=  dateadd(month,-1,dateadd(MONTH,datediff(MONTH,0,Getdate()),0))	--dateadd(year,2000,cast(getdate() as date))

insert into dwh_new.dbo.mt_GetDefaultCreditsAndBalanceByParam  with(tablockx)
select * from dwh_new.dbo.GetDefaultCreditsAndBalanceByParam(0, 1 , 4) where [start_date] >=  dateadd(month,-1,dateadd(MONTH,datediff(MONTH,0,Getdate()),0))

--select * from dwh_new.dbo.mt_GetDefaultCreditsAndBalanceByParam order by [start_date] desc
