...
CREATE PROCEDURE [etl].[base_etl_mt_requests_transition_crm]
...
select 
      ...
      ,zl.[Ответственный] as [ИсполнительСсылка]
	  ,u0.[Наименование] as [ИсполнительНаименование]
      ,zl.[ПричинаОтказа] as [ПричинаСсылка]
	  ,null  as [ПричинаНаименование]
	  ,r.[НаименованиеПолное] [ПричинаОтказаНаименование]
/*	  ,zrs.[Представление] as [ПричинаНаименование]
	  ,r.[Наименование] as [ПричинаОтказаНаименование]
*/
into #ts_a
  FROM stg._1cCRM.[РегистрСведений_СтатусыЗаявокНаЗаймПодПТС] zl with (nolock)
	LEFT JOIN stg._1cCRM.[Документ_ЗаявкаНаЗаймПодПТС] z  with (nolock) -- zayvka
	ON zl.[Заявка]=z.[Ссылка]
	LEFT JOIN stg._1cCRM.[Справочник_Пользователи] u0  with (nolock) --user
	ON zl.[Ответственный]=u0.[Ссылка]
	LEFT JOIN stg._1cCRM.[Справочник_СтатусыЗаявокПодЗалогПТС] zs with (nolock)
	ON zl.[Статус]=zs.[Ссылка]
/*	LEFT JOIN stg._1cCRM.[Перечисление_ГП_ПричиныСтатусаЗаявки] zrs with (nolock)
	ON zl.[Причина]=zrs.[Ссылка]
*/
  left join stg._1cCRM.[Справочник_CRM_ПричиныОтказов] r with (nolock) on zl.[ПричинаОтказа]=r.[Ссылка]

  WHERE  zl.[Заявка] in (select * from #request_for)
		--zl.[Период] >= @DateStartCurr2000 
		--and	z.[Номер]=N'19110710000004'
		--z.[Дата]>=@DateStart2 
 group by zl.[Заявка] ,z.[Номер] ,dateadd(year,-2000,z.[Дата]) ,zl.[Статус] ,zs.[Наименование] 
			,zl.[Ответственный] ,u0.[Наименование] ,zl.[ПричинаОтказа] /*,zrs.[Представление]*/,r.[НаименованиеПолное]
...
SELECT 
  ...
  FROM stg._1cCRM.[РегистрСведений_ПриостановкаВерификацииЗаявок] zs1  with (nolock) --zayvka stop
	LEFT JOIN stg._1cCRM.[Документ_ГП_Заявка] z1  with (nolock) -- zayvka
	ON zs1.[Заявка]=z1.[Ссылка]
	LEFT JOIN stg._1cCRM.[Справочник_Пользователи] u1  with (nolock) --user
	ON zs1.[Исполнитель]=u1.[Ссылка]
	LEFT JOIN stg._1cCRM.[Справочник_ПричиныПриостановкиВерификацииЗаявок] sr  with (nolock) --suspension reason 
...
delete from [dwh_new].[dbo].[mt_requests_transition_crm] 
...
insert into [dwh_new].[dbo].[mt_requests_transition_crm] (
...
/*
into [dwh_new].[dbo].[mt_requests_transition_crm]
*/
from #tmp
...
--select min([Период_Исх]) from [dwh_new].[dbo].[mt_requests_transition_crm]

-- select * from [dwh_new].[dbo].[mt_requests_transition_crm]