...
-- Description:	airflow etl   1c_cd_into_staging
-- exec etl.base_etl_1c_cd_into_staging '20191130','20200303 23:59:59'
-- select * from  staging.requests_history
-- before first run 
--					select *  into dwh_new_dev.staging.credits_delays from dwh_new.staging.credits_delays where 1=0

-- =============================================
CREATE PROCEDURE [etl].[base_etl_1c_cd_into_staging]
	 @start_date datetime
	,@end_date datetime
AS
BEGIN
...
begin try
  delete from staging.credits_delays where creation_date>=@start_date and creation_date<= @end_date

/** etl from airflow*/
	insert into staging.credits_delays
	select 
		r.Номер external_id ,
		dateadd(yy, -2000, rh.Период) creation_date,
		КоличествоПолныхДнейПросрочки overdue_days,
		ПросроченнаяЗадолженность overdue ,
		 r.Заявка as external_link
	from 
	[prodsql02].[mfo].[dbo].РегистрСведений_ГП_АналитическиеПоказателиЗайма rh
	...
	and КоличествоПолныхДнейПросрочки >0 
	/** etl from airflow*/
