...
CREATE PROCEDURE [etl].[base_etl_report_verification_fedor_prod_new0]
...
select t.*	--t.[ВремяЗатрачено]/ (select * from #uniq_Qty) AVG_queue_time
		,'Время заявки в ожидании очереди (queue time) на этапе' Indicator		
...
select 
		...
		,r.employee	
		,sum([ВремяЗатрачено]) [ВремяЗатрачено]
		,'Кол-во одобренных займов после этапа' Indicator
...
-- select * from #appr_Qty_2 where [ЗаявкаНомер_Исх]='20030300013507' employee = ''
...
select 
		...
		,[State_prev]
		,[State_next]
		,[frequency] = (case when not [accdate_month] is null and [accdate] is null then 'Месяц' when [accdate_month] is null then 'День' end)
		,[Qty] 
...
--select * from #verification_table where not [accdate_month] is null
...
--begin
	...
	delete 
	--select *
	from [dwh_new].[dbo].[mt_report_verification_fedor_prod_new] 
	 --where [accdate]>= @StartDate --  [begindate] >= @StartDate

	insert into [dwh_new].[dbo].[mt_report_verification_fedor_prod_new]  ([period]  
																,[rdate]
																,[accdate_month]
																,[accdate]
																,[Indicator]
...
--drop table [dwh_new].[dbo].[mt_report_verification_fedor_prod_new]
	select *
	--into [dwh_new].[dbo].[mt_report_verification_fedor_prod_new]    
...
--   select * from [dwh_new].[dbo].[mt_report_verification_fedor_prod_new] where [frequency]='Месяц' order by 3 desc
