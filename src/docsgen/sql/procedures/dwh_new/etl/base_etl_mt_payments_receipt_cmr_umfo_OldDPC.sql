...
--				
-- =============================================
CREATE PROCEDURE [etl].[base_etl_mt_payments_receipt_cmr_umfo_OldDPC]
...
set @DateStart= dateadd(MONTH,datediff(MONTH,0,dateadd(month,0,Getdate())),0);
...
select dd.[Ссылка]
  ...
  from [C1-VSR-SQL07].[umfo].[dbo].[Документ_АЭ_ЗаймПредоставленный] dd  with (nolock) --y
    left join [C1-VSR-SQL07].[umfo].[dbo].[Справочник_АЭ_ФинансовыеПродукты] fp  with (nolock) --y
...
from [C1-VSR-SQL07].[umfo].[dbo].[РегистрНакопления_АЭ_ВзаиморасчетыПоГрафикуЗаймовПредоставленных]  with (nolock)
...
select --	  [Период] ,[Регистратор_Ссылка] ,
      ...
      ,case when [ВидДвижения]=0 then [Сумма] else 0 end as [СуммаНачислено]
      ,case when [ВидДвижения]=1 then [Сумма] else 0 end as [СуммаОплачено]
from [C1-VSR-SQL07].[umfo].[dbo].[РегистрНакопления_АЭ_ВзаиморасчетыПоШтрафамЗаймовПредоставленных]  with (nolock)
where [Займ] in (select distinct [Ссылка] from BusinessLoan_UMFO)
