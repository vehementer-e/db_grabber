...
--	exec [etl].[base_etl_report_collecting_receipt_cmr_2]			
-- =============================================
CREATE PROCEDURE [etl].[base_etl_report_collecting_receipt_cmr_2]
...
into #TempTable_CollectingPayIn
  ...
  where [ДатаОперации] >= dateadd(month,datediff(month,0,dateadd(day,-1,Getdate())),0) and [ДатаОперации] < dateadd(day,datediff(day,0,Getdate()),0) --and [ПериодУчета]>=@DateStart 
...
--select * from #PaymentReceiptCMR_1c
...
where [Indicator] in (N'Платежи по ОД' ,N'Платежи по процентам' ,N'Платежи по пеням') and [cdate] >= dateadd(month,datediff(month,0,Getdate()),0) and [Факт/План] = N'Факт'
...
where [НаименованиеЛиста] in (N'Платежи по ОД' ,N'Платежи по процентам' ,N'Платежи по пеням') and [ДатаОперации] >= dateadd(month,datediff(month,0,Getdate()),0)
