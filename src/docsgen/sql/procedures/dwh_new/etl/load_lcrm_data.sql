...
CREATE procedure [etl].[load_lcrm_data]
as
begin    
  set nocount on
exec [log].[LogAndSendMailToAdmin] ' etl.load_lcrm_data','Info','procedure Started ',''
  ...
  set @dt=dateadd(day,-15,cast(getdate() as date))
  ...
  delete from staging.lcrm_tbl where UF_ACTUALIZE_AT>@dt
...
set @prev_i=@ii
  ...
  set @query='select  c1.id ,
     ...
     join     (select id from carmoney_light_crm   c2
       ...
       where 
        c2.id> '''''+format(@ii,'0')+'''''                
        and UF_ACTUALIZE_AT>'''''+format(@dt,'yyyy-MM-ddTHH:mm:ss')+'''''
order by c2.id
limit 10000
) c2 
      on c2.id=c1.id
               ' 
               /*
               where UF_ACTUALIZE_AT>'''''+format(@dt,'yyyy-MM-ddTHH:mm:ss')+'''''
               '
               */

 -- select @query
  set @tsql='INSERT INTO staging.lcrm_tbl with(tablockx) select * from OPENQUERY(LCRM,'''+@query+''')'
 -- set @tsql='select * from OPENQUERY(LCRM,'''+@query+''')'
  select @tsql 
   exec (@tsql)

--  
  set @ii=cast(isnull((select max(id) from staging.lcrm_tbl  where UF_ACTUALIZE_AT>@dt ),'0') as bigint)
