...
-- =============================================
CREATE PROCEDURE [etl].[base_etl_mt_credit_portfolio_mfo_OldDPC]
...
into #Repayment_UMFO
from [prodsql01].[UMFO].[dbo].[РегистрБухгалтерии_БНФОБанковский] rgpln  with (nolock)
	left join  [prodsql01].[UMFO].[dbo].[ПланСчетов_БНФОБанковский] pln1 with (nolock)
		on rgpln.[СчетДт]=pln1.[Ссылка]
	left join  [prodsql01].[UMFO].[dbo].[ПланСчетов_БНФОБанковский] pln2 with (nolock)
...
--		on rgpln.[Регистратор_Ссылка]=doc1.[Ссылка]	
	left join (	select zp.[Регистратор_Ссылка] ,zp.[Займ] ,zp.[Контрагент] 
				from  [prodsql01].[UMFO].[dbo].[РегистрНакопления_АЭ_ВзаиморасчетыПоЗаймамПредоставленным] zp with (nolock)
		...
		left join  [prodsql01].[UMFO].[dbo].[Документ_АЭ_ЗаймПредоставленный] dd0 with (nolock)
...
from [prodsql01].[UMFO].[dbo].[РегистрНакопления_АЭ_ВзаиморасчетыПоГрафикуЗаймовПредоставленных] ras with (nolock)
	...
	left join [prodsql01].[UMFO].[dbo].[Справочник_Контрагенты] cl with (nolock)
