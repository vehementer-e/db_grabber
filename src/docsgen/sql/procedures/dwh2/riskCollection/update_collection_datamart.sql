...
CREATE PROCEDURE [riskCollection].[update_collection_datamart] as 
begin
--exec [riskCollection].[update_collection_datamart]
declare @sp_name nvarchar(255) = OBJECT_NAME(@@PROCID);
declare @start_date date = '2025-02-17';
declare @end_date date = getdate();
declare @min_nd_dt as date;

BEGIN TRY
-------------------------------------------найти все несоответствия между балансами и витриной 
-------------------------------------------(по полям d,external_id, dpd_coll, dpd_p_coll,[остаток од], pay_total, prev_od)
drop table if exists #t;
select  *
into #t
from (
	select d,external_id, dpd_coll, dpd_p_coll,[остаток од], pay_total, prev_od
	from dbo.dm_CMRStatBalance stbal
	where d >= '2023-01-01'
	except
	select d,external_id, dpd_coll, dpd_p_coll,[остаток од], pay_total, prev_od
	from riskcollection.collection_datamart cdm
	where d >= '2023-01-01'
) t
/*
union
 select d,external_id, dpd_coll, dpd_p_coll,[остаток од], pay_total, prev_od
	...
	d>='2025-12-29'
	and ball_in_p is null
	*/
;
drop table if exists #new_dogs;
select stbal.* 
into #new_dogs 
from #t t
left join dbo.dm_CMRStatBalance stbal
	on t.external_id = stbal.external_id
	and t.d = stbal.d
;
set @min_nd_dt = (select min(d) from #new_dogs);
------------------------------------------------Кусок скрипта из create_collection_datamart, только источник теперь - new_dogs
...
where exists (select external_id from #new_dogs new_dogs where new_dogs.external_id = Deals.number)
;

drop table if exists #sotr; -- здесь вормируется "дата с" и "дата до" закрепления
select last_sotr.external_id 
...
select distinct(a.number) as external_id
...
where exists (select external_id from #new_dogs new_dogs where new_dogs.external_id = a.number)
;
-------------------------------------------Принтяие на баланс
...
(
	...
	and exists (select external_id from #new_dogs new_dogs where new_dogs.external_id = b.number)
	...
	and  exists (select external_id from #new_dogs new_dogs where new_dogs.external_id = Deals.number)
) t
;
-------------------------------------------Платежи от ФССП
...
and exists (select external_id from #new_dogs new_dogs where new_dogs.external_id = tt.number)
;
-------------------------------------------Признак банкротов
...
into #kk
from reports.dbo.dm_restructurings tt
...
,case when day(stbal.d) = 1 then stbal.prev_od --балансы входа в бакет на первое число
	...
	when stbal.d = cast(getdate() as date) and [dbo].[GetCollectionBucketName] (stbal.dpd_coll) <> stbal.[bucket_p_coll] then stbal.prev_od -- Для учета данных в текущий день
...
,stbal.bucket_last_coll
,case when day(stbal.d) = 1 then stbal.prev_od --балансы входа в бакет на первое число
	when day(stbal.d) > 1  and dwh2.[dbo].[GetCollectionBucketName] (stbal.prev_dpd_p_coll) <> stbal.[bucket_p_coll] then stbal.prev_od 
	when stbal.d = cast(getdate() as date) and dwh2.[dbo].[GetCollectionBucketName] (stbal.dpd_coll) <> stbal.[bucket_p_coll] then stbal.prev_od -- Для учета данных в текущий день
	else 0 --балансы входа в бакет в течение месяца
	end ball_in_p
into #balls
--from #new_dogs stbal
from dwh2.dbo.dm_CMRStatBalance stbal
where 1=1
and d >= dateadd(day,-30,@min_nd_dt)
and exists (select 1 from #new_dogs n_d where stbal.external_id = n_d.external_id)
;
...
select balls.d
,coalesce(
	dateadd(dd,-1, coalesce( lead(balls.d) over (partition by balls.external_id order by balls.d, balls.bucket_p_coll),
	lead(balls.d) over (partition by balls.external_id order by balls.d)) ), '2099-01-01'
	) as to_d
,balls.external_id
,balls.ball_in_p
into #balls_sb
...
and exists (select external_id from #new_dogs new_dogs where new_dogs.external_id = a.number)
group by a.number
...
where exists (select external_id from #new_dogs new_dogs where new_dogs.external_id = a.Код)
;
...
where exists (select external_id from #new_dogs new_dogs where new_dogs.external_id = dstage.external_id)
;
-------------------------------------------КА
...
where exists (select external_id from #new_dogs new_dogs where new_dogs.external_id = cstage.external_id)
and call_dt >= '2023-01-01'
;
-------------------------------------------Итог
...
,case when substring(stbal.bucket_p_coll, 2,1) > substring(stbal.bucket_coll, 2,1) 
	...
	then balls_sb.ball_in_p 
	else 0 
	end Saved_ballance
,case when substring(stbal.bucket_p_coll, 2,1) > substring(stbal.bucket_coll, 2,1) 
	and stbal.dpd_p_coll < 91
	and (stbal.pay_total > 0 or restr.restr = 1)
	then balls_sb.ball_in_p*(ac.k1-ac.k2)/nullif(ac.k1,0) 
	end reduced_balance
...
left join #External_Stage_history hi --на случай пропуска одного дня
	...
	and hi.rn = 1
	and hi.call_dt != '2025-03-17' --дата пробной страт., исключить
...
left join #ballance ballance
	...
	and stbal.d >= cast(ballance.AdoptionBalanceDate as date) 
	and ballance.rn = 1
...
left join #beznadega_dead beznadega
	on stbal.external_id = beznadega.external_id 
	and stbal.d >= cast(beznadega.createdate as date) 
	and beznadega.rn = 1 
	and beznadega.beznadega_status = 'Безнадёжное взыскание подтверждено'
left join #kk kk
...
BEGIN TRANSACTION
	...
	where not exists 
		(
		select 1 from dbo.dm_CMRStatBalance 
		where d = riskcollection.collection_datamart.d 
		and external_id = riskcollection.collection_datamart.external_id
		);
	
	merge into riskcollection.collection_datamart dm
	using #final cmr
		on dm.external_id = cmr.external_id
		and dm.d = cmr.d
	when matched then update set 
	dm.dpd_coll = cmr.dpd_coll,
	dm.dpd_p_coll = cmr.dpd_p_coll,
	dm.dpd_last_coll = cmr.dpd_last_coll,
	dm.bucket_coll = cmr.bucket_coll,
	dm.bucket_coll_num = cmr.bucket_coll_num,
	dm.bucket_p_coll = cmr.bucket_p_coll,
	dm.bucket_p_coll_num = cmr.bucket_p_coll_num,
	dm.bucket_last_coll = cmr.bucket_last_coll,
	dm.bucket_last_coll_num = cmr.bucket_last_coll_num,
	dm.bucket_last_p_coll = cmr.bucket_last_p_coll,
	dm.bucket_last_p_coll_num = cmr.bucket_last_p_coll_num,
	dm.[остаток од] = cmr.[остаток од],
	dm.prev_od = cmr.prev_od,
	dm.prev_dpd_coll = cmr.prev_dpd_coll,
	dm.prev_dpd_p_coll = cmr.prev_dpd_p_coll,
	dm.pay_total = cmr.pay_total,
	dm.ball_in_p1 = cmr.ball_in_p1,
	dm.inflow = cmr.inflow,
	dm.inflow_old = cmr.inflow_old,
	dm.Saved_ballance = cmr.Saved_ballance,
	dm.reduced_balance = cmr.reduced_balance,
	dm.ball_in_p = cmr.ball_in_p,
	dm.external_stage = cmr.external_stage,
	dm.ClaimantStage = cmr.ClaimantStage,
	dm.fio = cmr.fio,
	dm.crmclientstage = cmr.crmclientstage,
	dm.agent_name = cmr.agent_name,
	dm.claimant_fio = cmr.claimant_fio,
	dm.ballance_flag = cmr.ballance_flag,
	dm.[Сумма принятия на баланс] = cmr.[Сумма принятия на баланс],
	dm.death_flag = cmr.death_flag,
	dm.fssp_pays = cmr.fssp_pays,
	dm.beznadega_status = cmr.beznadega_status,
	dm.BankruptConfirmed = cmr.BankruptConfirmed,
	dm.kk_status = cmr.kk_status,
	dm.flag_IL = cmr.flag_IL,
	dm.claimant_ip_fio = cmr.claimant_ip_fio,
	dm.restr = cmr.restr,
	dm.bucket_p_portf = cmr.bucket_p_portf,
	dm.[остаток %] = cmr.[остаток %]
	...
	cmr.bucket_coll_num,
	cmr.bucket_p_coll,
	cmr.bucket_p_coll_num,
	cmr.bucket_last_coll,
	cmr.bucket_last_coll_num,
	cmr.bucket_last_p_coll,
	cmr.bucket_last_p_coll_num,
	cmr.[остаток од],
	cmr.prev_od,
	cmr.prev_dpd_coll,
	cmr.prev_dpd_p_coll,
	...
	cmr.claimant_fio,
	cmr.ballance_flag,
	cmr.[Сумма принятия на баланс],
	cmr.death_flag,
	cmr.fssp_pays,
	cmr.beznadega_status,
	cmr.BankruptConfirmed,
	cmr.kk_status,
	cmr.flag_IL,
	cmr.claimant_ip_fio,
	cmr.restr,
