-- =======================================================
-- Create: 29.05.2025. А.Никитин
-- Description:	заполнение линков между hub.БКИ_НФ_ИсторияСобытий и другими хабами
-- =======================================================
create   PROC link.exec_fill_link_between_БКИ_НФ_ИсторияСобытий_and_other
...
BEGIN
	...
	SELECT @eventName = 'dwh2.link.exec_fill_link_between_БКИ_НФ_ИсторияСобытий_and_other', @eventType = 'info', @SendEmail = 0

	BEGIN TRY

		DECLARE cur_LinkName CURSOR FOR
		SELECT S.LinkName, cnt = count(*)
		FROM link.БКИ_НФ_ИсторияСобытий_stage AS S
		GROUP BY S.LinkName
		ORDER BY cnt DESC
		...
		BEGIN  
			...
			begin
				begin try
					EXEC link.fill_link_between_БКИ_НФ_ИсторияСобытий_and_other
						@LinkName = @LinkName
				...
				begin CATCH
					...
					SELECT @message = 'Ошибка вызова link.fill_link_between_БКИ_НФ_ИсторияСобытий_and_other'
