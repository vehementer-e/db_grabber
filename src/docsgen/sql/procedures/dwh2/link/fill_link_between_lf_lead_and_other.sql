--
/*
exec link.fill_link_between_lf_lead_and_other 'link.lf_lead_product_type'

*/
--
CREATE   PROC link.fill_link_between_lf_lead_and_other
...
begin try
	...
	begin
		...
		declare @cmd_merge nvarchar(max) =
			...
			from (
					select distinct
						guid_lead,
						LinkGuid,
						nRow = ROW_NUMBER() over(partition by guid_lead order by created_at desc)
					from #t_2Insert
					) s
				where s.nRow = 1
			) s
			on	s.[guid_lead] =  t.[guid_lead]
		...
		delete t from link.lf_lead_stage t
