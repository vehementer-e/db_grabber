-- =======================================================
-- Create: 23.10.2024. А.Никитин
-- Description:	заполнение линков между hub.lf_referral_visit и другими хабами
-- =======================================================
CREATE   PROC link.exec_fill_link_between_lf_referral_visit_and_other
...
BEGIN
	...
	SELECT @eventName = 'dwh2.link.exec_fill_link_between_lf_referral_visit_and_other', @eventType = 'info', @SendEmail = 0

	BEGIN TRY

		DECLARE cur_LinkName CURSOR FOR
		SELECT S.LinkName, cnt = count(*)
		FROM link.lf_referral_visit_stage AS S
		GROUP BY S.LinkName
		ORDER BY cnt DESC
		...
		BEGIN  
			...
			begin
				begin try
					EXEC link.fill_link_between_lf_referral_visit_and_other
						 @LinkName = @LinkName
				...
				begin CATCH
					...
					SELECT @message = 'Ошибка вызова link.fill_link_between_lf_referral_visit_and_other'
