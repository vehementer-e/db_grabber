...
-- Description:	заполнение линков между hub.CRE_REJECT_LOG_302 и другими хабами
-- =======================================================
create   PROC link.exec_fill_link_between_CRE_REJECT_LOG_302_and_other
...
BEGIN
	...
	SELECT @eventName = 'dwh2.link.exec_fill_link_between_CRE_REJECT_LOG_302_and_other', @eventType = 'info', @SendEmail = 0

	BEGIN TRY

		DECLARE cur_LinkName CURSOR FOR
		SELECT S.LinkName, cnt = count(*)
		FROM link.CRE_REJECT_LOG_302_stage AS S
		GROUP BY S.LinkName
		ORDER BY cnt DESC
		...
		BEGIN  
			...
			begin
				begin try
					EXEC link.fill_link_between_CRE_REJECT_LOG_302_and_other
						@LinkName = @LinkName
				...
				begin CATCH
					...
					SELECT @message = 'Ошибка вызова link.fill_link_between_CRE_REJECT_LOG_302_and_other'
