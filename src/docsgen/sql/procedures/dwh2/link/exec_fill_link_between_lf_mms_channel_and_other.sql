...
-- Description:	заполнение линков между hub.lf_mms_channel и другими хабами
-- =======================================================
CREATE   PROC link.exec_fill_link_between_lf_mms_channel_and_other
...
BEGIN
	...
	SELECT @eventName = 'dwh2.link.exec_fill_link_between_lf_mms_channel_and_other', @eventType = 'info', @SendEmail = 0

	BEGIN TRY

		DECLARE cur_LinkName CURSOR FOR
		SELECT S.LinkName, cnt = count(*)
		FROM link.lf_mms_channel_stage AS S
		GROUP BY S.LinkName
		ORDER BY cnt DESC
		...
		BEGIN  
			...
			begin
				...
				begin CATCH
					...
					SELECT @message = 'Ошибка вызова link.fill_link_between_lf_mms_channel_and_other'
