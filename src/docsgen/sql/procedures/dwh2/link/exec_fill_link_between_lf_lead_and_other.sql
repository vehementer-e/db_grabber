...
-- Description:	заполнение линков между hub.lf_lead и другими хабами
-- =======================================================
CREATE PROC link.exec_fill_link_between_lf_lead_and_other
...
BEGIN
	...
	SELECT @eventName = 'dwh2.link.exec_fill_link_between_lf_lead_and_other', @eventType = 'info', @SendEmail = 0

	BEGIN TRY

		DECLARE cur_LinkName CURSOR FOR
		SELECT S.LinkName, cnt = count(*)
		FROM link.lf_lead_stage AS S (nolock)
		GROUP BY S.LinkName
		ORDER BY cnt DESC
		...
		BEGIN  
			...
			begin
				begin try
					EXEC link.fill_link_between_lf_lead_and_other
						 @LinkName = @LinkName
				...
				begin CATCH
					...
					SELECT @message = 'Ошибка вызова link.fill_link_between_lf_lead_and_other'
