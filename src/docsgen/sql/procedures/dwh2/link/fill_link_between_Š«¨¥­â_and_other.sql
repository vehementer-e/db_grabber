--EXEC link.fill_link_between_Клиент_and_other @LinkName = ''
CREATE   PROC link.fill_link_between_Клиент_and_other
...
begin try
	...
	begin
		...
		declare @cmd_merge nvarchar(max) =
			...
			from (
					select distinct
						...
						date_from,
						LinkGuid,
						nRow = ROW_NUMBER() over(partition by GuidКлиент,date_from order by created_at desc)
					from #t_2Insert
					) s
				where s.nRow = 1
			) s
			on s.GuidКлиент = t.GuidКлиент
		...
		delete t from link.Клиент_stage t
