--EXEC link.fill_link_between_ДоговорЗайма_and_other_SCD2 @LinkName = ''
create   PROC link.fill_link_between_ДоговорЗайма_and_other_SCD2
...
begin try
	...
	select top(0)
		...
		,created_at
	into #t_2Insert
	from link.ДоговорЗайма_stage_SCD2
	...
	begin
		...
		declare @cmd_merge nvarchar(max) =
			...
			from (
					select distinct
						...
						date_from,
						LinkGuid,
						nRow = ROW_NUMBER() over(partition by КодДоговораЗайма,date_from order by created_at desc)
					from #t_2Insert
					) s
				where s.nRow = 1
			) s
			on s.КодДоговораЗайма = t.КодДоговораЗайма
				and s.date_from = t.date_from
		...
		delete t from link.ДоговорЗайма_stage_SCD2 t
