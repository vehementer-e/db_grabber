...
*/
--
CREATE   PROC link.fill_link_between_lf_mms_decline_reason_and_other
...
begin try
	...
	select top(0)
		...
		,created_at
	into #t_2Insert
	from link.lf_mms_decline_reason_stage
	...
	begin
		...
		declare @cmd_merge nvarchar(max) =
			...
			from (
					select distinct
						guid_mms_decline_reason,
						LinkGuid,
						nRow = ROW_NUMBER() over(partition by guid_mms_decline_reason order by created_at desc)
					from #t_2Insert
					) s
				where s.nRow = 1
			) s
			on	s.guid_mms_decline_reason =  t.guid_mms_decline_reason
		...
		delete t from link.lf_mms_decline_reason_stage t
