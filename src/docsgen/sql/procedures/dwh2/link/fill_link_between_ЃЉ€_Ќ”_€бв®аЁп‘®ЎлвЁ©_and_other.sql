--EXEC link.fill_link_between_БКИ_НФ_ИсторияСобытий_and_other @LinkName = ''
create   PROC link.fill_link_between_БКИ_НФ_ИсторияСобытий_and_other
...
begin try
	...
	select top(0)
		...
		,created_at
	into #t_2Insert
	from link.БКИ_НФ_ИсторияСобытий_stage
	...
	begin
		...
		declare @cmd_merge nvarchar(max) =
			...
			from (
					select distinct
						GuidБКИ_НФ_ИсторияСобытий,
						LinkGuid,
						nRow = ROW_NUMBER() over(partition by GuidБКИ_НФ_ИсторияСобытий order by created_at desc)
					from #t_2Insert
					) s
				where s.nRow = 1
			) s
			on s.GuidБКИ_НФ_ИсторияСобытий = t.GuidБКИ_НФ_ИсторияСобытий
		...
		delete t from link.БКИ_НФ_ИсторияСобытий_stage t
