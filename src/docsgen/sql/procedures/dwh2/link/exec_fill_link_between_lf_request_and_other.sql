...
-- Description:	заполнение линков между lf_request и другими хабами
-- =======================================================
CREATE   PROC link.exec_fill_link_between_lf_request_and_other
...
BEGIN
	...
	SELECT @eventName = 'dwh2.link.exec_fill_link_between_lf_request_and_other', @eventType = 'info', @SendEmail = 0

	BEGIN TRY

		DECLARE cur_LinkName CURSOR FOR
		SELECT S.LinkName, cnt = count(*)
		FROM link.lf_request_stage AS S
		GROUP BY S.LinkName
		ORDER BY cnt DESC
		...
		BEGIN  
			...
			begin
				begin try
					EXEC link.fill_link_between_lf_request_and_other
						 @LinkName = @LinkName
				...
				begin CATCH
					...
					SELECT @message = 'Ошибка вызова link.fill_link_between_lf_request_and_other'
