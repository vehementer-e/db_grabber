--EXEC link.fill_link_between_ДоговорЗайма_and_other @LinkName = ''
CREATE PROC link.fill_link_between_ДоговорЗайма_and_other
...
begin try
	...
	begin
		...
		declare @cmd_merge nvarchar(max) =
			...
			from (
					select distinct
						КодДоговораЗайма,
						LinkGuid,
						nRow = ROW_NUMBER() over(partition by КодДоговораЗайма order by created_at desc)
					from #t_2Insert
					) s
				where s.nRow = 1
			) s
		...
		delete t from link.ДоговорЗайма_stage t
