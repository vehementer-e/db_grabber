--EXEC link.fill_link_between_Collection_JudicialProceeding_and_other @LinkName = ''
create   PROC link.fill_link_between_Collection_JudicialProceeding_and_other
...
begin try
	...
	select top(0)
		...
		,created_at
	into #t_2Insert
	from link.Collection_JudicialProceeding_stage
	...
	begin
		...
		declare @cmd_merge nvarchar(max) =
			...
			from (
					select distinct
						GuidCollection_JudicialProceeding,
						LinkGuid,
						nRow = ROW_NUMBER() over(partition by GuidCollection_JudicialProceeding order by created_at desc)
					from #t_2Insert
					) s
				where s.nRow = 1
			) s
			on s.GuidCollection_JudicialProceeding = t.GuidCollection_JudicialProceeding
		...
		delete t from link.Collection_JudicialProceeding_stage t
