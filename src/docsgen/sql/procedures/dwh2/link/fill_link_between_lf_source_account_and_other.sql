...
*/
--
CREATE   PROC link.fill_link_between_lf_source_account_and_other
...
begin try
	...
	select top(0)
		...
		,created_at
	into #t_2Insert
	from link.lf_source_account_stage
	...
	begin
		...
		declare @cmd_merge nvarchar(max) =
			...
			from (
					select distinct
						guid_source_account,
						LinkGuid,
						nRow = ROW_NUMBER() over(partition by guid_source_account order by created_at desc)
					from #t_2Insert
					) s
				where s.nRow = 1
			) s
			on	s.guid_source_account =  t.guid_source_account
		...
		delete t from link.lf_source_account_stage t
