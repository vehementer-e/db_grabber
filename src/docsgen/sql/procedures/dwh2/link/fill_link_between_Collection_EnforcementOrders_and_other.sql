--EXEC link.fill_link_between_Collection_EnforcementOrders_and_other @LinkName = ''
create   PROC link.fill_link_between_Collection_EnforcementOrders_and_other
...
begin try
	...
	select top(0)
		...
		,created_at
	into #t_2Insert
	from link.Collection_EnforcementOrders_stage
	...
	begin
		...
		declare @cmd_merge nvarchar(max) =
			...
			from (
					select distinct
						GuidCollection_EnforcementOrders,
						LinkGuid,
						nRow = ROW_NUMBER() over(partition by GuidCollection_EnforcementOrders order by created_at desc)
					from #t_2Insert
					) s
				where s.nRow = 1
			) s
			on s.GuidCollection_EnforcementOrders = t.GuidCollection_EnforcementOrders
		...
		delete t from link.Collection_EnforcementOrders_stage t
