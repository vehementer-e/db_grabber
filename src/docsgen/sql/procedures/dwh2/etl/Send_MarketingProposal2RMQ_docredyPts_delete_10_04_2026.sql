 ...
 -- [etl].[Send_MarketingProposal2RMQ_docredyPts_delete_10_04_2026] @env = 'prod', @CMRClientGUID = 'C6DCA609-08F9-11E8-A814-00155D941900'
CREATE      procedure [etl].[Send_MarketingProposal2RMQ_docredyPts_delete_10_04_2026]
...
begin
	...
	,@procedureName  nvarchar(255) = 'dwh2.[etl].[GetMarketProposal_docredy_pts_RMQ_JSON]'
	...
	exec logdb.dbo.[LogAndSendMailToAdmin] 
		'trying start [etl].[Send_MarketingProposal2RMQ_docredyPts_delete_10_04_2026]','Info',' started', ''
	...
	begin try
		...
		exec LogDb.etl.[RequestHttpWebService]
			...
			begin
				...
				declare @msg nvarchar(255)= Concat('Данные по проекту докреды ПТС загружены в RMQ'
					,', CMRClientGUID: ', isnull(@CMRClientGUID, '-')
					,isnull(' Отправлено: ' + cast(@TotalSendPackages as nvarchar(10)), '')
					)
				EXEC  [LogDb].dbo.SendToSlack_dwhNotification  @msg
				exec logdb.dbo.[LogAndSendMailToAdmin] 'exec [etl].[Send_MarkingProposal2RMQ_docredyPts]','Info','Done',''
			end
	...
	begin catch
		...
		set @error_description =CONCAT_WS(' '
			...
			,isnull('@CMRClientGUID:' + @CMRClientGUID, '')
			)
		...
		set @text=CONCAT_WS(':exclamation:','Ошибка загрузки данных по проекту докреды ПТС в RMQ',
			format(getdate(),'dd.MM.yyyy HH:mm:ss')
			,isnull('@CMRClientGUID:' + @CMRClientGUID, '')
			)
		EXEC  [LogDb].dbo.SendToSlack_dwhNotification  @text
      
		exec logdb.dbo.[LogAndSendMailToAdmin] 'catching error [etl].[Send_MarketingProposal2RMQ_docredyPts_delete_10_04_2026]','Error','Error',@error_description
