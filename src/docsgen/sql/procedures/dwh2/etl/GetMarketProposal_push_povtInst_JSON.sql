/*
--[etl].[GetMarketProposal_push_pov_installment_JSON] 'prod'
*/
CREATE    procedure  [etl].[GetMarketProposal_push_povtInst_JSON]
...
begin
	...
	where t.phoneInBlackList = 0
		...
		and (t.CMRClientGUID in (select trim(value) from string_split(@CMRClientGUIDs, ','))
...
begin
	...
	insert into #data2rmq(pushProfile2Send, approved_limit,phone,  push_communicationId, CMRClientGUID)
	select 
	...
	from (
		...
		and (t.CMRClientGUID in (select trim(value) from string_split(@CMRClientGUIDs, ','))
			or @CMRClientGUIDs is null)
		--and povt_inst.date2SendPush =  cast(getdate() as date)
		and phone is not null
