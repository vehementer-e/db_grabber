 ...
 --[etl].[Send_MarketingProposal2Naumen_docredy_pts] @env = 'prod', @isDebug = 1
 -- [etl].[Send_MarketingProposal2Naumen_docredy_pts] @env = 'uat',  @batchSizeToSendNaumen  = 1000,  @isDebug = 1,
-- set statistics io off
CREATE PROC [etl].[Send_MarketingProposal2Naumen_docredy_pts]
...
begin
	...
	EXEC [CommonDb].[SendNotification].[Send2GChat_dwhNotification]
				@text = @text
				,@threadKey = @subject
	exec logdb.dbo.[LogAndSendMailToAdmin] 'trying start [etl].[Send_MarketingProposal2Naumen_docredy_pts]','Info',' started', ''
	 declare 
			@status  int,
		...
		BEGIN  
			...
			begin
				...
				begin
					...
					exec logdb.dbo.[LogAndSendMailToAdmin] 'catching error [etl].[Send_MarketingProposal2Naumen_docredy_pts]','Error','Error',@error_description
	...
	;with cte_result_uat as (select TotalRecord = count(1), 
		...
		select @msg = concat(@msg, 
			...
			EXEC [CommonDb].[SendNotification].[Send2GChat_dwhNotification]
				@text = @msg
				,@threadKey = @subject
			exec logdb.dbo.[LogAndSendMailToAdmin] 'exec [etl].[Send_MarketingProposal2Naumen_docredy_pts]','Info','Done',''
