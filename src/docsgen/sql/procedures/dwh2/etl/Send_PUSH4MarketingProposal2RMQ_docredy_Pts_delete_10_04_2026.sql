...
exec [etl].[Send_PUSHForMarkingProposal2RMQ_docredy_pts] @env = 'uat'
	, @CMRClientGUIDs = @CMRClientGUIDs
	*/
CREATE       procedure [etl].[Send_PUSH4MarketingProposal2RMQ_docredy_Pts_delete_10_04_2026]
...
begin
	...
	,@procedureName  nvarchar(255) = 'dwh2.[etl].[GetMarketProposal_push_docredy_pts_JSON]'  -- was GetMarketProposal_push_docredy_pts_JSON
	...
	declare @error_description nvarchar(4000)=N''
		set @text=Concat('Маркетинговые предложения ПТС докред.'
	...
	exec logdb.dbo.[LogAndSendMailToAdmin] 'trying start [etl].[Send_PUSHForMarkingProposal2RMQ_docredy_pts]','Info',' started', ''
	...
	begin try
		...
		exec LogDb.[etl].[RequestHttpWebService]
			...
			begin
				...
				declare @msg nvarchar(255)= Concat('Данные для PUSH рассылки по проекту докред птс загружены в RMQ', 
					', CMRClientGUID: ', isnull(@CMRClientGUIDs, '-')
					, isnull(' Отправлено: ' + cast(@TotalSendPackages as nvarchar(10)), '')
					)
				EXEC  [LogDb].dbo.SendToSlack_dwhNotification  @msg
				exec logdb.dbo.[LogAndSendMailToAdmin] 'exec [etl].[Send_PUSHForMarkingProposal2RMQ_docredy_pts]','Info','Done',''
			end
	...
	begin catch
		...
		set @text=CONCAT_WS(' ',
			':exclamation: Ошибка загрузки  данных PUSH для рассылки по проекту докред ПТС в RMQ',
			format(getdate(),'dd.MM.yyyy HH:mm:ss')
			,ISNULL('CMRClientGUID: ' + @CMRClientGUIDs, '')
			)
		EXEC  [LogDb].dbo.SendToSlack_dwhNotification  @text
      
		exec logdb.dbo.[LogAndSendMailToAdmin] 'catching error [etl].[Send_PUSHForMarkingProposal2RMQ_docredy_pts]','Error','Error',@error_description
