 -- [etl].[end_MarketingProposal2RMQ_povtInst] @env = 'prod', @CMRClientGUID = '8391211A-0901-11E8-A814-00155D941900'
CREATE     procedure [etl].[Send_MarketingProposal2RMQ_povtInst_delete_10_04_2026]
...
begin
 ...
 declare @exchangeName nvarchar(255)  = 'dwh.marketProposal.1.1'
	...
	,@procedureName  nvarchar(255) = 'dwh2.[etl].[GetMarketProposal_povt_ints_RMQ_JSON]'
	...
	exec logdb.dbo.[LogAndSendMailToAdmin] 'trying start [etl].[Send_MarkingProposal2RMQ_povtInst]','Info',' started', ''
	...
	begin try
		...
		exec LogDb.etl.[RequestHttpWebService]
			...
			begin
				...
				declare @msg nvarchar(255)= Concat('Данные по проекту  повторники инстолмент загружены в RMQ'
					,', CMRClientGUIDs: ', isnull(@CMRClientGUIDs, '-')
					,isnull(' Отправлено: ' + cast(@TotalSendPackages as nvarchar(10)), '')
					)
				EXEC  [LogDb].dbo.SendToSlack_dwhNotification  @msg
				exec logdb.dbo.[LogAndSendMailToAdmin] 'exec [etl].[Send_MarkingProposal2RMQ_povtInst]','Info','Done',''
			end
	...
	begin catch
		...
		set @text=CONCAT_WS(' ', 
			':exclamation:','Ошибка загрузки данных по проекту повторники инстолмент в RMQ',
			format(getdate(),'dd.MM.yyyy HH:mm:ss')
			,'@CMRClientGUIDs:', @CMRClientGUIDs
			)
		EXEC  [LogDb].dbo.SendToSlack_dwhNotification  @text
      
		exec logdb.dbo.[LogAndSendMailToAdmin] 'catching error [etl].[Send_MarkingProposal2RMQ_povtInst]','Error','Error',@error_description
