 ...
 -- [etl].[Send_DeactivatedMarketingProposal2RMQ_povtPts] @env = 'prod', @CMRClientGUID = 'C6DCA609-08F9-11E8-A814-00155D941900'
CREATE       procedure [etl].[Send_DeactivatedMarketingProposal2RMQ_povtPts_delete_10_04_2026]
...
begin
	...
	declare @exchangeName nvarchar(255)  = 'dwh.marketProposal.1.1'
		...
		,@procedureName  nvarchar(255) = 'dwh2.[etl].[GetDeactivatedMarketProposal_povtPts_RMQ_JSON]'
	...
	declare @error_description nvarchar(4000)=N''
		...
		, ' Старт загрузки данных в RMQ для проекту повторники ПТС'
		, ' env:', @env
		, ' ', format(getdate(),'dd.MM.yyyy HH:mm:ss'))
	EXEC [LogDb].dbo.SendToSlack_dwhNotification  @text
	exec logdb.dbo.[LogAndSendMailToAdmin] 
		'trying start [etl].[Send_DeactivatedMarketingProposal2RMQ_povtPts]','Info',' started', ''
	...
	begin try
		...
		exec LogDb.etl.[RequestHttpWebService]
			...
			begin
				...
				declare @msg nvarchar(255)= Concat('Данные для деактивация маркетинговые пердложений по проекту повторники ПТС загружены в RMQ'
					,isnull(' Отправлено: ' + cast(@TotalSendPackages as nvarchar(10)), '')
					)
				EXEC  [LogDb].dbo.SendToSlack_dwhNotification  @msg
				exec logdb.dbo.[LogAndSendMailToAdmin] 'exec [etl].[Send_DeactivatedMarketingProposal2RMQ_povtPts]','Info','Done',''
			end
	...
	begin catch
		...
		set @text=concat(':exclamation: Ошибка загрузки данных для деактивация маркетинговые пердложений по проекту повторники ПТС в RMQ ', format(getdate(),'dd.MM.yyyy HH:mm:ss'))
		EXEC  [LogDb].dbo.SendToSlack_dwhNotification  @text
      
		exec logdb.dbo.[LogAndSendMailToAdmin] 'catching error [etl].[Send_DeactivatedMarketingProposal2RMQ_povtPts]','Error','Error',@error_description
