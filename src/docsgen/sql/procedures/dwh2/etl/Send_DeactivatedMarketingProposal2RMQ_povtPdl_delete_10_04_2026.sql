 ...
 -- [etl].[Send_DeactivatedMarketingProposal2RMQ_povtPdl] @env = 'prod', @CMRClientGUID = '8391211A-0901-11E8-A814-00155D941900'
CREATE        procedure [etl].[Send_DeactivatedMarketingProposal2RMQ_povtPdl_delete_10_04_2026]
...
begin
	...
	declare @exchangeName nvarchar(255)  = 'dwh.marketProposal.1.1'
		...
		,@procedureName  nvarchar(255) = 'dwh2.[etl].[GetDeactivatedMarketProposal_povtPdl_RMQ_JSON]'
	...
	exec logdb.dbo.[LogAndSendMailToAdmin] 'trying start [etl].[DeactivatedMarketingProposal2RMQ]','Info',' started', ''
	...
	begin try
		...
		exec LogDb.etl.[RequestHttpWebService]
			...
			begin
				...
				declare @msg nvarchar(255)= Concat('Данные для деактивация маркетинговые пердложений по проекту  повторники PDL загружены в RMQ'
					,isnull(' Отправлено: ' + cast(@TotalSendPackages as nvarchar(10)), '')
					)
				EXEC  [LogDb].dbo.SendToSlack_dwhNotification  @msg
				exec logdb.dbo.[LogAndSendMailToAdmin] 'exec [etl].[DeactivatedMarketingProposal2RMQ]','Info','Done',''
			end
	...
	begin catch
		...
		set @text=':exclamation: Ошибка загрузки данных по проекту повторники PDL в RMQ'+format(getdate(),'dd.MM.yyyy HH:mm:ss')
		EXEC  [LogDb].dbo.SendToSlack_dwhNotification  @text
      
		exec logdb.dbo.[LogAndSendMailToAdmin] 'catching error [etl].[DeactivatedMarketingProposal2RMQ]','Error','Error',@error_description
