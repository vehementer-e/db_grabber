 ...
 -- [etl].[Send_MarketingProposal2RMQ_povtPdl_delete_10_04_2026] @env = 'prod', @CMRClientGUID = '8391211A-0901-11E8-A814-00155D941900'
CREATE      procedure [etl].[Send_MarketingProposal2RMQ_povtPdl_delete_10_04_2026]
...
begin
 ...
 declare @exchangeName nvarchar(255)  = 'dwh.marketProposal.1.1'
	,@routingKey nvarchar(255) = 'dwh.marketProposal.1'
	,@procedureName  nvarchar(255) = 'dwh2.[etl].[GetMarketProposal_povt_pdl_RMQ_JSON]'
	...
	declare @error_description nvarchar(4000)=N''
		...
		,' Старт загрузки данных в RMQ для проекта повторники PDL'
		, ' CMRClientGUIDs: ', isnull(@CMRClientGUIDs, '-')
		, ' env:', @env
		, ' ', format(getdate(),'dd.MM.yyyy HH:mm:ss'))
	EXEC [LogDb].dbo.SendToSlack_dwhNotification  @text
	exec logdb.dbo.[LogAndSendMailToAdmin] 'trying start [etl].[Send_MarkingProposal2RMQ_povtPDL]','Info',' started', ''
	...
	begin try
		...
		exec LogDb.etl.[RequestHttpWebService]
			...
			begin
				...
				declare @msg nvarchar(255)= Concat('Данные по проекту  повторники PDL загружены в RMQ'
				...
				exec logdb.dbo.[LogAndSendMailToAdmin] 'exec [etl].[Send_MarkingProposal2RMQ_povtPDL]','Info','Done',''
			end
	...
	begin catch
		...
		set @error_description =CONCAT_WS(' ', 
			...
			,'@CMRClientGUIDs', @CMRClientGUID
			)
      set @text=CONCAT_WS(' ', 
			':exclamation:','Ошибка загрузки данных по проекту повторники PDL в RMQ',
			format(getdate(),'dd.MM.yyyy HH:mm:ss')
			,'@CMRClientGUIDs:', @CMRClientGUID
			)
		...
		exec logdb.dbo.[LogAndSendMailToAdmin] 'catching error [etl].[Send_MarkingProposal2RMQ_povtPDL]','Error','Error',@error_description
