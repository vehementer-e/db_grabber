 ...
 --[etl].[Send_dwhDwnloadConfirmation_2RMQ]
CREATE       procedure [etl].[Send_dwhDwnloadConfirmation_2RMQ_delete_10_04_2026]
	@env nvarchar(255) = 'uat'
as
begin
	declare 
	@ServiceUrl varchar(1024) =   concat('http://dwh-ex.cm.carmoney.ru/rmqSender/', @env, '/dwhDwnloadConfirmation.php')
	 ,@httpMethod varchar(10) = 'GET',
       @paramsValues varchar(1024) =concat('spForGetData', '=', 'etl.Get_DWHDownloadConfirmation_JSON'),
	...
	exec logdb.dbo.[LogAndSendMailToAdmin] 'trying start [etl].[Send_dwhDwnloadConfirmation_2RMQ]','Info',' started', ''
	...
	begin try
		...
		exec LogDb.etl.[RequestHttpWebService]
			...
			begin
				...
				exec logdb.dbo.[LogAndSendMailToAdmin] 'exec [etl].[Send_dwhDwnloadConfirmation_2RMQ]','Info','Done',''
			end
	...
	begin catch
		...
		exec logdb.dbo.[LogAndSendMailToAdmin] 'catching error [etl].[Send_dwhDwnloadConfirmation_2RMQ]','Error','Error',@error_description
