 ...
 --[etl].[Send_MarketingProposal2Naumen_povtInst_delete_10_04_2026] @env = 'prod' @batchSizeToSendNaumen = 2000
 -- [etl].[Send_MarkingProposal2Naumen_povtInst_uat] @env = 'uat',  @batchSizeToSendNaumen  = 1000,  @isDebug = 1,
-- set statistics io off
CREATE PROC [etl].[Send_MarketingProposal2Naumen_povtInst_delete_10_04_2026]
	@env nvarchar(255) = 'uat',
	@batchSizeToSendNaumen smallint = 100,
	@isDebug bit =0 
...
begin
	...
	set @subject =  concat_ws(' '
		, 'Загрузки данных в naumen для проекта повторники инстолмент'
	...
	exec logdb.dbo.[LogAndSendMailToAdmin] 'trying start [etl].[Send_MarkingProposal2Naumen_povtInst]','Info',' started', ''
		...
		begin
			;throw 51000, 'Id проекта - маркетинговые предложения(истолмент повторники) для заливки case в naumen не определен', 16
		end
		...
		begin
			set @batchSize =
				case  cast(getdate() as date)
					when '2025-04-03' then 1
					when '2025-04-04' then 30
					when '2025-04-05' then 30
					when '2025-04-06' then 30
				else 100 end
		...
		begin
			;throw 51000, 'Нет данных, по маркетинговым предложения(истолмент повторники)  для отправки в наумен',16 
		end
