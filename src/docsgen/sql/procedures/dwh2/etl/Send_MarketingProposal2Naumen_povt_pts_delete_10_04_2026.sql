 ...
 --[etl].[Send_MarketingProposal2Naumen_povt_pts_delete_10_04_2026] @batchSizeToSendNaumen = 2000
 -- [etl].[Send_MarketingProposal2Naumen_povt_pts_delete_10_04_2026] @env = 'uat',  @batchSizeToSendNaumen  = 1000,  @isDebug = 1,
-- set statistics io off
CREATE PROC [etl].[Send_MarketingProposal2Naumen_povt_pts_delete_10_04_2026]
...
begin
	...
	set @subject =  concat_ws(' '
		, 'Загрузки данных в naumen для проекта повторники pts'
		, format(getdate(), 'dd.MM.yyyy HH:mm'))
	set @text=Concat_ws(' ', 'Маркетинговые предложения.',
			'Старт загрузки данных в naumen для проекта повторники pts ',
			'env: ', @env,
	...
	begin try
		EXEC [CommonDb].[SendNotification].[Send2GChat_dwhNotification]
				@text = @text
				,@threadKey = @subject
		exec logdb.dbo.[LogAndSendMailToAdmin] 'trying start [etl].[Send_MarketingProposal2Naumen_povt_pts_delete_10_04_2026]','Info',' started', ''
		...
		begin
			...
			,@naumenProjectUUID = 'corebo00000000000mp6ibjtv3d4rjgc'

		end
		if LOWER(@env) = 'prod'
		begin
			select @authHeader =  'Basic ZHdoOjVMZCJedlYq'
			,@naumenProjectUUID = 'corebo00000000000palme673n6njdhs'
			
		end
		if nullif(@naumenProjectUUID,'') is null
		begin
			;throw 51000, 'Id проекта - маркетинговые предложения повторник pts для заливки case в naumen не определен', 16
		end
		...
		begin
			...
			select
					...
					naumenPriority = 
					--если новый 
					case when naumenInteractionTypeCode = 'isNew' 
						and was_closed_ago < 10 then 99
						else 0 end
					+ isnull(naumenPriority, 10)
			...
			from (
				select 
					...
					title = concat_ws(' '
						...
						,iif(t.hasPEP =1, 'ПЭП', null )		
						,isnull([hasCommissionProducts], 'Без кп')
						,iif(t.lastRate>0, format(t.lastRate/100, 'p1'), null)
					),
					market_proposal_type_name,
					naumenPriority= 100,
					clientTimeZone =  isnull(clientTimeZone, 3),
				...
				where market_proposal_message_guid is not null
		...
		begin
			;throw 51000, 'Нет данных, по маркетинговым предложения повторник pts для отправки в наумен',16 
		end
			...
			begin
				begin try
					...
					set @ParamValue = (select
						'id' = id
						,'title' = title 
						,'comment' = 'Повторные'--market_proposal_type_name 
				...
				begin
					...
					exec logdb.dbo.[LogAndSendMailToAdmin] 'catching error [etl].[Send_MarketingProposal2Naumen_povt_pts_delete_10_04_2026]','Error','Error',@error_description
		...
		declare @msg nvarchar(max)=concat(':white_check_mark: '
			, 'Загрузки данных по проекту повторник pts в naumen завершена.',
			'env: ', @env
			)
	if  lower(@env) = 'prod'
	BEGIN
	...
	;with cte_result_uat as (select TotalRecord = count(1), 
		...
		select @msg = concat(@msg, 
			...
			EXEC [CommonDb].[SendNotification].[Send2GChat_dwhNotification]
					@text = @text
					,@threadKey = @subject
			exec logdb.dbo.[LogAndSendMailToAdmin] 'exec [etl].[Send_MarketingProposal2Naumen_povt_pts_delete_10_04_2026]','Info','Done',''
	...
	begin catch
		...
		set @text=':exclamation: Ошибка загрузки данных по проекту повторник pts в naumen'+format(getdate(),'dd.MM.yyyy HH:mm:ss')
		EXEC [CommonDb].[SendNotification].[Send2GChat_dwhNotification]
						@text = @text
