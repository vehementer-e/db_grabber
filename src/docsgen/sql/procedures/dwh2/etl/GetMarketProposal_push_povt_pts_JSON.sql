/*
﻿--[etl].[GetMarketProposal_push_povt_pts_JSON] 'uat'
*/
CREATE     procedure  [etl].[GetMarketProposal_push_povt_pts_JSON]
...
begin
	...
	pushProfile2Send = case nRow
				when 1 then 'PUSH_ID_1343_ADDITIONAL_4'
	...
	from (
		...
		and (t.CRMClientGUID in (select trim(value) from string_split(@CMRClientGUIDs, ','))
			or @CMRClientGUIDs is null)
		--and povt_pts.date2SendPush =  cast(getdate() as date)
		and phone is not null
