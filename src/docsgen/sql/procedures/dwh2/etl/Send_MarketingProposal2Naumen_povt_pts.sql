 ...
 --[etl].[Send_MarketingProposal2Naumen_povt_pts] @batchSizeToSendNaumen = 2000
 -- [etl].[Send_MarketingProposal2Naumen_povt_pts] @env = 'uat',  @batchSizeToSendNaumen  = 1000,  @isDebug = 1,
-- set statistics io off
CREATE PROC [etl].[Send_MarketingProposal2Naumen_povt_pts]
...
begin
	...
	begin try
		EXEC [CommonDb].[SendNotification].[Send2GChat_dwhNotification]
				@text = @text
				,@threadKey = @subject
		exec logdb.dbo.[LogAndSendMailToAdmin] 'trying start [etl].[Send_MarketingProposal2Naumen_povt_pts]','Info',' started', ''
		...
		BEGIN  
			...
			begin
				...
				begin
					...
					exec logdb.dbo.[LogAndSendMailToAdmin] 'catching error [etl].[Send_MarketingProposal2Naumen_povt_pts]','Error','Error',@error_description
	...
	;with cte_result_uat as (select TotalRecord = count(1), 
		...
		select @msg = concat(@msg, 
			...
			EXEC [CommonDb].[SendNotification].[Send2GChat_dwhNotification]
					@text = @text
					,@threadKey = @subject
			exec logdb.dbo.[LogAndSendMailToAdmin] 'exec [etl].[Send_MarketingProposal2Naumen_povt_pts]','Info','Done',''
