/*
--[etl].[GetMarketProposal_email_pov_installment_JSON] 'prod'
--[etl].[GetMarketProposal_email_pov_installment_JSON] 'uat'
*/
CREATE     procedure  [etl].[GetMarketProposal_email_povt_inst_JSON]
...
begin
	...
	where cdate = cast(getdate() as date)
		...
		and povt_inst.market_proposal_category_code not in ('red')
			and povt_inst.Date2SendEmail =  cast(getdate() as date)
			and povt_inst.client_email is not null
			and povt_inst.email_communicationId is null
...
begin
	...
	(
		...
		order by povt_inst.external_id desc
	) t
end
	...
	begin 
		begin tran
			...
			from marketing.povt_inst_uat t
				inner join #data2rmq s on s.CMRClientGUID = t.CMRClientGUID
