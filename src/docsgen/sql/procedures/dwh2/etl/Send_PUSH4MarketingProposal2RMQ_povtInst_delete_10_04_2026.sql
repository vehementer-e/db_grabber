...
declare @CMRClientGUIDs nvarchar(max)
select 
	@CMRClientGUIDs = STRING_AGG(cast(CMRClientGUID as nvarchar(36)), ',')
from (select top(4)
	*
from marketing.povt_inst_uat t
		where cdate = cast(getdate() as date)
		and t.phoneInBlackList = 0
		and push_communicationId is null
		) t
select @CMRClientGUIDs
exec [etl].[Send_PUSHForMarkingProposal2RMQ_povtInst] @env = 'prod'
	, @CMRClientGUIDs = ' 683a3b9d-8344-4b3f-b831-e5b643b171b1'
	*/
CREATE       procedure [etl].[Send_PUSH4MarketingProposal2RMQ_povtInst_delete_10_04_2026]
...
begin
	...
	,@routingKey nvarchar(255) = 'push'
	,@procedureName  nvarchar(255) = 'dwh2.[etl].[GetMarketProposal_push_povtInst_JSON]'  -- was GetMarketProposal_PUSH_povtInst_JSON
	...
	declare @error_description nvarchar(4000)=N''
		set @text=Concat('Маркетинговые предложения инстолмент.'
		,' Старт PUSH рассылки по маркениговым предложениям'
		, ' CMRClientGUID: ', isnull(@CMRClientGUIDs, '-')
		,' env: ', @env
		, ' ', format(getdate(),'dd.MM.yyyy HH:mm:ss'))
	EXEC [LogDb].dbo.SendToSlack_dwhNotification  @text
	exec logdb.dbo.[LogAndSendMailToAdmin] 'trying start [etl].[Send_PUSHForMarkingProposal2RMQ_povtInst]','Info',' started', ''
	...
	begin try
		...
		exec LogDb.[etl].[RequestHttpWebService]
			...
			begin
				...
				declare @responseError nvarchar(1024)=concat_ws(' '
					,':interrobang: '
			...
			begin
				...
				declare @msg nvarchar(255)= Concat('Данные для PUSH рассылки по проекту повторники инстолмент загружены в RMQ', 
					', CMRClientGUID: ', isnull(@CMRClientGUIDs, '-')
					, isnull(' Отправлено: ' + cast(@TotalSendPackages as nvarchar(10)), '')
					)
				EXEC  [LogDb].dbo.SendToSlack_dwhNotification  @msg
				exec logdb.dbo.[LogAndSendMailToAdmin] 'exec [etl].[Send_PUSHForMarkingProposal2RMQ_povtInst]','Info','Done',''
			end
	...
	begin catch
        ...
        set @text=CONCAT_WS(' ',
			':exclamation: Ошибка загрузки  данных PUSH для рассылки по проекту повторники инстолмент в RMQ',
			format(getdate(),'dd.MM.yyyy HH:mm:ss')
			,ISNULL('CMRClientGUIDs: ' + @CMRClientGUIDs, '')
			)


		EXEC  [LogDb].dbo.SendToSlack_dwhNotification  @text
      
		exec logdb.dbo.[LogAndSendMailToAdmin] 'catching error [etl].[Send_PUSHForMarkingProposal2RMQ_povtInst]','Error','Error',@error_description
