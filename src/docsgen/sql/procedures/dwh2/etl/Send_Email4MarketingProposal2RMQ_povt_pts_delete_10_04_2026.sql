 ...
 --[etl].[Send_EmailForMarkingProposal2RMQ_povt_pts]
CREATE      procedure [etl].[Send_Email4MarketingProposal2RMQ_povt_pts_delete_10_04_2026]
...
begin
	...
	,@procedureName  nvarchar(255) = 'dwh2.[etl].[GetMarketProposal_email_povt_pts_JSON]'  -- was GetMarketProposal_email_povt_pts_JSON
	...
	declare @error_description nvarchar(4000)=N''
		...
		,'Старт email рассылки по маркениговым предложениям повторники ПТС'
		, 'env - ', @env
		, format(getdate(),'dd.MM.yyyy HH:mm:ss'))
	EXEC [LogDb].dbo.SendToSlack_dwhNotification  @text
	exec logdb.dbo.[LogAndSendMailToAdmin] 'trying start [etl].[Send_EmailForMarkingProposal2RMQ_povt_pts]','Info',' started', ''
	...
	begin try
		...
		exec LogDb.[etl].[RequestHttpWebService]
			...
			begin
				...
				declare @msg nvarchar(255)= Concat('Данные для email рассылки по проекту повторники ПТС загружены в RMQ', 
					isnull(' Отправлено: ' + cast(@TotalSendPackages as nvarchar(10)), '')
					)
				EXEC  [LogDb].dbo.SendToSlack_dwhNotification  @msg
				exec logdb.dbo.[LogAndSendMailToAdmin] 'exec [etl].[Send_EmailForMarkingProposal2RMQ_povt_pts]','Info','Done',''
			end
	...
	begin catch
		...
		set @text=':exclamation: Ошибка загрузки  данных Email для рассылки по проекту повторники ПТС в RMQ'+format(getdate(),'dd.MM.yyyy HH:mm:ss')
	--	EXEC  [LogDb].dbo.SendToSlack_dwhNotification  @text
      
		exec logdb.dbo.[LogAndSendMailToAdmin] 'catching error [etl].[Send_EmailForMarkingProposal2RMQ_povt_pts]','Error','Error',@error_description
