 ...
 --[etl].[Send_EmailForMarkingProposal2RMQ_povtPDL]
CREATE       procedure [etl].[Send_Email4MarketingProposal2RMQ_povtPDL_delete_10_04_2026]
...
begin
	...
	,@procedureName  nvarchar(255) = 'dwh2.[etl].[GetMarketProposal_email_povt_PDL_JSON]'  -- was GetMarketProposal_email_povt_PDL_JSON
	...
	declare @error_description nvarchar(4000)=N''
		...
		,'Старт email рассылки по маркениговым предложениям pdl'
		, 'env - ', @env
		, format(getdate(),'dd.MM.yyyy HH:mm:ss'))
	EXEC [LogDb].dbo.SendToSlack_dwhNotification  @text
	exec logdb.dbo.[LogAndSendMailToAdmin] 'trying start [etl].[Send_EmailForMarkingProposal2RMQ_povtPDL]','Info',' started', ''
	...
	begin try
		...
		exec LogDb.[etl].[RequestHttpWebService]
			...
			begin
				...
				declare @msg nvarchar(255)= Concat('Данные для email рассылки по проекту повторники PDL загружены в RMQ', 
					isnull(' Отправлено: ' + cast(@TotalSendPackages as nvarchar(10)), '')
					)
				EXEC  [LogDb].dbo.SendToSlack_dwhNotification  @msg
				exec logdb.dbo.[LogAndSendMailToAdmin] 'exec [etl].[Send_EmailForMarkingProposal2RMQ_povtPDL]','Info','Done',''
			end
	...
	begin catch
		...
		set @text=':exclamation: Ошибка загрузки  данных Email для рассылки по проекту повторники PDL в RMQ'+format(getdate(),'dd.MM.yyyy HH:mm:ss')
		EXEC  [LogDb].dbo.SendToSlack_dwhNotification  @text
      
		exec logdb.dbo.[LogAndSendMailToAdmin] 'catching error [etl].[Send_EmailForMarkingProposal2RMQ_povtPDL]','Error','Error',@error_description
