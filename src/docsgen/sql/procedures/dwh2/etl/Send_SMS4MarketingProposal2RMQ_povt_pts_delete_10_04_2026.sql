 ...
 --[etl].[Send_SMSForMarkingProposal2RMQ_povt_pts]
CREATE       procedure [etl].[Send_SMS4MarketingProposal2RMQ_povt_pts_delete_10_04_2026]
...
begin
	...
	,@procedureName  nvarchar(255) = 'dwh2.[etl].[GetMarketProposal_sms_povt_pts_JSON]'  -- was GetMarketProposal_sms_povt_pts_JSON
	...
	declare @error_description nvarchar(4000)=N''
		...
		,'Старт sms рассылки по маркениговым предложениям повторники ПТС'
		, 'env - ', @env
		, format(getdate(),'dd.MM.yyyy HH:mm:ss'))
	EXEC [LogDb].dbo.SendToSlack_dwhNotification  @text
	exec logdb.dbo.[LogAndSendMailToAdmin] 'trying start [etl].[Send_SMSForMarkingProposal2RMQ_povt_pts]','Info',' started', ''
	...
	begin try
		...
		exec LogDb.[etl].[RequestHttpWebService]
			...
			begin
				...
				declare @msg nvarchar(255)= Concat('Данные для SMS рассылки по проекту повторники ПТС загружены в RMQ', 
					isnull(' Отправлено: ' + cast(@TotalSendPackages as nvarchar(10)), '')
					)
				EXEC  [LogDb].dbo.SendToSlack_dwhNotification  @msg
				exec logdb.dbo.[LogAndSendMailToAdmin] 'exec [etl].[Send_SMSForMarkingProposal2RMQ_povt_pts]','Info','Done',''
			end
	...
	begin catch
		...
		set @text=':exclamation: Ошибка загрузки  данных для SMS рассылки по проекту повторники ПТС в RMQ'+format(getdate(),'dd.MM.yyyy HH:mm:ss')
		EXEC  [LogDb].dbo.SendToSlack_dwhNotification  @text
      
		exec logdb.dbo.[LogAndSendMailToAdmin] 'catching error [etl].[Send_SMSForMarkingProposal2RMQ_povt_pts]','Error','Error',@error_description
