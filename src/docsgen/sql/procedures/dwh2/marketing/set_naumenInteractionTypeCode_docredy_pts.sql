CREATE        procedure [marketing].[set_naumenInteractionTypeCode_docredy_pts]
...
begin try
	...
	from (
		select marketProposal_ID
			/*
				Если lastNaumen_AttemptResult in('consultation')
				и дата звонка = дата заявки 
		...
		--	,lastЗаявкаНаЗаймПодПТС_Дата
			...
			,lastNaumen_AttemptResult = iif(lastNaumen_IsPhoned = 0
				, 'Не дозвонились', lastNaumen_AttemptResult)
	--		,lastЗаявкаНаЗаймПодПТС_СтатусыЗаявкиНаименование
			,lastCRMЗаявка_СтатусНаименование
	...
	select 
		...
		= case 
				...
				when lastActionResult in('Отказ клиента', 'Клиент передумал') 
					then 
						case 
						--После отказа 1 звонок на 15 день
						when t.totalIsCustomerRejection = 1 and datediff(dd, lastDateAction, getdate())>=15
							then 'isCustomerRejection 1'
						--После каждые 30 дней,
						 when t.totalIsCustomerRejection > 1 and datediff(dd, lastDateAction, getdate()) >30
