CREATE      procedure [marketing].[set_param_forStratege_povt_inst]
...
begin try
	...
	and t.market_proposal_category_code in ('green')
	and (t.CMRClientGUID  in (select trim(value) from string_split(@CMRClientGUIDs, ',')) or @CMRClientGUIDs  is null)
			...
			*/
			,lastCRMЗаявка_Дата
				=iif( cast(lastCRMЗаявка_Дата  as date) =  cast(lastNaumen_AttemptDate as date)
					and lastNaumen_AttemptResult in('consultation')
