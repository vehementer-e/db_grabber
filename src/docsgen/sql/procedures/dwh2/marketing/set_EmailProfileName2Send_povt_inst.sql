...
/*
select * from marketing.povt_inst
where cdate = cast(getdate() as date)
and povt_inst.phoneInBlackList = 0
		and povt_inst.client_email is not null
	and Date2SendEmail is not null
	*/
CREATE       procedure [marketing].[set_EmailProfileName2Send_povt_inst]
as
begin
...
begin try
	...
	from marketing.povt_inst povt_inst
		inner join @EmailProfile ep on
			povt_inst.cdate between ep.fromDate and ep.toDate
		left  join (
			select last_Date2SendEmail = max(isnull(Date2SendEmail,'2000-01-01')),  
				CMRClientGUID
			from marketing.povt_inst povt_inst
			where cdate < cast(getdate() as date)
			and povt_inst.EmailProfile2Send in (select ep.emailProfileName from @EmailProfile ep)
			group by CMRClientGUID
		) last_emailSend on last_emailSend.CMRClientGUID = povt_inst.CMRClientGUID
		where cdate = cast(getdate() as date)
			and povt_inst.phoneInBlackList = 0
			and povt_inst.client_email is not null
			and povt_inst.market_proposal_category_code not in ('red')
	union
	select 
		povt_inst.CMRClientGUID,
		EmailProfile2Send =  ep.EmailProfileName,
		Date2SendEmail = povt_inst.cdate
	from marketing.povt_inst povt_inst 
	inner join @EmailProfile_after_loan_repaid ep on ep.interactionTypeCode = povt_inst.interactionTypeCode
	where  povt_inst.cdate = cast(getdate() as date)
			and povt_inst.phoneInBlackList = 0
			and povt_inst.market_proposal_category_code not in ('red')
			and povt_inst.client_email is not null
	...
	begin tran
		...
		left join #tEmailProfileName2Send t  on 
			t.CMRClientGUID = povt_inst.CMRClientGUID
		where cdate = cast(getdate() as date)
	commit tran
end try
begin catch
