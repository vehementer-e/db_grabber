...
and Date2SendSMS is  null
	...
	*/
CREATE         procedure [marketing].[set_SMSProfileCode_povt_pts]
as
begin
...
begin try
	...
	from marketing.povt_pts povt_pts
		inner join (
			...
			from marketing.docredy_pts povt_pts
			group by marketProposal_ID
		) t_first_Data 
		on t_first_Data.marketProposal_ID = povt_pts.marketProposal_ID
		...
		where cdate = @today
			...
			and not exists(select top(1) 1 from marketing.povt_pts  t1
				where t1.cdate<povt_pts.cdate
				and t1.CRMClientGUID = povt_pts.CRMClientGUID
