CREATE PROC [marketing].[fill_povt_inst]
	@CMRClientGUID nvarchar(36) = null
as
begin
	begin try
		if not exists(select top(1) 1 from risk.povt_inst_buffer)
		begin 
			;throw 51000, 'Отсутвутют данные в risk.povt_inst_buffer', 16
		end
		drop table if exists #t_povt_inst
		...
		drop table if exists #has_marketing_docked_pov
		create table #has_marketing_docked_pov(CRMClientGUID varchar(36))
		insert into #has_marketing_docked_pov(CRMClientGUID)
		select distinct CRMClientGUID from risk.docredy_buffer
		where category not in('Красный')
		union
		select CRMClientGUID from risk.povt_buffer
		where category not in('Красный')
		...
		select top(0) 
			...
			row_hash
		into #t_povt_inst
		from [marketing].[povt_inst]

		insert into #t_povt_inst
		(
			[external_id], 
			КлиентСсылка,
			[CMRClientGUID], 
		...
		inner join stg.dbo.v_1cMDS_ProductType  pt 
			on 1=1 
			and UPPER(pt.Name)  = UPPER('Installment')
		...
		left join  [Dictionary].categoryMarketProposal dic_mp on
			dic_mp.Name = 
				case --если клиент не задат и категория Розовый значт Красный --DWH-2580
		...
		left join [Dictionary].[typeCodeMarketProposal] dic_t on 
			dic_t.Наименование = 'Повторный заём инстолмент'
		left join (select 
			Партнер = CRM_КонтактнаяИнформация.Ссылка 
		...
		(lower(t.category) in ('Зеленый', 'Красный') and @CMRClientGUID is null
			...
			or  ( t.CMRClientGUID =  @CMRClientGUID
					and t.category = ('Розовый')
				)
				*/

		--DWH-2320 Использовать персональные данные из CRM
		--Если таких данных нет, тогда уже использовать данные из родительской таблицы (risk.povt_inst_buffer)
		UPDATE D
		SET passport_series = nullif(replace(A.Серия, ' ', ''),''), 
			passport_number = nullif(A.Номер,''),
			passportNotValid = 0
		FROM (
			SELECT 
				R.Физлицо_Ссылка,
		...
		from #t_povt_inst t
			where exists(select top(1) 1 from #has_marketing_docked_pov s
			where s.CRMClientGUID = t.CMRClientGUID)
