...
CREATE    procedure [marketing].[fill_povt_pdl_uat]
	@env nvarchar(255) = 'uat',
	@CMRClientGUID nvarchar(36) = null
as
begin
	begin try
		if not exists(select top(1) 1 from [marketing].[povt_pdl])
		begin 
			;throw 51000, 'Отсутвутют данные в marketing.povt_pdl', 16
		end
	...
	begin tran
		truncate table [marketing].[povt_pdl_uat]

		insert into [marketing].[povt_pdl_uat]
		(
			[external_id], 
			[CMRClientGUID], 
			[last_name], 
		...
		select 
		 ...
		 from [marketing].[povt_pdl] povt_pdl
			...
			uat where uat.Номер =povt_pdl.external_id
			)
