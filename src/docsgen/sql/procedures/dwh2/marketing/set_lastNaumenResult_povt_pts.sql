CREATE   procedure [marketing].[set_lastNaumenResult_povt_pts]
as
begin
...
--Сбор данных по результатам последнего звонка по маркетинговым предложениям повторники инстолмент
	...
	declare @projectIds table (project_id nvarchar(255) primary key)
		insert into @projectIds (project_id)
		select  project_id
		from (values
			('corebo00000000000mm1tts6og6rs2fk') --Докреды
			,('corebo00000000000n25qgnvfnogtjf8') --Докреды Новые
			,('corebo00000000000n25qereimll7884') --Докреды Перезвоны
			,('corebo00000000000mn2eg74l4nb9950') --Повторные - Перезвоны
	...
	select client_number = right(t.phone, 10) from marketing.povt_pts t
	where t.phone is not null
	group by   right(t.phone, 10)

	

	drop table if exists #t_client_number_result_on_date
	--звонки за последние 360 дней
	select 
	...
	from Reports.dbo.dm_report_DIP_detail_outbound_sessions t
		where cast(t.attempt_start as date)>=dateadd(dd,-360, getdate())
	...
	and exists(select top(1) 1 from  #povt_ptsClientPhone cp where cp.client_number  = right(t.client_number,10))
