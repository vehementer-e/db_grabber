...
BEGIN
 ...
 begin try	
  begin tran 
	...
	insert into #Registor(dat,dt,kt,numdog,beginDate,summ,mem,rzd,fin,findop,plat,reg,regNum,checkSubKonto,aKt,aDt)
	select
		dat=cast(a.Период as date)
		,dt=b.Код	
		,kt=c.Код
		,numDog= iif((b.Код ='47422'and c.Код='60322')or(b.Код ='61217' and c.Код ='60323') ,dog2.Номер,isnull(dog1.Номер,dog2.Номер)) --цессия
		,beginDate=iif((b.Код ='47422'and c.Код='60322')or(b.Код ='61217' and c.Код ='60323'),cast(dog2.Дата as date),cast(isnull(dog1.Дата,dog2.Дата) as date)) --цессия
		,summ=a.Сумма
		,mem= iif (b.Код ='60323' and c.Код ='47422',isnull(memDt.Наименование,a.Содержание),isnull(memCt.Наименование,a.Содержание))
				--case 
				--when b.Код ='60323' and c.Код ='47422' 
				--	then isnull(memDt.Наименование,a.Содержание)
				--when b.Код ='61217'and c.Код ='48801'
				--	then vvz.Имя
				--else isnull(memCt.Наименование,a.Содержание) end
	...
	insert into	#Provodki --dwh2.finAnalytics.tmp83 
			(dat,dt,kt,numdog,beginDate,mem,summIn,summCes,summOD,summPRC,summPeni,summGP,summDop,plat,reg,regNum)
	...
	insert into #tmp (dat,numdog,beginDate,summIn,summCes,summOD,summPRC,summPeni,summGP,summDop,plat)
		select 
			dat
			,numdog
			,beginDate
			...
			,sum(summDop) summDop
			,plat
		from #Provodki --dwh2.finanalytics.tmp83
		group by 
			dat 
			,numdog
		    ,beginDate
