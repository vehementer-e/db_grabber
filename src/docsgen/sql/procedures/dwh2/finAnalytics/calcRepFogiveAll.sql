

CREATE PROC [finAnalytics].[calcRepFogiveAll] 
    @repmonth date
AS
BEGIN

    DECLARE @sp_name NVARCHAR(255) = OBJECT_NAME(@@PROCID)
    DECLARE @procStartTimeraw datetime = getdate()  -- Переменная фиксирует начало выполнения
    DECLARE @subject NVARCHAR(255) = CONCAT (
				'Выполнение процедуры '
				,'"Расчет данных для отчета по прощениям"'
				)
       
    begin try

    --старт лог
       declare @log_IsError bit=0
       declare @log_Mem nvarchar(2000)	='Ok'
       declare @mainPrc nvarchar(255)=''
      if (select OBJECT_ID( N'tempdb..#mainPrc')) is not null
                          set @mainPrc=(select top(1) sp_name from #mainPrc)
      exec finAnalytics.sys_log @sp_name,0, @mainPrc

	exec finAnalytics.calcRepForgive1 @repmonth;
	exec finAnalytics.calcRepForgive2 @repmonth;
	EXEC finAnalytics.calcRepForgive2_2 @repmonth;
	exec finAnalytics.calcRepForgive3 @repmonth;
	exec finAnalytics.calcRepForgive4 @repmonth;
	exec finAnalytics.calcRepForgive5 @repmonth;

    
	DECLARE @procEndTimeraw datetime = getdate()   -- Переменная фиксирует окончание выполнения

    DECLARE @procStartTime varchar(40) = format(@procStartTimeraw,'dd.MM.yyyy HH:mm:ss', 'ru-RU')
    DECLARE @procEndTime varchar(40) =  format(@procEndTimeraw,'dd.MM.yyyy HH:mm:ss', 'ru-RU')
    DECLARE @timeDuration varchar(40) = concat(cast (datediff(second,@procStartTimeraw,@procEndTimeraw) as varchar),' секунд')

    DECLARE @maxDateRep1 NVARCHAR(30)
    set @maxDateRep1 = cast((select max(repmonth) from finAnalytics.repForgive1) as varchar)
	DECLARE @maxDateRep2 NVARCHAR(30)
    set @maxDateRep2 = cast((select max(repmonth) from finAnalytics.repForgive2) as varchar)
	DECLARE @maxDateRep3 NVARCHAR(30)
    set @maxDateRep3 = cast((select max(repmonth) from finAnalytics.repForgive3) as varchar)
	DECLARE @maxDateRep4 NVARCHAR(30)
    set @maxDateRep4 = cast((select max(repmonth) from finAnalytics.repForgive4) as varchar)
	DECLARE @maxDateRep5 NVARCHAR(30)
    set @maxDateRep5 = cast((select max(repmonth) from finAnalytics.repForgive5) as varchar)
    
	/*Фиксация времени расчета*/
	update dwh2.[finAnalytics].[reportReglament]
	set lastCalcDate = getdate(),[maxDataDate] = @maxDateRep1
	where [reportUID]= 8

    DECLARE @msg_good NVARCHAR(2048) = CONCAT (
				'Успешное выполнение процедуры - данные расчитаны за :'
				,FORMAT( eoMONTH(@REPMONTH), 'MMMM yyyy', 'ru-RU' )
                ,char(10)
                ,char(13)
                ,'Время начала выполнения: '
                ,@procStartTime
                ,char(10)
                ,char(13)
                ,'Время окончания выполнения: '
                ,@procEndTime
                ,char(10)
                ,char(13)
                ,'Время выполнения: '
                ,@timeDuration
                ,char(10)
                ,char(13)
                ,'Максимальная дата Первой части отчета: '
                ,@maxDateRep1
				,char(10)
                ,char(13)
                ,'Максимальная дата Второй части отчета: '
                ,@maxDateRep2
				,char(10)
                ,char(13)
                ,'Максимальная дата Третьей части отчета: '
                ,@maxDateRep3
				,char(10)
                ,char(13)
                ,'Максимальная дата Четвертой части отчета: '
                ,@maxDateRep4
				,char(10)
                ,char(13)
                ,'Максимальная дата Пятой части отчета: '
                ,@maxDateRep5
				)

	declare @emailList varchar(255)=''
	set @emailList = (select STRING_AGG(email,';') from finAnalytics.emailList where emailUID in (1,2,31))
    
	EXEC msdb.dbo.sp_send_dbmail @profile_name = 'Default'
			,@recipients = @emailList
			,@copy_recipients = ''
			,@body = @msg_good
			,@body_format = 'TEXT'
			,@subject = @subject;

	--финиш
   exec dwh2.finAnalytics.sys_log  @sp_name,1, @mainPrc

    end try
    
    begin catch

    DECLARE @msg_bad NVARCHAR(2048) = CONCAT (
				'Ошибка выполнения процедуры - '
				,@sp_name
				,'. Ошибка '
				,ERROR_MESSAGE()
				)

    IF @@TRANCOUNT > 0
    ROLLBACK TRANSACTION;
	
	----кэтч

   set  @log_IsError =1
   set  @log_Mem =ERROR_MESSAGE()
   exec finAnalytics.sys_log  @sp_name,1, @mainPrc,  @log_IsError,  @log_Mem

	set @emailList = (select STRING_AGG(email,';') from finAnalytics.emailList where emailUID in (1))
    EXEC msdb.dbo.sp_send_dbmail @profile_name = 'Default'
			,@recipients = @emailList
			,@copy_recipients = ''
			,@body = @msg_bad
			,@body_format = 'TEXT'
			,@subject = @subject;
        
    throw 51000 
			,@msg_bad
			,1;
    

    end catch
END
