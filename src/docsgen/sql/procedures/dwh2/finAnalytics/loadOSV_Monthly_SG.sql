...
CREATE   PROCEDURE [finAnalytics].[loadOSV_Monthly_SG] 
...
BEGIN
    ...
    declare @repmonthtemp date = (select min(CONVERT (date, [Отчетный месяц], 104)) from stg.[files].[OSV_MONTHLY_sg])
    declare @repmonth date = DATEFROMPARTS(DATEPART(year,@repmonthtemp),datepart(month,@repmonthtemp),1)
    --select @repmonth

    delete from finAnalytics.OSV_MONTHLY_sg where REPMONTH=@repmonth

  begin tran  
    
    INSERT INTO finAnalytics.OSV_MONTHLY_sg
	(
 ...
 from stg.[files].[OSV_MONTHLY_sg] a
 ...
 --exec dwh2.[finAnalytics].[calcRepPL] @repmonth
    ...
    set @maxDateRest = cast((select max(repmonth) from finAnalytics.OSV_MONTHLY_sg) as varchar)
    ...
    DECLARE @msg_good NVARCHAR(2048) = CONCAT (
				'Успешное выполнение процедуры - Загрузка ОСВ СГ за '
