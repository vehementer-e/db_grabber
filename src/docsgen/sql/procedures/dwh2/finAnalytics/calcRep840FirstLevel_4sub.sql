...
CREATE procedure [finAnalytics].[calcRep840FirstLevel_4sub]
	@dataLevel int,
    @prosCategory int,
    @repmonth date,
    @repdate date,
...
--and upper(isnull(a.nomenklGroup,''))=upper('PDL') --v1
and upper(a.zaymGroup)=upper('Займ физическому лицу необеспеченный') --v2
and a.PSK_prc >= 250 --v2
...
where upper(a.clientType)=upper('ФЛ')
--and upper(isnull(a.nomenklGroup,''))!=upper('PDL') --v1
and upper(a.zaymGroup)=upper('Займ физическому лицу обеспеченный') --v2
and a.PSK_prc < 250 --v2
and a.PROScategoy=@prosCategory
...
where 1=1
--and upper(a.clientType)!=upper('ФЛ') --v1
--and upper(isnull(a.nomenklGroup,''))!=upper('PDL')  --v1
and (
		((upper(a.clientType) like '%ЮЛ%' or upper(a.clientType) like '%ИП%') --v2
		and upper(a.zaymGroup)  in (
                        --upper('Займ индивидуальному предпринимателю обеспеченный'), --v1
                        upper('Займ субъекту малого и среднего предпринимательства обеспеченный') --v2
                        )
		and upper(kp.isMSPbyRepDate) = upper('Да')
		)
		or
		(upper(a.clientType) like '%ФЛ%' --v3
		and upper(a.zaymGroup)  in (
                        upper('Займ самозанятому обеспеченный') --v3
                        )
		)
		)
and a.PROScategoy=@prosCategory
...
and (
		...
		or
		(upper(a.clientType) like '%ФЛ%' --v3
		  and upper(a.zaymGroup)=upper('Займ самозанятому необеспеченный')
		)
		)
and a.PROScategoy=@prosCategory
...
where 1=1
--and upper(a.clientType)!=upper('ФЛ') --v1
and (upper(a.clientType) like '%ЮЛ%' or upper(a.clientType) like '%ИП%') --v2
--and upper(isnull(a.nomenklGroup,''))!=upper('PDL')  --v1
--and upper(a.zaymGroup)=upper('Займ субъекту малого и среднего предпринимательства обеспеченный')  --v1
and upper(a.zaymGroup) in (
                            upper('Займ индивидуальному предпринимателю обеспеченный'),
                            upper('Займ юридическому лицу обеспеченный')
                           ) --v2
and a.PROScategoy=@prosCategory
and upper(isnull(kp.isMSPbyRepDate,'Нет')) != upper('Да')
...
left join finAnalytics.PBR_MONTHLY kp on kp.repmonth=@repmonth /*and kp.repdate=@repdate*/ and a.dogNum=kp.dogNum
where 1=1
--upper(a.clientType)!=upper('ФЛ')  --v1
and (upper(a.clientType) like '%ЮЛ%' or upper(a.clientType) like '%ИП%') --v2
--and upper(isnull(a.nomenklGroup,''))!=upper('PDL')  --v1
/*
and (upper(a.zaymGroup)=upper('Займ субъекту малого и среднего предпринимательства необеспеченный')
...
*/ --v1
and upper(a.zaymGroup) in (
                            upper('Займ индивидуальному предпринимателю необеспеченный'),
                            upper('Займ июридическому лицу необеспеченный'),
							upper('Займ юридическому лицу необеспеченный')
                           ) --v2
...
*/  --v1
and upper(a.zaymGroup) in (
                            upper('Займ реструктурированный /рефинсированный необеспеченный')
                            ,upper('Займ реструктурированный физическому лицу краткосрочный')
                           ) --v2
and a.PSK_prc >= 250 --v2
and a.PROScategoy=@prosCategory
...
where 1=1
--and upper(isnull(a.nomenklGroup,''))!=upper('PDL') --v1
and upper(a.zaymGroup) in (upper('Займ реструктурированный /рефинсированный обеспеченный'),
						   upper('Займ самознятому реструктурированный /рефинсированный обеспеченный'))
and not (upper(a.clientType) = 'ФЛ' and a.PSK_prc >= 250)    --v2
and a.PROScategoy=@prosCategory
