CREATE   PROCEDURE [finAnalytics].[loadOSV_Monthly] 
...
BEGIN
    ...
    declare @repmonthtemp date = (select min(CONVERT (date, [Отчетный месяц], 104)) from stg.[files].[OSV_MONTHLY])
    declare @repmonth date = DATEFROMPARTS(DATEPART(year,@repmonthtemp),datepart(month,@repmonthtemp),1)
    --select @repmonth

    delete from finAnalytics.OSV_MONTHLY where REPMONTH=@repmonth

  begin tran  
    
    INSERT INTO finAnalytics.OSV_MONTHLY
	(
	 repMonth, acc2order, acc2orderName, accNum, accName, subconto1, subconto1UID, subconto2, subconto2UID, 
     subconto3, subconto3UID, branch, nomenkGroup, restIN_BU, restIN_NU, sumDT_BU, sumDT_NU, sumKT_BU, sumKT_NU, 
 ...
 , accNum = a.[СчетАналитическогоУчета]
 , accName = a.[СчетАналитическогоУчетаНаименование]
 ...
 from stg.[files].[OSV_MONTHLY] a
 where a.[СчетАналитическогоУчета] is not null
 ...
 /*Расчет данных для ф 842*/
 EXEC dwh2.[finAnalytics].[calcRep842] @repmonth
 
 /*Расчет данных для ф 845*/
 EXEC [finAnalytics].[calcRep845] @repmonth

 /*ДАПП*/
 exec [finAnalytics].[addDAPP] @repmonth
 
 /*НМА ОС*/
 exec [finAnalytics].[addNMAOS]

 /*Расчет данных для Публикуемой*/
 EXEC [finAnalytics].[calcRepPublic] @repmonth
    ...
    set @maxDateRest = cast((select max(repmonth) from finAnalytics.OSV_MONTHLY) as varchar)
    ...
    DECLARE @msg_good NVARCHAR(2048) = CONCAT (
				'Успешное выполнение процедуры - Загрузка ОСВ за '
