...
/*
процедура создана для задачи Дмитрия нужна информация по месяно о параметре [Корректировка регистра(не учитывается)]
процедру наполняет таблицу dwh2.finAnalytics.CessionUbt
*/
CREATE PROCEDURE [finAnalytics].[updateCessionUbt] 
	@repdate date
    
AS
BEGIN
	declare @repyear int=year(@repdate)
	...
	declare @subjectHeader  nvarchar(250) ='Рассчет предельной суммы убытка', @subject nvarchar(250)
	declare @msgHeader nvarchar(max)=concat('Обновление данных в таблице CessionUbt : ',@repyear ,' год',char(10))
 ...
 begin try	
  begin tran 
	delete dwh2.finAnalytics.CessionUbt
	where year(dateDogCession)=@repyear
	...
	insert into finAnalytics.CessionUbt (dateDogCession,corrRegistr)
		select
		 l1.dateDogCession
		 ,sum(l1.corrRegistr)
		from (
		select 
			  dateDogCession=datefromparts(year([Дата договора цессии]),month([Дата договора цессии]),1)
			  ...
			  --,[Предельная  %% ставка]
		from #ReestrCession )l1
		group by l1.dateDogCession
		order by l1.dateDogCession
