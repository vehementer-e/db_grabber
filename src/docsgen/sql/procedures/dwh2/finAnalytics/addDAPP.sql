...
-- exec [finAnalytics].[addDAPP] '2026-02-01'


/*

*/
CREATE PROCEDURE [finAnalytics].[addDAPP] 
    @repmonth date
AS
BEGIN
 ...
 begin try	
  begin tran 
	...
	;with cte_62001 as(
		...
		--хард-код для Автомобиль CHEVROLET AVEO, г/в 2008, цвет серебристый (VIN KL1SF69DJB296848) нет проводки о принятии нап баланс 
		union all
		select
	...
	create table #balanceIn (dat date,/* dat1 date,*/ numdog varchar(200),client nvarchar(500),summ float,nom nvarchar(1000),nomLink binary(16), reg binary(16))

	insert into #balanceIn (dat,numdog,client,summ,nom,nomLink,reg)
		select
			--dat=l1.dat
			--, convert(date, dateadd(year, -2000, l1.dat)) as dat1
			 convert(date, dateadd(year, -2000, l1.dat)) as dat
			,numdog=l1.numdog
			,client=l1.client
			,summ=l1.summ
			,nom=l1.nom
...
--- 3. Блок определения статуса и задолжности по всем договорам для авто на отчетную дату и на дату погашения залогом
	...
	left join stg._1cumfo.РегистрСведений_СЗД_ПоказателиЗаймовПредоставленных z1 on bal.dat = z1.ОтчетнаяДата and bal.numdog=z1.НомерДоговора
	--задолность на отчетную дату
	left join stg._1cumfo.РегистрСведений_СЗД_ПоказателиЗаймовПредоставленных z2 on z2.ОтчетнаяДата = convert(date, dateadd(year, -2000, @repmonth)) and bal.numdog=z2.НомерДоговора
	--состояние договра
...
--10. формируем таблицу по договорам кол-во дней просрочки на дату погашения залогом
	drop table if exists #infoDogDpd 
	create table #infoDogDpd (nomLink binary(16),/*numdog varchar(20),*/dpd int)
	insert into #infoDogDpd (nomLink /*,numdog*/,dpd)
	select
		l1.nomlink
		,dpd=max(l1.dpd)
	from( 
		select
			nomlink=a.nomlink
			,numDog=a.numDog
			,dpd=cmr.dpd_begin_day
		from #tmpInfo a
		left join #dPZ dpz on a.nomLink=dpz.nomLink
		left join dwh2.dbo.dm_cmrstatbalance cmr on a.numDog=cmr.external_id and dateadd(year,-2000,dpz.datePogaZalog)=cmr.d
		) l1
	group by l1.nomLink


/*	--доводим таблицу-по договорам у которых на момент даты погашения нет записи о DPD выбираем последнюю запист с DPD
	--определяе последню запись в CMR по договорам у которых нет данных DPD на момент залога
	;with cte_dogNullDpd as (
	select
		numdog=external_id
		,dmax=max(d)
	...
	inner join (select
				...
				from #infoDogDpd
				where dpd is null
				) b on a.external_id=b.numdog
	group by external_id
	)
	--по этим договорам на получившиеся даты определям DPD
	select
		numdog=external_id
		,dpd=dpd_begin_day
	into #dogNullDpd
	from dwh2.dbo.dm_cmrstatbalance  a
	inner join cte_dogNullDpd b on a.external_id=b.numdog and a.d=b.dmax 
	--обновляем данные по DPD по договорам у которых не было на дату погашения данных
	merge #infoDogDpd trg
	using #dogNullDpd src
		on  trg.numdog=src.numdog
		when matched then update set trg.dpd=src.dpd;
*/
	
	--- 11. Блок сборки всех блоков в единую таблицу, внутри блока определям также статус банкротства
	--declare @repmonth date ='4025-08-31'
...
insert into dwh2.finAnalytics.DAPP
			...
			,checkPriceBalance
			 ,dateOrderFRMinus,sumOrderFRMinus,dpd)
	select 
		...
		left join #infoDogDpd dpd on bal.nomLink=dpd.nomlink
