...
CREATE PROC [finAnalytics].[calcRepPublic_5_1] 
    @repmonth date
AS
BEGIN
    ...
    begin try
		...
		, [sumAmountItog] = 0
		from dwh2.[finAnalytics].[SPR_repPublicPL_5_1] a
		...
		,rowName
		from #rep
		where rowName in ('1','1(1)','2','3','4','5')
		) t2 on (t1.rowName = t2.rowName)
		when matched then update
		set t1.[sumAmountItog] = t2.[sumAmountItog];

		delete from dwh2.[finAnalytics].[repPublicPL_5_1] where repmonth = @repmonth

		insert into dwh2.[finAnalytics].[repPublicPL_5_1]
   ...
   exec dwh2.finAnalytics.sys_log  @sp_name,1, @mainPrc
    ...
    DECLARE @msg_bad NVARCHAR(2048) = CONCAT (
				'Ошибка выполнения процедуры расчета даных для публикуемой отчетности Таблица 5.1'
				,'. Ошибка '
				,ERROR_MESSAGE()
				)
	----кэтч
   ...
   exec finAnalytics.sys_log  @sp_name,1, @mainPrc,  @log_IsError,  @log_Mem
	...
	declare @subject  nvarchar(200) = 'Ошибка расчета 5.1 для Публикуемой'
