...
CREATE PROC [finAnalytics].[calcRepPublic_8_4] 
    @repmonth date
AS
BEGIN
    ...
    begin try
		...
		create table #rep(
			...
			[is3] [int] NULL,
			[is4] [int] NULL,
			[sumAmountCol1] float null,
			[sumAmountCol2] float null,
		...
		, [is3]
		, [is4]
		, [sumAmountCol1] = case when is3=1 then cast(isnull(osv.restOUT_BU * a.[Aplicator],0) as float) else 0 end
		, [sumAmountCol2] = case when is4=1 then cast(isnull(osv.restOUT_BU * a.[Aplicator],0) as float) else 0 end
		, [sumAmountItog] = 0
		from dwh2.[finAnalytics].[SPR_repPublicPL_8_4] a
		...
		using(
		select
		[sumAmountCol1] = sum([sumAmountCol1])
		,[sumAmountCol2] = sum([sumAmountCol2])
		from #rep
		where razdel = 1
		) t2 on (t1.rowName = '1')
		when matched then update
		...
		,[sumAmountCol2] = sum([sumAmountCol2])
		from #rep
		where rowName in ('1','2','3','4','5','6','7')
		) t2 on (t1.rowName = '8')
		when matched then update
		set t1.[sumAmountCol1] = t2.[sumAmountCol1],
			t1.[sumAmountCol2] = t2.[sumAmountCol2];

		/*Итого между графой 3 и 4*/
		merge into #rep t1
		using(
		select
		...
		delete from dwh2.[finAnalytics].[repPublicPL_8_4] where repmonth = @repmonth

		insert into dwh2.[finAnalytics].[repPublicPL_8_4]
		([repmonth], [RowNum], [Razdel], [RowName], [Pokazatel], [Acc2], [Aplicator], [isBold], [is3], [is4], [sumAmountCol1], [sumAmountCol2], [sumAmountItog])
		select * from #rep
   ...
   exec dwh2.finAnalytics.sys_log  @sp_name,1, @mainPrc
    ...
    DECLARE @msg_bad NVARCHAR(2048) = CONCAT (
				'Ошибка выполнения процедуры расчета даных для публикуемой отчетности Таблица 8.4'
				,'. Ошибка '
				,ERROR_MESSAGE()
				)
	----кэтч
   ...
   exec finAnalytics.sys_log  @sp_name,1, @mainPrc,  @log_IsError,  @log_Mem
	...
	declare @subject  nvarchar(200) = 'Ошибка расчета 8.4 для Публикуемой'
