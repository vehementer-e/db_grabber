...
CREATE PROCEDURE [finAnalytics].[loadSPR_comm] 
...
BEGIN
    ...
    delete from dwh2.[finAnalytics].[SPR_comm_correction]

  begin tran  
    
    INSERT INTO dwh2.[finAnalytics].[SPR_comm_correction]
	(
	[repmonth], [rowName], [sheetName], [correctionAmount], [created]
	 )
     

	select
   [repmonth] = a.[Отчетный месяц]
   , [rowName] = a.[№ строки]
   , [sheetName] = a.[Лист]
   , [correctionAmount] = a.[Сумма корректировки]
   , [dateLoad] = a.[created]
 
 from stg.[files].[spr_comm] a
    ...
    set @maxDateRest = cast((select max(repmonth) from dwh2.[finAnalytics].[SPR_comm_correction]) as varchar)
    

    DECLARE @msg_good NVARCHAR(2048) = CONCAT (
				'Успешное выполнение процедуры - Загрузка Справочника корректировок по комиссиям.'
