...
CREATE PROCEDURE [finAnalytics].[calcRep840FirstLevel_21_v3]
	@repmonth date,
    @repdate date
    --,    @sumRang int
AS
...
,[Примечание] = 'Дт 612, 47422 Кт 48801, 49401,48701, признак : Дистанционный (с использованием средств телекоммуникаций);  Для проводок Дт 612 Кт 48801,49401, 48701  - Признак: Полное досрочное погашение, Частичное списание;'
,73
from (
SELECT 
/*
...
,[Значение] = ROUND(cast(isnull(sum(l1.[Сумма БУ]),0) as money),3)
,[Примечание] = 'Дт 48809, 49409, 48709 612 Кт 48802, 49402, 48702, признак : Дистанционный (с использованием средств телекоммуникаций);  Для проводок Дт 612 Кт 48802,49402, 48702  - Признак: Полное досрочное погашение, Частичное списание;'
,75
from (
SELECT 
/*
...
,[Значение] = ROUND(cast(isnull(sum(l1.[Сумма БУ]),0) as money),3)
,[Примечание] = 'Дт 47422 612  Кт 60323, признак : Дистанционный (с использованием средств телекоммуникаций);  Для проводок Дт 612 Кт 6023  - Признак: Полное досрочное погашение, Частичное списание; ВидыПлатежейПоЗаймам - не равен госпошлины, проценты, мошенники'
,77
from (
