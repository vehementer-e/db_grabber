...
CREATE PROCEDURE [finAnalytics].[loadINCOMMING_BALANCE_MONTHLY_SG] 
...
BEGIN
    ...
    declare @repmonthtemp date = (select min([Дата периода]) from stg.[files].[INCOMING_BALANCE_SG])
  ...
  begin try
	...
	set @repMonthCountCheck = (select
								...
								from stg.[files].[INCOMING_BALANCE_SG]
								) l1)
	...
	delete from dwh2.finanalytics.INCOMING_BALANCE_SG where DATEFROMPARTS(DATEPART(year,[repDate]),datepart(month,[repDate]),1) = @repmonth

	insert into dwh2.finanalytics.INCOMING_BALANCE_SG
    ...
    set @maxDateRest = cast((select max([repDate]) from dwh2.finanalytics.INCOMING_BALANCE_SG) as varchar)
    ...
    DECLARE @msg_good NVARCHAR(2048) = CONCAT (
				'Успешное выполнение процедуры - Загрузка Отчета по ежедневным остаткам на счетах 2-го порядка СГ за '
