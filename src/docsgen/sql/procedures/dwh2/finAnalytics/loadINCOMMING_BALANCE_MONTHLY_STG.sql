...
CREATE PROCEDURE [finAnalytics].[loadINCOMMING_BALANCE_MONTHLY_STG] 
...
BEGIN
    ...
    declare @repmonthtemp date = (select min([Дата периода]) from stg.[files].[INCOMING_BALANCE_STG])
  ...
  begin try
	...
	set @repMonthCountCheck = (select
								...
								from stg.[files].[INCOMING_BALANCE_STG]
								) l1)
	...
	delete from dwh2.finanalytics.INCOMING_BALANCE_STG where DATEFROMPARTS(DATEPART(year,[repDate]),datepart(month,[repDate]),1) = @repmonth

	insert into dwh2.finanalytics.INCOMING_BALANCE_STG
    ...
    set @maxDateRest = cast((select max([repDate]) from dwh2.finanalytics.INCOMING_BALANCE_STG) as varchar)
    ...
    DECLARE @msg_good NVARCHAR(2048) = CONCAT (
				'Успешное выполнение процедуры - Загрузка Отчета по ежедневным остаткам на счетах 2-го порядка СТГ за '
