...
CREATE PROCEDURE [finAnalytics].[loadSPR_OS_NMA] 
...
BEGIN
    ...
    DECLARE @subject NVARCHAR(255) = 'Выполнение процедуры загрузки справочников ОС и НМА'
    --SELECT 1
    ...
    delete from dwh2.finanalytics.SPR_PL_OS_NMA --where REPMONTH=@repmonth

  begin tran  
    
    INSERT INTO dwh2.finanalytics.SPR_PL_OS_NMA
	select
	 REPMONTH = DATEfromparts(year(a.[Отчетный месяц]), month(a.[Отчетный месяц]),1)--@REPMONTH
	,[Показатель]
	,[Сумма корректировки]
	, dataLoadDate = created
   from stg.[files].[spr_OS_NMA] a
    ...
    set @maxDateRest = cast((select max(repmonth) from dwh2.finanalytics.SPR_PL_OS_NMA) as varchar)
    

    DECLARE @msg_good NVARCHAR(2048) = CONCAT (
				'Успешное выполнение процедуры - Загрузка Справочника по ОС и НМА для отчета PL для публикуемой.'
