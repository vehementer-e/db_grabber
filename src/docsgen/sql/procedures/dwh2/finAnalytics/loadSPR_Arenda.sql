...
CREATE PROCEDURE [finAnalytics].[loadSPR_Arenda] 
...
BEGIN
    ...
    --declare @repmonthtemp date = (select min(CONVERT (date, [Отчетная дата], 104)) from stg.[files].[SPR_arenda])
    --declare @repmonth date = DATEFROMPARTS(DATEPART(year,@repmonthtemp),datepart(month,@repmonthtemp),1)
    --select @repmonth

    delete from finAnalytics.SPR_Arenda --where REPMONTH=@repmonth

  begin tran  
    
    INSERT INTO finAnalytics.SPR_Arenda
	(
	repMonth, client, dogNum, sum60804, sum60805, sum60806, sumOverflow, sum90days, dataLoadDate
	 )
 ...
 , sum60804 = a.[Сумма АФПП на счете № 60804 на отчетную дату]
 , sum60805 = a.[Сумма амортизации на счете № 60805 на отчетную дату]
 , sum60806 = a.[Сумма обязательства на счете № 60806 на отчетную дату]
 , sumOverflow = a.[Превышение обязательств на стоимостью активов с учетом амортизац]
 , sum90days = a.[Сумма обязательств сроком погашения до 90 дней ]
 , dataLoadDate = created
 
 from stg.[files].[SPR_arenda] a
    ...
    DECLARE @msg_good NVARCHAR(2048) = CONCAT (
				'Успешное выполнение процедуры - Загрузка Справочника по Аренде.'
