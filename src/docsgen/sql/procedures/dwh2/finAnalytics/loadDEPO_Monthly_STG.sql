...
CREATE PROCEDURE [finAnalytics].[loadDEPO_Monthly_STG] 
...
BEGIN
    ...
    declare @repmonthtemp date = (select min(CONVERT (date, [Отчетная дата], 104)) from stg.[files].[DEPO_MONTHLY_STG])
  ...
  --  end
    ...
    delete from finAnalytics.DEPO_MONTHLY_STG where REPMONTH=@repmonth

  begin tran  
    
    INSERT INTO finAnalytics.DEPO_MONTHLY_STG
	(
	...
	from (
        ...
        from stg.[files].[DEPO_MONTHLY_STG] a
    ...
    DECLARE @msg_good NVARCHAR(2048) = CONCAT (
				'Успешное выполнение процедуры - Загрузка Отчета по привлечению СТГ за '
