...
CREATE PROC [finAnalytics].[loadOSV_RestsCheck] 
				@repmonth date
	...
	declare @isNullPDN2 varchar(max) = null
		...
		insert into #Data 
					...
					where 
						...
						and (
							upper(a.[Адрес регистрации]) not like upper('%Донецк%')
							or upper(a.[Адрес регистрации]) not like upper('%Луганск%')
						    or upper(a.[Адрес регистрации]) not like upper('%Запорожск%')
							or upper(a.[Адрес регистрации]) not like upper('%Херсон%')
