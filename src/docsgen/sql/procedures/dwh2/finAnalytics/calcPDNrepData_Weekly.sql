...
CREATE PROCEDURE [finAnalytics].[calcPDNrepData_Weekly] 
    @REP_MONTH date,
    @CALC_DATE date
AS
BEGIN
    ...
    DROP TABLE IF EXISTS #ID_LIST
        ...
        */
        ...
        inner join #rep_days d on r.ОтчетнаяДата = convert(date,d.dt)
  ...
  begin tran  
        ...
        ,[UMFOValue] = (
                        ...
                        where 1=1
                        and cast(Период as date) between dateadd(year,2000,@rep_month) and dateadd(year,2000,@CALC_DATE)
       ...
       ,[groupName] = case when L =0 then 'в т.ч. ПДН <=50%'
        ...
        delete from finAnalytics.repPDNcheckDetail where repMonth=@REP_MONTH and calcDate=@CALC_DATE and repType='WEEKLY'
  ...
  commit tran
    ...
    DECLARE @msg_good NVARCHAR(2048) = CONCAT (
				'Успешное выполнение процедуры - Расчет недельных данных для отчета ПДН за '
                ,FORMAT( @CALC_DATE, 'MMMM yyyy', 'ru-RU' )
                ,char(10)
                ,char(13)
