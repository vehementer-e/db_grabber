CREATE   PROCEDURE [finAnalytics].[loadSPR_bankWOLicense] 
...
BEGIN
    ...
    --declare @repmonthtemp date = (select min(CONVERT (date, [Отчетная дата], 104)) from stg.[files].[SPR_bankwolicense])
    --declare @repmonth date = DATEFROMPARTS(DATEPART(year,@repmonthtemp),datepart(month,@repmonthtemp),1)
    --select @repmonth
    --select * from stg.[files].[SPR_bankwolicense]

    delete from finAnalytics.SPR_BanksWOLicense --where REPMONTH=@repmonth

  begin tran  
    
    INSERT INTO finAnalytics.SPR_BanksWOLicense
	(
	repMonth, bankName, bikName, INN, accNum, licNum, licReturnDate, dataLoadDate
	 )
     

	select
   REPMONTH = DATEfromparts(year(a.[Отчетная дата]), month(a.[Отчетная дата]),1)--@REPMONTH
 , bankName = a.[Полное наименование]
 , bikName = a.[БИК]
 , INN = a.[ИНН]
 , accNum = a.[Номер счета]
 , licNum = a.[Номер лицензии]
 , licReturnDate = a.[Дата отзыва лицензии]
 , dataLoadDate = created
 
 from stg.[files].[SPR_bankwolicense] a
...
-- select * from stg.[files].[SPR_bankwolicense]
    ...
    set @maxDateRest = cast((select max(repmonth) from finAnalytics.SPR_BanksWOLicense) as varchar)
    

    DECLARE @msg_good NVARCHAR(2048) = CONCAT (
				'Успешное выполнение процедуры - Загрузка Справочника Банков с отозванной лицензией.'
