...
BEGIN
	...
	/*Расчет среднедневного остатка для Фондирования*/
		...
		--declare @dateFromTmp date = dateadd(day,-1,@repmonth)
		...
		declare @dateFrom date = dateadd(day,-1,@repmonth)
		declare @dateTo date = dateadd(day,-1,eomonth(@repmonth))
		...
		where ОтчетнаяДата between @dateFrom and @dateTo--dateadd(year, -2000, @dateFrom) and dateadd(year, -2000, @dateTo)
