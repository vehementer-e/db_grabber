...
CREATE PROCEDURE [finAnalytics].[loadRepReservData] 
    @repmonth date
AS
BEGIN
	...
	/*Данные для основного отчета*/
    exec finAnalytics.calcRepReservPart1 @repmonth
    exec finAnalytics.calcRepReservPart2 @repmonth
	/*Данные для сокращенного отчета*/
	exec [finAnalytics].[calcRepReservLena] @repMonth

	DECLARE @maxDateRest NVARCHAR(30)
    set @maxDateRest = cast((select max(repmonth) from dwh2.[finAnalytics].[repReservMassive]) as varchar)
    ...
    DECLARE @msg_good NVARCHAR(2048) = CONCAT (
				'Успешное выполнение процедуры расчета данных для отчета ОД+Резервы Новый разрез за '
