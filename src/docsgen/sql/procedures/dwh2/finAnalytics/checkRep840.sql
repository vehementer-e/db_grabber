...
,[Данные УМФО] = (
                    ...
                    from #registr--stg._1cUMFO.РегистрСведений_СЗД_ПоказателиЗаймовПредоставленных
                    where 1=1
                    --and ОтчетнаяДата = convert(date, EOMONTH(@repMonth))
...
,[Данные УМФО] = (
                    ...
                    from #registr--stg._1cUMFO.РегистрСведений_СЗД_ПоказателиЗаймовПредоставленных
                    where 1=1
                    --ОтчетнаяДата = convert(date, EOMONTH(@repMonth))
                    and substring(СчетОД,1,5) in ('48701')
                )
) l1
...
,[Данные УМФО] = (
                    ...
                    where 
                    a.ОтчетнаяДата = convert(date, EOMONTH(@repMonth))
...
,[Данные УМФО] = (
                    select  
                    cast(count(a.НомерДоговора) as int)
                    from #registr--stg._1cUMFO.РегистрСведений_СЗД_ПоказателиЗаймовПредоставленных a
                    where 1=1
                    --and a.ОтчетнаяДата = convert(date, EOMONTH(@repMonth))
