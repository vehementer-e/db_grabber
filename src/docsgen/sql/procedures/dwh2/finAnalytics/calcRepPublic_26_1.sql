...
CREATE PROC [finAnalytics].[calcRepPublic_26_1] 
    @repmonth date
AS
BEGIN
	...
	drop table if exists #OSV
		...
		from dwh2.[finAnalytics].[SPR_repPublicPL_26_1] a
		left join (
		select
		acc2order
		,[Symbol]
		...
		merge into #rep t1
		using(
		select [sumAmountItog] = sum(restOUT_BU) from #OSV
			where acc2order ='71101'
			and Symbol ='44113'
			and upper(subconto1) != upper('Арендные обязательства, процентный расход(44113,сч.71101)')
		) t2 on (t1.rowName = '10.11')
		when matched then update
		set t1.[sumAmountItog] = t2.[sumAmountItog];
		...
		[sumAmountItog] = sum([sumAmountItog])
		from #rep
		where rowName in ('9','10','11','12','13','14','15')
		) t2 on (t1.rowName = '8')
		when matched then update
		set t1.[sumAmountItog] = t2.[sumAmountItog];
		...
		[sumAmountItog] = sum([sumAmountItog])
		from #rep
		where rowName in ('1','8')
		) t2 on (t1.rowName = '16')
		when matched then update
		set t1.[sumAmountItog] = t2.[sumAmountItog];


		delete from dwh2.[finAnalytics].[repPublicPL_26_1] where repmonth = @repmonth

		insert into dwh2.[finAnalytics].[repPublicPL_26_1]
   ...
   exec dwh2.finAnalytics.sys_log  @sp_name,1, @mainPrc
    ...
    DECLARE @msg_bad NVARCHAR(2048) = CONCAT (
				'Ошибка выполнения процедуры расчета даных для публикуемой отчетности Таблица 26.1'
				,'. Ошибка '
				,ERROR_MESSAGE()
				)
	----кэтч
   ...
   exec finAnalytics.sys_log  @sp_name,1, @mainPrc,  @log_IsError,  @log_Mem
	...
	declare @subject  nvarchar(200) = 'Ошибка расчета 26.1 для Публикуемой'
