...
CREATE PROCEDURE [finAnalytics].[calcRep840FirstLevel_22]
	@repmonth date,
    @repdate date
    --,    @sumRang int
AS
...
,[Раздел] = '2.2'
,[Пункт] = '2.11.3.4'
,[Показатель] = '        физическим лицам, применяющим специальный налоговый режим «Налог на профессиональный доход» и не являющимся индивидуальными предпринимателями'
,[Значение] = ROUND(cast(isnull(sum([zadolgOD]),0) as money),3)
,[Примечание] = 'Берем строки где "Признак заемщика" = ФЛ; "Состояние"= действует; "Способ выдачи займа"= Онлайн; "Номенклатурная группа" = %займ для Самозанятых%; Installment до 1 млн. Рыночный. Самозанятый; Счет учета = 488 Сумма по полю "Задолженность ОД"'
,10
from #pbr a
...
,[Значение] = ROUND(cast(isnull(sum(zadolgPrc),0) as money) 
					...
					,3)
,[Примечание] = 'Берем строки где "Признак заемщика" = ФЛ; "Состояние"= действует; "Способ выдачи займа"=Онлайн; "Номенклатурная группа" = %займ для Самозанятых%; Installment до 1 млн. Рыночный. Самозанятый, Счет учета = 488; Сумма по полю "Задолженность проценты" и полю "Сумма пени счета"'
,20
...
--and upper(a.nomenkGroup) like upper('%займ для Самозанятых%')
and upper(a.nomenkGroup) like upper('%Самозанят%')
and upper(a.[saleType])=upper('Онлайн')/*Дистанционный*/
and substring(a.AccODNum,1,3) = '488'
and a.saleDate<=EOMONTH(a.REPMONTH)
...
,[Примечание] = 'Схема действует после 01.07.2024 года; Берем строки где "Состояние"= действует; "Способ выдачи займа"=Онлайн; Счет учета = 488; "Признак заемщика" = ФЛ; "Номенклатурная группа" = %займ для Самозанятых%, Installment до 1 млн. Рыночный. Самозанятый; Дата выдачи займа не может быть больше отчетного периода; считаем договора, у одного заещика может быть несколько договоров'
,30
from #pbr a
...
,[Примечание] = 'Берем строки где "Состояние"= действует; "Способ выдачи займа"=Онлайн;Счет учета = 488; Признак заемщика = ФЛ; "Номенклатурная группа" = %займ для Самозанятых%; Считаем контрагентов, они не должны повторятся, лучше сверять ИНН. Дата выдачи займа(действующего договора) не может быть больше отчетного периода'
,40
from #pbr a
...
,[Показатель] = '        физическим лицам, применяющим специальный налоговый режим «Налог на профессиональный доход» и не являющимся индивидуальными предпринимателями, в том числе:'
,[Значение] = isnull(count(distinct a.dogNum),0)
,[Примечание] = 'Берем строки где "Признак заемщика" = ЮЛ; Счет учета = 488; "Способ выдачи займа"=Онлайн; " Дата выдачи = текущему отчетному периоду; "Состояние" не равно "Отменен";  "Номенклатурная группа" =%займ для Самозанятых%; Installment до 1 млн. Рыночный. Самозанятый, Считаем количество договоров, у одного заемщика может быть больше одного договора. '
,50
from #pbr a
...
,[Примечание] = 'Берем строки где "Признак заемщика" = ФЛ; Счет учета = 488; "Способ выдачи займа"=Онлайн; " Дата выдачи = текущему отчетному периоду; "Номенклатурная группа" = %займ для Самозанятых%, Installment до 1 млн. Рыночный. Самозанятый; "Состояние" не равно "Отменен"; Сумма по полю "Сумма займа"'
,60    
from #pbr a
...
,[Примечание] = 'Дт 612, 47422, Кт 48801, 49401,48701, признак : Онлайн;  Для проводок Дт 612 Кт 48801,49401, 48701  - Признак: Полное досрочное погашение, Частичное списание;Полное списание'
,61
from (
SELECT 
/*
...
,[Значение] = ROUND(cast(isnull(sum(l1.[Сумма БУ]),0) as money),3)
,[Примечание] = 'Дт 48809, 49409, 48709 612 Кт 48802, 49402, 48702, признак : Онлайн;  Для проводок Дт 612 Кт 48802,49402, 48702  - Признак: Полное досрочное погашение, Частичное списание;Полное списание'
,63
from (
SELECT 
/*
...
,[Значение] = ROUND(cast(isnull(sum(l1.[Сумма БУ]),0) as money),3)
,[Примечание] = 'Дт 47422 612  Кт 60323, признак : Онлайн;  Для проводок Дт 612 Кт 6023  - Признак: Полное досрочное погашение, Частичное списание, Полное списание; ВидыПлатежейПоЗаймам - не равен госпошлины, проценты, мошенники'
,65
from (
