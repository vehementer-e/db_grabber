...
*/
CREATE PROCEDURE [finAnalytics].[addReestr20501_STG] 
    
AS
BEGIN
	declare @sp_name NVARCHAR(255) = OBJECT_NAME(@@PROCID)
	declare @repmonthtemp date = (select min(CONVERT (date, [Отчетный месяц], 104)) from stg.files.Rep20501_stg)
    declare @repmonth date = DATEFROMPARTS(DATEPART(year,@repmonthtemp),datepart(month,@repmonthtemp),1)
	
	declare @subjectHeader  nvarchar(250) ='Реестр 20501 СТГ', @subject nvarchar(250)
	declare @msgHeader nvarchar(max)=concat('Внесение данных в реестр 20501 СТГ: ',FORMAT(@repmonth, 'MMMM yyyy', 'ru-RU' ),char(10))
...
-- Первый этап
    delete from finAnalytics.Reestr20501_stg where REPMONTH=@repmonth

	begin tran  
		insert into finAnalytics.Reestr20501_stg (REPDATE,Dt,Kt,inSumm,outSumm,Client,typeClient,innClient,senderPay,purPay,numAcc,nameBank,bikBank,codBank
