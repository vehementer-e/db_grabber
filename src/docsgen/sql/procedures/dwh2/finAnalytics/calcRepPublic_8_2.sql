...
CREATE PROC [finAnalytics].[calcRepPublic_8_2] 
    @repmonth date
AS
BEGIN
    ...
    begin try
		...
		, [sumAmountItog] = 0
		from dwh2.[finAnalytics].[SPR_repPublicPL_8_2] a
		...
		delete from dwh2.[finAnalytics].[repPublicPL_8_2] where repmonth = @repmonth

		insert into dwh2.[finAnalytics].[repPublicPL_8_2]
   ...
   exec dwh2.finAnalytics.sys_log  @sp_name,1, @mainPrc
    ...
    DECLARE @msg_bad NVARCHAR(2048) = CONCAT (
				'Ошибка выполнения процедуры расчета даных для публикуемой отчетности Таблица 8.2'
				,'. Ошибка '
				,ERROR_MESSAGE()
				)
	----кэтч
   ...
   exec finAnalytics.sys_log  @sp_name,1, @mainPrc,  @log_IsError,  @log_Mem
	...
	declare @subject  nvarchar(200) = 'Ошибка расчета 8.2 для Публикуемой'
