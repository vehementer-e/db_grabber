...
*/
CREATE PROCEDURE [finAnalytics].[addReestr20501] 
    
AS
BEGIN
	declare @sp_name NVARCHAR(255) = OBJECT_NAME(@@PROCID)
	declare @repmonthtemp date = (select min(CONVERT (date, [Отчетный месяц], 104)) from stg.files.Rep20501)
    declare @repmonth date = DATEFROMPARTS(DATEPART(year,@repmonthtemp),datepart(month,@repmonthtemp),1)

	declare @subjectHeader  nvarchar(250) ='Реестр 20501', @subject nvarchar(250)
	declare @msgHeader nvarchar(max)=concat('Внесение данных в реестр 20501: ',FORMAT(@repmonth, 'MMMM yyyy', 'ru-RU' ),char(10))
...
-- Первый этап
    delete from finAnalytics.Reestr20501 where REPMONTH=@repmonth

	begin tran  
		insert into finAnalytics.Reestr20501 (REPDATE,Dt,Kt,inSumm,outSumm,Client,typeClient,innClient,senderPay,purPay,numAcc,nameBank,bikBank,codBank
...
-- Второй этап
	...
	-------- рассылка по отчету Справочник Контрагентов в случаи отсутвия ошибок
	else 
		begin
			--declare @subject varchar(500),@message varchar(200)
			set @subject='Обновились данные для отчета Справочник по Контрагентам'
			set @message= concat('Справочник Контрагентов обновлен',char(10))
			exec finAnalytics.sendEmail @subject, @message ,@strRcp ='1,35'
		end
