...
-- создаем дополнительно новую витрину reports.dbo.dm_CollectionNonPaymentReasonDetail
-- =======================================================
-- Modify: 7.02.2022. А.Никитин
-- Description:	DWH-1492 Доработка отчета по причинам неплатежа
-- =======================================================
create         procedure [collection].[reportCollection_NonPaymentReason]
  @year int
  ,@month int
as 
...
with p2 as (
 ...
 select date , NonPaymentReason, Client_Stage , count(distinct [CRMClientGUID]) as cnt
  ...
  group by  date, NonPaymentReason, Client_Stage
  	  ...
  	  delete from  reports.dbo.dm_CollectionNonPaymentReasonDetail where dt_year = @year and dt_month = @month
      insert into reports.dbo.dm_CollectionNonPaymentReasonDetail
      select    
		@year  as dt_year
		, @month as dt_month
		, NonPaymentReason
		...
		, cnt
		, date
	  --into reports.dbo.dm_CollectionNonPaymentReasonDetail
      from #pr2
  ...
  --SELECT * FROM #pr2
	  ...
	  delete from  reports.dbo.dm_CollectionNonPaymentReason where dt_year = @year and dt_month = @month
      insert into reports.dbo.dm_CollectionNonPaymentReason
      select    
	   @year  as dt_year
      , @month as dt_month
	  , NonPaymentReason
	  ...
	  --into reports.dbo.dm_NonPaymentReason
      from #pr1
      --where date>='20200401' and date<'20200501'
...
with p2 as (
 ...
 select distinct date , NonPaymentReason, Client_Stage ,  p2.[CRMClientGUID] , clients.CRMClientFIO
  ...
  --group by  date, NonPaymentReason, Client_Stage
  	  ...
  	  delete from  reports.dbo.dm_CollectionNonPaymentReasonFullDetail where dt_year = @year and dt_month = @month
      insert into reports.dbo.dm_CollectionNonPaymentReasonFullDetail
      select    
		@year  as dt_year
		, @month as dt_month
		, NonPaymentReason
	  ...
	  --into reports.dbo.dm_CollectionNonPaymentReasonFullDetail
      from #pr3
   --   --where date>='20200401' and date<'20200501'
	  where year(date) = @year 
		and month(date) = @month
