...
drop table if exists #rep_weekly_portf_coll;
			   ...
			   c.external_id as [ID Кредитного Договора],

			   c.person_id as [ID Клиента],
			   ...
			   concat(z.Фамилия, ' ', z.Имя, ' ', Отчество) as [ФИО],

			   dateadd(year,-2000,cast( датарождения as date)) [Дата_рождения],
			   ...
			   c.start_date as [Дата Выдачи Займа],
			   dateadd(month,c.term,c.start_date) [Дата Окончания КД],
			   c.amount as [сумма займа], --datsyplakov 13/07/2020: добавлена сумма выдачи
			   c.term as [Срок Займа],			   
			 ...
			 --  select * into #z from stg._1cmfo.документ_ГП_заявка
			  ...
			  --   select * into #b from dwh_new.dbo.stat_v_balance2 b
			   ...
			   on c.external_id=b.external_id 
			   ...
			   on c.external_id = ac.external_id 
			   ...
			   on c.external_id = v.external_id 
			   and r_date = @rdt --cast(dateadd(day,-1,current_timestamp) as date)
			   
			   left join #payments_30d  p30  on c.external_id=p30.external_id
			   left join #payments_90d  p90  on c.external_id=p90.external_id
			   left join #payments_180d p180 on c.external_id=p180.external_id
			   left join #payments_270d p270 on c.external_id=p270.external_id
			   left join #payments_360d p360 on c.external_id=p360.external_id
			   left join #payments_all  al   on c.external_id=al.external_id
			   left join #payments_last plst on c.external_id = plst.external_id
			   
			   --left join [dwh_new].[Dialer].[StrategyDataMart2] sdm 
			   left join dm.Collection_StrategyDataMart sdm
			   on c.external_id = sdm.external_id 
			   and sdm.StrategyDate = @rdt -- cast(dateadd(day,-1,current_timestamp) as date)
			   
			   left join #last_overdue lo on c.external_id=lo.external_id
			   
			   left join #registration_address res on c.external_id=res.номер
			   
			   left join stg._Collection.Deals cd on c.external_id=cd.Number

			   left join stg._1cmfo.Справочник_ГП_СтатусПодтверждающегоЛица kl 
			   on z.КЛСтатус = kl.ссылка

			   left join #judge_requirements jreq
			   on c.external_id = jreq.external_id

			   left join #space spc
			   on c.external_id = spc.external_id
