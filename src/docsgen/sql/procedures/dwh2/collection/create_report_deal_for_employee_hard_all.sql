...
CREATE PROC collection.create_report_deal_for_employee_hard_all 
  ...
  begin try
	...
	select 															
			...
			,concat(e.LastName, ' ', e.FirstName, ' ', e.MiddleName) fio_claimant
 ...
 general as (	select 															
	...
	left join collection.say_sprav_field_actual_region		far on far.employee_id = bci.ClaimantId				--devdb.dbo.say_sprav_field_actual_region									
   ...
   --left join #type_product s_1 on d.number=s_1.external_id
	...
	where 1 = 1															
			...
			,cs.name													
			,d.OverdueDays													
			,collection.bucket_dpd(d.OverdueDays)													
			,bci.ClaimantId													
			,beh.fio_claimant													
...
begin transaction 
 ...
 delete from [collection].[report_deal_for_employee_hard_current_day];
	insert [dwh2].[collection].[report_deal_for_employee_hard_current_day] (
 ...
 select 
  ...
  from #Final_table
  where [дата закрепления клиента] >= cast(dateadd(dd,-1,getdate()) as date);
  /*select 															
			cast(getdate() as date) 'дата отчёта'													
			,d.Number 'номер договора'
			,concat(c.LastName, ' ', c.Name, ' ', c.MiddleName) 'фио клиента'	
