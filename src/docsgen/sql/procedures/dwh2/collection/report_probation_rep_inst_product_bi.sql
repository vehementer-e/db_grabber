 ...
 CREATE PROCEDURE [collection].[report_probation_rep_inst_product_bi] 
  ...
  begin try
	...
	drop table if exists #deals_Installment;
	select t1.number
	into #deals_Installment
	from  stg._collection.[Deals] t1
	left join stg._collection.customers t2 on t2.Id = t1.IdCustomer
	where t1.Installment = 1
			and [collection].customer_fio(t2.id) not in (
													  'ТЕСТ ТЕСТ ТЕСТ'
													  ,'ХЭШ МП ТЕСТ'
														)
	group by t1.number;
	...
	from #deals_Installment deals_Installment
	join dbo.dm_cmrstatbalance balance_cmr on balance_cmr.external_id = deals_Installment.number ---  change dwh_new.dbo.v_balance_cmr 
	where balance_cmr.d >= '2021-12-01' --cdate
	;
	...
	left join (
				select
						comm_res_name CommunicationResultContacr
				from 
						[dwh2].[collection].[say_hard_comm_for_kk_sprav]
				where 1=1
						and contact = 1
				group by comm_res_name
				) t6 on t6.CommunicationResultContacr = t4.Name							
	where t1.Installment = 1							
	...
	delete from  [collection].[report_probation_rep_inst_product];
	insert [collection].[report_probation_rep_inst_product] (
...
getdate() r_date
			,t1.number							
			,1 cnt					
			,coalesce(cast(tmp_v_credits.StartDate as date),DATEFROMPARTS ('20'+SUBSTRING(t1.number, 1, 2), SUBSTRING(t1.number, 3, 2), SUBSTRING(t1.number, 5, 2))) 'Дата выдачи'  --start_date
			/*coalesce(cast(tmp_v_credits.start_date as date),DATEFROMPARTS ('20'+SUBSTRING(t1.number, 1, 2), SUBSTRING(t1.number, 3, 2), SUBSTRING(t1.number, 5, 2))) 'Дата выдачи'*/
			...
			,case when coalesce(t3.Name,'Current') = 'Closed'
				  ...
				  then 1
				   when coalesce(pay.total_sum_pay,0) >= td_payment_schedule.today_plan_sum_pay
				  then 1 
	...
	left join (
				select 
						number
						,sum([СуммаПлатежа]) over (partition by number) today_plan_sum_pay
				from
					(
					select distinct
							d.код number
							,g.[СуммаПлатежа]
					from 
							[Stg].[_1cCMR].[РегистрСведений_ДанныеГрафикаПлатежей_today]				g
							join [Stg].[_1cCMR].Справочник_Договоры										d on g.Договор=d.Ссылка
					where 1=1
							and cast(dateadd(yyyy,-2000,[ДатаПлатежа]) as date) < cast(getdate() as date)
					...
					(
						select
								d.код number
								,g.[СуммаПлатежа]
								,ROW_NUMBER() over (partition by d.код order by cast([ДатаПлатежа] as date)) rn
						from 
								[Stg].[_1cCMR].[РегистрСведений_ДанныеГрафикаПлатежей_today]		g
								join [Stg].[_1cCMR].Справочник_Договоры								d on g.Договор=d.Ссылка
						where 1=1
				...
				)td_payment_schedule on td_payment_schedule.number = t1.Number
	left join dbo.dm_overdueindicators					tmp_v_credits on tmp_v_credits.Number = t1.Number  --dwh_new.dbo.tmp_v_credits	
	/*dwh_new.dbo.tmp_v_credits					tmp_v_credits on tmp_v_credits.external_id = t1.Number*/
	where t1.Installment = 1							
