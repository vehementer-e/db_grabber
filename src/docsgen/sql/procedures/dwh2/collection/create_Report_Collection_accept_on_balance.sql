...
-- Description:	DWH-2332 Реверс инжиниринг отчета Коллекшн. ИП. Принятые на баланс
-- =============================================
CREATE  PROC [collection].[create_Report_Collection_accept_on_balance]
as
begin
begin TRY
	SET XACT_ABORT ON;
	...
	declare  @dt_begin_of_month date = cast(format(cast('2023-01-01' as date),'yyyyMM01') as date)
	declare  @dt_next_month date = dateadd(year, 1, @DateCalculate)
	...
	FROM monitor AS M
	WHERE M.rn=1
		AND M.AdoptionBalanceDate BETWEEN @dt_begin_of_month and @dt_next_month


	DROP TABLE IF EXISTS #t_accept_on_balance
	CREATE TABLE #t_accept_on_balance(
		id_yearmonth int,
		Период varchar(50), 
		Уникальные_договора int,
		Сумма_принятия_на_баланс numeric(18, 2)
	)


	INSERT #t_accept_on_balance
	(
	    id_yearmonth,
	    Период,
	    Уникальные_договора,
	    Сумма_принятия_на_баланс
	)
	SELECT 
		A.id_yearmonth,
		Период = A.year_month_name, 
		Уникальные_договора = count(DISTINCT A.Уникальные_договора),
		Сумма_принятия_на_баланс = sum(A.Сумма_принятия_на_баланс)
	FROM (
		SELECT DISTINCT
			C.id_yearmonth,
			C.year_month_name, 
			Дата_Принятия_На_Баланс = monitoring.AdoptionBalanceDate,
			Уникальные_договора = Deals.Number,
			Сумма_принятия_на_баланс = monitoring.AmountDepositToBalance
		FROM Stg._Collection.EnforcementOrders as eo -- исполнительный лист
			...
			INNER JOIN cubes.v_Collection_Calendar AS C
				ON C.DT = monitoring.AdoptionBalanceDate
	...
	GROUP BY A.id_yearmonth, A.year_month_name

	SELECT 
		--T.id_yearmonth,
		T.Период,
		T.Уникальные_договора,
		T.Сумма_принятия_на_баланс
	FROM #t_accept_on_balance AS T
	ORDER BY T.id_yearmonth
