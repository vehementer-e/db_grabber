 ...
 CREATE PROCEDURE [collection].[create_say_jp_sym_pay_jp_all_sum_pay] 
...
drop table if exists #calendar;
	select convert(date,'01.06.2022',104) Month_Value into #calendar

	WHILE (select max(Month_Value) from #calendar) < dateadd(dd,1, EOMONTH(cast(getdate() as date), -1))            ---[collection].dt_st_month(cast(getdate() as date)) --reports.dbo.dt_st_month всех местах
	...
	select
			...
			,balance_cmr.d cdate
			,cast(coalesce(balance_cmr.[сумма поступлений],0) as int) total_CF --change balance_mfo
			,balance_cmr.overdue
			...
			,case when balance_cmr.[dpd day-1] = 0 
					and balance_cmr.[сумма поступлений] > 0 --pay_total on [сумма поступлений]
					...
					,balance_cmr.[Тип Продукта] ----добавленно поле 01.09.2024
	into #v_balance_cmr
	from dbo.dm_cmrstatbalance				balance_cmr  --change reports.dbo.dm_CMRStatBalance_2
	/*left join dbo.dm_cmrstatbalance				balance_mfo	on balance_mfo.external_id = balance_cmr.external_id --put out
	...
	from
					  ...
					  (
						...
						from
								...
								left join #base_dt_closed			bdc on bdc.number = bc.external_id
																		and dateadd(dd,1, EOMONTH(cast(bdc.dt_closed as date), -1))  = bc.cdate                                          ----[collection].dt_st_month(bdc.dt_closed)
						where 1=1
			...
			left join 
					  (
						select 
								r_month cdate --[collection].dt_st_month(cdate)
	...
	*/
	...
	select
			...
			,pay.sum_pay_91
			,t.count_deals
			,t.count_closed_deals
			,t.[Тип Продукта] ----добавленно поле 01.09.2024
	...
	from
		(	select Month_Value
			...
			,portf.count_deals
			,portf.count_closed_deals
			,portf.[Тип Продукта]------добавленно поле 01.09.2024
			...
			left join 
					  (
						...
						where 1=1
								...
								and bc.overdue_days_p_corr >= 1
						group by
								cdate,[Тип Продукта] ----добавленно поле 01.09.2024
					  )													portf on portf.cdate = ca.Month_Value
			...
			left join 
					  (
						...
						where 1=1
								and overdue_days_p_corr >= 1
						group by
								r_month, [Тип Продукта] ----добавленно поле 01.09.2024 --[collection].dt_st_month(cdate)
					  )													pay on pay.cdate = t.Month_Value and t.[Тип Продукта]=pay.[Тип Продукта]
	;
	...
	(
		...
		union all*/
		select *
		from dbo.dm_cmrstatbalance  --RiskDWH.dbo.stg_coll_bal_mfo
	...
	select 
			...
			,[Тип Продукта] ---добавленно поле 01.09.2024
	into #base_pay_all
	...
	select distinct
			...
			,b1.[Тип Продукта] ---добавленно поле 01.09.2024
	into #base_pay_jp_month
	from base_pay_all															b1
	left join (select number
						,r_month --[collection].dt_st_month(b1.d) r_month
...
BEGIN TRANSACTION
	...
	delete from [collection].[say_jp_sym_pay_jp_all_sum_pay];
	insert [collection].[say_jp_sym_pay_jp_all_sum_pay] (

	[отчётный месяц]
      ,[cash 1-90]
      ,[cash 91+]
      ,[cash на всех стадиях СП]
      ,[cash на стадии отправка ЗТ]
      ,[cash на стадии подача иска]
      ,[cash на стадии вынесения решения]
      ,[cash на стадии получения ИЛ]
      ,[cash на стадии возбуждения СП]
      ,[доля в cash стадии отправка ЗТ]
      ,[доля в cash стадии подача иска]
      ,[доля в cash стадии вынесения решения]
      ,[доля в cash стадии получения ИЛ]
      ,[доля в cash стадии возбуждения СП]
      ,[доля cash 91+ в cash 1+]
      ,[доля в cash ИП в cash 1+]
	  ,[Тип Продукта] ---добавленно поле 01.09.2024
	...
	select 
			pca.Month_Value 'отчётный месяц'
			,pca.sum_pay_1_90 'cash 1-90'
			,pca.sum_pay_91 'cash 91+'
			,bpm.sum_pay_jp_all 'cash на всех стадиях СП'
			,bpm.sum_pay_submission_claim 'cash на стадии отправка ЗТ'
			,bpm.sum_pay_Court_Claim_Sending 'cash на стадии подача иска'
			,bpm.sum_pay_Receipt_Judgment 'cash на стадии вынесения решения'
			,bpm.sum_pay_Receipt_IL 'cash на стадии получения ИЛ'
			,bpm.sum_pay_excitation_ep 'cash на стадии возбуждения СП'
			,isnull(cast(bpm.sum_pay_submission_claim as float) / nullif((cast(pca.sum_pay_1_90 as float)  + cast(pca.sum_pay_91 as float)),0),0) 'доля в cash стадии отправка ЗТ'
			,isnull(cast(bpm.sum_pay_Court_Claim_Sending as float) / nullif((cast(pca.sum_pay_1_90 as float)  + cast(pca.sum_pay_91 as float)),0),0) 'доля в cash стадии подача иска'
			,isnull(cast(bpm.sum_pay_Receipt_Judgment as float) /nullif((cast(pca.sum_pay_1_90 as float)  + cast(pca.sum_pay_91 as float)),0),0) 'доля в cash стадии вынесения решения'
			,isnull(cast(bpm.sum_pay_Receipt_IL as float) / nullif((cast(pca.sum_pay_1_90 as float)  + cast(pca.sum_pay_91 as float)),0),0) 'доля в cash стадии получения ИЛ'
			,isnull(cast(bpm.sum_pay_excitation_ep as float) / nullif((cast(pca.sum_pay_1_90 as float)  + cast(pca.sum_pay_91 as float)),0),0) 'доля в cash стадии возбуждения СП'
			,isnull(cast(pca.sum_pay_91 as float) / nullif((cast(pca.sum_pay_1_90 as float)  + cast(pca.sum_pay_91 as float)),0),0) 'доля cash 91+ в cash 1+'
			,isnull(cast(bpm.sum_pay_jp_all as float) / nullif((cast(pca.sum_pay_1_90 as float)  + cast(pca.sum_pay_91 as float)),0),0) 'доля в cash ИП в cash 1+'
			/*,cast(bpm.sum_pay_submission_claim as float) / (cast(pca.sum_pay_1_90 as float)  + cast(pca.sum_pay_91 as float)) 'доля в cash стадии отправка ЗТ'
			,cast(bpm.sum_pay_Court_Claim_Sending as float) / (cast(pca.sum_pay_1_90 as float)  + cast(pca.sum_pay_91 as float)) 'доля в cash стадии подача иска'
			,cast(bpm.sum_pay_Receipt_Judgment as float) / (cast(pca.sum_pay_1_90 as float)  + cast(pca.sum_pay_91 as float)) 'доля в cash стадии вынесения решения'
			,cast(bpm.sum_pay_Receipt_IL as float) / (cast(pca.sum_pay_1_90 as float)  + cast(pca.sum_pay_91 as float)) 'доля в cash стадии получения ИЛ'
			,cast(bpm.sum_pay_excitation_ep as float) / (cast(pca.sum_pay_1_90 as float)  + cast(pca.sum_pay_91 as float)) 'доля в cash стадии возбуждения СП'
			,cast(pca.sum_pay_91 as float) / (cast(pca.sum_pay_1_90 as float)  + cast(pca.sum_pay_91 as float)) 'доля cash 91+ в cash 1+'
			,cast(bpm.sum_pay_jp_all as float) / (cast(pca.sum_pay_1_90 as float)  + cast(pca.sum_pay_91 as float)) 'доля в cash ИП в cash 1+'*/
			,pca.[Тип Продукта] ---добавленно поле 01.09.2024
	...
	from 
			...
			join (
					select
							r_month
							,sum(sum_pay_jp_all) sum_pay_jp_all
							,sum(sum_pay_submission_claim) sum_pay_submission_claim
							,sum(sum_pay_Court_Claim_Sending) sum_pay_Court_Claim_Sending
							,sum(sum_pay_Receipt_Judgment) sum_pay_Receipt_Judgment
							,sum(sum_pay_Receipt_IL) sum_pay_Receipt_IL
							,sum(sum_pay_excitation_ep) sum_pay_excitation_ep
							,[Тип Продукта] ---добавленно поле 01.09.2024
					from 
							#base_pay_jp_month
					group by
							r_month,[Тип Продукта]
					)																					bpm on bpm.r_month = pca.Month_Value and bpm.[Тип Продукта]=pca.[Тип Продукта] ---добавленно поле 01.09.2024
	order by 1 
	;
