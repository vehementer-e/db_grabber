...
--exec [risk].[copy_to_povt_inst_buffer_history];
CREATE PROCEDURE [risk].[copy_to_povt_inst_buffer_history]
AS
BEGIN TRY
	IF OBJECT_ID('risk.povt_inst_buffer_history') IS NULL
	BEGIN
		SELECT TOP (0) [cdate] = cast(getdate() AS DATE)
			,t.*
		INTO risk.povt_inst_buffer_history
		FROM [risk].povt_inst_buffer t

		CREATE CLUSTERED INDEX cix_cdate_external_id ON risk.povt_inst_buffer_history (
			cdate
			,external_id
			)
	END
	...
	BEGIN TRAN
	--alter table risk.povt_inst_buffer_history
	...
	*/
	DELETE
	FROM risk.povt_inst_buffer_history
	WHERE [cdate] = cast(getdate() AS DATE)

	INSERT INTO [risk].[povt_inst_buffer_history] (
		[cdate]
		,[external_id]
		,[CMRClientGUID]
		,[person_id]
	...
	SELECT [cdate] = cast(getdate() AS DATE)
		...
		,fl_low_PDscore_ByPerson
	FROM [risk].povt_inst_buffer WITH (NOLOCK)
