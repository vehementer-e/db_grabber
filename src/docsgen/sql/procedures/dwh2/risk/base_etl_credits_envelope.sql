CREATE   procedure [risk].[base_etl_credits_envelope]
as
begin
	...
	begin try
		...
		EXEC [CommonDb].[SendNotification].[Send2GChat_dwhNotification]
				@text = @text
				,@threadKey = @subject
		exec logdb.dbo.[LogAndSendMailToAdmin] 'trying start risk.base_etl_credits','Info',' started', ''
	
		
		exec [risk].[base_etl_credits]

		exec logdb.dbo.[LogAndSendMailToAdmin] 'exec risk.base_etl_credits - completed successfully','Info','Done', ''
		
		set @text=concat(':heavy_check_mark: ', 
			'Формирование  risk.credits завершено успешно.',
			format(getdate(),'dd.MM.yyyy HH:mm:ss')
			)
		EXEC [CommonDb].[SendNotification].[Send2GChat_dwhNotification]
			@text = @text
	...
	begin catch
		...
		set @text=concat(':exclamation: ', 
			'Формирование  risk.credits завершено с ошибкой!',
			format(getdate(),'dd.MM.yyyy HH:mm:ss'))
		EXEC [CommonDb].[SendNotification].[Send2GChat_dwhNotification]
			@text = @text
      
		exec logdb.dbo.[LogAndSendMailToAdmin] 'catching error risk.base_etl_credits','Error','Error',@error_description
