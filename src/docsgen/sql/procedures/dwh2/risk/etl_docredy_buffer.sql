...
CREATE PROCEDURE [risk].[etl_docredy_buffer]
AS

begin
--рефакторирнг по задаче RDWH-28
...
if OBJECT_ID('risk.docredy_buffer') is null
begin
	select top(0) * into risk.docredy_buffer
	from #docredy_buffer
end;

BEGIN TRANSACTION
	delete from risk.docredy_buffer;  

	insert into risk.docredy_buffer (
	external_id
	,category
	,Type
	,main_limit
...
if OBJECT_ID('risk.povt_buffer') is null
begin
	select top(0) * into risk.povt_buffer
	from #povt_buffer_new_collateral_total
end;

BEGIN TRANSACTION
	delete from risk.povt_buffer; 

	insert into risk.povt_buffer (
	external_id,
	startdate,
	category,
	TYPE,
