...
--exec [risk].[etl_repbi_monitoring_pdn]
CREATE PROCEDURE [risk].[etl_repbi_monitoring_pdn]
AS
BEGIN
...
-------окт25-------
		...
		TRUNCATE TABLE risk.repbi_monitoring_pdn_mpl;

		INSERT INTO risk.repbi_monitoring_pdn_mpl
		SELECT a.startdate
			,a.daily AS period
			,a.amount_more50
			,a.amount_more80
		...
		TRUNCATE TABLE risk.repbi_monitoring_pdn_mpl_pts;

		INSERT INTO risk.repbi_monitoring_pdn_mpl_pts
		SELECT a.startdate
			,a.daily AS period
			,a.amount_more50
			,a.amount_more80
		...
		TRUNCATE TABLE risk.repbi_monitoring_pdn_mpl_autocred;

		INSERT INTO risk.repbi_monitoring_pdn_mpl_autocred
		SELECT a.startdate
			,a.daily AS period
			,a.amount_more50
			,a.amount_more80
...
select distinct a.external_id as КодДоговораЗайма, coalesce(umfo.pdn, cmr1.pdn, cmr2.pdn_logs) as pdn
into #cmr_pdn
...
FROM (
    ...
    AND number < 365
		...
		TRUNCATE TABLE risk.repbi_monitoring_pdn_mpl_cmr;

		INSERT INTO risk.repbi_monitoring_pdn_mpl_cmr
		SELECT 
			a.startdate,
			a.daily AS period,
		...
		DROP TABLE IF EXISTS #stg_mpl_day_additional_cmr;
			SELECT 
				d.startdate,
				ISNULL(sum(CASE WHEN b.pdn > 0.8 THEN a.amount ELSE 0 END), 0) amount_more80,
				ISNULL(sum(CASE WHEN b.pdn > 0.5 AND b.pdn <= 0.8 THEN a.amount ELSE 0 END), 0) amount_more50,
		...
		TRUNCATE TABLE risk.repbi_monitoring_pdn_mpl_cmr_nobig;

		INSERT INTO risk.repbi_monitoring_pdn_mpl_cmr_nobig
		SELECT 
			a.startdate,
			a.daily AS period,
			ISNULL(a.amount_more50, 0) AS amount_more50,
		...
		DROP TABLE IF EXISTS #stg_mpl_day_additional_cmr_nobig;
			SELECT 
				d.startdate,
				ISNULL(sum(CASE WHEN b.pdn > 0.8 THEN a.amount ELSE 0 END), 0) amount_more80,
				ISNULL(sum(CASE WHEN b.pdn > 0.5 AND b.pdn <= 0.8 THEN a.amount ELSE 0 END), 0) amount_more50,
		...
		TRUNCATE TABLE  risk.repbi_monitoring_pdn_mpl_cmr_bigonly;

		INSERT INTO  risk.repbi_monitoring_pdn_mpl_cmr_bigonly
		SELECT 
			a.startdate,
			a.daily AS period,
			ISNULL(a.amount_more50, 0) AS amount_more50,
		...
		TRUNCATE TABLE  risk.repbi_monitoring_pdn_mpl_cmr_bigmarket;

		INSERT INTO  risk.repbi_monitoring_pdn_mpl_cmr_bigmarket
		SELECT 
			a.startdate,
			a.daily AS period,
			ISNULL(a.amount_more50, 0) AS amount_more50,
		...
		TRUNCATE TABLE risk.repbi_monitoring_pdn_mpl_pts_cmr;

		INSERT INTO risk.repbi_monitoring_pdn_mpl_pts_cmr
		SELECT 
			a.startdate,
			a.daily AS period,
			ISNULL(a.amount_more50, 0) AS amount_more50,
		...
		TRUNCATE TABLE risk.repbi_monitoring_pdn_mpl_autocred_cmr;

		INSERT INTO risk.repbi_monitoring_pdn_mpl_autocred_cmr
		SELECT 
			a.startdate,
			a.daily AS period,
			ISNULL(a.amount_more50, 0) AS amount_more50,
