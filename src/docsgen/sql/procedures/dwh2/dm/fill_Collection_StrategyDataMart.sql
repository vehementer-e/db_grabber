...
CREATE     PROC [dm].[fill_Collection_StrategyDataMart]
...
--  num att	кол-во попыток звонков (за всю историю, контактные и неконтактные попытки)
 ...
 group by CrmCustomerId
	 ...
	 /*Изменили источник в рамка задачи DWH-536*/
	   	
	drop table if exists #plan_date
	;with base_plan_date as 
	(
	select external_id = t.Код
		, Current_НомерПлатежа =min(t.НомерПлатежа)
		,paymentDate = min(t.ДатаПлатежа)
		,end_date_payment_schedule = max(t.ДатаПлатежа)
		,prev_paymentDate =  max(prev.ДатаПлатежа)
		from dwh2.dm.CMRExpectedRepayments t
		left join dwh2.dm.CMRExpectedRepayments  prev
			on prev.Код =t.Код
			and prev.ДатаПлатежа<@today
			and prev.СуммаПлатежа<>0
	where  t.ДатаПлатежа>=@today
	and t.СуммаПлатежа<>0
	group by t.Код
	 )
	 select 
		...
		pd.external_id
		  ,pd. paymentDate 
		 , paymentSum = cur_payment.СуммаПлатежа 
		  , prev_paymentDate =pd.prev_paymentDate
		  , pd.end_date_payment_schedule

	  into #plan_date
	 from  base_plan_date pd
	 left join 	 dwh2.dm.CMRExpectedRepayments cur_payment
		on cur_payment.Код = pd.external_id
		and cur_payment.ДатаПлатежа = pd.paymentDate
...
with dpd0 as(
 ...
 inner join #cmr_p cmr on cmr.external_id = pd.external_id
	and cast(cmr.dt as date) = 
	dateadd(day,1,cast(pd.prev_paymentDate  as date))
	and cmr.dpd = 0
 ...
 ,
  ...
  SELECT de.Код external_id
    ...
    where cast(g.Дата as date) > cast(dateadd(year,2000, prev_paymentDate) as date)
...
create clustered index cix on #t_last_sending_action(CrmCustomerId)
	...
	--dwh-578
		 ...
		 --  , old_plan_date			 = dateadd(year,-2000,pd.old_plan_date)  --DWH-1149
		   , end_date_payment_schedule = pd.end_date_payment_schedule --DWH-1155
