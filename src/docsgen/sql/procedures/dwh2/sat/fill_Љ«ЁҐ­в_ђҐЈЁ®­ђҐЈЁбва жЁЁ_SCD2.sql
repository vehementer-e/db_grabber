...
CREATE   PROC sat.fill_Клиент_РегионРегистрации_SCD2
...
begin try
	...
	--DELETE R
	--FROM sat.Клиент_РегионРегистрации_SCD2_stg AS R
	...
	if OBJECT_ID('sat.Клиент_РегионРегистрации_SCD2') is null
	begin
		select top(0)
			GuidКлиент,
            СсылкаКлиент,
            ...
            spFillName,
            ВерсияДанных
		into sat.Клиент_РегионРегистрации_SCD2
		from #t_Клиент_РегионРегистрации_SCD2

		alter table sat.Клиент_РегионРегистрации_SCD2
		...
		ALTER TABLE sat.Клиент_РегионРегистрации_SCD2
			ADD CONSTRAINT PK_Клиент_РегионРегистрации_SCD2 PRIMARY KEY CLUSTERED (GuidКлиент, date_from)
	end
	
	DELETE s
	FROM #t_Клиент_РегионРегистрации_SCD2 AS s
	WHERE s.РегионРегистрации IS NULL

	-- удалить то, что не изменилось
	DELETE s
	FROM #t_Клиент_РегионРегистрации_SCD2 AS s
		INNER JOIN sat.Клиент_РегионРегистрации_SCD2 AS t
			ON t.GuidКлиент = s.GuidКлиент
			AND s.date_from BETWEEN t.date_from AND t.date_to
			AND isnull(t.РегионРегистрации,'*') = isnull(s.РегионРегистрации,'*')
			AND isnull(t.GMTРегионРегистрации,99) = isnull(s.GMTРегионРегистрации,99)
	...
	FROM #t_Клиент_РегионРегистрации_SCD2 AS s
		INNER JOIN (
			...
			WHERE NOT EXISTS(
				SELECT TOP(1) 1
				FROM sat.Клиент_РегионРегистрации_SCD2 AS X
	...
	begin tran
		merge sat.Клиент_РегионРегистрации_SCD2 AS t
		using #t_Клиент_РегионРегистрации_SCD2 AS s
			on t.GuidКлиент = s.GuidКлиент
		...
		when matched and (
			...
			OR t.РегионРегистрации <> s.РегионРегистрации
			OR t.GMTРегионРегистрации <> s.GMTРегионРегистрации
			)
		then update SET
