...
CREATE PROC sat.fill_Клиент_РегионФактическогоПроживания
	@mode int = 1
as
begin
	--truncate table sat.Клиент_РегионФактическогоПроживания
begin try
	...
	drop table if exists #t_Клиент_РегионФактическогоПроживания

	if OBJECT_ID ('sat.Клиент_РегионФактическогоПроживания') is not null
		and @mode = 1
	begin
		set @rowVersion = isnull((select max(ВерсияДанных) - 1000 FROM sat.Клиент_РегионФактическогоПроживания), 0x0)
	end

	select distinct
		GuidКлиент = cast([dbo].[getGUIDFrom1C_IDRREF](Партнеры.Ссылка) as uniqueidentifier),
		СсылкаКлиент = Партнеры.Ссылка,
		РегионФактическогоПроживания = РегионПроживания.Наименование,
		GMTРегионФактическогоПроживания = РегионПроживания.CRM_ВремяПоГринвичу_GMT,
		created_at							= CURRENT_TIMESTAMP,
		updated_at							= CURRENT_TIMESTAMP,
		spFillName							= @spName,
		ВерсияДанных = cast(Партнеры.ВерсияДанных AS binary(8))
	into #t_Клиент_РегионФактическогоПроживания
	--SELECT *
	from Stg._1cCRM.Справочник_Партнеры AS Партнеры
		LEFT JOIN Stg._1cCRM.Справочник_БизнесРегионы AS РегионПроживания
			ON РегионПроживания.Ссылка = Партнеры.РегионФактическогоПроживания
	where Партнеры.ВерсияДанных >= @rowVersion
		--AND (РегионПроживания.Наименование IS NOT NULL
		--	OR РегионПроживания.CRM_ВремяПоГринвичу_GMT IS NOT NULL
		--)
		AND РегионПроживания.Наименование IS NOT NULL

	if OBJECT_ID('sat.Клиент_РегионФактическогоПроживания') is null
	begin
		select top(0)
			GuidКлиент,
            СсылкаКлиент,
            ...
            spFillName,
            ВерсияДанных
		into sat.Клиент_РегионФактическогоПроживания
		from #t_Клиент_РегионФактическогоПроживания

		alter table sat.Клиент_РегионФактическогоПроживания
			alter column GuidКлиент uniqueidentifier not null

		ALTER TABLE sat.Клиент_РегионФактическогоПроживания
			ADD CONSTRAINT PK_Клиент_РегионФактическогоПроживания PRIMARY KEY CLUSTERED (GuidКлиент)
	end

	begin tran
		merge sat.Клиент_РегионФактическогоПроживания t
		using #t_Клиент_РегионФактическогоПроживания s
		...
		--DWH-2923
		/*
		INSERT sat.Клиент_РегионФактическогоПроживания_SCD2_stg
		(
			portion_id,
			GuidКлиент,
			СсылкаКлиент,
		...
		SELECT 
			...
			R.spFillName,
			R.ВерсияДанных 
		FROM #t_Клиент_РегионФактическогоПроживания AS R

		EXEC sat.fill_Клиент_РегионФактическогоПроживания_SCD2
			@portion_id = @portion_id
		*/
		...
		FROM #t_Клиент_РегионФактическогоПроживания AS R
			INNER JOIN hub.БизнесРегион AS B
				ON B.Наименование = R.РегионФактическогоПроживания

		-- удалить то, что не изменилось
		DELETE s
		FROM #t_Клиент_РегионФактическогоПроживания_SCD2 AS s
			INNER JOIN link.v_Клиент_РегионФактическогоПроживания_SCD2 AS t
				ON t.GuidКлиент = s.GuidКлиент
		...
		FROM #t_Клиент_РегионФактическогоПроживания_SCD2 AS s
			INNER JOIN (
				...
				WHERE NOT EXISTS(
					SELECT TOP(1) 1
					FROM link.v_Клиент_РегионФактическогоПроживания_SCD2 AS X
		...
		SELECT 
			R.GuidКлиент,
			R.date_from,
			LinkName = 'link.Клиент_РегионФактическогоПроживания_SCD2',
			R.GuidБизнесРегион,
			TargetColName = 'GuidБизнесРегион'
		FROM #t_Клиент_РегионФактическогоПроживания_SCD2 AS R
