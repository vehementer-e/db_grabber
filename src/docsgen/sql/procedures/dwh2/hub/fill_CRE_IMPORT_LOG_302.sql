/*
exec hub.fill_CRE_IMPORT_LOG_302
*/
CREATE PROC hub.fill_CRE_IMPORT_LOG_302
	@mode int = 1 -- 0 - full, 1 - increment
	--,@GuidCRE_IMPORT_LOG_302 uniqueidentifier = NULL
	,@ID int = null
	,@InsertDate datetime = null
	,@isDebug int = 0
as
begin
	--truncate table hub.CRE_IMPORT_LOG_302
begin TRY
	...
	if OBJECT_ID ('hub.CRE_IMPORT_LOG_302') is not NULL
		AND @mode = 1
		and @ID is null
		and @InsertDate is null
	begin
		set @INSERT_DATE = isnull((select dateadd(day, -2, max(INSERT_DATE)) from hub.CRE_IMPORT_LOG_302), '2000-01-01')
	end

	select distinct
		GuidCRE_IMPORT_LOG_302 = try_cast(hashbytes('SHA2_256', cast(L.ID as varchar(30))) AS uniqueidentifier),
		L.ID,
	...
	--SELECT *
	FROM Stg._CRE.IMPORT_LOG_302 AS L
	...
	--create index ix1
	--on #t_CRE_IMPORT_LOG_302(EVENT_DATE_dt, ТипОбъекта, EVENT_ID, GuidОбъекта, INSERT_DATE)

	--create index ix2
	--on #t_CRE_IMPORT_LOG_302(EVENT_DATE_dt, EVENT_ID, GuidОбъекта, INSERT_DATE)

	create index ix3
	on #t_CRE_IMPORT_LOG_302(EVENT_DATE_dt, EVENT_ID, НомерОбъекта, INSERT_DATE)

	create index ix5
	on #t_CRE_IMPORT_LOG_302(EVENT_DATE_dt, EVENT_ID, НаименованиеКлиент, INSERT_DATE)

	IF @isDebug = 1 BEGIN
		DROP TABLE IF EXISTS ##t_CRE_IMPORT_LOG_302
		SELECT * INTO ##t_CRE_IMPORT_LOG_302 FROM #t_CRE_IMPORT_LOG_302
		--RETURN 0
	END


	if OBJECT_ID('link.CRE_IMPORT_LOG_302_stage') is null
	begin
		CREATE TABLE link.CRE_IMPORT_LOG_302_stage
		(
			Id uniqueidentifier NOT NULL CONSTRAINT [DF_CRE_IMPORT_LOG_302_stage__Id] DEFAULT (newid()),
			GuidCRE_IMPORT_LOG_302 uniqueidentifier NOT NULL,
			...
			created_at datetime NOT NULL CONSTRAINT [DF_CRE_IMPORT_LOG_302_stage__created_at] DEFAULT (getdate()),
			TargetColName nvarchar(255) NULL
		) ON [PRIMARY]

		ALTER TABLE link.CRE_IMPORT_LOG_302_stage
		ADD CONSTRAINT PK__CRE_IMPORT_LOG_302_stage PRIMARY KEY CLUSTERED (Id) ON [PRIMARY]

		CREATE NONCLUSTERED INDEX [ix_LinkName] ON link.CRE_IMPORT_LOG_302_stage (LinkName) ON [PRIMARY]
	end

	insert into link.CRE_IMPORT_LOG_302_stage(
	...
	select distinct 
		...
		LinkGuid,
		TargetColName
	from #t_CRE_IMPORT_LOG_302
		CROSS APPLY (
			VALUES 
				(Link_GuidБКИ_НФ_События, 'link.CRE_IMPORT_LOG_302_События', 'GuidБКИ_НФ_События')
				,(Link_GuidДоговорЗайма, 'link.CRE_IMPORT_LOG_302_ДоговорЗайма', 'GuidДоговораЗайма')
				,(Link_GuidЗаявка, 'link.CRE_IMPORT_LOG_302_Заявка', 'GuidЗаявки')
				,(Link_GuidКлиент, 'link.CRE_IMPORT_LOG_302_Клиент', 'GuidКлиент')
	...
	select 
		a.GuidCRE_IMPORT_LOG_302,
		a.LinkName,
		a.LinkGuid,
		a.TargetColName
	from (
		select --distinct 
			e.GuidCRE_IMPORT_LOG_302,
			LinkName = 'link.CRE_IMPORT_LOG_302_ИсторияСобытий',
			LinkGuid = h.GuidБКИ_НФ_ИсторияСобытий,
			TargetColName = 'GuidБКИ_НФ_ИсторияСобытий',
			rn = row_number() over(
				partition by e.GuidCRE_IMPORT_LOG_302
				order by h.Дата desc, h.ДатаЗаписи desc, getdate()
			)
		from #t_CRE_IMPORT_LOG_302 as e
	...
	*/
	...
	--BEGIN TRY
		EXEC link.exec_fill_link_between_CRE_IMPORT_LOG_302_and_other
		--EXEC msdb.dbo.sp_start_job N'DWH2. fill_link_between_CRE_IMPORT_LOG_302_and_other'
	--END TRY
	--BEGIN CATCH
	--	--??
	--END CATCH

	if OBJECT_ID('hub.CRE_IMPORT_LOG_302') is null
	begin
		select top(0)
			GuidCRE_IMPORT_LOG_302,
			ID,
            ...
            updated_at,
            spFillName
		into hub.CRE_IMPORT_LOG_302
		from #t_CRE_IMPORT_LOG_302

		alter table hub.CRE_IMPORT_LOG_302
			alter COLUMN GuidCRE_IMPORT_LOG_302 uniqueidentifier not null

		ALTER TABLE hub.CRE_IMPORT_LOG_302
			ADD CONSTRAINT PK__CRE_IMPORT_LOG_302 PRIMARY KEY CLUSTERED (GuidCRE_IMPORT_LOG_302)

		create index ix_INSERT_DATE
		on hub.CRE_IMPORT_LOG_302(INSERT_DATE)

		--create index ix_EVENT_DATE_EVENT_ID_IMPORT_ID_UUID
		--on hub.CRE_IMPORT_LOG_302(EVENT_DATE, EVENT_ID, IMPORT_ID, UUID)
	end
	
	--begin tran
		merge hub.CRE_IMPORT_LOG_302 t
		using #t_CRE_IMPORT_LOG_302 s
			on t.GuidCRE_IMPORT_LOG_302 = s.GuidCRE_IMPORT_LOG_302
		...
		--)
		then update SET
			--t.GuidCRE_IMPORT_LOG_302 = s.GuidCRE_IMPORT_LOG_302,
			t.ID = s.ID,
