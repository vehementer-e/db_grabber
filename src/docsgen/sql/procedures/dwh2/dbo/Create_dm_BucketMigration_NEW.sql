...
---- =============================================
CREATE   PROC dbo.Create_dm_BucketMigration_NEW
...
------------------------------
 ...
 if object_id('tempdb.dbo.#change_bucket') is not null drop table #change_bucket
	...
	*/  
	...
	IF @isDebug = 1 BEGIN
		DROP TABLE IF EXISTS ##change_bucket_NEW
		SELECT * INTO ##change_bucket_NEW FROM #change_bucket
	END
	...
	IF @isDebug = 1 BEGIN
		DROP TABLE IF EXISTS ##change_bucket_by_day_NEW
		SELECT * INTO ##change_bucket_by_day_NEW FROM #change_bucket_by_day
	END
	...
	IF @isDebug = 1 BEGIN
		DROP TABLE IF EXISTS ##change_bucket_by_day_and_tomorrow_NEW
		SELECT * INTO ##change_bucket_by_day_and_tomorrow_NEW FROM #change_bucket_by_day_and_tomorrow
	END
	...
	IF @isDebug = 1 BEGIN
		DROP TABLE IF EXISTS ##change_bucket_final_NEW
		SELECT * INTO ##change_bucket_final_NEW FROM #change_bucket_final
	END
...
left join (
		...
		delete from dbo.dm_BucketMigration_NEW
		where Период = Format(dateadd(year,-2000,@dt_begin_of_month),'yyyy-MM')

	IF @isDebug = 1 BEGIN
		DROP TABLE IF EXISTS ##bucket_transfer_NEW
		SELECT * INTO ##bucket_transfer_NEW FROM #bucket_transfer
	END
		 ...
		 -- добавим ОД
		  ...
		  insert into dbo.dm_BucketMigration_NEW
		  Select 
