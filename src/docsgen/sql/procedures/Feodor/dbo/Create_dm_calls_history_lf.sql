...
CREATE PROC [dbo].[Create_dm_calls_history_lf]
as 
begin


 declare @max_attempt_start datetime =  isnull( (select max(attempt_start) from Feodor.dbo.dm_calls_history_lf), '20250301')
  declare @start_creating_ch datetime = getdate()

 declare @long_update int = case when 
 not exists (select top 1 1 from Feodor.dbo.dm_calls_history_monitoring where job=-1 and start_creating_lh>=cast(getdate() as date)   )
 ...
 set  @update_dt=case when @long_update=1 then  cast(@max_attempt_start-2 as date) else dateadd(hour, -1, @max_attempt_start) end
   select @update_dt



--select  count(*) over(partition by uuid ), *  from #case4
--order by 1	  desc

   drop table if exists #call1
select case_uuid, session_id, project_id, project_title ,phone,   attempt_start,  getdate() row_created, login, attempt_end, hangup_initiator , attempt_result  into #call1 from analytics.dbo.v_call
where project_id in (
			select id from analytics.dbo.v_naumen_project
where project like 'crm%'
 	)  
and 	attempt_start>= '20250301'
and 	attempt_start>= cast(@update_dt as datetime2)


   drop table if exists #call2

select a.*, b.Звонок_ссылка, u.Номер НомерЗаявки, u.original_lead_id original_lead_id, b.ВзаимодействиеПрикрепленное_Ссылка  , b.ДатаВремяВзаимодействия , b.Результат, b.ДлительностьЗвонка, case when
a.attempt_start>isnull(isnull(z.[Предварительное одобрение], z.Отказано )	  , getdate())		then 1 else 0 end call_after_call1
 into #call2  

from 	#call1 a 
left join analytics.dbo.v_communication_crm b on a.session_id=b.Session_id
left join analytics.dbo.[lead_case_crm] u on u.uuid=a.case_uuid
left join analytics.dbo.v_request z on z.НомерЗаявки=b.НомерЗаявки

;with v  as (select *, row_number() over(partition by session_id order by ДлительностьЗвонка desc) rn from #call2 ) delete from v where rn>1

begin tran
--drop table if exists analytics.dbo.[lead_call_crm]
--select * into analytics.dbo.[lead_call_crm] from #call2
delete a from analytics.dbo.lead_call_crm	a join 	#call2 b on a.session_id=b.session_id
insert into analytics.dbo.lead_call_crm
SELECT 
               a.[case_uuid] 
,   a.[session_id] 
,   a.[project_id] 
...
,   a.[НомерЗаявки] 
,   a.[original_lead_id] 
,   a.[ВзаимодействиеПрикрепленное_Ссылка] 
,   a.[ДатаВремяВзаимодействия] 
,   a.[Результат] 
,   a.[ДлительностьЗвонка] 
,   a.[call_after_call1] 
,   a.attempt_end 
,   a.hangup_initiator 
    , attempt_result 
 ...
 [#call2] a
commit tran
--alter table lead_call_crm add hangup_initiator nvarchar(20)
--alter table lead_call_crm add attempt_result nvarchar(20)
 ...
 select distinct idexternal project_id from feodor.dbo.dm_feodor_project2
 ) p
...
SELECT  cc.uuid
	   ...
	   , trim( isnull( cast(cf.lead_id as nvarchar(50)), lccrm.original_lead_id)) lead_id
    into #t
	...
	left join Analytics.dbo.[lead_call_crm]	lccrm on lccrm.session_id=dos.session_id	 and  lccrm.call_after_call1=0
--	DWH-1871
  ...
  delete from  feodor.dbo.dm_calls_history_lf   
  ...
  insert into  feodor.dbo.dm_calls_history_lf
    select  [uuid]
      ,[creationdate]
      ,[timezone]
      ,[phonenumbers]
...
(
	...
	where cast(creationdate as date) >= @start_date
	group by lcrm_id
	,        ch.projectuuid
	,        creationdate
)
...
--					
--select 	a.lead_id into #t1 from #t	  a
--left join lead_tbl B ON A.lead_id=B.ID
--WHERE isnull(B.ВремяПоследнейПопытки, '20010101') <a.attempt_start 
--
--
--
--insert into lead_for_update
--select 	 a.lead_id , 'call_reference', getdate()   from 	#t1			 a
--left join lead_for_update b on a.lead_id=b.id
--where b.id is null
--group by a.lead_id
...
--					
--select 	a.lead_id into #t2 from dm_calls_history_lf	  a
--left join lead_tbl B ON A.lead_id=B.ID
--WHERE isnull(B.ВремяПоследнейПопытки, '20010101') <a.attempt_start 
--
--
--insert into lead_for_update
--select 	 a.lead_id , 'call_reference long', getdate()  from 	#t2			 a
--left join lead_for_update b on a.lead_id=b.id
--where b.id is null
--group by a.lead_id
--
